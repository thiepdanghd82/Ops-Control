...
- **Desktop setup wizard "Failed to fetch" (CORS)** — wizard rendered at `data:text/html` URL has null origin, renderer `fetch()` to embedded Express is cross-origin, blocked silently. The probe now runs in the main process, where no CORS layer applies, and `probeServer()` reaches it over `ipcRenderer.invoke`. Server-side CORS policy untouched. _Sprint S-WIZARD-CORS; the original `/__probe__` sentinel was replaced by IPC in S-FIRSTRUN-IPC (PR #279) — it could not work from the dialog's opaque `data:` origin._
