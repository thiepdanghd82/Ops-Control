...
app.whenReady().then(async () => {
  ...
  // Recovery: clients stranded in an unbootable state by an earlier install
  // (firstRunCompleted=true + thin + remoteUrl='') would otherwise
  // loadURL('') → ERR_INVALID_URL (-300) and need config.json hand-edited.
  // Must run BEFORE the wizard gate below, and must clear BOTH halves of the
  // first-run state — the electron-store flag AND setup-done.json — or
  // isFirstRun() keeps reporting false and boot skips the wizard anyway.
  // Bug surfaced 2026-06-11 (CCL Vietnam soft-launch), still stranding
  // clients 2026-09-10 because only the store flag was being cleared.
  if (BUILD_ROLE === 'client' && store.get('firstRunCompleted')) {
    const stranded =
      store.get('mode') === 'thin' && String(store.get('remoteUrl') || '').length === 0;
    if (stranded) {
      log.warn('[main] recovering from invalid state: thin + empty remoteUrl → re-running wizard');
    ...
    if (BUILD_ROLE !== 'generic' && !store.get('firstRunCompleted')) {
      try {
        if (BUILD_ROLE === 'server') {
          ...
          await showClientFirstRunDialog({ store, log });
