...
async function main() {
  ...
  // last `vite build` ship STALE inside the DMG even though the source
  // file looks current. MES-3-FIX-55 (2026-06-17) — surfaced when
  // PR #157's column reorder didn't appear in DMG rc2 despite the
  // commit on disk; bundle hash hadn't rolled because nothing rebuilt
  // client/dist between rc1 and rc2.
