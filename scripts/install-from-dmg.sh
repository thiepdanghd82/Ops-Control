...
#
# Why this exists:
#   CCL Vietnam ships unsigned ad-hoc DMGs (no Apple Developer ID — the
#   Apple program costs $99/year per company and blocks shipping). On
#   first launch macOS quarantines the .app and shows a scary modal.
...
if [[ -f "$CHECKSUMS" ]]; then
  ...
  ( cd "$(dirname "$DMG")" && shasum -a 256 -c <(grep "$(basename "$DMG")" "$CHECKSUMS") ) \
    || { echo "✗ Checksum FAILED. DMG may be corrupted or tampered. Re-download." >&2; exit 2; }
  ...
  echo "[1/4] ⚠ Checksum file missing — skipped (FORCE=1)"
fi
...
if [[ ! -x "$TARGET/Contents/MacOS/"* ]]; then
