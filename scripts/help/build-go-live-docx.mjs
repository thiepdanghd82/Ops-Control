 ...
 *
 * Output: 4. CLAUDE OUTPUT/OpsControl_GoLiveGuide_v1.2.docx
 *
...
import {
  ...
  Footer, AlignmentType, HeadingLevel, BorderStyle,
...
const OUT_DIR = path.join(ROOT, '..', '..', '4. CLAUDE OUTPUT');
const OUT_FILE = path.join(OUT_DIR, 'OpsControl_GoLiveGuide_v1.2.docx');
...
function inlineRuns(text, baseOpts = {}) {
  ...
  while (i < text.length) {
    ...
    if (text[i] === '`') {
      ...
      if (end > 0) {
        flush(text.slice(i + 1, end), { font: 'Consolas', shading: { type: ShadingType.CLEAR, fill: COLOR_CODE_BG, color: 'auto' } });
        i = end + 1;
        continue;
      }
    }
...
const heading = (text, level) => {
  ...
  return new Paragraph({
    children: [new TextRun({ text, bold: true, font: FONT, size: sizes[level] || 22, color: colors[level] || '000000' })],
...
function parseTable(rows) {
  ...
  const splitCells = (line) => line.replace(/^\|/, '').replace(/\|$/, '').split('|').map(s => s.trim());
  ...
  const headerRow = new TableRow({
    ...
    children: header.map(h => new TableCell({
      ...
      children: [new Paragraph({
        children: [new TextRun({ text: h, bold: true, font: FONT, size: 18, color: COLOR_ACCENT })],
      })],
    })),
  });
    ...
    footers: {
      default: new Footer({
        children: [new Paragraph({
          ...
          children: [
            new TextRun({ text: 'Ops Control v1.2 — Go-Live Guide · Henry Dang — NPI Manager · CCL Design Vietnam · ', font: FONT, size: 16, color: '6B7280' }),
...
console.log(`[build-go-live-docx] wrote ${OUT_FILE}`);
