/**
 * Self-check — Sprint 10 go-live smoke test (renamed from self-test
 * in v1.3 GA so the `node --test` filename glob doesn't pick it up).
 *
 ...
 *
 * Browser globals (window/document) inside `page.evaluate()` callbacks
 * are added via a per-file override in `eslint.config.js` — those
 * callbacks execute in the page context Puppeteer ferries over, not
 * in Node.
 */
...
try {
  ...
  await page.evaluate((u) => {
    const i = [...document.querySelectorAll('input')].find(
      (x) => x.type !== 'password' && /user|name/i.test(x.placeholder + ' ' + x.name)
    );
    if (i) {
  ...
  await page.evaluate(() => {
    [...document.querySelectorAll('button')]
      .find((b) => /sign\s*in/i.test(b.textContent) && !/change/i.test(b.textContent))
      ?.click();
  });
  ...
  for (const probe of TAB_PROBES) {
    ...
    const a11y = await page.evaluate(() => {
      ...
      const missing = tabs.filter(
        (t) => !t.getAttribute('aria-label') && !t.textContent.trim()
      ).length;
