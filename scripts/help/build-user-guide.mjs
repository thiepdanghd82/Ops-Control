...
const h3 = (text, color = COLOR_ACCENT) =>
  new Paragraph({
...
async function renderEntryAsync(entry, sectionMeta) {
  ...
  const blocks = [];
  blocks.push(
    p(`${sectionMeta.label.en} · ${sectionMeta.label.vi}`, {
      size: 18,
      bold: true,
      color: '6B7280',
    })
      ...
      hdrRows.push(
        new TableRow({
          children: [
            ...
            cellText(
              `${entry.function.en}${entry.function.vi ? '  ·  ' + entry.function.vi : ''}`,
  ...
  if (entry.purpose) {
    blocks.push(h3('Purpose · Mục đích'));
    blocks.push(...bi(entry.purpose.en, entry.purpose.vi));
  }
  if (entry.whenToUse) {
    blocks.push(h3('When to use · Khi nào dùng'));
    blocks.push(...bi(entry.whenToUse.en, entry.whenToUse.vi));
  }

  if (entry.features?.length) {
    blocks.push(h3('Features · Tính năng'));
    for (const f of entry.features) blocks.push(bullet(f));
  }
    ...
    for (let i = 0; i < entry.procedures.length; i++) {
      ...
      if (prc.screenshot) {
        const shotPath = path.resolve(
          __dirname,
          '../../client/public/help/screenshots',
          prc.screenshot
        );
            ...
            new Paragraph({
              children: [
                new ImageRun({
                  ...
                  altText: {
                    title: prc.title.en,
                    description: `Procedure: ${prc.title.en}`,
                    name: `proc_${entry.id}_${i}`,
                  },
                }),
              ],
  ...
  if (entry.screenshot) {
    ...
    if (fs.existsSync(shotPath)) {
          ...
          children: [
            new ImageRun({
              ...
              altText: {
                ...
                description: `Screenshot of ${entry.id}`,
                name: entry.id,
              },
            }),
          ],
      ...
      blocks.push(
        p(`[Screenshot pending: ${entry.screenshot}]`, { size: 18, italics: true, color: '9CA3AF' })
      );
    }
  }
    ...
    for (const f of entry.formulas) {
      ...
      if (f.example) {
        ...
        blocks.push(
          new Paragraph({
            children: [
              new TextRun({ text: f.example, font: 'Consolas', size: 19, color: '14532D' }),
            ],
            shading: { fill: 'F0FDF4', type: ShadingType.CLEAR },
  ...
  if (entry.appendices?.length) {
    ...
    for (let i = 0; i < entry.appendices.length; i++) {
      ...
      blocks.push(
        p(`${letter}. ${app.title.vi} — ${app.title.en}`, {
          bold: true,
          size: 22,
          color: '1E40AF',
      ...
      for (const r of app.rows) {
        rows.push(
          new TableRow({
            children: app.columns.map((c) => cellText(String(r[c.key] ?? ''), { width: colWidth })),
          })
        );
      }
      blocks.push(buildTable(widths, rows));
  ...
  if (entry.relatedTabs?.length) {
    ...
    const related = entry.relatedTabs
      .map((rid) => HELP_CONTENT[rid])
      .filter(Boolean)
...
const titlePage = [
  new Paragraph({
    children: [
      new TextRun({ text: 'Ops Control', font: FONT, size: 64, bold: true, color: COLOR_ACCENT }),
    ],
...
indexHeader.push(
  new TableOfContents('Click this line, then press F9 in Word (or Tools → Update) to populate.', {
    hyperlink: true,
    headingStyleRange: '1-2',
  })
);
indexHeader.push(
  p("— A plain index follows for viewers that don't auto-update fields —", {
    size: 18,
    italics: true,
    color: '6B7280',
    alignment: AlignmentType.CENTER,
...
for (const sec of HELP_SECTIONS) {
  ...
  body.push(
    p(`${entries.length} tab${entries.length === 1 ? '' : 's'}`, {
      size: 20,
      italics: true,
      color: '6B7280',
    })
...
const footer = new Footer({
  children: [
    new Paragraph({
      children: [
        new TextRun({
          text: 'Ops Control · User Guide · CCL Design Vietnam',
          font: FONT,
          size: 16,
          color: '6B7280',
        }),
...
const REVIEW_OUT = path.resolve(
  __dirname,
  '../../../../4. CLAUDE OUTPUT/OpsControl_UserGuide.docx'
);
