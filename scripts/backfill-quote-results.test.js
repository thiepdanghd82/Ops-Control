...
function stdState(overrides = {}) {
  return {
    moq: 1000, annual_qty: 10000, selling_price: 0.2, trade_mode: 'USD(Normal)', site: 'VN',
    product_lifetime: 1, active_moq_idx: 0, extra_moqs: [], num_moq: 1,
    part_width: 80, part_length_md: 50, web_width_td: 80, sheet_length: 50, num_webs: 1,
    parts_in_md: 1, parts_web_across: 1, min_gap_md: 2, rotary_cols: 0, pcs_per_roll: 0,
    materials: [{ code: 'M001', width: 80, usage: 1, s_price: 5, g_price: 5, latest: 0, offcut_pct: 0, offcut_yn: 'N', slitting_yn: 'N' }],
    inks: [],
    processes: [{ workcenter: 'Flexo-A', process_type: 'Flexo', speed: 10, layout: 4, efficiency: 0.85, setup_h: 0.5, scrap_pct: 0.03, tool_cost: 0, tool_type: '', tool_life: 0, product_life: 1, eau_ovr: 0, repeat: 1 }],
    pcs_per_bag: 100, bags_per_box: 10, container_cost: 0.5, box_cost: 0.5, other_packing: 0,
    shipping_cost: 100, other_ship: 0, ship_qty: 1000,
    ...overrides,
  };
}
...
test('backfillQuotes: thin quotes updated, full-schema untouched, bad ones skipped', () => {
  const lib = makeLib();
  const alreadyFull = {
    id: 2, type: 'standard', state: stdState(),
    result: { sp: 0.2, s_ttl: 0.05, gm: 0.75, va: 0.8, s_mat_cost: 0.02, tooling: 0, packing_ship: 0.01, labor_cost: 0.003 },
  };
  const thin = { id: 1, type: 'standard', state: stdState(), result: { gm: 0.1, va: 0.2, s_ttl: 0.05 } };
...
test('backfillQuotes: idempotent — re-run after successful backfill is a no-op', () => {
  const lib = makeLib();
  const q = { id: 1, type: 'standard', state: stdState(), result: { gm: 0.5, va: 0.5, s_ttl: 0.05 } };
