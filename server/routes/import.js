...
import {
  getBackupRoot, resolveBackupTarget, checkQuota,
...
import {
  shadowWriteBom, shadowWriteRouting,
  shadowWriteInventory, shadowWriteMaterials,
  shadowClearBom, shadowClearRouting,
  shadowClearInventory, shadowClearMaterials,
...
router.post('/inventory', requireRole(4), upload.single('file'), requireValidUpload, async (req, res) => {
  try {
...
router.post('/finished-goods', requireRole(4), upload.single('file'), requireValidUpload, async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ error: 'No file uploaded' });
...
router.post('/raw-materials', requireRole(4), upload.single('file'), requireValidUpload, async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ error: 'No file uploaded' });
...
router.post('/routing', requireRole(4), upload.single('file'), requireValidUpload, async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ error: 'No file uploaded' });
...
const CLEAR_TARGETS = {
  bom:           { file: ['Manufacturing_Structures', 'mfg_structures_data.js'], varName: 'window._CCL_MFG_DATA', label: 'Manufacturing Structures' },
  routing:       { file: ['Routing_Operations',       'routing_ops_data.js'],    varName: 'window._CCL_ROP_DATA', label: 'Routing Operations' },
  inventory:     { file: ['IFS_Inventory',            'inventory_data.js'],      varName: 'window._CCL_INV_DATA', label: 'Full Inventory' },
  'finished-goods': { file: ['IFS_Inventory',         'finished_good_data.js'],  varName: 'window._CCL_FG_DATA',  label: 'Finished Goods' },
  'raw-materials':  { file: ['IFS_Inventory',         'raw_materials_data.js'],  varName: 'window._CCL_RM_DATA',  label: 'Raw Materials' },
};
...
const NPI_HEADER_MAP = {
  // date
  'date': 'date', 'update date': 'date', 'updated': 'date', 'ngày': 'date',
  // name / material
  'name': 'name', 'material': 'name', 'material name': 'name', 'tên vật tư': 'name',
  // price
  'price': 'price', 'usd/m²': 'price', 'usd per m2': 'price', 'unit price': 'price',
  // type / description
  'type': 'type', 'description': 'type', 'desc': 'type', 'type / description': 'type',
  // thickness
  'thick': 'thick', 'thickness': 'thick', 'độ dày': 'thick',
  // color / surface / adhesive
  'color': 'color', 'màu': 'color',
  'surface': 'surface', 'finish': 'surface',
  'adhesive': 'adhesive', 'glue': 'adhesive',
  // moq / leadtime
  'moq': 'moq', 'min qty': 'moq', 'min order': 'moq',
  'lt': 'lt', 'lead time': 'lt', 'leadtime': 'lt',
  // supplier / note
...
const SOURCING_HEADER_MAP = {
  // date
  'month': 'month', 'req. date': 'month', 'req date': 'month', 'request date': 'month', 'date': 'month',
  // requester
  'requester': 'req', 'req': 'req', 'by': 'req',
  // customer
  'customer': 'cust', 'cust': 'cust', 'project': 'cust', 'customer / project': 'cust',
  // material inquiry
  'material': 'material', 'material inquiry': 'material', 'inquiry': 'material',
  // size
  'size': 'size', 'spec': 'size', 'size / spec': 'size',
  // prices
  'exw': 'exw', 'exw price': 'exw', 'exw price (usd/m²)': 'exw',
  'dap': 'dap', 'dap price': 'dap', 'dap price (usd/m²)': 'dap',
  // moq / lead time
  'moq': 'moq', 'supplier moq': 'moq', 'min order': 'moq',
  'lt': 'lt', 'lead time': 'lt', 'leadtime': 'lt',
  // supplier / status
  'supplier': 'supplier', 'vendor': 'supplier',
  'status': 'status', 'remark': 'status', 'remarks': 'status', 'status / remark': 'status',
};
...
function handleMaterialsImport(cfg) {
  return async (req, res) => {
    try {
      ...
      try { fs.unlinkSync(req.file.path); } catch { /* ignore cleanup */ }
      ...
      try { if (req.file) fs.unlinkSync(req.file.path); } catch { /* ignore cleanup */ }
...
router.post('/npi-materials', requireRole(4), upload.single('file'), requireValidUpload, handleMaterialsImport({
  label: 'NPI Materials',
...
const RATE_HEADER_MAP = {
  ...
  'workcenter': 'workcenter', 'work center': 'workcenter', 'work centre': 'workcenter',
  'wc': 'workcenter', 'wc name': 'workcenter', 'workcenter name': 'workcenter',
  'name': 'workcenter', 'operation': 'workcenter', 'tên': 'workcenter',
  // Crew — operators required
  'crew': 'crew', 'crew size': 'crew', 'operators': 'crew', 'people': 'crew',
  // Machine rate ($/hr)
  'machine': 'machine_rate', 'machine rate': 'machine_rate', 'machine usd/h': 'machine_rate',
  'machine $/h': 'machine_rate', 'mach rate': 'machine_rate', 'mc rate': 'machine_rate',
  // Labor rate ($/hr)
  'labor': 'labor_rate', 'labour': 'labor_rate', 'labor rate': 'labor_rate',
  'labor usd/h': 'labor_rate', 'labour usd/h': 'labor_rate', 'labor $/h': 'labor_rate',
  // Speed unit
  'speed uom': 'speed_uom', 'speed': 'speed_uom', 'uom': 'speed_uom', 'unit': 'speed_uom',
  'speed unit': 'speed_uom',
  // Overhead cost
  'oh cost': 'oh_cost', 'overhead': 'oh_cost', 'oh': 'oh_cost', 'overhead cost': 'oh_cost',
  // Machine center / machine cost reference
  'w/c': 'mc_cost', 'mc cost': 'mc_cost', 'machine center': 'mc_cost', 'mc': 'mc_cost',
};
...
router.post('/rate', requireRole(4), upload.single('file'), requireValidUpload, async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ ok: false, error: 'No file uploaded' });
    ...
    if (mapped.length === 0) {
      return res.status(400).json({
        ok: false,
        error: 'No rate rows recognised. Expected headers like: Workcenter, Crew, Machine USD/H, Labor USD/H, Speed UOM, OH Cost',
      });
    }
...
router.post('/sourcing-db', requireRole(4), upload.single('file'), requireValidUpload, handleMaterialsImport({
  label: 'Sourcing DB',
...
const DATASET_FILES = {
  ...
  bom:           { file: ['Manufacturing_Structures', 'mfg_structures_data.js'], label: 'BOM / Mfg Structures' },
...
router.post('/backup', requireRole(4), validateBody({
  ...
  key: { type: 'enum', required: true, values: Object.keys({
    inventory: 1, finishedGoods: 1, rawMaterials: 1, bom: 1, routing: 1,
  }) },
  ...
  try {
    ...
    if (!fs.existsSync(src)) return res.status(404).json({ ok: false, error: 'Source data file not found' });
...
router.post('/backup/verify', requireRole(4), validateBody({
