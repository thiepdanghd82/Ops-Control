...
const APP_LEVEL_AUTH = ['routes/shared.js', 'routes/planning.js', 'routes/import.js'];
...
function scanRouteFile(rel) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const inlineAuth = /getSessionUser\s*\(|getPreauthSession\s*\(|requireWriter\s*\(|requireUser\s*\(|isAdminPlus\s*\(|isSys\s*\(|roleLevel\s*\(|cu\.role|req\.user\?\./.test(body);
    ...
    findings.push({
      ...
      source: sigAuth ? 'middleware-sig'
        : inlineAuth ? 'inline-check'
        : appLevelAuth ? 'app-level-authMiddleware'
        : fileUseGate ? 'router-use-gate'
        : 'NONE',
    });
  }
  return findings;
...
test('every mutating route has an auth/role gate or is on the public allowlist', () => {
  ...
  const unprotected = all.filter(r => !r.protected && !PUBLIC_ALLOWLIST.has(`${r.verb} ${r.path}`));
...
test('app-level authMiddleware mounts are still wired up in index.js', () => {
  ...
  for (const rel of APP_LEVEL_AUTH) {
    const base = path.basename(rel, '.js');
    const pattern = new RegExp(`app\\.use\\([^)]*['"]/api/${base}['"]\\s*,\\s*authMiddleware`, 'i');
    assert.ok(pattern.test(src),
      `${rel} is on APP_LEVEL_AUTH but index.js no longer mounts /api/${base} with authMiddleware — ` +
      `update this test OR restore the mount before shipping.`);
  }
});
