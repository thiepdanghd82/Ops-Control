...
test('propagates thrown errors, releases lock so next caller proceeds', async () => {
  ...
  let caught;
  try {
    await withLock('err', async () => { throw new Error(errMsg); });
  } catch (e) { caught = e; }
...
test('Map cleanup: key drops after last task drains', async () => {
  ...
  await withLock('ephemeral', async () => { await new Promise(r => setTimeout(r, 1)); });
...
test('Map keeps key while queue has pending tasks', async () => {
  let release1;
  const held1 = new Promise(r => { release1 = r; });
  const t1 = withLock('queued', async () => { await held1; });
...
test('multi-instance: lock file is created + released', async () => {
  ...
  try {
    ...
    assert.ok(fs.existsSync(stub), 'lock stub file persists after release (proper-lockfile pattern)');
...
test('multi-instance: concurrent writers still serialize (same key)', async () => {
  ...
  try {
    let counter = 0;
    const N = 5;
    const writers = Array.from({ length: N }, () => withLock('ctr', async () => {
      const snapshot = counter;
...
test('multi-instance: error in fn releases the file lock', async () => {
  ...
  try {
    await assert.rejects(
      withLock('err-key', async () => { throw new Error('boom'); }),
      /boom/,
    );
...
test('single-instance (default): no lock directory created', async () => {
  ...
  try {
    ...
    assert.ok(!fs.existsSync(locksDir), 'no locks dir should exist when multi-instance flag is off');
  } finally {
