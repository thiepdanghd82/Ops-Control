...
const seed = Array.from({ length: seedCount }, (_, i) => ({
  id: i + 1, username: `user${i + 1}`, role: 'cost',
  pwd_bcrypt: '$2b$10$dummy', lastPwdChange: new Date().toISOString(),
  permissions: {}, full_name: '', english_name: '', id_no: '', email: '', phone: '',
}));
...
test('N parallel updateUsers calls each land a distinct field in the final file', async () => {
  ...
  for (let i = 0; i < N; i++) {
    ...
    assert.equal(u?.email, `race-${i + 1}@test`,
...
test('updateUsers propagates thrower errors and still releases the lock', async () => {
  await assert.rejects(
    () => updateUsers(() => { throw new Error('mutator boom'); }),
    /mutator boom/
  );
  // Subsequent call must still succeed — lock wasn't left held.
  await updateUsers(users => { users[0].full_name = 'post-boom'; });
