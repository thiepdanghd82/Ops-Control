...
const INVENTORY_BASE_ALIASES = aliasMap({
  'Part No': VN.partNo,
  // `Part Description In Use` is the header IFS actually exports for the
  // human-readable description. Getting this wrong is what filled the
  // description column with the literal string "Finished Goods" for all
  // 9096 rows of the 2026-06 import — that value belongs to
  // `Accounting Group Description`, which is a category with its own
  // canonical below. Never alias the two together.
  'Part Description': [...VN.partDesc, 'Part Description In Use'],
  Site: VN.site,
  'Lot/Batch No': ['Lot No', 'Batch No', 'Lot', 'Batch', 'Số lô'],
  'Location No': ['Location', 'Bin', 'Vị trí'],
  'Qty On Hand': ['Qty', 'On Hand', 'OH', 'SL Tồn', 'Tồn kho'],
  UOM: [...VN.uom, 'Inventory UoM'],
  'Unit Cost': ['Cost', 'Standard Cost', 'Giá vốn', 'Đơn giá'],
  'Total Cost': ['Stock Value', 'Giá trị tồn'],
  'Receipt Date': ['Date Received', 'Receipt', 'Ngày nhập'],
  'Expiry Date': ['Expiry', 'Expiration', 'Ngày hết hạn'],
  // ── Part-master columns carried by the IFS Full Inventory export ──
  // VND and USD stay two columns: the export supplies both and the FX rate
  // it used is not in the file, so collapsing them would silently bake in
  // an unknown rate.
  'Inventory Value (VND)': ['Inv Value VND', 'Giá trị tồn (VND)'],
  'Inventory Value (USD)': ['Inv Value USD', 'Giá trị tồn (USD)'],
  'Accounting Group Description': ['Accounting Group Desc'],
  'Accounting Group': [],
  'Part Product Family Description': ['Product Family Description'],
  'Part Product Family': ['Product Family'],
  'Part Status Description': ['Status Description'],
  'Part Status': ['Status'],
  'Part Type': [],
  'Type Designation': [],
  'Dimension/ Quality': ['Dimension/Quality', 'Dimension / Quality'],
  Thick: ['Thickness'],
  Width: [],
  Len: ['Length'],
  Uldes: [],
  ...
  'Std Lot Size': ['Standard Lot Size'],
});
...
const INVENTORY_DATASET = {
  ...
  canonicalHeaders: [
    ...
    'Receipt Date',
    'Expiry Date',
    // Part-master columns from the IFS Full Inventory export (2026-09-10).
    'Inventory Value (VND)',
    'Inventory Value (USD)',
  ...
  columnTypes: {
    ...
    'Unit Cost': 'number',
    'Total Cost': 'number',
    'Inventory Value (VND)': 'number',
    'Inventory Value (USD)': 'number',
    Thick: 'number',
    Width: 'number',
    Len: 'number',
    'Std Lot Size': 'number',
