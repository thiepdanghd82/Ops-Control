...
- **Fix**: new helper `isIndigoPrintType(printType)` in `client/src/services/printTypeUtils.js` doing `String(s).startsWith('Indigo')`. Wired into all 4 callsites. Sprint S-NEW-QUOTE-FIX (SHA: `<NEW_PR_SHA>`). Reproducer post-fix: `Indigo6800` ink → `layout_indigo_disp = 8`, `run_s = 0.003700` (was 0).
