...
#### MES-3-FIX-53 — Cpx tier MOQ/EAU/Price writes to wrong state slice
...
- **Data-integrity follow-up**: Cpx multi-tier quotes SAVED between 2026-05-11 (PR #41 ship) and this hotfix may have tier>0 price/eau/price_vnd/target/target_vnd silently missing from cplxState. Scope: any Cpx quote where operator typed values in MOQ2+ tier columns and saved. Audit + remediation (`SELECT id FROM quotes WHERE type='complex' AND … extra_moqs[*].price IS NULL`) is OUT OF SCOPE for this hotfix — file as separate follow-up after confirming production impact.
- **Effort**: S (~30 LOC reducer + ~150 LOC test + ticket); fix already shipped post-DIAGNOSE in same session.
- **Priority**: P0 closed by this ticket; data-integrity audit follow-up = P1 (post-hotfix prod data review before next save-heavy operator session).
