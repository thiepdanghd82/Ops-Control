...
- **Root cause analysis (partial, 2026-05-11)**: `CalcInks.jsx:161` gates CLICKS input on `ink.print_type === 'Indigo'` (per-row `print_type` field, NOT quote-level Design Process). The trap: operator sets quote-level Design Process to Indigo but each ink row's `print_type` defaults to something else → CLICKS disabled. Ink Setup/Run/Total = "—" portion may also be downstream of FIX-32 (calcInk needs `pitch > 0` which requires `sheet_length` + `min_gap_md` filled). Post-FIX-32 the residual scope is narrower — verify whether the per-row `print_type` cascade from quote-level Design Process is the only remaining gap.
- **Acceptance**: investigate Indigo ink calc path — (a) cascade quote-level Design Process → per-row `print_type` on add-ink-row OR document the manual step, (b) verify CLICKS becomes enterable after operator sets `print_type='Indigo'`, (c) trace why BASE MAT renders "300" instead of material code (joined-field display bug?). Re-test after FIX-32 ships — may auto-close.
- **Effort**: S-M (re-scope after FIX-32 verifies on hardware; ~50 LOC if cascade, ~10 LOC if docs-only)
- **Priority**: P2 (DOWNGRADED from P1 after FIX-32 closed the pitch=0 cascade; per-row `print_type` UX gap is real but operator can workaround via manual per-row dropdown)
