...
- **Priority**: **P0 CLOSED** 2026-06-16 (SHA: `6f3e0e8` via PR #155). Data-integrity follow-up below remains **OPEN**.
- **Data-integrity follow-up — OPEN (P1)**: Cpx multi-tier quotes SAVED between 2026-05-11 (PR #41 ship) and 2026-06-16 (this hotfix merge) may have tier>0 `price` / `eau` / `price_vnd` / `target` / `target_vnd` silently missing from `cplxState.extra_moqs[i]`. Scope: any Cpx quote where the operator typed values in MOQ2+ tier columns and saved within the 36-day window. Audit query (`SELECT id FROM quotes WHERE type='complex' AND json_extract(state,'$.num_moq') > 1 AND json_extract(state,'$.extra_moqs[0].price') IS NULL`) + remediation plan = OUT OF SCOPE for the hotfix itself. Owner: TBD. Must confirm production-data impact before any back-fill writes. File a separate ticket when production audit kicks off.
