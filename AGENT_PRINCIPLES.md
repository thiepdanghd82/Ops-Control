<!-- ============================================================
     Đặt file này ở thư mục gốc repo (cạnh CLAUDE.md).
     Cách nối vào CLAUDE.md (chọn 1):
       (A) Thêm 1 dòng ở đầu CLAUDE.md, ngay sau "# Ops Control — Agent Playbook":
               @AGENT_PRINCIPLES.md
           (dùng khi agent của bạn hỗ trợ cú pháp @import của Claude Code)
       (B) Nếu không chắc agent hỗ trợ @import: copy toàn bộ nội dung dưới đây
           dán vào đầu CLAUDE.md (sau dòng tiêu đề, trước blockquote sprint history).

     Nguồn tham khảo (đã chắt lọc, không copy nguyên):
       - Andrej Karpathy notes on LLM coding pitfalls (4 nguyên tắc gốc)
       - shanraisshan/claude-code-best-practice (trục Research→Plan→Execute→Review→Ship)
       - mattpocock/skills (các "working mode" + CONTEXT.md shared-language)
============================================================ -->
