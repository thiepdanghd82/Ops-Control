...
export default function ApprovalActionsCell({ quote, user, onAfterTransition }) {
  ...
  const runTransition = useCallback(async (action, reason) => {
    if (!quote?.id) {
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: '#fff', width: 480, maxWidth: '92vw',
          border: '1px solid #e0e0e0', boxShadow: '0 20px 40px rgba(0,0,0,0.18)',
        }}
      >
        <div style={{
          padding: '12px 18px', borderBottom: '1px solid #e0e0e0',
        ...
        <div style={{ padding: 18 }}>
          <div style={{ fontSize: 12, color: '#525252', marginBottom: 10 }}>
            Current status: <b>{currentStatus}</b>. Rejecting returns the quote
            to <b>rejected</b>; the cost engineer can edit and re-submit.
          </div>
          <label style={{ fontSize: 11, fontWeight: 600, color: '#525252', textTransform: 'uppercase', letterSpacing: 0.3 }}>
            Reason (required)
          </label>
          <textarea
            autoFocus
            ...
            style={{
              ...
              border: '1px solid #c6c6c6', fontFamily: 'inherit', fontSize: 12,
              resize: 'vertical', color: '#161616',
            }}
          />
        ...
        <div style={{
          padding: '10px 18px', borderTop: '1px solid #e0e0e0',
