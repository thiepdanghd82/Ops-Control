...
export default function CapacityPlanning() {
  ...
  const utilization = useMemo(() => {
    ...
    const activeOrders = orders.filter(
      (o) => o.status === 'Planned' || o.status === 'Released' || o.status === 'In Progress'
    );
    ...
    activeOrders.forEach((order) => {
      ...
      ops.forEach((op) => {
        ...
        if (wc && wcMap[wc]) {
          wcMap[wc].scheduledHours += totalHrs;
  ...
  const filtered =
    selectedWC === 'all' ? utilization : utilization.filter((u) => u.name === selectedWC);
    ...
    <div className="capacity-planning">
      <div className="tab-header">
        <div>
          ...
          <p className="tab-subtitle">
            Weekly view &bull;{' '}
            {usingFallback
              ? `Fallback ${DEFAULT_WEEKLY_CAPACITY}h/week (no finance data)`
              : 'Per-WC capacity from finance data'}
          </p>
        </div>
      ...
      {usingFallback && (
        <div className="cap-fallback-banner" role="alert">
          ⚠ Finance work-center hours unavailable — every WC is using the default{' '}
          {DEFAULT_WEEKLY_CAPACITY}h/week. Utilization figures below are approximate; configure
          finance hours in <strong>Library &rarr; Finance</strong> for accurate numbers.
        </div>
      )}
      ...
      <div className="cap-kpis">
        ...
        <div className="kpi-card kpi-ok">
          <span className="kpi-value">
            {utilization.filter((u) => u.scheduledHours > 0 && u.utilization <= 85).length}
          </span>
      ...
      <div className="cap-chart">
        {filtered.length === 0 ? (
          <EmptyState
            ...
            hint="Configure work centers in Library to see capacity."
          />
        ) : (
          filtered.map((wc) => (
            <div key={wc.name} className="cap-row">
              <div className="cap-label">
                ...
                <span className="cap-hours">
                  {wc.scheduledHours}h / {wc.capacity}h
                </span>
              </div>
