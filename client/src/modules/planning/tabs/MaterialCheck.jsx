      ...
      .map((item) => {
        ...
        const coverageRaw =
          item.totalRequired > 0
            ...
            : item.onHand > 0
              ? 9999
              : 100; // no demand + on-hand > 0 → effectively unlimited
  ...
  return (
    <div className="material-check">
      ...
      <div className="mc-kpis">
        ...
        <button
          className={kpiClass('ok') + ' kpi-ok'}
          onClick={setOrToggle('ok')}
        ...
        <button
          className={kpiClass('low') + ' kpi-low'}
          onClick={setOrToggle('low')}
        ...
        <button
          className={kpiClass('shortage') + ' kpi-shortage'}
          onClick={setOrToggle('shortage')}
          aria-pressed={filter === 'shortage'}
        >
          <span className="kpi-value">{stats.shortages}</span>
          <span className="kpi-label">Shortage</span>
        </button>
        <button
          className={kpiClass('over') + ' kpi-over'}
          onClick={setOrToggle('over')}
          aria-pressed={filter === 'over'}
        >
      ...
      <div className="orders-table-container">
        <table className="data-table">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan="8" className="mc-empty-cell">
                  <EmptyState
                    icon="🔍"
                  ...
                  <td>
                    <div className="coverage-bar">
                      ...
                      <span className="coverage-text">
                        {mat.coverage > 999 ? '∞' : `${mat.coverage}%`}
                      </span>
                    </div>
                  </td>
                  <td>
                    <span
                      className={`status-badge status-${mat.status.toLowerCase().replace(' ', '-')}`}
                    >
                      {mat.status}
                    </span>
                  </td>
