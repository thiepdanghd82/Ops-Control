  ...
  async function generateWorkOrders() {
    ...
    try {
      ...
      if (newWOs.length > 0) {
        ...
        if (statusFailedCount > 0) parts.push(`${statusFailedCount} parent order(s) still show Planned (refresh to retry)`);
    ...
    <div className="work-orders">
      <div className="tab-header">
        <div>
          ...
          <p className="tab-subtitle">{workOrders.length} work orders &bull; {orders.length} planned orders</p>
        </div>
      </div>
              ...
              <thead>
                <tr>
                  <th className="wo-col-check">
                    <input type="checkbox" checked={selectedOrders.size === orders.length && orders.length > 0} onChange={toggleAll} aria-label="Select all" />
                  </th>
                  <th>Order ID</th>
                  <th>Product</th>
                  <th>Qty</th>
              ...
              <tbody>
                {orders.map(o => (
                  <tr key={o.id} className={selectedOrders.has(o.id) ? 'selected' : ''}>
                    <td><input type="checkbox" checked={selectedOrders.has(o.id)} onChange={() => toggleOrder(o.id)} aria-label={`Select order ${o.id}`} /></td>
      ...
      <div className="orders-table-container">
        <table className="data-table">
          ...
          <tbody>
            {workOrders.length === 0 ? (
              <tr><td colSpan="7" className="wo-empty-cell">
                <EmptyState icon="📋" title="No work orders yet" hint="Select a planned order above and click Generate." />
              </td></tr>
            ) : workOrders.map(wo => (
              <tr key={wo.id}>
