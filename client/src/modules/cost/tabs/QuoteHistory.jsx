...
function SortTh({ children, id, className, style, sortKey, sortDir, toggleSort }) {
  ...
  return (
    <th className={`qh-col ${className||''} ${active ? 'qh-sort-on' : ''}`} style={style}
...
function fmtPrice(v) { return v == null || v === '' ? '—' : Number(v).toFixed(4).replace(/\.?0+$/, ''); }
...
export default function QuoteHistory() {
  ...
  const { data: rawQuotes, setData: setQuotes, loading, error: loadError, refresh } = useAbortableFetch(
  ...
  useEffect(() => {
    ...
    function handleKey(e) { if (e.key === 'Escape') setCtxMenu(null); }
  ...
  function handleContextMenu(e, quote) {
    ...
    const rect = e.currentTarget.closest('.qh-root')?.getBoundingClientRect() || { left: 0, top: 0 };
    setCtxMenu({
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      r = r.filter(q => {
        ...
        return [s.ccl_pn, s.direct_cu, s.project_name, s.end_cu || s.project, q.label, s.rfq_number, s.npi_owner, q.npi_owner, s.sale_owner, s.direct_cu_pn, s.end_cu_pn]
  ...
  function handleDelete(q) {
    ...
    if (!confirm(`Move quote #${q.id} — ${q.state?.ccl_pn || q.state?.rfq_number || ''} to Trash?\n\nYou can restore it later from the Trash tab.`)) return;
    costApi.trashQuote(q.id).then(() => {
  ...
  const openTrash = useCallback(() => {
    ...
    costApi.getTrashedQuotes().then(items => {
  ...
  const handleRestore = (q) => {
    if (!confirm(`Restore quote #${q.id} — ${q.state?.ccl_pn || q.state?.rfq_number || ''}?`)) return;
    costApi.restoreQuote(q.id).then(() => {
      refreshTrash();
      refresh();
    ...
    <div className="qh-root">
      ...
      <div className="qh-headerbar">
        <div className="qh-hb-icon">
          <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="white" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M3 3h18v4H3z"/><path d="M3 10h18v4H3z"/><path d="M3 17h18v4H3z"/></svg>
        </div>
        <div className="qh-hb-titleblock">
          ...
          <div className="qh-hb-meta">
            ...
            <span className="qh-hb-refresh-info" title={lastRefreshedAt ? `Last sync: ${lastRefreshedAt.toLocaleString('vi-VN')}` : 'Auto-refresh every 30s'}>
        ...
        <div className="qh-hb-search-wrap">
          <svg className="qh-si" viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="#94a3b8" strokeWidth="2.5" strokeLinecap="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
          <input className="qh-hb-search" placeholder="Search CCL PN / RFQ / Customer / Project / NPI Owner / Sale Owner…"
            value={search} onChange={e => { setSearch(e.target.value); setPage(0); }} />
          {search && <button className="qh-hb-sclear" onClick={() => { setSearch(''); setPage(0); }}>×</button>}
        </div>
        <div className="qh-hb-filters">
          {[{id:'all',label:'All'},{id:'standard',label:'◇ Standard'},{id:'complex',label:'◈ Complex'}].map(f => (
            <button key={f.id} className={`qh-hb-fbtn ${filterType===f.id?'active':''}`} onClick={() => { setFilterType(f.id); setPage(0); }}>
              {f.label}
            </button>
          ))}
        ...
        <div className="qh-table-flush">
          <table className="qh-table">
            <thead>
              ...
              <tr className="qh-g2">
                <th className="qh-col qh-c-num" style={{width:28}}>#</th>
                <SortTh {...sortProps} id="date" style={{width:88}}>{t('qh.date')}</SortTh>
                <SortTh {...sortProps} id="rfq" style={{width:120}}>{t('qh.rfq_number')}</SortTh>
                <th className="qh-col qh-c-ctr" style={{width:90}}>{t('qh.ver')}</th>
                <th className="qh-col qh-c-ctr" style={{width:32}}>{t('qh.ul')}</th>
                <SortTh {...sortProps} id="npi" style={{width:100}}>{t('qh.owner')}</SortTh>
                <SortTh {...sortProps} id="direct_cu" style={{width:100}}>{t('qh.direct_cu')}</SortTh>
                <SortTh {...sortProps} id="end_cu" style={{width:90}}>{t('qh.end_cu')}</SortTh>
                <SortTh {...sortProps} id="project" style={{width:90}}>{t('qh.project')}</SortTh>
                <SortTh {...sortProps} id="ifs" className="qh-c-ifs" style={{width:130}}>{t('qh.ifs_code')}</SortTh>
                <SortTh {...sortProps} id="dcu_pn" style={{width:110}}>{t('qh.direct_cu_pn')}</SortTh>
                <th className="qh-col" style={{width:90}}>{t('qh.end_cu_pn')}</th>
                <th className="qh-col" style={{width:85}}>{t('qh.size')}</th>
                <th className="qh-col" style={{width:110}}>{t('qh.materials')}</th>
                <th className="qh-col" style={{width:80}}>{t('qh.trade_mode')}</th>
                <th className="qh-col" style={{width:75}}>{t('qh.design')}</th>
                <SortTh {...sortProps} id="moq" className="qh-c-r" style={{width:75}}>{t('qh.moq')}</SortTh>
                <SortTh {...sortProps} id="sell" className="qh-c-r" style={{width:80}}>{t('qh.sell_price')}</SortTh>
                <SortTh {...sortProps} id="price_vnd" className="qh-c-r" style={{width:100}}>{t('qh.price_vnd')}</SortTh>
                <th className="qh-col qh-c-r" style={{width:65}}>{t('qh.target')}</th>
                <SortTh {...sortProps} id="va" className="qh-c-r" style={{width:50}}>{t('qh.va_pct')}</SortTh>
                <SortTh {...sortProps} id="contr" className="qh-c-r" style={{width:60}}>{t('qh.contr_pct')}</SortTh>
                <SortTh {...sortProps} id="gm" className="qh-c-ctr" style={{width:55}}>{t('qh.gm_pct')}</SortTh>
                <SortTh {...sortProps} id="status" style={{width:120}}>{t('qh.status')}</SortTh>
                <th className="qh-col qh-c-ctr" style={{width:170}}>{t('qh.approve_reject')}</th>
                <th className="qh-col qh-c-ctr" style={{width:90}}>{t('qh.layout')}</th>
              </tr>
            </thead>
            <tbody>
              {paged.length === 0 ? (
                <tr><td colSpan={26} style={{ padding: 0 }}>
                  <EmptyState
                    icon="📊"
                ...
                const matStr = Array.isArray(rawMats)
                  ? rawMats.slice(0,2).map(m => typeof m === 'object' ? (m.code || m.name || '') : String(m)).filter(Boolean).join(', ')
                  : '';
                ...
                const contrVal = r.contribution != null ? r.contribution
                  : r.contr_pct != null ? r.contr_pct / 100 : null;
                ...
                return (
                  <tr key={q.id||idx}
                    ...
                    <td className="qh-d qh-d-rfq" style={{ color: rfqColors[s.rfq_number] || undefined }}>{s.rfq_number || '—'}</td>
                    ...
                    <td className="qh-d qh-d-contr">{contrVal != null ? fmtPct(contrVal) : '—'}</td>
      ...
      {totalPages > 1 && (
        <div className="qh-pag">
          <button className="qh-pb" disabled={page===0} onClick={() => setPage(0)}>«</button>
          <button className="qh-pb" disabled={page===0} onClick={() => setPage(p => p-1)}>‹ Prev</button>
          <span className="qh-pi">Page {page+1} / {totalPages} <span className="qh-pr">({filtered.length} rows)</span></span>
          <button className="qh-pb" disabled={page>=totalPages-1} onClick={() => setPage(p => p+1)}>Next ›</button>
          <button className="qh-pb" disabled={page>=totalPages-1} onClick={() => setPage(totalPages-1)}>»</button>
        </div>
      )}
      ...
      <Modal open={!!trashModal} onClose={closeTrash} size="xl" severity="warning" ariaLabelledBy="qh-trash-title">
        <Modal.Header
          id="qh-trash-title"
          title="Trash"
        ...
        <Modal.Body className="flush">
          ...
          {trashModal && !trashModal.loading && (trashModal.items || []).length === 0 && (
            <div style={{ padding: 32, textAlign: 'center', color: '#6f6f6f', fontStyle: 'italic' }}>
              ...
              <tbody>
                {trashModal.items.map(q => (
                  <tr key={q.id} style={{ borderBottom: '1px solid #e8e8e8' }}>
                    ...
                    <td style={{ padding: '6px 10px' }}><b>{q.state?.ccl_pn || q.state?.rfq_number || q.label || '—'}</b></td>
        ...
        <Modal.Footer>
          <button className="op-btn op-btn-ghost" onClick={refreshTrash}>↻ Refresh</button>
          <button className="op-btn op-btn-primary" onClick={closeTrash}>Close</button>
        </Modal.Footer>
      </Modal>
      ...
      {ctxMenu && (
        <div ref={ctxRef} className="qh-ctx-menu" style={{ left: ctxMenu.x, top: ctxMenu.y }}>
          <div className="qh-ctx-header">
            Quote #{ctxMenu.quote.id} — {ctxMenu.quote.state?.ccl_pn || ctxMenu.quote.state?.rfq_number || 'Untitled'}
          </div>
          <div className="qh-ctx-divider" />
          <button className="qh-ctx-item" onClick={() => { handleLoad(ctxMenu.quote); setCtxMenu(null); }}>
          ...
          <button className="qh-ctx-item" onClick={() => { handleCopy(ctxMenu.quote); setCtxMenu(null); }}>
            <span className="qh-ctx-icon">📑</span>
            <span>Copy</span>
            ...
            {RFQ_COLOR_PALETTE.map(c => {
              ...
              return (
                <button
                  ...
                  title={c.label}
                  onClick={() => { if (rfq) setRfqColor(rfq, c.color); setCtxMenu(null); }}
                />
              );
            })}
            <button
              ...
              title="Clear color"
              onClick={() => { const rfq = ctxMenu.quote.state?.rfq_number; if (rfq) setRfqColor(rfq, null); setCtxMenu(null); }}
            >×</button>
          </div>
          <div className="qh-ctx-divider" />
          {canDelete && (
            <button className="qh-ctx-item qh-ctx-danger" onClick={() => {
