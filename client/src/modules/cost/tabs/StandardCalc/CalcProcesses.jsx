...
import { calcProcess, getActiveTierState, getProcessOptions, getWCOptionsByType, getRateByWC, getToolLife } from '../../../../services/calcEngine';
...
function fmtN(v, d = 4) { return _fmtN(v, d); }
...
export default function CalcProcesses() {
  ...
  const setSetupHActive = useCallback((procIdx, value) => {
    if (activeMoqIdx === 0) {
    ...
    arr[procIdx] = (value === '' || value == null) ? null
  ...
  const getSetupHActive = useCallback((proc, procIdx) => {
  ...
  const handleToolType = useCallback((idx, toolType) => {
      ...
      <div className="sc-card">
        <div className="sc-card-header sc-header-green">
          ...
          <div className="sc-header-totals">
            <span className="sc-header-total-item">Mach: <b>${fmtN(totals.setupMach + totals.runMach)}</b></span>
            <span className="sc-header-total-item">Labor: <b>${fmtN(totals.setupLabor + totals.runLabor)}</b></span>
            <span className="sc-header-total-item">Tool: <b>${fmtN(totals.tooling)}</b></span>
        ...
        <div className="sc-card-body sc-table-wrap">
          <table className="sc-table sc-proc-table">
            <thead>
              <tr>
                ...
                <th className="sc-col-derived" style={{ width: 60 }} title="Machine UPH from rate table">MC UPH</th>
                <th style={{ width: 60 }} title="Manual labor UPH (per person)">Man UPH</th>
                <th style={{ width: 70 }}>Tool Cost</th>
                <th style={{ width: 90 }}>Tool Type</th>
                <th style={{ width: 65 }}>Tool Life</th>
                <th className="sc-col-derived" style={{ width: 65 }} title="Production time in hours">Prod Time</th>
                <th className="sc-col-result" style={{ width: 65 }}>S.Mach</th>
                <th className="sc-col-result" style={{ width: 65 }}>S.Labor</th>
                <th className="sc-col-result" style={{ width: 65 }}>R.Mach</th>
                <th className="sc-col-result" style={{ width: 65 }}>R.Labor</th>
              ...
              {visibleProcs.map((proc, vi) => {
                ...
                return (
                  <tr key={proc._mid || `idx-${i}`}>
                    ...
                    <td>
                      <select value={proc.workcenter || ''} onChange={e => handleWorkcenter(i, e.target.value)} className="sc-input-sm sc-select-bare">
                        <option value="">--</option>
                        {wcOpts.map(w => <option key={w} value={w}>{w}</option>)}
                      </select>
                    </td>
                    <td><input type="number" min="1" value={proc.repeat ?? 1} onChange={e => handleField(i, 'repeat', e.target.value, true)} className="sc-input-sm sc-input-num" style={{ background: '#fef3c7' }} /></td>
                    <td className="sc-td-auto">{r ? r.crew || proc.crew || '\u2014' : proc.crew || '\u2014'}</td>
                    <td><input type="text" inputMode="numeric" value={proc.speed || ''} onChange={e => handleField(i, 'speed', e.target.value, true)} placeholder="—" className="sc-input-sm sc-input-num" /></td>
                    ...
                    /></td>
                    <td><input type="number" min="0" max="100" value={proc.efficiency != null ? Math.round(proc.efficiency * 100) : 85} onChange={e => handleField(i, 'efficiency', (parseLocaleNumber(e.target.value) || 0) / 100)} className="sc-input-sm sc-input-num" /></td>
                    {/* Sprint 1.6 — Setup H tracks active MOQ, amber when override. */}
                    <td><DecimalInput
                      value={getSetupHActive(proc, i)}
                      onChange={v => setSetupHActive(i, v)}
                      placeholder="—"
                      title={isSetupHOverride(i)
                        ? `MOQ ${activeMoqIdx + 1} override (base = ${proc.setup_h ?? 0})`
                        : (activeMoqIdx === 0 ? 'Base value (MOQ 1)' : `Inherits MOQ 1 base (${proc.setup_h ?? 0}) — type to override`)}
                      className={`sc-input-sm sc-input-num ${isSetupHOverride(i) ? 'sc-smt-inp-ovr' : ''}`}
                    /></td>
                    <td><input type="number" min="0" max="100" value={proc.scrap_pct != null ? Math.round(proc.scrap_pct * 100) : 3} onChange={e => handleField(i, 'scrap_pct', (parseLocaleNumber(e.target.value) || 0) / 100)} className="sc-input-sm sc-input-num" /></td>
                    <td className="sc-td-derived" style={{ color: '#1a4a8a', fontWeight: 700 }}>{r && r.uph ? Math.round(r.uph).toLocaleString() : '\u2014'}</td>
                    <td><DecimalInput value={proc.manual_uph} onChange={v => setProcessField(i, 'manual_uph', v)} placeholder="—" className="sc-input-sm sc-input-num" style={{ color: '#b45309' }} /></td>
                    <td><input type="text" inputMode="numeric" value={proc.tool_cost || ''} onChange={e => handleField(i, 'tool_cost', e.target.value, true)} placeholder="—" className="sc-input-sm sc-input-num" /></td>
                    <td>
                      <select value={proc.tool_type || ''} onChange={e => handleToolType(i, e.target.value)} className="sc-input-sm sc-select-bare">
                        <option value="">--</option>
                        {toolTypeOpts.map(t => <option key={t} value={t}>{t}</option>)}
                      </select>
                    </td>
                    <td><input type="text" inputMode="numeric" value={proc.tool_life || ''} onChange={e => handleField(i, 'tool_life', e.target.value, true)} placeholder="—" className="sc-input-sm sc-input-num" /></td>
                    <td className="sc-td-derived">{r && r.total_time ? (r.total_time / 60).toFixed(2) : '\u2014'}</td>
          ...
          <div className="sc-add-row">
            <button className="op-btn op-btn-tertiary" onClick={addRow}>+ Add Process Row</button>
          </div>
        </div>
      </div>
