...
function fmtN(v, d = 2) { return _fmtN(v, d); }
...
export default function ProcessBalancing() {
  ...
  const active = useMemo(() => {
    ...
    processes.forEach((p, i) => {
      ...
      list.push({
        i, visIdx, p, r, mcUPH, manUPH, baseCrew, adjCrew, canAdj,
        setupHrs, runHrsBefore, runHrsAfter, totalTimeBefore, totalHrsAfter,
        effUPH, ctSec, repeat,
      });
    });
    return list;
  ...
  const autoTarget = useMemo(() => {
    const machMin = withUPH.filter(x => x.mcUPH > 0).reduce((mn, x) => Math.min(mn, x.mcUPH), Infinity);
  ...
  const adjustCrew = useCallback((procIdx, delta) => {
    ...
    <div className="pb">
      ...
      <div className="pb-header">
        <div className="pb-header-left">
          ...
          <div>
            ...
            <div className="pb-header-sub">Bottleneck detection &middot; Crew simulation &middot; Line efficiency</div>
          </div>
        </div>
          ...
          <div className="pb-kpi pb-kpi-bn">
            ...
            <div>
              ...
              <div className="pb-kpi-val pb-kpi-val-bn">
                Process {bnProc?.visIdx} &mdash; {bnProc?.p.workcenter || bnProc?.p.process_type || '?'}
              </div>
            </div>
          </div>
          {/* Balance Eff */}
          <div className={`pb-kpi ${lineEff >= 80 ? 'pb-kpi-green' : lineEff >= 60 ? 'pb-kpi-amber' : 'pb-kpi-red'}`}>
        ...
        <table className="pb-table">
          <thead>
            <tr className="pb-group-row">
              <th colSpan={3} className="pb-group pb-group-info">PROCESS INFO</th>
              <th colSpan={2} className="pb-group pb-group-uph">THROUGHPUT (UPH)</th>
              <th colSpan={3} className="pb-group pb-group-time">TIME BEFORE ADJ (HRS) &larr; FROM PROCESS DB</th>
              <th colSpan={2} className="pb-group pb-group-crew">CREW SIMULATION</th>
              <th colSpan={2} className="pb-group pb-group-speed">LINE SPEED</th>
              <th colSpan={3} className="pb-group pb-group-analysis">ANALYSIS</th>
            </tr>
            <tr>
              <th className="pb-th">PROCESS</th>
              <th className="pb-th">TYPE</th>
              <th className="pb-th">WORKCENTER</th>
              <th className="pb-th right">MC UPH<br /><span className="pb-th-sub">MACHINE</span></th>
              <th className="pb-th right">MANUAL UPH<br /><span className="pb-th-sub">PER PERSON</span></th>
              <th className="pb-th right">SETUP<br /><span className="pb-th-sub">(HRS)</span></th>
              <th className="pb-th right">RUNNING<br /><span className="pb-th-sub">(HRS)</span></th>
              <th className="pb-th right">PROD TIME<br /><span className="pb-th-sub">BEFORE (HRS)</span></th>
              <th className="pb-th center">ADJ.<br /><span className="pb-th-sub">CREW</span></th>
              <th className="pb-th right">PROD TIME<br /><span className="pb-th-sub">AFTER (HRS)</span></th>
              <th className="pb-th right">EFF. UPH<br /><span className="pb-th-sub">WITH CREW</span></th>
              <th className="pb-th right">CYCLE TIME<br /><span className="pb-th-sub">PER PIECE</span></th>
              <th className="pb-th">LOAD <span className="pb-th-sub" style={{ fontWeight: 400 }}>| TARGET</span></th>
          ...
          <tbody>
            {active.map(proc => {
              ...
              const barPct = maxCT > 0 && proc.ctSec ? Math.min(100, (proc.ctSec / maxCT) * 100) : 0;
              const taktPct = targetCT && maxCT > 0 ? Math.min(100, (targetCT / maxCT) * 100) : null;
              const barColor = isBN ? '#ef4444' : isSlow ? '#f59e0b' : '#22c55e';
              const timeDelta = proc.totalTimeBefore > 0 ? proc.totalHrsAfter - proc.totalTimeBefore : 0;
              const showDelta = proc.canAdj && proc.adjCrew !== proc.baseCrew && Math.abs(timeDelta) > 0.001;
              ...
              if (proc.canAdj && effectiveTarget > 0 && proc.manUPH > 0) {
                ...
                if (delta > 0) { suggText = `+${delta} needed \u2192 ${req}`; suggColor = '#dc2626'; }
                else if (delta < 0) { suggText = `${delta} excess \u2192 ${req}`; suggColor = '#0369a1'; }
                else { suggText = '\u2713 Balanced'; suggColor = '#16a34a'; }
              }

              // CT display
              let ctDisp = '\u2014';
              if (proc.ctSec != null) {
                ctDisp = proc.ctSec >= 60
              ...
              return (
                <tr key={proc.i} className={isBN ? 'pb-row-bn' : isSlow ? 'pb-row-slow' : ''}>
                  ...
                  {/* UPH */}
                  <td className="pb-td-num" style={{ color: '#1a4a8a' }}>{proc.mcUPH || '\u2014'}</td>
                  <td className="pb-td-num" style={{ color: '#b45309' }}>{proc.manUPH || '\u2014'}</td>

                  {/* Time before */}
                  <td className="pb-td-num" style={{ background: '#faf5ff', color: '#6d28d9' }}>{fmtN(proc.setupHrs)}</td>
                  <td className="pb-td-num" style={{ background: '#f0fdfa', color: '#0e7490' }}>{fmtN(proc.runHrsBefore)}</td>
                  <td className="pb-td-num pb-td-bold" style={{ background: '#eff6ff', color: '#0369a1' }}>{fmtN(proc.totalTimeBefore)}</td>

                  {/* Crew simulation */}
                  <td className="pb-td-crew">
                    {proc.canAdj ? (
                      <div className="pb-crew-ctrl">
                        <button className="pb-crew-btn pb-crew-minus" onClick={() => adjustCrew(proc.i, -1)}>&minus;</button>
                        <span className="pb-crew-val">{proc.adjCrew}</span>
                        <button className="pb-crew-btn pb-crew-plus" onClick={() => adjustCrew(proc.i, 1)}>+</button>
                      </div>
                    ) : (
                  ...
                  {/* Prod time after */}
                  <td className="pb-td-num" style={{ background: ptAfterBg, fontWeight: 600, color: showDelta && timeDelta < 0 ? '#16a34a' : showDelta && timeDelta > 0 ? '#dc2626' : '#0369a1' }}>
                    {fmtN(proc.totalHrsAfter)}
                    {showDelta && (
                      <span className={`pb-delta ${timeDelta < 0 ? 'pb-delta-down' : 'pb-delta-up'}`}>
                        {timeDelta < 0 ? '\u25BC' : '\u25B2'}{Math.abs(timeDelta).toFixed(2)}
                      </span>
                    )}
                  </td>

                  {/* Line speed */}
                  <td className="pb-td-num" style={{ color: isBN ? '#dc2626' : '#0369a1', fontWeight: isBN ? 800 : 600 }}>
                    {proc.effUPH > 0 ? proc.effUPH.toLocaleString() : '\u2014'}
                  </td>
                  <td className="pb-td-num" style={{ color: '#374151', fontSize: 11 }}>{ctDisp}</td>

                  {/* Load bar */}
                  <td className="pb-td-load">
                    <div className="pb-bar-wrap">
                      <div className="pb-bar-fill" style={{ width: barPct + '%', background: barColor }} />
                      {taktPct != null && (
                        <div className="pb-bar-target" style={{ left: taktPct + '%' }} />
                      )}
                    </div>
                    <span className="pb-bar-text" style={{ color: barColor }}>{barPct > 0 ? barPct.toFixed(0) + '% of BN' : ''}</span>
                  </td>
      ...
      <div className="pb-footer">
        <div className="pb-footer-left">
          ...
          <b>Prod Time After</b> &mdash; recalculated when Adj. Crew changes (manual component scales by 1/crew). Simulation only &mdash; does <b>not</b> affect cost.
        </div>
