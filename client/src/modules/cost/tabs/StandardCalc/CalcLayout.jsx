...
export default function CalcLayout() {
  ...
  // S-LAYOUT-DEFAULT (2026-05-05) — open Print Design first; engineer
  // typically locks plate cylinder before fitting the die.
  ...
  const layoutPerSheet = useMemo(() => {
    try {
      return calcLayoutPerSheet(st);
    } catch {
      return 0;
    }
  }, [st]);
  ...
  const validation = useMemo(() => {
    try {
      return validateLayout(st, st.materials, {}, activeProfile);
    } catch {
            ...
            <div className="cl-press-picker">
              <span className="cl-press-meta">
                ...
                {activeProfile.common_dies?.length
                  ...
                  : ''}
                {activeProfile.speed_max_m_min ? ` · ${activeProfile.speed_max_m_min} m/min` : ''}
              </span>
            </div>
          )}
          ...
          <div className="cl-top-grid">
            ...
            {activeSub === 'print' ? (
              ...
              <>
                ...
                <CutLayoutSummary state={st} pitch={pitch} layoutPerSheet={layoutPerSheet} />
              </>
            )}
...
export function AdvancedLayoutBlock({ state, onField }) {
  const hasNonDefault = !!(
    ...
    state.unwind_direction ||
    state.print_direction_md ||
    state.include_reg_marks ||
    state.plate_thickness_mm ||
    state.anilox_bcm ||
        ...
        <div className="cl-adv-body">
          <div className="cl-adv-group">
            ...
            <div className="sc-grid4">
              <div
                className="sc-field"
                title="Compound die multiplier — e.g. 2 for a 2-up die (one cavity cuts 2 labels). Default = 1."
              >
                <label>Parts per die</label>
                <DecimalInput
                  value={state.parts_per_die}
                  onChange={(v) => onField('parts_per_die', v)}
                  className="sc-input"
                />
              </div>
              <div
                className="sc-field"
                title="If enabled, optimizer will NOT try 90° rotated placement (e.g. when text direction must match roll unwind)."
              >
                <label>
                  <input
                    type="checkbox"
                    checked={!!state.orient_locked}
              ...
              <div
                className="sc-field"
                title="Added to effective MD gap when a perforation cut is present between repeat labels."
              >
                <label>Perf offset (mm)</label>
                <DecimalInput
                  value={state.perf_offset_mm}
                  onChange={(v) => onField('perf_offset_mm', v)}
                  className="sc-input"
                />
              </div>
              <div
                className="sc-field"
                title="Customer-specified target pcs/roll. Optimizer grants a small score bonus when pcs_per_shot divides this evenly."
              >
                <label>Target pcs/roll</label>
                <DecimalInput
                  value={state.target_pcs_per_roll}
                  onChange={(v) => onField('target_pcs_per_roll', v)}
                  className="sc-input"
                />
              </div>
            </div>
          ...
          <div className="cl-adv-group">
            ...
            <div className="sc-grid4">
              <div
                className="sc-field"
                title="Print-to-Cut tolerance. Validator warns if min_gap_md < 2 × this."
              >
                <label>Tol P2C (mm)</label>
                <DecimalInput
                  value={state.tol_p2c_mm}
                  onChange={(v) => onField('tol_p2c_mm', v)}
                  className="sc-input"
                />
              </div>
              <div
                className="sc-field"
                title="Cut-to-Cut tolerance. Validator warns if min_gap_td < 2 × this."
              >
                <label>Tol C2C (mm)</label>
                <DecimalInput
                  value={state.tol_c2c_mm}
                  onChange={(v) => onField('tol_c2c_mm', v)}
                  className="sc-input"
                />
              </div>
              <div
                className="sc-field"
                title="Slitting tolerance. Validator warns if edge_margin_td < 3 × this."
              >
                <label>Tol Slit (mm)</label>
                <DecimalInput
                  value={state.tol_slit_mm}
                  onChange={(v) => onField('tol_slit_mm', v)}
                  className="sc-input"
                />
              </div>
            </div>
          ...
          <div className="cl-adv-group">
            <div className="cl-adv-group-title">
              Orientation + registration · Hướng cuộn + mark đăng ký
            </div>
            <div className="sc-grid4">
              <div
                className="sc-field"
                title="Unwind direction: face-in (label facing inward on roll) or face-out (label facing outward). Affects labeler feed + packaging."
              >
                <label>Unwind Direction</label>
                <select
                  value={state.unwind_direction || ''}
              ...
              <div
                className="sc-field"
                title="Print direction in MD — head-first or tail-first. Determines text orientation relative to unwind."
              >
                <label>Print Direction MD</label>
                <select
                  value={state.print_direction_md || ''}
              ...
              <div
                className="sc-field"
                title="Check if a registration mark / color bar strip must sit at the web edge (typical 5mm). Edge margin will need to accommodate."
              >
                <label>
                  <input
                    type="checkbox"
                    checked={!!state.include_reg_marks}
              ...
              <div
                className="sc-field"
                title="Width of reg-mark / color-bar strip when enabled. Default = 5 mm."
              >
                <label>Reg Mark Width (mm)</label>
                <DecimalInput
                  value={state.reg_mark_width_mm}
          ...
          <div className="cl-adv-group">
            ...
            <div className="sc-grid4">
              <div
                className="sc-field"
                title="Flexo plate thickness in mm. Common: 1.14 (DFM / HD plate), 1.70 (narrow-web rotary). Informational."
              >
                <label>Plate Thickness (mm)</label>
                <select
                  value={state.plate_thickness_mm || 0}
              ...
              <div
                className="sc-field"
                title="Anilox BCM (billion cubic microns). Determines ink lay-down: 2-3 for white/line work, 4-6 for CMYK process, 6-10 for opaque spot."
              >
                <label>Anilox BCM</label>
                <DecimalInput
                  value={state.anilox_bcm}
              ...
              <div
                className="sc-field"
                title="Print-to-cut MD offset (distance in mm between print cylinder center and cutting cylinder center on the press). Used in layout drawings, not pricing. Typical 50-200mm."
              >
                <label>Print→Cut MD offset</label>
                <DecimalInput
                  value={state.print_to_cut_offset_mm}
      ...
      <div className="cl-pc-shared">
        ...
        <div className="sc-grid4">
          <div className="sc-field">
            <label>Web Width TD (mm)</label>
            <DecimalInput
              value={state.web_width_td}
              onChange={(v) => onField('web_width_td', v)}
              className="sc-input"
              placeholder="—"
            />
          </div>
          <div className="sc-field">
            <label>Sheet Length MD (mm)</label>
            <DecimalInput
              value={state.sheet_length}
              onChange={(v) => onField('sheet_length', v)}
              className="sc-input"
              placeholder="—"
            />
          </div>
          <div className="sc-field">
            <label># Webs</label>
            <DecimalInput
              value={state.num_webs}
              onChange={(v) => onField('num_webs', v)}
              className="sc-input"
              placeholder="1"
            />
          </div>
          <div className="sc-field">
            <label>Rotary Cols</label>
            <DecimalInput
              value={state.rotary_cols}
              onChange={(v) => onField('rotary_cols', v)}
              className="sc-input"
              placeholder="0 sheet"
            />
          </div>
          <div
            className="sc-field"
            title="Distance from LEFT liner edge to first product (0 = symmetric default)"
          >
            <label>Edge Left (mm)</label>
            <DecimalInput
              value={state.edge_margin_td_left}
              onChange={(v) => onField('edge_margin_td_left', v)}
              className="sc-input"
              placeholder="0 sym"
            />
          </div>
          <div
            className="sc-field"
            title="Distance from RIGHT liner edge to first product (0 = symmetric default)"
          >
            <label>Edge Right (mm)</label>
            <DecimalInput
              value={state.edge_margin_td_right}
              onChange={(v) => onField('edge_margin_td_right', v)}
              className="sc-input"
              placeholder="0 sym"
            />
          </div>
          <div className="sc-field">
            <label>Pcs/Roll</label>
            <DecimalInput
              value={state.pcs_per_roll}
              onChange={(v) => onField('pcs_per_roll', v)}
              className="sc-input"
              placeholder="auto"
            />
          </div>
  ...
  return (
    <div className="cl-pc-body">
      <div className="cl-pc-hint">
        ...
        <b> ② Image Area</b> = safe design boundary (what holds text/logo) ·<b> ③ Bleed</b> = ink
        overshoot past the cut line (typically 1 mm).
        <br />
      ...
      <div className="sc-grid4">
        <div
          className="sc-field"
          title="① Finished product width TD (mm) the print designer is targeting. Should equal the Cut tab's Die Width TD; the validator above flags any divergence."
        >
        ...
        <div
          className="sc-field"
          title="① Finished product length MD (mm) the print designer is targeting. Should equal the Cut tab's Die Length MD."
        >
        ...
        <div
          className="sc-field"
          title="② Image Area Width TD — the SAFE DESIGN BOUNDARY where text, logo, barcode must sit. Smaller than Product Size by 2×bleed."
        >
          <label>② Image Area Width TD (mm)</label>
          <DecimalInput
            value={state.part_net_width}
            onChange={(v) => onField('part_net_width', v)}
            className="sc-input"
          />
        </div>
        <div className="sc-field" title="② Image Area Length MD — safe design boundary in MD.">
          ...
          <DecimalInput
            value={state.part_net_length}
            onChange={(v) => onField('part_net_length', v)}
            className="sc-input"
          />
        </div>
        <div
          className="sc-field"
          title="③ Bleed each side TD (mm) — ink overshoot past the cut line. Protects against die registration drift. Product = Image Area + 2×Bleed."
        >
          <label>③ Bleed TD (mm)</label>
          <DecimalInput
            value={state.bleed_td_mm}
            onChange={(v) => onField('bleed_td_mm', v)}
            className="sc-input"
          />
        </div>
        <div className="sc-field" title="③ Bleed each side MD (mm).">
          ...
          <DecimalInput
            value={state.bleed_md_mm}
            onChange={(v) => onField('bleed_md_mm', v)}
            className="sc-input"
          />
        </div>
        <div
          className="sc-field"
          title="Part rows in the MD (running) direction. Shared with Cutting tab."
        >
          <label>Parts in MD</label>
          <DecimalInput
            value={state.parts_in_md}
            onChange={(v) => onField('parts_in_md', v)}
            className="sc-input"
          />
        </div>
        <div
          className="sc-field"
          title={`Cavities per cut lane (TD). ${slit ? `Slit ON → print web has ${slitCount} × ${partsAcross} = ${printCavAcross} cav across; each slit lane gets ${partsAcross}.` : 'Single-web: print cavities = cut cavities.'} Shared with Cutting tab.`}
        >
          <label>Cut Parts/Web Across</label>
          <DecimalInput
            value={state.parts_web_across}
            onChange={(v) => onField('parts_web_across', v)}
            className="sc-input"
          />
        </div>
        <div
        ...
        <div
          className="sc-field"
          title="Minimum gap between parts along MD (mm). Shared with Cutting tab. Display padded to 3 decimals so synced engineering values (e.g. 7.225) keep their precision visible."
        >
          <label>Min Gap MD (mm)</label>
          <DecimalInput
            value={state.min_gap_md}
            onChange={(v) => onField('min_gap_md', v)}
            className="sc-input"
            decimals={3}
          />
        </div>
        <div
          className="sc-field"
          title="Minimum gap between parts along TD (across web). Only (cavities − 1) gaps exist, not × cavities. Default = 3 mm if blank. Shared with Cutting tab."
        >
          <label>Min Gap TD (mm)</label>
          <DecimalInput
            value={state.min_gap_td}
            onChange={(v) => onField('min_gap_td', v)}
            className="sc-input"
        ...
        <div
          className="sc-field"
          title="Plate mounting cylinder tooth count. Leave 0 to let the optimizer/magnetic tooth drive pitch."
        >
          <label>Plate Tooth (T)</label>
          <DecimalInput
            value={state.plate_tooth}
            onChange={(v) => onField('plate_tooth', v)}
            className="sc-input"
          />
        </div>
        ...
        <div
          className="sc-field"
          title="Tổng số màu in (CMYK, spot, white, UV...). Validator warn nếu vượt số trạm in của Cut Press đã chọn."
        >
          <label>Color Count</label>
          <DecimalInput
            value={state.color_count}
            onChange={(v) => onField('color_count', v)}
            className="sc-input"
            placeholder="e.g. 4"
          />
        </div>
        <div
          className="sc-field"
          title="Print-to-print (color registration) tolerance. Chuẩn: flexo ≤ 0.1mm, HP Indigo ≤ 0.05mm. Validator check bleed ≥ 2× tol khi color_count ≥ 2."
        >
          <label>Tol P2P · Color Reg (mm)</label>
          <DecimalInput
            value={state.tol_p2p_mm}
      ...
      {slit && (
        <div className="cl-pc-banner cl-pc-banner--info">
          ℹ Slit workflow ON — Print web in <b>{printCavAcross}</b> cav × {partsInMd} MD ={' '}
          <b>{printTotal}</b> pcs/print-repeat · slit {slitCount} lanes · mỗi lane cắt riêng (xem
          Cutter Cavity ở tab Cut).
        </div>
      )}
...
function CutSubTab({ state, onField }) {
  ...
  const edgeSym = Number(state.edge_margin_td) || 3;
  const edgesTotal = edgeLeft > 0 || edgeRight > 0 ? edgeLeft + edgeRight : 2 * edgeSym;
  const webNeededTd =
    partW > 0 ? partsAcross * partW + Math.max(0, partsAcross - 1) * gapTdVal + edgesTotal : 0;
  ...
  const cutterCavity =
    Number(state.cutter_cavity) > 0 ? Number(state.cutter_cavity) : cutterCavityAuto;
    ...
    <div className="cl-pc-body">
      ...
      <div className="sc-grid4">
        <div className="sc-field" title="Die outer width TD (mm) — total cut area.">
          ...
          <DecimalInput
            value={state.part_width}
            onChange={(v) => onField('part_width', v)}
            className="sc-input"
          />
        </div>
        <div className="sc-field" title="Die outer length MD (mm).">
          ...
          <DecimalInput
            value={state.part_length_md}
            onChange={(v) => onField('part_length_md', v)}
            className="sc-input"
          />
        </div>
        <div className="sc-field">
        ...
        <div
          className="sc-field"
          title="Khoảng cách tối thiểu giữa 2 part theo MD (chiều chạy). Dùng trong công thức pitch: pitch = part_length_md + min_gap_md. Hiển thị 3 chữ số sau dấu phẩy để giữ nguyên độ chính xác từ Design Tools sync (vd 7.225)."
        >
          <label>Min Gap MD (mm)</label>
          <DecimalInput
            value={state.min_gap_md}
        ...
        <div
          className="sc-field"
          title="Khoảng cách tối thiểu giữa 2 part theo TD (ngang web). Chỉ cộng vào (cavities − 1) khoảng, không cộng vào edge. Default = 3 mm nếu để trống."
        >
          <label>Min Gap TD (mm)</label>
          <DecimalInput
            value={state.min_gap_td}
        ...
        <div
          className="sc-field"
          title="Perforation offset added to effective MD gap if a perf line runs between labels."
        >
          <label>Perf Offset (mm)</label>
          <DecimalInput
            value={state.perf_offset_mm}
        ...
        <div
          className="sc-field"
          title="Kiss = face only, Through = cut liner too, Both = compound die, Perf-only = perforation, Emboss = raised/recessed (no through-cut)."
        >
          <label>Cut Type</label>
          <select
            value={state.cut_type || ''}
            onChange={(e) => onField('cut_type', e.target.value)}
            className="sc-input"
          >
        ...
        <div
          className="sc-field"
          title="Die corner radius in mm. 0 = square. Min 0.5mm (magnetic die physical limit). Max typically part_width / 4."
        >
          <label>Corner Radius (mm)</label>
          <DecimalInput
            value={state.corner_radius_mm}
            onChange={(v) => onField('corner_radius_mm', v)}
            className="sc-input"
          />
        </div>
        <div
          className="sc-field"
          title="Clearance around the die for registration marks, bearer bars, vision fiducials. Typical: 5-8mm on rotary. 0 = no quiet zone."
        >
          <label>Die Quiet Zone (mm)</label>
          <DecimalInput
            value={state.die_quiet_zone_mm}
            onChange={(v) => onField('die_quiet_zone_mm', v)}
            className="sc-input"
            placeholder="5-8 typ."
          />
        </div>
        <div
          className="sc-field"
          title="Magnetic die-cut cylinder tooth count. Leave 0 for optimizer to pick."
        >
          <label>Magnetic Tooth (T)</label>
          <DecimalInput
            value={state.magnetic_tooth}
            onChange={(v) => onField('magnetic_tooth', v)}
            className="sc-input"
          />
        </div>
      ...
      {partW > 0 && (
        <div
          className={`cl-pc-banner ${webDeltaTd < 0 ? 'cl-pc-banner--err' : webDeltaTd < 5 ? 'cl-pc-banner--warn' : 'cl-pc-banner--ok'}`}
        >
          <span>
            <b>{webDeltaTd < 0 ? '✕' : webDeltaTd < 5 ? '⚠' : '✓'} Web Width TD required:</b>{' '}
            <code>
              {partsAcross} × {partW} + ({partsAcross} − 1) × {gapTdVal} + ({edgeLeft || edgeSym} +{' '}
              {edgeRight || edgeSym}) = <b>{webNeededTd.toFixed(2)} mm</b>
            </code>
            <span className="cl-pc-banner--md">
              Web hiện tại: <b>{webHaveTd} mm</b>{' '}
              {webDeltaTd < 0
      ...
      <div className="cl-pc-shared cl-pc-shared--spaced">
        <div className="cl-pc-shared-title">
          Slit workflow + Cutter cavity · Slit & số cavity mỗi dập
        </div>
        <div className="sc-grid4">
          <div
            className="sc-field"
            title="Check if the press prints a WIDE web that is then slit into multiple narrower webs before die-cutting."
          >
            <label>
              <input
                type="checkbox"
          ...
          <div
            className="sc-field"
            title="Number of narrower lanes produced by the slitter. Each lane typically gets its own cutter."
          >
            <label>Slit Lanes</label>
            <DecimalInput
              value={state.slit_lane_count}
              onChange={(v) => onField('slit_lane_count', v)}
              disabled={!slit}
              className="sc-input"
            />
          </div>
          <div
            className="sc-field"
            title="Derived: total cavities across the WIDE print web = slit_lanes × parts_web_across (when slit) or num_webs × parts_web_across (no slit)."
          >
          ...
          >
            <label>
              Cutter Cavity
              {Number(state.cutter_cavity) === 0 ? ` (auto = ${cutterCavityAuto})` : ''}
            </label>
            <DecimalInput
              value={state.cutter_cavity}
              onChange={(v) => onField('cutter_cavity', v)}
              className="sc-input"
              style={
                Number(state.cutter_cavity) > 0 ? { color: '#b45309', fontWeight: 700 } : undefined
              }
            />
          </div>
          <div
          ...
          >
            ...
            <input
              type="text"
              value={cutTotalPerShot}
              disabled
              className="sc-input sc-derived"
              style={
                slit ? { color: '#6d28d9', fontWeight: 700, background: '#f5f3ff' } : undefined
              }
            />
          </div>
          <div
            className="sc-field"
            title="Override min slit-lane width in mm. Defaults: 25mm (general RDC), 30mm (Brotech). Validator errors when lane < this value. 0 = use industry default 25mm."
          >
            <label>Min Slit Lane (mm)</label>
            <DecimalInput
              value={state.min_slit_lane_width_mm}
          ...
          <div className="cl-pc-banner cl-pc-banner--info">
            <span>
              ...
              <span className="cl-pc-banner--md">
                ...
                {cutterCavityAuto}/lane)
              </span>
              <span className="cl-pc-banner--md">
                • <b>Cut</b> mỗi cutter dập ra <b>{cutterCavity}</b> parts/shot
                {Number(state.cutter_cavity) > 0 ? ' (đã override)' : ' (= auto)'}
              </span>
              <span className="cl-pc-banner--md">
                • Chạy {slitCount} cutter song song → tổng throughput = {slitCount} × {cutterCavity}{' '}
                = {slitCount * cutterCavity} parts/stamp-cycle
              </span>
            </span>
          </div>
        )}
        {!slit && numWebs > 1 && (
          <div className="cl-pc-banner cl-pc-banner--ok">
            ℹ In-line ({numWebs} webs) — cùng 1 máy cut đồng thời tất cả web: Cut Total = {numWebs}{' '}
            × {cutterCavity} = {cutTotalPerShot} parts/shot.
          </div>
        )}
      </div>
      <PrintCutRatioHint state={state} />
...
function PrintCutSizeMismatch({ state, onField }) {
  ...
  if (!printSet && cutSet) {
    return (
      <div className="cl-pc-banner cl-pc-banner--warn">
        <span>
          ⚠ Print product size not set. Cut tab has{' '}
          <b>
            {cutW}×{cutL}
          </b>{' '}
          mm.
        ...
        <span className="cl-pc-banner-actions">
          <button type="button" onClick={syncFromCut} className="sc-inline-btn">
            Sync from Cut
          </button>
        </span>
      </div>
  ...
  if (printSet && !cutSet) {
    return (
      <div className="cl-pc-banner cl-pc-banner--warn">
        <span>
          ⚠ Cut tab's Die Size not set. Print has{' '}
          <b>
            {prW}×{prL}
          </b>{' '}
          mm.
        ...
        <span className="cl-pc-banner-actions">
          <button type="button" onClick={syncToCut} className="sc-inline-btn">
            Sync to Cut
          </button>
        </span>
      </div>
  ...
  if (same) {
    return (
      <div className="cl-pc-banner cl-pc-banner--ok">
        ✓ Print product size matches Cut die size (
        <b>
          {cutW}×{cutL}
        </b>{' '}
        mm).
  ...
  return (
    <div className="cl-pc-banner cl-pc-banner--err">
      <span>
        ⚠ <b>Print vs Cut product-size mismatch.</b> Print:{' '}
        <b>
          {prW}×{prL}
        </b>{' '}
        mm · Cut die:{' '}
        ...
        mm. Likely a documentation mistake — the die-cut output should match the print designer's
        target.
      </span>
      <span className="cl-pc-banner-actions">
        <button type="button" onClick={syncFromCut} className="sc-inline-btn">
...
function PrintCutRatioHint({ state }) {
  ...
  const inv = mt / pt;
  const isInteger =
    Math.abs(ratio - Math.round(ratio)) < 0.02 || Math.abs(inv - Math.round(inv)) < 0.02;
  const ratioText =
    ratio > 1
  ...
  return (
    <div className="cl-pc-ratio cl-pc-ratio-warn">
      ⚠ Non-integer ratio {ratio.toFixed(3)} ({pt}T / {mt}T) — cylinders will drift out of
      registration. Pick a tooth pair that is an integer multiple (1:1, 2:1, 3:1, …).
    </div>
  );
}
  ...
  return (
    <div className="cl-summary">
      ...
      <div className="cl-sum-row">
        ...
        <b>
          {pitchDisplay}
          {pitchHint && <span className="cl-sum-ok">{pitchHint.replace(/^ · /, '')}</span>}
        </b>
      </div>
      <div className="cl-sum-row">
        <span>Print Cav Across</span>
        <b>
          {printCavAcross}
          {slit
            ? ` (${slitCount}×${partsAcross})`
            : numWebs > 1
              ? ` (${numWebs}×${partsAcross})`
              : ''}
        </b>
        ...
        <>
          ...
          {plateMismatch && (
            <div className="cl-sum-mismatch">
              ...
              <b>
                {plateMismatch.expectedT}T → {plateMismatch.expectedMm} mm
              </b>
            </div>
          )}
        </>
...
export function CutLayoutSummary({ state, pitch, layoutPerSheet }) {
  const sizeStr =
    state.part_width && state.part_length_md
  ...
  const partsAcross = Math.max(1, Number(state.parts_web_across) || 1);
  const partsInMd = Math.max(1, Number(state.parts_in_md) || 1);
  const printCavAcross = slit
    ? slitCount * partsAcross
    : Math.max(1, Number(state.num_webs) || 1) * partsAcross;
  const cutterCavityAuto = partsAcross * partsInMd;
  const cutterCavity =
    Number(state.cutter_cavity) > 0 ? Number(state.cutter_cavity) : cutterCavityAuto;
  ...
  return (
    <div className="cl-summary">
      ...
      <div className="cl-sum-row">
        <span>Pitch (mm)</span>
        <b>{pitch > 0 ? pitch.toFixed(2) : '—'}</b>
      </div>
      <div className="cl-sum-row">
        <span>Magnetic Pitch</span>
        <b>{magPitch}</b>
      </div>
      {slit && (
        <>
          ...
          <div className="cl-sum-row">
            <span>Slit Lanes</span>
            <b>{slitCount}</b>
          </div>
        </>
      )}
      <div className="cl-sum-row">
        <span>Cutter Cavity</span>
        <b>
          {cutterCavity}
          {Number(state.cutter_cavity) > 0 ? ' *' : ''}
        </b>
      </div>
      <div className="cl-sum-row">
        <span>Layout/Sheet</span>
        <b>{layoutPerSheet > 0 ? layoutPerSheet : '—'}</b>
      </div>
      <div className="cl-sum-row">
        <span>Webs</span>
        <b>{state.num_webs || '—'}</b>
      ...
      <div className="cl-sum-row">
        <span>Pcs/Roll</span>
        <b>{pcsRollEntered > 0 ? pcsRollEntered.toLocaleString() : '—'}</b>
      </div>
    </div>
  );
}
...
export function MdTdLegendCard() {
  ...
  const toggle = () => {
    setOpen((prev) => {
      const next = !prev;
      try {
        localStorage.setItem('ops_mdtd_open', next ? '1' : '0');
      } catch {
  ...
  return (
    <div className={`cl-mdtd-card ${open ? 'is-open' : 'is-collapsed'}`}>
      ...
      >
        <span className="cl-mdtd-icon" aria-hidden="true">
          🧭
        </span>
          ...
          <div className="cl-mdtd-grid">
            <div className="cl-mdtd-col">
              ...
              <div className="cl-mdtd-en">
                <b>Machine Direction</b> — the direction the web travels through the press as the
              ...
              <div className="cl-mdtd-fields">
                ...
                <code>sheet_length</code>, <code>min_gap_md</code>,<code>part_length_md</code>,{' '}
                <code>bleed_md_mm</code>,<code>perf_offset_mm</code>, <code>parts_in_md</code>
              </div>
            </div>
            <div className="cl-mdtd-col">
              ...
              <div className="cl-mdtd-fields">
                <span className="cl-mdtd-label">Fields · Các trường:</span>
                <code>web_width_td</code>, <code>part_width</code>,<code>bleed_td_mm</code>,{' '}
                <code>edge_margin_td_left</code>,<code>edge_margin_td_right</code>,{' '}
                <code>parts_web_across</code>
              </div>
            </div>
          </div>
          <div className="cl-mdtd-viz-grid">
            <div className="cl-mdtd-viz-col">
              <div className="cl-mdtd-viz-cap">General axes · Trục tổng quát</div>
