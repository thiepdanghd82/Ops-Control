...
export default function CalcHeader() {
    ...
    (idx, field, value) => {
      ...
      if (rate > 0 && typeof v === 'number') {
        ...
        if (mirrorField)
          dispatch({
            type: 'SET_EXTRA_MOQ',
            payload: { idx, field: mirrorField, value: mirrorVal },
          });
      }
    },
  ...
  // PR #C (Sprint S-ALT-MAT) — material-row tier override field name
  // routes through state.materials_active. SET_MATERIAL_FIELD writes
  // directly to a row in the active set via the reducer (PR #A mirror
  // invariant); the per-tier override field is _alt-suffixed when in
  // alt mode so calc engine picks the matching variant.
  const stMatActive = st.materials_active === 'alt' ? 'alt' : 'main';
  const tierLmField = stMatActive === 'alt' ? 'mat_setup_lm_alt' : 'mat_setup_lm';

  const setMatSetupForTier = useCallback(
    (tierIdx, matIdx, value) => {
      // tierIdx 0 = base (writes mat.setup_lm via mirror-aware reducer);
      // 1+ = override in extra_moqs[tierIdx-1].mat_setup_lm[_alt]
      if (tierIdx === 0) {
        dispatch({
          type: 'SET_MATERIAL_FIELD',
          payload: { idx: matIdx, field: 'setup_lm', value: coerceBase(value) },
        });
        return;
      }
      const ei = tierIdx - 1;
    ...
    [dispatch, st.extra_moqs, tierLmField]
  );

  const setProcSetupForTier = useCallback(
    (tierIdx, procIdx, value) => {
      if (tierIdx === 0) {
        dispatch({
          type: 'SET_PROCESS_FIELD',
          payload: { idx: procIdx, field: 'setup_h', value: coerceBase(value) },
        });
        return;
      }
      const ei = tierIdx - 1;
              ...
              <table className="sc-moq-table">
                ...
                <tbody>
                  ...
                  <tr className={`sc-moq-tr ${(st.active_moq_idx || 0) === 0 ? 'active' : ''}`}>
                    ...
                    <td>
                      <DecimalInput
                        value={st.selling_price}
                        onChange={(v) => setPriceUsd('selling_price', 'selling_price_vnd', v)}
                        className="sc-moq-inp sc-moq-inp-gold"
                      />
                    </td>
                    <td>
                      <DecimalInput
                        value={st.selling_price_vnd}
                        onChange={(v) => setPriceVnd('selling_price_vnd', 'selling_price', v)}
                        className="sc-moq-inp sc-moq-inp-gold"
                        thousandSep
                      />
                    </td>
                    <td>
                      <DecimalInput
                        value={st.target}
                        onChange={(v) => setPriceUsd('target', 'target_vnd', v)}
                        className="sc-moq-inp"
                      />
                    </td>
                    <td>
                      <DecimalInput
                        value={st.target_vnd}
                        onChange={(v) => setPriceVnd('target_vnd', 'target', v)}
                        className="sc-moq-inp"
                        thousandSep
                      />
                    </td>
                  ...
                  {(st.extra_moqs || []).map((em, idx) => (
                    <tr
                      key={idx}
                      className={`sc-moq-tr ${(st.active_moq_idx || 0) === idx + 1 ? 'active' : ''}`}
                    >
                      <td className="sc-moq-td-tier">
                        <label className="sc-moq-radio">
                          <input
                      ...
                      <td>
                        <DecimalInput
                          value={em.moq}
                          onChange={(v) => updateExtraMoq(idx, 'moq', v)}
                          className="sc-moq-inp"
                          thousandSep
                        />
                      </td>
                      <td>
                        <DecimalInput
                          value={em.eau}
                          onChange={(v) => updateExtraMoq(idx, 'eau', v)}
                          className="sc-moq-inp"
                          thousandSep
                        />
                      </td>
                      <td>
                        <DecimalInput
                          value={em.price}
                          onChange={(v) => updateExtraMoq(idx, 'price', v)}
                          className="sc-moq-inp sc-moq-inp-gold"
                        />
                      </td>
                      <td>
                        <DecimalInput
                          value={em.price_vnd}
                          onChange={(v) => updateExtraMoq(idx, 'price_vnd', v)}
                          className="sc-moq-inp sc-moq-inp-gold"
                          thousandSep
                        />
                      </td>
                      <td>
                        <DecimalInput
                          value={em.target}
                          onChange={(v) => updateExtraMoq(idx, 'target', v)}
                          className="sc-moq-inp"
                        />
                      </td>
                      <td>
                        <DecimalInput
                          value={em.target_vnd}
                          onChange={(v) => updateExtraMoq(idx, 'target_vnd', v)}
                          className="sc-moq-inp"
                          thousandSep
                        />
                      </td>
                    ...
                    <thead>
                      <tr>
                        ...
                        {Array.from({ length: numMoq }, (_, ti) => {
                          const qty =
                            ti === 0 ? st.moq || 0 : (st.extra_moqs[ti - 1] || {}).moq || 0;
                          return (
                      ...
                      <tr className="sc-smt-row-moq">
                        ...
                        {Array.from({ length: numMoq }, (_, ti) => {
                          const qty =
                            ti === 0 ? st.moq || 0 : (st.extra_moqs[ti - 1] || {}).moq || 0;
                          return (
                            <td key={ti} className="sc-smt-td-num sc-smt-td-moq">
                              {fmtIntLocal(qty)}
                            </td>
                          );
                        })}
                        ...
                        return (
                          <tr key={`m${i}`}>
                            ...
                            {Array.from({ length: numMoq }, (_, ti) => {
                              ...
                              const ovrVal =
                                !isBase && em[tierLmField] && em[tierLmField][i] != null
                                  ? em[tierLmField][i]
                                  : null;
                                    ...
                                    <DecimalInput
                                      ...
                                      title={
                                        isBase
                                          ? 'Base (MOQ 1) value'
                                          : isOvr
                                            ? `MOQ ${ti + 1} override — click ⟲ to revert to base`
                        ...
                        return (
                          <tr key={`p${i}`}>
                            ...
                            {Array.from({ length: numMoq }, (_, ti) => {
                              ...
                              const ovrVal =
                                !isBase && em.proc_setup_h && em.proc_setup_h[i] != null
                                  ? em.proc_setup_h[i]
                                  : null;
                              const isOvr = ovrVal != null;
