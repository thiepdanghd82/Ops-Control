...
function NewJobTab({ onSaved, initialJob, onInitialConsumed }) {
  ...
  useEffect(() => {
    ...
    setConfig((prev) => ({
      ...
      bgColor: Array.isArray(dc.bg_color) ? dc.bg_color
      ...
      mergeThreshold: Number.isFinite(dc.merge_threshold) ? dc.merge_threshold : prev.mergeThreshold,
      chromaBoost: typeof dc.chroma_boost === 'boolean' ? dc.chroma_boost : prev.chromaBoost,
      rescueOutliers: typeof dc.rescue_outliers === 'boolean' ? dc.rescue_outliers : prev.rescueOutliers,
    ...
    if (initialJob.artwork_file) {
      ...
      (async () => {
        try {
          const blob = await sharedApi.getDataFile(initialJob.artwork_file, { signal: controller.signal });
          ...
          onFileSelected(file);
          setOpenedFromLibrary((prev) => prev && prev.artworkFile === initialJob.artwork_file
            ? { ...prev, artworkLoading: false } : prev);
        } catch (err) {
          if (err?.name === 'AbortError') return;
          setOpenedFromLibrary((prev) => prev && prev.artworkFile === initialJob.artwork_file
            ? { ...prev, artworkLoading: false, artworkError: err?.message || 'Fetch failed' } : prev);
        }
      })();
  ...
  const onFileSelected = useCallback((f) => {
    ...
    const mimeOk = /^image\/(png|jpe?g|webp|gif|bmp|svg\+xml)$/.test(mime)
    ...
    const isPdfLike = mime === 'application/pdf'
      || mime === 'application/illustrator'
      || mime === 'application/postscript'
      || /\.(pdf|ai)$/.test(name);
  ...
  const runAnalyze = useCallback(async () => {
    if (!file) { setError('Upload an artwork first'); return; }
    ...
    try {
      ...
      r.colors.forEach((c) => { on[`c${c.idx}`] = false; });
      setOverlayOn(on);
      setStatus(null);
  ...
  const paintCanvas = useCallback((r, overlays) => {
    ...
    if (active.length > 0) {
      ...
      for (let i = 0; i < src.length; i += 4) {
        ...
        for (const c of active) {
          const dr = pr - c.rgb[0], dg = pg - c.rgb[1], db = pb - c.rgb[2];
          if (Math.sqrt(dr * dr + dg * dg + db * db) <= merge) {
            dst[i] = c.rgb[0]; dst[i + 1] = c.rgb[1]; dst[i + 2] = c.rgb[2]; dst[i + 3] = 255;
            break;
          }
        }
      }
    ...
    if (liveRoi) {
      ...
      if (norm) {
        ...
        if (cfgWmm > 0 && cfgHmm > 0) {
          ...
          ctx.fillRect(norm.x + norm.w / 2 - xMetrics.width / 2 - padX, xLblY - fontPx * 0.6,
                       xMetrics.width + padX * 2, fontPx * 1.2);
          ctx.fillStyle = '#fff';
          ...
          ctx.fillStyle = '#2563eb';
          ctx.fillRect(-yMetrics.width / 2 - padX, -fontPx * 0.6,
                       yMetrics.width + padX * 2, fontPx * 1.2);
          ctx.fillStyle = '#fff';
    ...
    // has moved away.
    if (pickedPixel && pickedPixel.x >= 0 && pickedPixel.y >= 0
        && pickedPixel.x < w && pickedPixel.y < h) {
  ...
  }, [config.mergeThreshold, config.widthMm, config.heightMm, config.scaleRatio, manualRoi, roiDrag, pickedPixel]);
  ...
  const toggleColorExcluded = useCallback((idx) => {
    setResult((prev) => {
      if (!prev) return prev;
      ...
      if (target) {
        setConfig((cfgPrev) => {
          const cur = new Set((cfgPrev.userExcludedHex || []).map(h => String(h).toUpperCase()));
          const hex = String(target.hex).toUpperCase();
          if (target.excluded) cur.delete(hex); // un-ignoring
          else cur.add(hex);
  ...
  const paintAnalysisCanvas = useCallback((r, overlays) => {
  ...
  const analysisDims = useMemo(() => {
    ...
    if (result.crop) {
      ...
      return {
        wMm, hMm,
        px_w: result.crop.w, px_h: result.crop.h,
        area_mm2: wMm * hMm,
  ...
  useEffect(() => {
    ...
    const same = (manualRoi && cur
      && manualRoi.x === cur.x && manualRoi.y === cur.y
      && manualRoi.w === cur.w && manualRoi.h === cur.h)
      || (!manualRoi && !cur);
    if (same) return;
    setAutoBusy(true);
    const id = setTimeout(() => {
    ...
    return () => { clearTimeout(id); setAutoBusy(false); };
  ...
  useEffect(() => {
    function onKey(e) {
      ...
      if (e.key === 'Escape') {
        if (roiDrag) { setRoiDrag(null); return; }
        if (roiMode) { setRoiMode(false); return; }
        if (pickMode) { setPickMode(false); return; }
        if (pickedPixel) { setPickedPixel(null); return; }
      } else if ((e.metaKey || e.ctrlKey) && e.key === 'Enter' && !inField) {
        if (file) { e.preventDefault(); runAnalyze(); }
      }
    }
  ...
  const onCanvasMouseUp = useCallback(() => {
    if (!roiMode || !roiDrag) return;
    const canvas = canvasRef.current;
    if (!canvas) { setRoiDrag(null); return; }
  ...
  const onCanvasClick = useCallback((e) => {
    ...
    for (const c of result?.colors || []) {
      ...
      if (d < bestDist) { bestDist = d; cluster = c; }
    }
  ...
  const save = useCallback(async () => {
    if (!result) return;
    if (!sku.trim()) { setError('SKU (mã) is required to save'); return; }
    setSaving(true);
    setError(null);
    try {
      ...
      <div className="pa-panel">
        ...
        {openedFromLibrary && (
          <div className="pa-info-banner" role="status" aria-live="polite">
            ...
            {openedFromLibrary.updatedAt && ` · ${String(openedFromLibrary.updatedAt).slice(0, 16).replace('T', ' ')}`}
            <div style={{ marginTop: 2, fontSize: 11, color: '#1e40af' }}>
              ...
              {openedFromLibrary.artworkError && (
                <span style={{ color: '#b91c1c' }}>
                  ⚠ Couldn&rsquo;t load artwork ({openedFromLibrary.artworkError}) — drop the source file manually.
                </span>
              )}
              {!openedFromLibrary.artworkLoading && !openedFromLibrary.artworkError && (
                <>✓ Artwork re-loaded
                  {openedFromLibrary.artworkFile && <> (<code>{openedFromLibrary.artworkFile.split('/').pop()}</code>)</>}
        ...
        <div className="pa-field">
          ...
          <input type="text" value={sku} onChange={(e) => setSku(e.target.value)} placeholder="e.g. 109M8-A00" />
        </div>
        <div className="pa-field">
          <label>Product name</label>
          <input type="text" value={productName} onChange={(e) => setProductName(e.target.value)} placeholder="Optional" />
        </div>
        <div className="pa-row">
          <div className="pa-field">
            <label>Width (mm) *</label>
            <DecimalInput value={config.widthMm} onChange={(v) => setConfig({ ...config, widthMm: Number(v) || 0 })} />
          </div>
          <div className="pa-field">
            <label>Height (mm) *</label>
            <DecimalInput value={config.heightMm} onChange={(v) => setConfig({ ...config, heightMm: Number(v) || 0 })} />
          </div>
          <div className="pa-field" style={{ maxWidth: 120 }}>
            ...
            <DecimalInput value={config.bleedMm || 0} onChange={(v) => setConfig({ ...config, bleedMm: Math.max(0, Number(v) || 0) })} />
          </div>
        </div>
        <div className="pa-row">
          <div className="pa-field">
          ...
          <div className="pa-field">
            ...
            <select value={config.dpi} onChange={(e) => setConfig({ ...config, dpi: Number(e.target.value) })}>
        ...
        {Number(config.scaleRatio) !== 1 && (
          <div className="pa-scale-hint">
            Physical = {(Number(config.widthMm) / Number(config.scaleRatio)).toFixed(2)} × {(Number(config.heightMm) / Number(config.scaleRatio)).toFixed(2)} mm
          </div>
        )}

        <h4 className="pa-panel-title" style={{ marginTop: 14 }}>Print method</h4>
        <div className="pa-method-grid">
          {Object.values(INK_PROFILES).map((p) => (
            ...
            >
              ...
              />
              <span>{p.label}</span>
              <span className="pa-method-meta">×{p.transfer_factor} · {p.film_thickness_um}µm</span>
            </label>
          ))}
        </div>
        ...
        >
          {file
            ...
            : (
              <div>
                <svg className="pa-dropzone-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                  <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                  <polyline points="17 8 12 3 7 8" />
                  <line x1="12" y1="3" x2="12" y2="15" />
                </svg>
                <strong>Drop artwork here</strong>
                <span className="pa-dropzone-hint">click to browse · or paste with <kbd>{navigator.platform.includes('Mac') ? '⌘V' : 'Ctrl V'}</kbd></span>
        ...
        <h4 className="pa-panel-title" style={{ marginTop: 14 }}>Detection</h4>
        <div className="pa-field">
          <label>Background mode</label>
          <select value={config.bgMode} onChange={(e) => setConfig({ ...config, bgMode: e.target.value })}>
        ...
        <div className="pa-field">
          ...
          <div className="pa-radio-row">
            {[
              { k: 'dieline',  label: 'Die-line (auto-detect stroke)', hint: 'Finds the thin closed stroke (any color) around the label and crops to its bbox — most accurate when artwork has a die-cut outline' },
              { k: 'physical', label: 'Physical (input-driven)',       hint: 'Center-crop to W×H on content centroid' },
              { k: 'auto',     label: 'Auto (erode + bbox)',           hint: 'Legacy: crop to densest content bbox' },
              { k: 'none',     label: 'None',                          hint: 'Analyze full canvas as-is' },
            ].map(({ k, label, hint }) => (
              <label key={k} className="pa-radio-opt" title={hint}>
                <input type="radio" name="pa-cropmode" value={k}
          ...
          {manualRoi && <em style={{ color: '#6b7280', fontSize: 11 }}>Disabled while manual ROI is set — manual ROI always wins.</em>}
        </div>
        ...
        <div className="pa-field">
          ...
          <input type="range" min="0" max="3" value={config.thinStrokeIterations || 0}
        ...
        <div className="pa-field">
          ...
          <select value={String(config.rotation ?? 'auto')}
            onChange={(e) => {
        ...
        <h4 className="pa-panel-title" style={{ marginTop: 14 }}>Accuracy (Sprint 8)</h4>
        <div className="pa-check-row">
          <label title="Use perceptual Lab color space (ΔE76) for cluster merging and nearest-centroid assignment. Default ON — merges JPG-artifact duplicates the eye can't distinguish. Turn off to reproduce pre-Sprint-8 numbers.">
            <input type="checkbox" checked={config.colorMetric !== 'rgb'}
              onChange={(e) => setConfig({ ...config, colorMetric: e.target.checked ? 'lab' : 'rgb' })} />
        ...
        <div className="pa-check-row">
          <label title="Give anti-aliased edge pixels a fractional ink weight (0..1) instead of counting them as full ink. Default ON — matches how the press actually lays ink. Shrinks coverage by 2-5% on vector art.">
            <input type="checkbox" checked={config.aaWeighting !== false}
        ...
        <div className="pa-check-row">
          <label title="Fold press dot gain into the ink-volume math. Halftone screens grow on press (50% file ≈ 68% on substrate for flexo). Affects ink volume only — file-coverage % stays the true on-file number.">
            <input type="checkbox" checked={config.applyDotGain !== false}
        ...
        <div className="pa-check-row">
          <label title="Exclude magenta / pink clusters (CAD dieline / cut-mark convention) from the printed total. You can still override any color by clicking its row in the results.">
            <input type="checkbox" checked={config.autoExcludeDieline}
        ...
        <div className="pa-action-bar">
          <button className="pa-analyze-btn" disabled={!file} onClick={runAnalyze}
        ...
        <h4 className="pa-panel-title" style={{ marginTop: 14 }}>Notes</h4>
        <div className="pa-field">
          <textarea rows={3} value={notes} onChange={(e) => setNotes(e.target.value)}
            style={{ padding: 6, border: '1px solid #d1d5db', borderRadius: 5, fontFamily: 'inherit', fontSize: 12 }} />
        </div>
      </div>
      ...
      <div className="pa-preview-wrap">
        <div className="pa-canvas-pair">
          <div className="pa-canvas-side">
            ...
            <div className={`pa-canvas-box ${status ? 'is-analyzing' : ''} ${originalZoom !== 1 ? 'is-zoomed' : ''}`}>
              {objectUrl || result
                ? <canvas
                    ref={canvasRef}
            ...
            <div className="pa-canvas-caption">
              {(() => {
                ...
                return (
                  <>
                    <b>{w.toFixed(2)} × {h.toFixed(2)} mm</b>
          ...
          <div className="pa-canvas-side">
            <div className="pa-canvas-caption-top">
              ...
              {analysisDims?.source === 'manual' && <span className="pa-caption-tag pa-tag-manual">manual ROI</span>}
              {analysisDims?.source === 'auto' && <span className="pa-caption-tag pa-tag-auto">auto-crop</span>}
              {analysisDims?.source === 'full' && <span className="pa-caption-tag pa-tag-full">full canvas</span>}
            ...
            <div className={`pa-canvas-box ${autoBusy ? 'is-analyzing' : ''} ${analysisZoom !== 1 ? 'is-zoomed' : ''}`}>
              {result
                ...
                : (
                  <div className="pa-canvas-empty">
                    ...
                    <span>Press <kbd>{navigator.platform.includes('Mac') ? '⌘↵' : 'Ctrl ↵'}</kbd> or click <em>Analyze</em></span>
                  </div>
                )}
            </div>
            <div className="pa-canvas-caption">
              {analysisDims
                ? <>
                    <b>{analysisDims.wMm.toFixed(2)} × {analysisDims.hMm.toFixed(2)} mm</b>
                    <span className="pa-canvas-caption-sub"> · {analysisDims.area_mm2.toFixed(2)} mm² · {analysisDims.px_w}×{analysisDims.px_h} px</span>
                  </>
                : '—'}
            </div>
          </div>
        ...
        {result?.rotation_applied > 0 && (
          <div className="pa-info-banner">
            Rotated artwork {result.rotation_applied}° {result.rotation_source === 'auto' ? '(auto-detect)' : '(manual)'} to match label aspect.
          </div>
        )}
        {result?.bleed_mm > 0 && (
          <div className="pa-info-banner">
            Bleed {result.bleed_mm} mm applied: frame {result.frame_width_mm.toFixed(1)}×{result.frame_height_mm.toFixed(1)} mm, trim {result.trim_width_mm.toFixed(1)}×{result.trim_height_mm.toFixed(1)} mm.
            Ink volumes include bleed; trim-only mm² shown via &times;{result.trim_ratio.toFixed(3)}.
          </div>
        )}
        {result?.dieline && (() => {
          const dw = result.dieline.detected_width_mm;
          const dh = result.dieline.detected_height_mm;
          const dimDrift = (Math.abs(dw - config.widthMm) / config.widthMm) > 0.02
          ...
          return (
            <div className="pa-info-banner pa-dieline-banner">
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                ({result.dieline.pixel_count.toLocaleString()} px, stroke score {result.dieline.stroke_score.toFixed(1)})
                {' · '}
                <b>{dw.toFixed(2)} × {dh.toFixed(2)} mm</b>
                {dimDrift && <em style={{ color: '#8a5a00', marginLeft: 6 }}>
                  ({((dw - config.widthMm) / config.widthMm * 100).toFixed(1)}% W / {((dh - config.heightMm) / config.heightMm * 100).toFixed(1)}% H drift vs typed)
                </em>}
              </div>
              {dimDrift && (
                <button type="button" className="pa-roi-btn"
                  style={{ flexShrink: 0 }}
        ...
        {result && (
          <div className="pa-overlay-toggles">
            ...
            >Original</span>
            {result.crop && <span className="pa-overlay-chip" style={{ background: '#dcfce7', color: '#166534', cursor: 'default' }}>
            ...
            {result.colors.map((c) => (
              <span
                ...
                onClick={() => setOverlayOn({ ...overlayOn, [`c${c.idx}`]: !overlayOn[`c${c.idx}`] })}
        ...
        {pickedPixel && (
          <div className="pa-pick-info">
            ...
            <div className="pa-pick-meta">
              <div className="pa-pick-row"><b>{pickedPixel.hex}</b> · RGB({pickedPixel.rgb.join(', ')})</div>
              <div className="pa-pick-row pa-pick-sub">@ ({pickedPixel.x}, {pickedPixel.y}) px · nearest: {pickedPixel.name}</div>
              {pickedPixel.cluster ? (
                <button
                  type="button"
                  className={`pa-pick-row pa-pick-ink ${overlayOn[`c${pickedPixel.cluster.idx}`] ? 'overlay-on' : ''}`}
                  onClick={() => setOverlayOn((prev) => ({
                    ...prev,
                    [`c${pickedPixel.cluster.idx}`]: !prev[`c${pickedPixel.cluster.idx}`],
                  }))}
                  title="Click to highlight this ink's area on the canvas"
                >
                  <span className="pa-pick-swatch sm" style={{ background: pickedPixel.cluster.hex }} />
                  Ink #{pickedPixel.cluster.idx} · {pickedPixel.cluster.name} · {(pickedPixel.cluster.print_area_pct * 100).toFixed(1)}%
                  {pickedPixel.cluster.excluded && <em style={{ color: '#92400e', marginLeft: 4 }}>(ignored)</em>}
                  {overlayOn[`c${pickedPixel.cluster.idx}`] && <span className="pa-overlay-dot">●</span>}
                </button>
              ) : (
                <div className="pa-pick-row pa-pick-sub">No analysis yet — run Analyze to map to detected inks.</div>
              )}
              {(() => {
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      setConfig((prev) => {
                        const cur = new Set((prev.pinnedSpotHex || []).map(h => String(h).toUpperCase()));
                        if (cur.has(hex)) cur.delete(hex); else cur.add(hex);
              ...
              <button type="button" className="pa-pick-clear" onClick={() => setPickedPixel(null)}>× clear</button>
            </div>
          </div>
        )}
        ...
        {Array.isArray(config.pinnedSpotHex) && config.pinnedSpotHex.length > 0 && (
          <div className="pa-pinned-row" role="list" aria-label="Pinned spot inks">
            ...
            <span className="pa-pinned-hint">Re-run <b>Analyze</b> to apply.</span>
          </div>
        )}
        ...
        <div className="pa-panel">
          <h4 className="pa-panel-title pa-title-row pa-results-header">
            <span className="pa-results-header-title">
              ...
              {result && <span style={{ fontSize: 10, fontWeight: 400, color: '#6b7280', marginLeft: 8, textTransform: 'none', letterSpacing: 0 }}>Click a row to toggle ignore</span>}
            </span>
            <div className="pa-results-header-tools">
              <button
                type="button"
                className={`pa-roi-btn ${roiMode ? 'active' : ''}`}
                onClick={() => { setRoiMode((m) => !m); setPickMode(false); }}
              ...
              <button
                ...
                onClick={() => { setPickMode((m) => !m); setRoiMode(false); }}
              ...
              {manualRoi && (
                <button type="button" className="pa-roi-btn pa-roi-clear" onClick={clearRoi}
          ...
          {roiPhysical && (
            <div className="pa-roi-info">
              <div><b>X = {roiPhysical.wMm.toFixed(2)} mm</b> · <b>Y = {roiPhysical.hMm.toFixed(2)} mm</b></div>
              <div>Area: {roiPhysical.area_mm2.toFixed(2)} mm² · {roiPhysical.area_m2.toExponential(3)} m² · {roiPhysical.px_w}×{roiPhysical.px_h} px</div>
              {manualRoi && !roiDrag && <div style={{ marginTop: 3, opacity: 0.85 }}>↻ Re-run <b>Analyze</b> to apply this ROI</div>}
            </div>
          )}
          ...
          {!result && <div style={{ color: 'var(--pa-text-subtle)', fontSize: 12, fontStyle: 'italic' }}>Run analysis to see per-color breakdown.</div>}
          ...
          {result?.vector_ink?.plates?.length > 0 && (
            <div className="pa-panel pa-panel-plates">
              <h4 className="pa-panel-title pa-title-row">
                <span>
                  🧪 INK PLATES <span style={{ fontSize: 10, fontWeight: 400, color: '#6b7280' }}>
                    · Vector pre-press analysis · {result.vector_ink.plates.length} plate{result.vector_ink.plates.length === 1 ? '' : 's'}
                  </span>
                </span>
                <span style={{ fontSize: 10, fontWeight: 400, color: '#6b7280', textTransform: 'none', letterSpacing: 0 }}>
              ...
              {result.vector_ink.warnings?.length > 0 && (
                <div className="pa-bg-sanity pa-bg-sanity-warning" style={{ marginBottom: 8 }}>
                  {result.vector_ink.warnings.map((w, i) => <div key={i}>⚠ {w}</div>)}
                </div>
              )}
              <table className="pa-results">
                <thead>
                  <tr>
                    ...
                    <th className="num" title="Number of vector objects drawn on this plate">Objects</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={p.key} className={p.type === 'raster' ? 'excluded' : ''}>
                      ...
                      <td>
                        <span
                          className="pa-swatch"
                          style={{
                            background: p.type === 'raster' ? 'repeating-linear-gradient(45deg,#e5e7eb 0 4px,#f3f4f6 4px 8px)' : getPlateSwatch(p.color),
                          }}
                        />
                      </td>
                      <td>
                        <b>{p.name}</b>
                        {p.type === 'spot' && <span className="pa-excluded-tag" style={{ background: '#fef3c7', color: '#92400e' }}>SPOT</span>}
                      </td>
                      <td style={{ fontSize: 11, color: '#6b7280', textTransform: 'capitalize' }}>{p.type}</td>
                      <td className="num pa-pct-cell" style={{ '--pa-pct': `${Math.min(100, p.coveragePct * 100)}%` }}>
                        {(p.coveragePct * 100).toFixed(2)}%
                      </td>
                      <td className="num">{p.areaMm2.toFixed(2)}</td>
                      <td className="num" style={{ color: '#6b7280' }}>{p.objectCount}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              <div style={{ fontSize: 10.5, color: '#6b7280', marginTop: 6, fontStyle: 'italic' }}>
                Plate areas are computed directly from the PDF content stream (Shoelace integration of vector geometry), not from pixel clusters. This matches the pre-press convention used by Esko Ink Wizard / PitStop / Hybrid Packz.
              </div>
            </div>
          )}
          ...
          {result && (() => {
            ...
            return (
              <>
                ...
                <div className="pa-stack-bar" role="img"
            ...
            <table className="pa-results">
              <thead>
                <tr>
                  ...
                  <th className="num">Area %</th>
                  <th className="num">Area mm²</th>
                  <th className="num" title="Wet ink volume per single label, given the active print method">µL/label</th>
                  <th className="num" title="Wet ink volume to print 1,000 labels of this size">mL/1k</th>
                </tr>
              </thead>
              <tbody>
                {result.colors.map((c) => {
                  ...
                  return (
                    <tr
                      ...
                      title={c.excluded ? 'Click to include in total · click # or swatch to highlight on canvas' : 'Click to ignore in total · click # or swatch to highlight on canvas'}
                    >
                      <td className="pa-show-cell" onClick={toggleOverlay} title="Show this ink area on the canvas">
                        {c.idx}
                        {overlayActive && <span className="pa-overlay-dot" aria-hidden="true">●</span>}
                      </td>
                      <td className="pa-show-cell" onClick={toggleOverlay} title="Show this ink area on the canvas">
                        <span className="pa-swatch" style={{ background: c.hex }} />
                      </td>
                      <td>{c.name}{c.excluded && <span className="pa-excluded-tag">ignored{c.excluded_reason === 'dieline' ? ' · dieline' : c.excluded_reason === 'user' ? ' · user' : ''}</span>}</td>
                      <td style={{ fontFamily: 'monospace', fontSize: 11 }}>{c.hex}</td>
                      <td className="num pa-pct-cell" style={{ '--pa-pct': `${Math.min(100, c.print_area_pct * 100)}%` }}>
              ...
              <tfoot>
                <tr>
                  <td colSpan={4} style={{ textAlign: 'right' }}>Total print coverage (excluding ignored)</td>
                  <td className="num">{totalPct}%</td>
                  <td className="num">{totalMm2}</td>
                  <td className="num">
                    {(result.colors.filter((c) => !c.excluded).reduce((s, c) => s + (c.ink_uL_per_label || 0), 0)).toFixed(3)}
                  </td>
                  <td className="num">
                    {(result.colors.filter((c) => !c.excluded).reduce((s, c) => s + (c.ink_mL_per_1k || 0), 0)).toFixed(2)}
                  </td>
                </tr>
              </tfoot>
            </table>
...
function LibraryTab({ jobs, loading, error, onReload, onOpen }) {
  ...
  const clamped = useMemo(() => {
    ...
    const MW = 160, MH = 88;  // approximate menu dimensions
    return {
  ...
  return (
    <div style={{ padding: 14 }}>
      <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
        ...
        />
        <button onClick={onReload} style={{ padding: '6px 12px', border: '1px solid #d1d5db', borderRadius: 5, background: '#fff', cursor: 'pointer' }}>↻ Reload</button>
      </div>
      ...
      {!loading && filtered.length === 0 && (
        <div style={{ padding: 40, textAlign: 'center', color: '#9ca3af' }}>No saved measurements yet.</div>
      )}
      {filtered.length > 0 && (
          ...
          <tbody>
            {filtered.map((j) => (
              ...
              >
                ...
                <td style={{ fontSize: 11 }}>{j.print_method ? (INK_PROFILES[j.print_method]?.label || j.print_method) : '—'}</td>
                <td>
                  {(j.colors || []).map((c) => (
                    <span key={c.idx} className="pa-swatch" style={{ background: rgbToHex(c.rgb), marginRight: 3, width: 18, height: 14 }} title={`${c.name} ${(c.print_area_pct * 100).toFixed(1)}%`} />
                  ))}
                </td>
                <td style={{ textAlign: 'right' }}>{j.totals ? (j.totals.total_print_pct * 100).toFixed(2) + '%' : '—'}</td>
                <td style={{ textAlign: 'right' }}>{j.totals ? j.totals.total_print_mm2.toFixed(2) : '—'}</td>
                <td style={{ fontSize: 11, color: '#6b7280' }}>{(j.created_at || '').slice(0, 16).replace('T', ' ')}</td>
                <td>
                  <button
                    type="button"
      ...
      {menu && clamped && (
        ...
        >
          <div className="pa-context-menu-header">
            {menu.job.sku}
          </div>
          <button
            type="button"
  ...
  return (
    <div className="pa-legend-wrap">
      <div className="pa-legend-card">
        ...
        <ol>
          <li>Render artwork to a canvas sized <code>widthMm × heightMm × (dpi / 25.4)</code> pixels (PDF/AI via pdf.js, raster via createImageBitmap).
            <div className="pa-bi-vi">Render artwork lên canvas kích thước <code>widthMm × heightMm × (dpi / 25.4)</code> pixel (PDF/AI dùng pdf.js, ảnh raster dùng createImageBitmap).</div>
          </li>
          <li>Detect background: sample 4 corners, pick the most-agreed color (robust to one rogue corner).
            <div className="pa-bi-vi">Phát hiện nền: lấy mẫu 4 góc, chọn màu có nhiều phiếu nhất (chịu được 1 góc bất thường).</div>
          </li>
          <li>Crop to region of interest — <b>manual ROI wins</b>; else auto-erode dim lines + take bbox; else use full canvas.
            <div className="pa-bi-vi">Cắt vùng quan tâm — <b>ROI thủ công ưu tiên</b>; không thì auto erode + lấy bbox; cuối cùng dùng full canvas.</div>
          </li>
          <li>Mask out BG pixels using Euclidean RGB distance ≤ tolerance (transparent px always BG).
            <div className="pa-bi-vi">Loại pixel nền theo khoảng cách RGB Euclid ≤ tolerance (pixel trong suốt luôn = nền).</div>
          </li>
          <li>Quantize remaining pixels into K clusters via median-cut.
            <div className="pa-bi-vi">Phân cụm pixel còn lại thành K cluster bằng median-cut.</div>
          </li>
          <li>Merge clusters whose centroids are within <code>mergeThreshold</code> — removes JPG halo duplicates.
            <div className="pa-bi-vi">Gộp cluster có centroid gần nhau trong <code>mergeThreshold</code> — loại halo trùng do nén JPG.</div>
          </li>
          <li>Apply scale ratio: physical mm² = (entered mm / scale)² so output reflects the real label, not the drawing.
            <div className="pa-bi-vi">Áp tỷ lệ vẽ: mm² thực = (mm nhập / scale)² để kết quả phản ánh nhãn thật, không phải bản vẽ.</div>
          </li>
          <li>Apply ink profile: per-color µL = area_mm² × film_µm × transfer_factor × 0.001.
            <div className="pa-bi-vi">Áp ink profile: µL/màu = area_mm² × film_µm × transfer_factor × 0.001.</div>
          </li>
          <li>Report pixel count → <code>% of canvas</code> → <code>mm² of physical area</code> → <code>µL/label</code> + <code>mL/1k</code>.
            <div className="pa-bi-vi">Báo: số pixel → <code>% canvas</code> → <code>mm² thực tế</code> → <code>µL/nhãn</code> + <code>mL/1k</code>.</div>
          </li>
        </ol>
      </div>
      <div className="pa-legend-card">
        <h3>Print methods (ink transfer factor + film thickness) / Phương pháp in (hệ số truyền + độ dày màng mực)</h3>
        <table className="pa-results">
          <thead>
            <tr>
      ...
      <div className="pa-legend-card">
        ...
        <ul>
          <li><b>Manual ROI</b> — toggle "Draw selection on canvas", drag a rectangle on the preview; the analysis runs only inside that box. Use this when auto-crop picks up unwanted technical annotations, multi-up layouts, or front+back drawings on the same sheet.
            <div className="pa-bi-vi"><b>ROI thủ công</b> — bật "Draw selection on canvas", kéo 1 hình chữ nhật trên preview; phân tích chỉ chạy trong khung đó. Dùng khi auto-crop bắt nhầm chú thích kỹ thuật, layout nhiều mặt, hoặc bản vẽ mặt trước+sau cùng trang.</div>
          </li>
          <li><b>Drawing scale</b> — pick a ratio when the artwork file is drawn at non-1:1. Example: a 60×40 mm artwork drawn at 2:1 prints as 30×20 mm physical. Pixel % is unchanged; mm² + µL outputs use the physical dims.
            <div className="pa-bi-vi"><b>Tỷ lệ bản vẽ</b> — chọn khi file artwork không vẽ 1:1. Ví dụ: artwork 60×40 mm vẽ tỷ lệ 2:1 → in thực 30×20 mm. % pixel không đổi; mm² + µL dùng kích thước thực.</div>
          </li>
        </ul>
      </div>
      <div className="pa-legend-card">
        <h3>Color separations export / Xuất tách màu</h3>
        <p>Click <b>⬇ Separations</b> in the Results panel to download one PNG per detected color. Each PNG is a film-positive — the color's pixels rendered as solid black on white, sized to the analysed region. Drop these straight into your screen-burning workflow (silkscreen) or plate-imaging step (flexo / letterpress).</p>
        <p className="pa-bi-vi">Click <b>⬇ Separations</b> ở panel Results để tải 1 PNG cho mỗi màu phát hiện. Mỗi PNG là film positive — pixel của màu đó vẽ thành đen đặc trên nền trắng, đúng kích thước vùng đã phân tích. Chuyển thẳng sang quy trình burning lưới (silkscreen) hoặc lên bản (flexo / letterpress).</p>
      </div>
      <div className="pa-legend-card">
        <h3>Tuning / Điều chỉnh tham số</h3>
        <ul>
          <li><b>BG tolerance</b> — raise for noisy scans, lower for clean vector art (default 12).
            <div className="pa-bi-vi"><b>Tolerance nền</b> — tăng cho ảnh scan nhiễu, giảm cho vector sạch (mặc định 12).</div>
          </li>
          <li><b>K colors</b> — upper bound (1–16); merge step reduces to true color count. Default 8 reserves headroom for rare spot inks.
            <div className="pa-bi-vi"><b>K màu</b> — giới hạn trên (1–16); bước merge giảm về số màu thật. Mặc định 8 chừa chỗ cho mực pha hiếm.</div>
          </li>
          <li><b>Merge threshold</b> — 18 works for most CMYK labels; raise to 24+ for heavy JPG compression.
            <div className="pa-bi-vi"><b>Ngưỡng merge</b> — 18 phù hợp đa số nhãn CMYK; tăng lên 24+ với ảnh nén JPG nặng.</div>
          </li>
          <li><b>DPI</b> — 300 is the standard label print DPI; 600 doubles accuracy and render time.
            <div className="pa-bi-vi"><b>DPI</b> — 300 là DPI in nhãn chuẩn; 600 tăng gấp đôi độ chính xác và thời gian render.</div>
          </li>
        </ul>
      </div>

      <div className="pa-legend-card">
        <h3>Sprint 9 — Spot-color detection (red warnings, brand inks) / Phát hiện màu pha (cảnh báo đỏ, mực thương hiệu)</h3>
        <p>Pre-Sprint-9, MMCQ (median-cut quantization) absorbed tiny but visually distinct spot inks into dominant clusters because the algorithm is pixel-density-biased. The classic failure: a mostly-black label with 0.3% red "Wash / Rinse / Spin" warning text lost the red entirely. Three independent safeguards now guarantee spot inks survive, each with a distinct failure mode.</p>
        <p className="pa-bi-vi">Trước Sprint 9, MMCQ (median-cut quantization) gộp các mực pha nhỏ nhưng rõ thị giác vào cluster chính vì thuật toán thiên về mật độ pixel. Lỗi điển hình: nhãn gần như đen với 0.3% chữ đỏ "Wash / Rinse / Spin" mất hoàn toàn màu đỏ. Giờ có 3 lớp bảo vệ độc lập, mỗi lớp xử lý một dạng thất bại khác nhau.</p>
        <ol>
          <li>
            <b>Chroma-weighted sampling</b> <em>(on by default)</em> — before MMCQ runs, pixels with perceptual chroma C* &gt; 20 in Lab space are replicated 1–8× (scaling with chroma) so the color-space histogram reads spot inks as "dense enough" to claim a cluster. Replication only affects centroid SELECTION; the subsequent nearest-centroid counting pass runs against the original pixel list, so coverage totals are unaffected.
            <div className="pa-bi-vi"><b>Lấy mẫu theo chroma</b> <em>(mặc định bật)</em> — trước khi chạy MMCQ, pixel có chroma cảm nhận C* &gt; 20 trong Lab được nhân 1–8× (theo chroma) để histogram màu coi mực pha là "đủ dày" để giữ cluster. Việc nhân chỉ ảnh hưởng CHỌN centroid; pass đếm sau đó chạy trên pixel gốc nên tổng coverage không đổi.</div>
          </li>
          <li>
            <b>Outlier rescue</b> <em>(on by default)</em> — after quantize + merge, every printable pixel's distance to the nearest centroid is measured; pixels beyond ΔE &gt; 15 are sub-quantized (up to 3 new centroids). Candidates whose weight is ≥ 0.1% of the printable total are promoted to real clusters and the full pixel set is re-counted. This is the safety net when chroma-boost alone cannot surface a rare ink.
            <div className="pa-bi-vi"><b>Cứu outlier</b> <em>(mặc định bật)</em> — sau khi quantize + merge, đo khoảng cách của mỗi pixel in được tới centroid gần nhất; pixel vượt ΔE &gt; 15 sẽ được phân cụm con (tối đa 3 centroid mới). Ứng viên có trọng lượng ≥ 0.1% tổng pixel in được sẽ thành cluster thật, sau đó đếm lại toàn bộ. Đây là lưới an toàn khi chroma-boost một mình không bắt được mực hiếm.</div>
          </li>
          <li>
            <b>📌 Pin as spot ink</b> <em>(manual override)</em> — toggle <b>🎯 Inspect color</b>, click the exact pixel of the color you want preserved, then press <b>📌 Pin as spot ink</b>. The hex is added to the job's pinned list, and every future Analyze forces a centroid at that color. Pinned hexes persist with the saved job. Use this when both automatic safeguards miss a sub-visual-threshold ink (e.g., a &lt; 0.05% lot-number stamp).
            <div className="pa-bi-vi"><b>📌 Pin làm mực pha</b> <em>(can thiệp thủ công)</em> — bật <b>🎯 Inspect color</b>, click chính xác vào pixel màu cần giữ, rồi nhấn <b>📌 Pin as spot ink</b>. Hex được thêm vào danh sách pinned của job, và mỗi lần Analyze sau đều ép 1 centroid tại màu đó. Hex pinned được lưu cùng job. Dùng khi cả 2 lớp tự động đều bỏ qua mực dưới ngưỡng cảm nhận (ví dụ con dấu lot-number &lt; 0.05%).</div>
          </li>
        </ol>
        <p className="pa-legend-disclaimer">All three safeguards preserve total printed mm² exactly — they only change how that total is split between per-color rows. Pricing math based on per-ink volume therefore becomes more accurate, never less.</p>
        <p className="pa-bi-vi pa-legend-disclaimer">Cả 3 lớp bảo vệ giữ NGUYÊN tổng mm² in được — chỉ thay đổi cách phân chia giữa các dòng màu. Tính giá theo µL mỗi mực chỉ chính xác hơn, không kém đi.</p>
      </div>
      <div className="pa-legend-card">
        <h3>Storage / Lưu trữ</h3>
        <p>Jobs are saved keyed by <b>SKU / mã sản phẩm</b>. Re-measuring the same SKU overwrites its prior entry. The artwork file is uploaded once and referenced by hash to avoid duplicates on disk.</p>
        <p className="pa-bi-vi">Job được lưu khoá theo <b>SKU / mã sản phẩm</b>. Đo lại cùng SKU sẽ ghi đè bản trước. File artwork được upload 1 lần và tham chiếu theo hash để tránh trùng trên disk.</p>
      </div>
      <div className="pa-legend-card">
        <h3>Extension — import from product layout / Mở rộng — import từ product layout</h3>
        <p>A future button in Standard / Complex calc will hand over <code>{'{ sku, widthMm, heightMm, artworkUrl }'}</code> directly to this tab, so dimensions never need to be re-entered. The core pipeline already accepts this shape.</p>
        <p className="pa-bi-vi">Nút sắp tới ở Standard / Complex calc sẽ chuyển <code>{'{ sku, widthMm, heightMm, artworkUrl }'}</code> trực tiếp vào tab này, không phải nhập lại kích thước. Pipeline lõi đã chấp nhận shape này.</p>
      </div>
    </div>
  );
}
...
export default function PrintAreaCalc() {
  ...
  if (sub === 'new') body = (
    <NewJobTab
      onSaved={reload}
      initialJob={openJob}
      onInitialConsumed={() => setOpenJob(null)}
    />
  );
  else if (sub === 'library') body = (
    <LibraryTab
      jobs={jobs} loading={loading} error={error} onReload={reload}
      onOpen={(job) => { setOpenJob(job); setSub('new'); }}
    />
  );
  else body = <LegendTab />;
          ...
          <nav className="pa-tabs" role="tablist" aria-label="Print Area sub-tabs">
            <TabBarOverflow ariaLabel="Print Area sub-tabs">
              <button role="tab" aria-selected={sub === 'new'}
                ...
                onClick={() => setSub('new')}>New Job</button>
              <button role="tab" aria-selected={sub === 'library'}
                className={`pa-tab ${sub === 'library' ? 'active' : ''}`}
                onClick={() => setSub('library')}>
                Library <span className="pa-tab-count">{jobs.length}</span>
              </button>
              <button role="tab" aria-selected={sub === 'legend'}
                className={`pa-tab ${sub === 'legend' ? 'active' : ''}`}
                onClick={() => setSub('legend')}>Legend</button>
            </TabBarOverflow>
          </nav>
        </div>
      </header>
