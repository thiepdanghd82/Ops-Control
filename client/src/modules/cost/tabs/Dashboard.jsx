...
export default function Dashboard() {
  ...
  const load = useCallback((rng = range) => {
    abortRef.current?.abort();
    const ctrl = new AbortController();
    abortRef.current = ctrl;
    setLoading(true); setErr(null);
    const qs = rng && rng !== 'all' ? `?days=${rng}` : '';
    api.get(`/shared/dashboard${qs}`, { signal: ctrl.signal })
  ...
  if (loading && !data) return <div style={{ padding: 24 }}><SkeletonTable rows={8} cols={4} /></div>;
  if (err) return (
    <div style={{ padding: 24 }}>
      <EmptyState
        ...
        hint={err}
        action={<button onClick={() => load(range)} className="op-btn op-btn-primary op-btn-sm">{t('common.retry')}</button>}
      />
    </div>
  );
  if (!data) return null;
  ...
  return (
    <div style={{ padding: 24, background: 'var(--surface-subtle)', minHeight: '100%', color: 'var(--text-primary)', fontFamily: "'IBM Plex Sans', 'Helvetica Neue', Arial, sans-serif" }}>
      {/* Header + range + export */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        <div style={{ fontSize: 20, fontWeight: 600, color: 'var(--text-primary)' }}>{t('dashboard.title')}</div>
        <div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>
          {data.range?.days
            ? (data.range.days === 30 ? t('common.last_30_days')
               : data.range.days === 90 ? t('common.last_90_days')
               : data.range.days === 365 ? t('common.last_year') : `${data.range.days}d`)
            : t('common.all_time')}
        </div>

        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
          <label style={{ fontSize: 11, color: 'var(--text-secondary)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.4 }}>{t('common.range')}</label>
          <select
            value={range}
          ...
          <button onClick={() => load(range)} className="op-btn op-btn-secondary op-btn-sm" title={t('common.refresh')}>↻ {t('common.refresh')}</button>
          {canExport && (
            <button onClick={handleExport} className="op-btn op-btn-primary op-btn-sm" title={t('common.export_csv')}>⇩ {t('common.export_csv')}</button>
          )}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 20 }}>
        <KpiCard label={t('dashboard.kpi.total_quotes')} value={fmtInt(overview.total) === '\u2014' ? '0' : fmtInt(overview.total)} sub={`${overview.by_type.standard || 0} std / ${overview.by_type.complex || 0} cplx`} />
        <KpiCard
...
function KpiCard({ label, value, sub, tone }) {
  ...
  const color = tone === 'good' ? '#16a34a' : tone === 'warn' ? '#d97706' : tone === 'bad' ? '#dc2626' : 'var(--text-primary)';
  return (
    <div style={{ background: 'var(--surface-card)', padding: 16, border: '1px solid var(--border-strong)' }}>
      <div style={{ fontSize: 11, color: 'var(--text-secondary)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.4 }}>{label}</div>
      <div style={{ fontSize: 22, fontWeight: 600, color, marginTop: 8, fontVariantNumeric: 'tabular-nums' }}>{value}</div>
...
function Panel({ title, children }) {
  return (
    <div style={{ background: 'var(--surface-card)', border: '1px solid var(--border-strong)' }}>
      <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--border-strong)', fontSize: 13, fontWeight: 600, color: 'var(--text-primary)' }}>{title}</div>
      {children}
    </div>
  );
}
