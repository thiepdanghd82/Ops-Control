...
import {
  createGallusInputs, rankPrintCylinders, crossDirection,
  rankMagneticCylinders, jobSummary, validateInputs, suggestGap,
  pickWinners, suggestWebWidth,
  inkConsumption, recommendAnilox, dieRisk, validateColorSequence,
} from './gallusEngine';
...
const STATUS_TONE = {
  OK: 'good', '✓ MATCH': 'good',
  '⚠ NEAR': 'warn', 'GAP < MIN': 'warn', 'GAP > MAX': 'warn',
  '✗ MISMATCH': 'bad', 'TOO SMALL': 'bad',
};
...
export default function GallusCalc() {
  ...
  const [inputs, setInputs] = usePersistentInputs(
    'ops_design_tools_gallus_inputs', createGallusInputs,
  );
  ...
  const reloadCylinders = useCallback(async () => {
    ...
    } catch { /* silent — engine falls back to factory defaults */ }
  }, []);
  useEffect(() => { reloadCylinders(); }, [reloadCylinders]);
  ...
  const handleArtworkUpload = useCallback(async (file) => {
    ...
    if (file.size > 1.5 * 1024 * 1024) {
      showToast(`Large file (${(file.size / 1024 / 1024).toFixed(1)} MB) — preview works but may not persist on tab switch`, 'warning');
    }
  ...
  const saveAsNew = useCallback(async ({ notice = '', parentId = null } = {}) => {
  ...
  const saveUpdate = useCallback(async ({ notice = '' } = {}) => {
  ...
  const deleteFromHistory = useCallback(async (rec) => {
    if (!rec?.id) return;
    if (!window.confirm(
  ...
  const applyToPricing = useCallback((targetType) => {
    if (hasErrors || !top5[0]) {
    ...
    showToast(`Sent to Pricing (${targetType === 'standard' ? 'Std' : 'Cpx'}) — switch tabs to see prefilled fields`);
  ...
  return (
    <div className="gc-root">
      ...
      <div className="gc-subtitle">
        ...
        <span className="gc-bi-vi"> · Công cụ thiết kế cylinder &amp; layout cho máy Gallus ECS340</span>
      </div>
      ...
      <div className="gc-identbar">
        <div className="gc-identbar-fields">
          <div className="gc-identbar-field">
            <label>End CU PN <span className="gc-bi-vi">/ Mã sản phẩm khách</span></label>
            <input
              type="text"
          ...
          <div className="gc-identbar-field">
            <label>Project <span className="gc-bi-vi">/ Dự án</span></label>
            <input
              type="text"
          ...
          <div className="gc-identbar-field gc-identbar-grow">
            <label>Designer note <span className="gc-bi-vi">/ Ghi chú thiết kế</span></label>
            <input
              type="text"
        ...
        <div className="gc-identbar-actions">
          {activeDesignId != null && (
            <span className="gc-version-badge" title={`Loaded record #${activeDesignId} · Save will prompt: Update v${activeVersion} or branch as new`}>
      ...
      <SaveChoiceDialog
        ...
        onUpdate={(notice) => { setSaveChoiceOpen(false); saveUpdate({ notice }); }}
        onSaveAsNew={(notice) => { setSaveChoiceOpen(false); saveAsNew({ notice, parentId: activeDesignId }); }}
      />
...
function InputsPanel({
  inputs, setNum, set,
  issues, errorFields, warnFields,
  gapSuggestion, applyGapSuggestion,
}) {
  return (
      ...
      <div className="gc-card-body">
        <Section title="1A. Down-web (chiều chạy)">
          <Field
            ...
            value={inputs.L} onChange={setNum('L')}
          />
          <Field
            ...
            value={inputs.G} onChange={setNum('G')}
            footer={
              gapSuggestion && (
                <GapSuggestion suggestion={gapSuggestion} currentG={inputs.G} onApply={applyGapSuggestion} />
              )
            }
          />
          <Field
            ...
            unit="mm" hint="Default 2.0 — die-cut min"
            value={inputs.G_min} onChange={setNum('G_min')}
          />
          <Field
            en="G_max — Max acceptable gap"
            vi="Gap tối đa chấp nhận"
            unit="mm" hint="Default 8.0 — over này phí web"
            value={inputs.G_max} onChange={setNum('G_max')}
          />
          <Field
            en="K — Print zone overhead"
            vi="Vùng cylinder không in (clamp + reg marks)"
            unit="mm" hint="Default 9.9 — Gallus ECS340"
            warn={warnFields.has('K')}
            value={inputs.K} onChange={setNum('K')}
          />
          ...
          <Field
            ...
            unit="pcs" hint="0 = no preference · auto-optimise"
            value={inputs.parts_md} onChange={setNum('parts_md')}
          />
          <CheckField
        ...
        <Section title="1B. Cross-web (chiều ngang web)">
          <Field
            ...
            value={inputs.W} onChange={setNum('W')}
          />
          <Field
            ...
            value={inputs.Pw} onChange={setNum('Pw')}
          />
          ...
          <Field
            ...
            unit="mm" hint="2 mm = standard label · 0 = sheet/guillotine"
            value={inputs.bleed_mm} onChange={setNum('bleed_mm')}
          />
          <Field
            en="E — Edge margin per side"
            vi="Vùng mép web không in"
            unit="mm" hint="Default 5 mm/side"
            value={inputs.E} onChange={setNum('E')}
          />
          <Field
            en="Lg — Target lane gap (cross-direction)"
            vi="Gap mục tiêu giữa các lane"
            unit="mm" hint="Default 2.5 mm"
            value={inputs.Lg} onChange={setNum('Lg')}
          />
          ...
          <Field
            ...
            unit="lanes" hint="0 = no preference · auto-optimise"
            value={inputs.parts_td} onChange={setNum('parts_td')}
          />
        </Section>
        ...
        <Section title="1C. Die-cut & cost / Khuôn cắt & chi phí">
          <Field
            ...
            unit="T" hint="0 = same as print"
            value={inputs.Z_die} onChange={setNum('Z_die')}
          />
          <Field
            en="Plate cost"
            vi="Đơn giá khuôn in"
            unit="USD/m²" hint="Flexo digital ~$80"
            value={inputs.plate_cost} onChange={setNum('plate_cost')}
          />
          <Field
            en="Web length / job (running m)"
            vi="Chiều dài cuộn web cho 1 job (mét chạy)"
            unit="m" hint="Bidirectional — edits Lot Q'ty too"
            value={inputs.web_length_m} onChange={setNum('web_length_m')}
          />
          <Field
            en="Lot run Q'ty (pieces)"
            vi="Số lượng cần in 1 lot (cái)"
            unit="pcs" hint="Bidirectional — edits Web length too"
            value={inputs.lot_run_qty} onChange={setNum('lot_run_qty')}
          />
          <Field
            en="# Print colors"
            vi="Số màu in (số plate)"
            unit=""
            value={inputs.n_colors} onChange={setNum('n_colors')}
          />
          {/* Sprint S-FLEXO-3 — plate gripper + tape gutter overhead. */}
          <Field
            en="Plate overhead — gripper + tape gutter"
            vi="Phần plate tape & gripper"
            unit="mm" hint="Default 13 mm (CCL standard)"
            value={inputs.plate_overhead_mm} onChange={setNum('plate_overhead_mm')}
          />
          ...
          <Field
            ...
            unit="m" hint="100 m × n_colors typical · 0 = rerun"
            value={inputs.setup_waste_m} onChange={setNum('setup_waste_m')}
          />
        </Section>
        ...
        <Section title="1D. Ink & Anilox / Mực & Anilox">
          ...
          {inputs.material_type === 'custom' && (
            <Field
              en="Stretch (custom)"
              vi="Độ giãn (custom)"
              unit="%" hint="Override material stretch"
              value={inputs.stretch_pct} onChange={setNum('stretch_pct')}
            />
          )}
          <Field
            en="Coverage per color"
            vi="Tỷ lệ phủ mực mỗi màu"
            unit="%" hint="30 = standard process · 60+ = solid spot"
            value={inputs.coverage_pct} onChange={setNum('coverage_pct')}
          />
          <Field
            en="Anilox volume"
            vi="Volume anilox"
            unit="BCM" hint="4-12 — engine recommends below"
            value={inputs.anilox_bcm} onChange={setNum('anilox_bcm')}
          />
          <Field
            en="Ink density"
            vi="Tỷ trọng mực"
            unit="g/cm³" hint="1.0 water · 1.1 UV · 1.4 metallic"
            value={inputs.ink_density_g_cm3} onChange={setNum('ink_density_g_cm3')}
          />
        </Section>
        ...
        <Section title="1E. Ranking weights / Trọng số xếp hạng">
          <Field
            ...
            unit="" hint="Higher = prefer waste-minimising cylinder"
            value={inputs.weight_yield} onChange={setNum('weight_yield')}
          />
          <Field
            en="Cost weight"
            vi="Trọng số cost / 1k imp"
            unit="" hint="Higher = prefer plate-cost-minimising cylinder"
            value={inputs.weight_cost} onChange={setNum('weight_cost')}
          />
          <Field
            en="Gap match weight"
            vi="Trọng số khớp gap target G"
            unit="" hint="Higher = prefer cylinder closest to your G"
            value={inputs.weight_gap} onChange={setNum('weight_gap')}
          />
          <Field
            en="Layout match weight"
            vi="Trọng số khớp Part MD/TD"
            unit="" hint="Higher = prefer cylinder hitting parts targets"
            value={inputs.weight_match} onChange={setNum('weight_match')}
          />
        </Section>
      </div>
    </div>
...
function PrintDesignTab({ ranked, top5, cross, summary, inputs, winners, webSuggestion, set, onUploadArtwork, onClearArtwork, artworkLoading, isAdmin, onReloadCylinders }) {
  return (
    <div className="gc-tab-body">
      ...
      {top5[0] && cross.n_across > 1 && (
        <DieRiskPill cross={cross} />
      )}
      ...
      {top5[0] && (
        <InkBanner top1={top5[0]} cross={cross} inputs={inputs} />
      )}
      ...
      {webSuggestion && (
        <WebSuggestionBanner sug={webSuggestion} onApply={() => set('W', webSuggestion.suggested_mm)} />
      )}

      <div className="gc-card">
        <div className="gc-card-hdr">
          <span className="gc-card-icon" style={{ color: '#0f62fe' }}>★</span>
          <div>
            <b>2. Top 5 recommended cylinders (sorted by yield)</b>
            <span className="gc-bi-vi"> · Top 5 cylinder — sort theo yield (giảm waste)</span>
          </div>
        </div>
              ...
              <tbody>
                {top5.map((r, i) => (
                  <tr key={r.z} className={i === 0 ? 'gc-row-best' : ''}>
                    ...
                    <td className="num"><b>{r.z}</b></td>
                    <td className="num">{fmt.n(r.pitch, 2)}</td>
                    <td className="num">{r.n_down}</td>
                    <td className="num">{fmt.n(r.actual_gap, 3)}</td>
                    <td className="num"><b style={{ color: '#0e6027' }}>{fmt.pct(r.material_yield_pct)}</b></td>
                    <td className="num">{Number.isFinite(r.cost_per_1k) ? `$${fmt.n(r.cost_per_1k, 4)}` : '—'}</td>
      ...
      <div className="gc-card">
        <div className="gc-card-hdr">
          <span className="gc-card-icon" style={{ color: '#0043ce' }}>▦</span>
          <div style={{ flex: 1 }}>
        ...
        <div className="gc-card-body gc-flush">
          <ShotLayoutViz inputs={inputs} result={top5[0]} cross={cross} artworkUrl={inputs.artwork_data_url} />
        </div>
      </div>
...
function CrossDirectionCard({ inputs, cross }) {
  return (
    <div className="gc-card">
      <div className="gc-card-hdr">
        <span className="gc-card-icon" style={{ color: '#059669' }}>↔</span>
        <div>
      ...
      {cross.lane_gap_below_target && (
        <div className="gc-validation gc-val-warn" style={{ borderTop: '1px solid #e0e0e0' }}>
          ...
          <div className="gc-val-list" style={{ paddingLeft: 22, fontSize: 11 }}>
            ...
            Layout vẫn fit được {cross.n_across} lanes vật lý — chấp nhận hoặc tăng Lg để giảm số lane.
          </div>
        </div>
      )}
      <div className="gc-card-body gc-grid-kv">
        ...
        <KV en="Available width = W − 2E" vi="Chiều rộng khả dụng" v={`${fmt.n(cross.avail, 1)} mm`} />
        <KV en="# Across (max lanes)" vi="Số lane chiều ngang" v={cross.n_across || '—'} highlight />
        <KV en="Lane gap actual" vi="Gap thực tế giữa lanes" v={cross.n_across > 1 ? `${fmt.n(cross.lane_gap_actual, 2)} mm` : '—'} />
...
function JobSummaryCard({ summary }) {
  if (!summary) {
    return (
      <div className="gc-card">
        <div className="gc-card-hdr">
          <span className="gc-card-icon" style={{ color: '#525252' }}>≡</span>
          <div>
            <b>4. Job summary</b>
        ...
        <div className="gc-card-body">
          <Empty en="No OK cylinder yet — fill inputs above." vi="Chưa có cylinder phù hợp — điền inputs ở trên." />
        </div>
      </div>
    );
  }
  return (
    <div className="gc-card">
      ...
      <div className="gc-card-body gc-grid-kv">
        <KV en="Recommended cylinder" vi="Cylinder đề xuất" v={`Z = ${summary.cylinder_z} (${fmt.n(summary.pitch, 2)} mm)`} highlight />
        <KV en="Step / Lay" vi="Step (chiều chạy)" v={`${fmt.n(summary.step, 3)} mm`} />
        <KV en="N down × N across" vi="Layout (MD × TD)" v={`${summary.n_down} × ${summary.n_across}`} />
        <KV en="Pieces per running meter" vi="Sản phẩm / mét web" v={`${fmt.n(summary.pieces_per_m, 2)} pcs/m`} />
        <KV en="Lot run Q'ty (resolved)" vi="Số lượng lot (đã quy đổi)" v={fmt.i(summary.lot_run_qty)} highlight />
        <KV en="Web length consumed" vi="Web tiêu thụ" v={`${fmt.n(summary.web_used_m, 1)} m`} />
        <KV en="Material area" vi="Diện tích vật liệu" v={`${fmt.n(summary.material_area_m2, 2)} m²`} />
        ...
        <KV en="Total impressions / job" vi="Tổng impressions" v={fmt.i(summary.total_imp)} highlight />
        <KV en="Plate area / color" vi="Diện tích plate / màu" v={`${fmt.n(summary.plate_area_per_color, 4)} m²`} />
        <KV en="Total plate area (× n colors)" vi={`Tổng plate (${summary.n_down ? fmt.i(summary.total_plate_area / summary.plate_area_per_color || 0) : '?'} màu)`} v={`${fmt.n(summary.total_plate_area, 4)} m²`} />
        <KV en="★ Total plate cost" vi="Tổng chi phí plate" v={`$${fmt.n(summary.total_plate_cost, 2)}`} highlight />
        <KV en="Cost per 1,000 imp" vi="Chi phí / 1,000 imp" v={`$${fmt.n(summary.cost_per_1k_imp, 4)}`} />
      </div>
    </div>
  );
}
...
function MasterPrintTable({ ranked, isAdmin, onReload }) {
  ...
  const toggleAvail = useCallback(async (z, currentlyAvailable) => {
    setBusyZ(z);
    try {
  ...
  const submitAdd = useCallback(async () => {
    ...
    try {
      ...
      setNewZ(''); setNewNote(''); setAddOpen(false);
    } catch (err) {
  ...
  return (
    <div className="gc-card">
      <div className="gc-card-hdr">
        <span className="gc-card-icon" style={{ color: '#7c3aed' }}>☰</span>
        <div style={{ flex: 1 }}>
...
function CuttingDesignTab({ inputs, setNum, top5, magRanked, printStep, cross, onUploadArtwork, onClearArtwork, artworkLoading }) {
  ...
  const cutResult = useMemo(() => {
    ...
    const chosenDie = inputs.Z_die > 0
      ? magRanked.find(r => r.z === inputs.Z_die)
      : (top3[0] || null);
  ...
  return (
    <div className="gc-tab-body">
      ...
      <div className="gc-card">
        ...
        <div className="gc-card-body gc-flush">
          <ShotLayoutViz inputs={inputs} result={cutResult} cross={cross} artworkUrl={inputs.artwork_data_url} />
        </div>
      </div>
      ...
      <div className="gc-card">
        ...
        <div className="gc-card-body gc-flush">
          {!top5[0]
            ? <Empty en="Need a Top-1 print cylinder first — go to Print Design." vi="Cần có cylinder in Top-1 trước — chuyển sang Print Design." />
            : top3.length === 0
              ? <Empty en="No magnetic cylinder fits the inputs." vi="Không có magnetic die nào phù hợp." />
              : (
                <table className="gc-table">
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {top3.map((r, i) => (
                      <tr key={r.z} className={i === 0 ? 'gc-row-best' : ''}>
                        ...
                        <td><StatusBadge status={r.match} /></td>
                        <td>{r.note || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
      ...
      <ManualOverrideCard inputs={inputs} setNum={setNum} top5={top5} magRanked={magRanked} printStep={printStep} />
      ...
      <div className="gc-card">
        ...
        <div className="gc-card-body gc-flush">
          <table className="gc-table gc-table-compact">
            ...
            <tbody>
              {magRanked.length === 0 && (
                <tr><td colSpan={8} className="gc-empty">Set a Top-1 print cylinder to populate this table.</td></tr>
              )}
              {magRanked.map(r => (
                <tr key={r.z}>
                  <td className="num"><b>{r.z}</b></td>
...
function ManualOverrideCard({ inputs, setNum, top5, magRanked, printStep }) {
  ...
  return (
    <div className="gc-card">
      <div className="gc-card-hdr">
        <span className="gc-card-icon" style={{ color: '#b45309' }}>✎</span>
        <div>
      ...
      <div className="gc-card-body gc-grid-kv">
        ...
        {customMag ? (
          <>
            ...
            <KV en="Die step actual" vi="Die step thực tế" v={`${fmt.n(customMag.die_step, 3)} mm`} />
            ...
            <div className="gc-kv">
              ...
              <span><StatusBadge status={customMag.match} /></span>
            </div>
          </>
        ) : (
          <Empty en="Pick a Z_die that exists in the magnetic inventory above." vi="Chọn Z_die có trong bảng magnetic inventory ở trên." />
        )}
      </div>
    </div>
  );
...
function Field({ en, vi, unit, hint, value, onChange,
                 required = false, invalid = false, warn = false, footer = null }) {
    ...
    <div className="gc-field">
      <label>
        <span className="gc-l-en">
          {en}{unit ? <span className="gc-l-unit"> ({unit})</span> : null}
          {required && <span className="gc-l-req" title="Required · Bắt buộc">*</span>}
        </span>
  ...
  return (
    <div className={`gc-validation ${errors.length > 0 ? 'gc-val-err' : 'gc-val-warn'}`}>
      <div className="gc-val-hdr">
        ...
        {errors.length > 0
          ? <b>{errors.length} required field{errors.length > 1 ? 's' : ''} missing · {errors.length} trường bắt buộc còn thiếu</b>
          : <b>{warns.length} warning{warns.length > 1 ? 's' : ''} · {warns.length} cảnh báo</b>}
      </div>
...
function GapSuggestion({ suggestion, currentG, onApply }) {
  ...
  if (reason === 'already_optimal') {
    return (
      <div className="gc-suggest gc-suggest-good">
        <span aria-hidden>✓</span>
        <span>
          <b>Optimal · Tối ưu</b> — your G fits Z={cylinder_z} exactly with N={n_down}, film {(product_film_pct * 100).toFixed(1)}%.
  ...
  if (reason === 'no_ok_at_current_g') {
    return (
      <div className="gc-suggest gc-suggest-warn">
        <span aria-hidden>💡</span>
        <span>
          <b>No OK match</b> at current G — try <b>{dpy(gap)} mm</b> for an exact fit with Z={cylinder_z} (N={n_down}, film {(product_film_pct * 100).toFixed(1)}%).
          <span className="gc-bi-vi"> Không có cylinder OK ở G hiện tại — thử {dpy(gap)} mm để khớp Z={cylinder_z}.</span>
        </span>
        {needsApply && (
  ...
  return (
    <div className="gc-suggest gc-suggest-info">
      <span aria-hidden>💡</span>
      <span>
        <b>Suggested · Đề xuất:</b> <b>{dpy(gap)} mm</b> for exact fit with Z={cylinder_z} (N={n_down}, film {(product_film_pct * 100).toFixed(1)}%).
        <span className="gc-bi-vi"> Đề xuất G={dpy(gap)} mm để khớp chính xác cylinder Z={cylinder_z}.</span>
      </span>
      <button type="button" className="gc-suggest-apply" onClick={onApply}>
        Apply {dpy(gap)}
      </button>
...
function WinnersBar({ winners, inputs }) {
  ...
  return (
    <div className="gc-winners">
      <WinnerCard
        title="Yield winner" vi="Tối ưu vật liệu" axis="yield" cyl={winners.yield}
        rationale={`${fmt.pct(winners.yield.material_yield_pct)} of web → product. Best for HIGH VOLUME.`}
      />
      <WinnerCard
        title="Cost winner" vi="Rẻ nhất / 1k imp" axis="cost" cyl={winners.cost}
        rationale={winners.cost && Number.isFinite(winners.cost.cost_per_1k)
          ? `$${fmt.n(winners.cost.cost_per_1k, 4)} / 1,000 imp. Best for SMALL VOLUME.`
          : 'Set web length + plate cost to enable.'}
      />
      <WinnerCard
        title="Gap target" vi="Khớp G nhất" axis="gap" cyl={winners.gap}
      ...
      {(inputs.parts_md > 0 || inputs.parts_td > 0) && (
        <WinnerCard
          title="Layout match" vi="Khớp layout mong muốn" axis="match" cyl={winners.match}
          rationale={
            winners.match
...
function WinnerCard({ title, vi, axis, cyl, rationale }) {
  if (!cyl) return (
    <div className="gc-winner-card gc-winner-empty">
      <div className="gc-winner-title">{title}<span className="gc-bi-vi"> · {vi}</span></div>
...
function ArtworkUploadButton({ current, loading, onUpload, onClear }) {
  ...
  return (
    <div className="gc-artwork-bar">
      ...
      />
      {loading ? (
        <span className="gc-artwork-name">⏳ Rendering… <span className="gc-bi-vi">đang xử lý</span></span>
      ) : current ? (
        <>
          <span className="gc-artwork-name" title={current}>📎 {current}</span>
          <button type="button" className="gc-mini-btn" onClick={() => inputRef.current?.click()}>Replace</button>
          <button type="button" className="gc-mini-btn gc-mini-btn-danger" onClick={onClear}>Clear</button>
        </>
      ) : (
...
function SaveChoiceDialog({ open, onClose, activeId, activeVersion, endCuPn, onUpdate, onSaveAsNew }) {
  ...
  return (
    ...
    >
      ...
      <Modal.Body>
        <p>
          This design was loaded from history. How would you like to save your changes?
        </p>
        ...
        <ul className="op-modal-choice-list">
          <li>
            <b>Update existing</b> — overwrite #{activeId} in place, bump to <b>v{activeVersion + 1}</b>.
            Anyone who reloads this record will see your changes.
          </li>
          <li>
            <b>Save as new version</b> — keep #{activeId} untouched and create a separate
        ...
        <div className="gc-change-notice">
          <label htmlFor="gc-change-notice-input" className="gc-change-notice-label">
            <b>Design change notice <span style={{ color: '#da1e28' }}>*</span></b>
  ...
  return (
    <div className="gc-history">
      <div className="gc-history-hdr">
        ...
        />
        <button className="gc-mini-btn" onClick={onClose} aria-label="Close">×</button>
      </div>
      {filtered.length === 0 ? (
        <Empty
          ...
          <tbody>
            {filtered.map(r => (
              <tr key={r.id}>
                ...
                <td>{r.saved_by || '—'}</td>
                <td className="num">
                  {r.result?.cylinder_z ? `${r.result.cylinder_z}T (${(r.result.pitch || 0).toFixed(1)}mm)` : '—'}
                </td>
                <td className="num">
                  {r.inputs ? `${r.inputs.L} × ${r.inputs.Pw}` : '—'}
                </td>
                <td>{r.designer_note || ''}</td>
                <td>
                  <button className="gc-mini-btn" onClick={() => onLoad(r)}>Load</button>
                  <button
...
function InkBanner({ top1, cross, inputs }) {
  ...
  const total_imp = top1.step_lay > 0 && inputs.web_length_m > 0
  ...
  return (
    <div className="gc-banner gc-banner-info" role="status">
      ...
      <span style={{ marginLeft: 8 }}>
        {totalKg.toFixed(2)} kg total ({n} colors × {ink.mass_kg.toFixed(3)} kg/color)
        · printed area {printed_area_m2.toFixed(1)} m²
      </span>
      <div style={{ fontSize: 11, color: '#525252', marginTop: 4 }}>
        Recommended anilox for {inputs.coverage_pct}% coverage: <b>{recommended.bcm} BCM</b> ({recommended.range})
        {!usingRecommended && (
