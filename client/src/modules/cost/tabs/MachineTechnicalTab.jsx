...
function isBoolField(key) { return key.startsWith('has_'); }
function isNumberField(key) { return key.endsWith('_mm') || key.endsWith('_m') || key.endsWith('_count') || key.endsWith('_max') || key.endsWith('_min') || key.endsWith('_tonnes') || key.endsWith('_rpm_max') || key.endsWith('_mm_min'); }
...
export default function MachineTechnicalTab() {
  ...
  const tableCols = useMemo(() => {
    if (kind === 'printing') {
      return ['brand', 'model', 'printing_type', 'num_print_stations', 'max_usable_web_width_mm', 'max_repeat_length_mm', 'feasible_max_run_speed', 'availability'];
    }
    return ['brand', 'model', 'cutting_type', 'cutting_mode', 'num_die_stations', 'max_web_width_mm', 'max_run_speed', 'availability'];
  }, [kind]);
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(m =>
      [m.brand, m.model, m.availability, m.printing_type, m.cutting_type, m.notes]
  ...
  return (
    <div className="mt-tab">
      ...
      <div className="mt-header">
        <div className="mt-header-title">
          ...
          <div>
            ...
            <p>Equipment capability database — {items.length} {kind} machine(s) on record</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-sub-tabs" role="tablist">
        <button role="tab" aria-selected={kind === 'printing'}
          className={`mt-sub-tab ${kind === 'printing' ? 'active' : ''}`}
          onClick={() => setKind('printing')}>🖨 Printing Machine</button>
        <button role="tab" aria-selected={kind === 'cutting'}
          className={`mt-sub-tab ${kind === 'cutting' ? 'active' : ''}`}
          onClick={() => setKind('cutting')}>✂ Cutting Machine</button>
      </div>
      ...
      <div className="mt-toolbar">
        ...
        />
        <button className="mt-btn" onClick={handleExport}>📥 Export Excel</button>
        {isAdmin && (
          <>
            <button className="mt-btn" disabled={importing}
      ...
      {/* Table */}
      {filtered.length === 0 ? (
        <EmptyState icon="🏭" title="No machines yet"
          hint={isAdmin ? 'Click "+ Add Machine" or import an Excel file.' : 'Ask an admin to add equipment records.'} />
      ) : (
        <div className="mt-table-wrap">
          <table className="mt-table">
            <thead>
              <tr>
                {tableCols.map(c => <th key={c}>{labelFor(c)}</th>)}
                <th style={{ width: 80 }} />
              </tr>
            </thead>
            <tbody>
              {filtered.map(m => (
                <tr key={m.id} onClick={() => setEditing(m)} style={{ cursor: 'pointer' }}>
                  ...
                  <td onClick={e => e.stopPropagation()} className="mt-row-actions">
                    <button onClick={() => setEditing(m)} title="View / Edit">✎</button>
                    {isAdmin && <button onClick={() => handleDelete(m.id)} title="Delete" className="mt-del">×</button>}
                  </td>
                </tr>
              ))}
            </tbody>
...
function renderCellValue(v) {
  ...
  if (typeof v === 'boolean') return v ? <span className="mt-yes">✓</span> : <span className="mt-no">✕</span>;
...
function MachineDetailDrawer({ kind, record, fields, cuttingTypes, isAdmin, isNew, existingIds, onClose, onSaved, onReloadMeta }) {
  ...
  function set(key, val) {
    setForm(f => {
      ...
      if (key === 'max_repeat_length_vp_mm' && kind === 'printing') {
        const plainUntouched = f.max_repeat_length_mm == null
  ...
  async function handleSave() {
    ...
    if (!rec.id || !rec.id.trim()) {
      ...
      rec.id = String(rec.brand || 'unknown').toLowerCase().replace(/\s+/g, '-') + '-' +
               String(rec.model || Date.now()).toLowerCase().replace(/\s+/g, '-');
    }
  ...
  const sections = kind === 'printing' ? [
    { title: 'Identity', keys: ['brand', 'model', 'starting_date', 'availability'] },
    { title: 'Power + structure', keys: ['working_voltage', 'equipment_power', 'printing_type', 'equipment_structure'] },
    { title: 'Stations + tolerance', keys: ['num_print_stations', 'num_die_stations', 'die_cut_mode', 'p2p_tolerance', 'p2c_tolerance'] },
    { title: 'Speed + geometry', keys: ['max_design_run_speed', 'feasible_max_run_speed', 'total_web_path_m', 'max_web_width_mm', 'max_usable_web_width_mm', 'min_repeat_length_mm', 'max_repeat_length_vp_mm', 'max_repeat_length_mm', 'max_material_roll_diameter_mm'] },
    { title: 'Curing + options', keys: ['curing_system_ink_types', 'has_turn_bar', 'has_rotary_screen', 'has_adhesive_deadener', 'has_rotary_cold_foil', 'has_corona_treatment', 'has_web_cleaner', 'has_curing_screen_unit'] },
  ] : [
    { title: 'Identity', keys: ['brand', 'model', 'starting_date', 'availability'] },
    { title: 'Power + structure', keys: ['working_voltage', 'equipment_power', 'cutting_type', 'cutting_mode', 'equipment_structure'] },
    { title: 'Capacity (rotary)', keys: ['num_die_stations', 'max_web_width_mm', 'min_repeat_length_mm', 'max_repeat_length_mm', 'magnetic_tooth_max', 'max_run_speed'] },
    { title: 'Capacity (flatbed / power press)', keys: ['max_sheet_size_td_mm', 'max_sheet_size_md_mm', 'max_cutting_force_tonnes', 'material_thickness_min_mm', 'material_thickness_max_mm'] },
    { title: 'Capacity (CNC)', keys: ['working_area_td_mm', 'working_area_md_mm', 'axis_count', 'max_travel_speed_mm_min', 'spindle_rpm_max', 'tool_changer_count', 'accuracy_mm', 'supported_file_formats', 'typical_use'] },
    { title: 'Quality + options', keys: ['registration_tolerance_mm', 'has_stripping', 'has_foil_capability', 'has_emboss_capability', 'has_perf_capability', 'has_turn_bar', 'has_web_cleaner', 'has_corona_treatment'] },
  ];

  return (
    <>
      <div className="mt-drawer-scrim" onClick={onClose} />
      <aside className="mt-drawer" data-ops-draggable-card role="dialog" aria-label="Machine detail">
        <header className="mt-drawer-head" data-ops-drag-handle>
          <div>
            ...
            <div className="mt-drawer-sub">
              {kind === 'printing' ? '🖨 Printing' : '✂ Cutting'} · ID: <code>{form.id || '(auto)'}</code>
            </div>
          </div>
        ...
        <fieldset disabled={!isAdmin} className="mt-form">
          ...
          <label className="mt-field mt-field-wide">
            <span>ID (slug)</span>
            <input type="text" value={form.id || ''} disabled={!isNew}
          ...
          {sections.map(sec => (
            <div key={sec.title} className="mt-section">
              ...
              <div className="mt-grid">
                {sec.keys.filter(k => fields.includes(k)).map(k => (
                  <FieldInput key={k} k={k} value={form[k]}
                    onChange={v => set(k, v)}
                    cuttingTypes={cuttingTypes}
                    isAdmin={isAdmin}
        ...
        <footer className="mt-drawer-foot">
          <button className="mt-btn" onClick={onClose}>{isAdmin ? 'Cancel' : 'Close'}</button>
          {isAdmin && (
...
function EditableEnumSelect({
  label, value, onChange, options, enumKey, placeholder, isAdmin, onReloadMeta,
}) {
    ...
    <label className="mt-field mt-field-enum">
      ...
      <div className="mt-enum-row">
        <select value={value || ''} onChange={e => onChange(e.target.value)}>
          ...
          {(options || []).map(ct => <option key={ct} value={ct}>{ct}</option>)}
        </select>
        <button
          type="button"
...
function ManageEnumModal({ label, options, enumKey, onClose, onSaved }) {
  ...
  function removeItem(idx) {
    if (!confirm(`Remove "${list[idx]}" from the list?\n\nMachines already set to this value will keep it but the option will no longer appear in new selections.`)) return;
  ...
  return (
    <Modal open onClose={onClose} size="sm" severity="info" ariaLabelledBy="mt-enum-title">
      <Modal.Header id="mt-enum-title" title={`Manage — ${label}`} subtitle="Edit, reorder or add dropdown options" severity="info" />
      <Modal.Body>
        <div className="mt-enum-body">
          <div className="mt-enum-help">
            Add / rename / reorder the options available in the <b>{label}</b> dropdown.
            Changes take effect immediately for all users after you click <b>Save</b>.
          </div>
          {list.length === 0 && (
            <div className="mt-enum-empty">No values yet — add one below.</div>
          )}
          <ul className="mt-enum-list">
            {list.map((item, idx) => (
              <li key={idx} className="mt-enum-row-item">
                ...
                />
                <button onClick={() => moveItem(idx, -1)} disabled={idx === 0} title="Move up" className="mt-enum-move">▲</button>
                <button onClick={() => moveItem(idx, +1)} disabled={idx === list.length - 1} title="Move down" className="mt-enum-move">▼</button>
                <button onClick={() => removeItem(idx)} title="Remove" className="mt-enum-remove">×</button>
              </li>
            ))}
          </ul>
          <div className="mt-enum-add">
            <input
              ...
              className="mt-enum-input"
            />
            <button onClick={addItem} className="mt-btn mt-btn-primary">+ Add</button>
          </div>
        </div>
      </Modal.Body>
      <Modal.Footer>
