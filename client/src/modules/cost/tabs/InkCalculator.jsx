...
function MeshSpecTab({ data, search, onUpdate, onAdd, onDelete }) {
  ...
  const handleChange = (idx, field, value, isNum = false) => {
    ...
    if (realIdx < 0) return;
    const parsed = isNum ? (typeof value === 'number' ? value : parseFloat(value) || 0) : value;
    const row = { ...data[realIdx], [field]: parsed };
    const recalced = ['thread_dia', 'mesh_thickness', 'mesh_opening', 'open_area_recipe'].includes(
      field
    )
      ? meshRecalc(row)
      : row;
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <table className="ink-mesh-table">
        <thead>
          <tr>
            ...
            <th>Mesh Code</th>
            <th colSpan={2}>
              Mesh Count <span style={{ fontWeight: 400, opacity: 0.7 }}>n</span>
            </th>
            <th>
              Thread Dia <span style={{ fontWeight: 400, opacity: 0.7 }}>d</span>
            </th>
            <th>
            ...
            <th>
              Open Area <span style={{ fontWeight: 400, opacity: 0.7 }}>&alpha;</span>
            </th>
            <th>
          ...
          <tr>
            ...
            <th className="unit-row"></th>
            <th className="unit-row" colSpan={2}>
              n/cm
            </th>
            ...
            <tr key={fi}>
              ...
              <td>
                <input
                  type="text"
                  value={r.mesh_code || ''}
                  onChange={(e) => handleChange(fi, 'mesh_code', e.target.value)}
                />
              </td>
              <td colSpan={2}>
                <DecimalInput
                  value={r.mesh_count}
                  onChange={(v) => handleChange(fi, 'mesh_count', v, true)}
                />
              </td>
              <td>
                <DecimalInput
                  value={r.thread_dia}
                  onChange={(v) => handleChange(fi, 'thread_dia', v, true)}
                />
              </td>
              <td>
                <DecimalInput
                  value={r.mesh_thickness}
                  onChange={(v) => handleChange(fi, 'mesh_thickness', v, true)}
                />
              </td>
              <td>
                <input
                  type="text"
                  value={r.tolerance ?? ''}
                  onChange={(e) => handleChange(fi, 'tolerance', e.target.value)}
                />
              </td>
              <td>
                <DecimalInput
                  value={r.mesh_opening}
                  onChange={(v) => handleChange(fi, 'mesh_opening', v, true)}
                />
              </td>
              <td>
                <DecimalInput
                  value={r.open_area}
                  onChange={(v) => handleChange(fi, 'open_area', v, true)}
                />
              </td>
              <td>
                <DecimalInput
                  value={r.open_area_recipe}
                  className="ink-calc-input"
                  onChange={(v) => handleChange(fi, 'open_area_recipe', v, true)}
                />
              </td>
              <td>
                <DecimalInput
                  value={r.theo_ink_volume}
                  onChange={(v) => handleChange(fi, 'theo_ink_volume', v, true)}
                />
              </td>
              <td>
                <DecimalInput
                  value={r.volume_recipe}
                  className="ink-calc-input"
                  onChange={(v) => handleChange(fi, 'volume_recipe', v, true)}
                />
              </td>
      ...
      <div className="ink-add-row">
        <button className="ink-add-btn" onClick={onAdd}>
          + Add Row
        </button>
      </div>
    </div>
      ...
      <table className="ink-anilox-table">
        <thead>
          <tr>
            ...
            <th>Anilox Code</th>
            <th>
              Line Count <span className="unit-hint">lpi</span>
            </th>
            <th>
              BCM <span className="unit-hint">10&sup6; in&sup3;/in&sup2;</span>
            </th>
            <th>
            ...
            <tr key={fi}>
              ...
              <td>
                <input
                  type="text"
                  value={r.anilox_code || ''}
                  onChange={(e) => handleChange(fi, 'anilox_code', e.target.value)}
                />
              </td>
              <td>
                <DecimalInput value={r.lpi} onChange={(v) => handleChange(fi, 'lpi', v, true)} />
              </td>
              <td>
                <DecimalInput
                  value={r.bcm}
                  onChange={(v) => handleChange(fi, 'bcm', v, true, true)}
                />
              </td>
              <td>
                <DecimalInput
                  value={r.depth_um}
                  onChange={(v) => handleChange(fi, 'depth_um', v, true)}
                />
              </td>
              <td>
                <input
              ...
              <td>
                <DecimalInput
                  value={r.cell_opening_um}
                  onChange={(v) => handleChange(fi, 'cell_opening_um', v, true)}
                />
              </td>
              <td>
                <DecimalInput
                  value={r.open_area_pct}
                  onChange={(v) => handleChange(fi, 'open_area_pct', v, true)}
                />
              </td>
              <td>
                <input
                  className="ink-auto-field"
                  type="number"
                  value={r.calc_vol ?? ''}
                  readOnly
                  title="= BCM x 1.55"
              ...
              <td>
                <DecimalInput
                  value={r.transfer_eff}
                  onChange={(v) => handleChange(fi, 'transfer_eff', v, true, true)}
                />
              </td>
              <td>
                <input
                  ...
                  value={r.vol_recipe ?? ''}
                  readOnly
                  title="= Calc Vol x Transfer Eff / 100"
                />
              </td>
              <td className="col-act">
                <button
...
function QPACostTab({ data, type, onRun }) {
  ...
  const n = (v, d = 3) =>
    v !== undefined && v !== '' && !isNaN(v) && v !== 0 ? fmtN(v, d) : '\u2014';
      ...
      <div className="ink-qpa-toolbar">
        ...
        {data.length > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            <div className="ink-summary-badge ink-badge-unit">
              Unit:{' '}
              <span style={{ fontFamily: 'monospace', fontSize: 13 }}>${fmtN(totalUnit, 5)}</span>
            </div>
            <div className="ink-summary-badge ink-badge-setup">
              Setup:{' '}
              <span style={{ fontFamily: 'monospace', fontSize: 13 }}>${fmtN(totalSetup, 5)}</span>
            </div>
            <div className="ink-summary-badge ink-badge-total">
              Total:{' '}
              <span style={{ fontFamily: 'monospace', fontSize: 14, fontWeight: 900 }}>
                ${fmtN(totalCost, 5)}
              </span>
            </div>
            <div className="ink-summary-badge ink-badge-moq">
              MOQ: {displayMoq.toLocaleString()}
            </div>
          </div>
        )}
      </div>
            ...
            <thead>
              <tr>
                <th
                  className="section"
                  colSpan={2}
                  style={{ background: isSilk ? '#1e3a5f' : '#4c1d95' }}
                >
                  Syns
                </th>
                <th
                  className="section"
                  colSpan={3}
                  style={{ background: isSilk ? '#1e40af' : '#1e40af' }}
                >
                  {specLabel} infor
                </th>
              ...
              <tr>
                ...
                <th style={{ background: '#0f766e', textAlign: 'center' }}>
                  Density
                  <br />
                  (g/cm&sup3;)
                </th>
            ...
            <tbody>
              {data.map((r, i) => (
                <tr key={r.row_label ? `${r.row_label}-${r.color || ''}-${i}` : `ic-${i}`}>
                  ...
                  <td style={{ textAlign: 'right', color: '#4c1d95', fontWeight: 700 }}>
                    {r[isSilk ? 'mesh_count' : 'lpi'] || '\u2014'}
                  </td>
                  ...
                  <td style={{ textAlign: 'right', color: '#6d28d9', fontWeight: 700 }}>
                    {r.total_area ? Math.round(r.total_area).toLocaleString() : '\u2014'}
                  </td>
                  <td style={{ textAlign: 'right', color: '#7c3aed' }}>{n(r.ink_volume, 4)}</td>
                  <td style={{ textAlign: 'right', color: '#7c3aed', fontWeight: 700 }}>
                    {n(r.actual_ink_vol, 4)}
                  </td>
                  <td style={{ textAlign: 'right', color: '#7c3aed', fontWeight: 700 }}>
                    {n(r.ink_weight, 4)}
                  </td>
                  ...
                  <td
                    style={{ textAlign: 'center', color: '#92400e', fontSize: 10, fontWeight: 600 }}
                  >
                    {r.setup_kg_ovr > 0
                      ? fmtN(r.setup_kg_ovr, 4)
                      : r.auto_setup_kg > 0
                        ? fmtN(r.auto_setup_kg, 4)
                        : '\u2014'}
                  </td>
                  <td style={{ textAlign: 'right', color: '#dc2626' }}>{n(r.waste, 4)}</td>
                  <td style={{ textAlign: 'right', color: '#b45309', fontWeight: 700 }}>
                    {n(r.weight_per_time, 4)}
                  </td>
                  <td style={{ textAlign: 'right', color: '#92400e' }}>{n(r.theo_supply, 4)}</td>
                  <td style={{ textAlign: 'right', color: '#166534', fontWeight: 700 }}>
                    {r.unit_per_kg ? r.unit_per_kg.toLocaleString() : '\u2014'}
                  </td>
                  <td style={{ textAlign: 'right', color: '#166534' }}>
                    {r.m2_per_kg ? r.m2_per_kg.toLocaleString() : '\u2014'}
                  </td>
                  <td style={{ textAlign: 'right', color: '#475569' }}>
                    {r.layout_cavities || '\u2014'}
                  </td>
                  <td style={{ textAlign: 'right', color: '#1d4ed8', fontWeight: 700 }}>
                    {n(r.qpa_kg, 6)}
                  </td>
                  <td
                    style={{ textAlign: 'center', color: '#7c3aed', fontWeight: 700, fontSize: 10 }}
                  >
                    {r.ink_price_ovr > 0
                      ? fmtN(r.ink_price_ovr, 2)
                      : r.auto_price > 0
                        ? fmtN(r.auto_price, 2)
                        : '\u2014'}
                  </td>
  ...
  const formulas = isSilk
    ? [
        ...
        [
          'L',
          'Actual Ink Vol (cm³)',
          'Thể tích mực thực tế (cm³)',
          '= Ink Vol Max × Print Area %',
          '#ede9fe',
          '#6d28d9',
        ],
        [
    ...
    : [
        ...
        [
          'J',
          'V_r (Volume Recipe)',
          'V_r (Recipe khối lượng)',
          '= Calc Vol × Transfer Eff / 100',
          '#dbeafe',
          '#1e40af',
        ],
        [
    ...
    <div className="ink-legend">
      <div className="ink-legend-header">
        ...
        <div className="ink-legend-sub">
          <div>
            Reference guide for formulas, color coding, and {isSilk ? 'Silkscreen' : 'Flexo'}{' '}
            printing notes
          </div>
          <div className="ink-legend-sub-vi">
            Hướng dẫn công thức, mã màu và ghi chú in {isSilk ? 'lụa (Silkscreen)' : 'Flexo'}
          </div>
        </div>
      </div>
        ...
        <div className="ink-formula-grid">
          {formulas.map(([col, nameEn, nameVi, formula, bg, color]) => (
            <div key={col} className="ink-formula-item" style={{ background: bg }}>
              <div className="ink-formula-badge" style={{ background: color }}>
                {col}
              </div>
              <div>
                <div className="ink-formula-name">{nameEn}</div>
                <div className="ink-formula-name-vi">{nameVi}</div>
                <div className="ink-formula-expr" style={{ color }}>
                  {formula}
                </div>
              </div>
            </div>
...
export default function InkCalculator() {
  ...
  // Mesh Spec handlers
  const updateMeshRow = useCallback(
    (idx, row) => {
      const newData = {
        ...inkCalc,
        silkscreen: {
          ...inkCalc.silkscreen,
          meshSpec: inkCalc.silkscreen.meshSpec.map((r, i) => (i === idx ? row : r)),
        },
      };
      setInkCalc(newData);
    },
  ...
  const deleteMeshRow = useCallback(
    (idx) => {
      const newData = {
        ...inkCalc,
        silkscreen: {
          ...inkCalc.silkscreen,
          meshSpec: inkCalc.silkscreen.meshSpec.filter((_, i) => i !== idx),
        },
      };
      setInkCalc(newData);
    },
  ...
  // Anilox DB handlers
  const updateAniloxRow = useCallback(
    (idx, row) => {
      const newData = {
        ...inkCalc,
        flexo: {
          ...inkCalc.flexo,
          aniloxDB: inkCalc.flexo.aniloxDB.map((r, i) => (i === idx ? row : r)),
        },
      };
      setInkCalc(newData);
    },
  ...
  const deleteAniloxRow = useCallback(
    (idx) => {
      const newData = {
        ...inkCalc,
        flexo: { ...inkCalc.flexo, aniloxDB: inkCalc.flexo.aniloxDB.filter((_, i) => i !== idx) },
      };
      setInkCalc(newData);
    },
    [inkCalc]
  ...
  const runSilkscreen = useCallback(
    (source) => {
      if (!inkCalc) return;
  ...
  const silkTotal =
    (inkCalc.silkscreen?.meshSpec?.length || 0) + (inkCalc.silkscreen?.qpaCost?.length || 0);
  ...
  if (mainTab === 'silkscreen') {
    if (silkSub === 'meshSpec') {
      content = (
        <MeshSpecTab
          data={inkCalc.silkscreen.meshSpec}
          search={search}
          onUpdate={updateMeshRow}
          onAdd={addMeshRow}
          onDelete={deleteMeshRow}
        />
      );
    } else if (silkSub === 'qpaCost') {
      content = (
        <QPACostTab data={inkCalc.silkscreen.qpaCost} type="silkscreen" onRun={runSilkscreen} />
      );
    } else {
    ...
    if (flexoSub === 'aniloxDB') {
      content = (
        <AniloxDBTab
          data={inkCalc.flexo.aniloxDB}
          search={search}
          onUpdate={updateAniloxRow}
          onAdd={addAniloxRow}
          onDelete={deleteAniloxRow}
        />
      );
  ...
  return (
    <div className="ink-calc">
      ...
      <div className="ink-calc-header">
        <div className="ink-calc-header-top">
          ...
          <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
            <button
              className={`ink-tab-btn ${mainTab === 'silkscreen' ? 'active' : ''}`}
              onClick={() => {
                setMainTab('silkscreen');
                setSearch('');
              }}
            >
            ...
            <button
              className={`ink-tab-btn ${mainTab === 'flexo' ? 'active' : ''}`}
              onClick={() => {
                setMainTab('flexo');
                setSearch('');
              }}
      ...
      <div className="ink-body">
        <div className="ink-panel">{content}</div>
      </div>
    </div>
  );
}
