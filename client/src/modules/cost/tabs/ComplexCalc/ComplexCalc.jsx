...
import { calcPacking, calcShipping, aggregateComplex, serializeResultForPersist } from '../../../../services/calcEngine';
import { addSubProduct, removeSubProduct, setSpField, setSpMaterialField, setSpProcessField } from '../../../../context/calcReducer';
...
export default function ComplexCalc() {
  ...
  useEffect(() => {
    ...
    showToast('Prefilled from Design Tools / RFQ — geometry for each SP must still be set per sub-product');
    clearPendingQuote();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pendingQuote, loadQuote, clearPendingQuote]);
  ...
  const { spResults, aggregate, calcErrors } = useMemo(() => {
    ...
    const { aggregate: agg, pass2, errors } = aggregateComplex(cs, sps, lib, tierIdx, { bomQtyEnabled, spMoqScalingEnabled });
    ...
    for (const e of errors) {
      console.error(`[ComplexCalc] pass${e.pass} calcAll failed for SP "${e.code}" (idx=${e.spi}): ${e.message}`);
    }
    ...
    if (sp > 0) {
      ...
      agg.contribution = 1 - ((agg.s_mat_cost || 0) + (agg.tooling || 0) + (agg.packing_ship || 0) + (agg.labor_cost || 0)) / sp;
    } else {
  ...
  const handleAddSp = useCallback(() => {
    ...
    let code = 'SP A';
    for (const l of letters) { const c = `SP ${l}`; if (!usedCodes.includes(c)) { code = c; break; } }
  ...
  const removeSp = useCallback((idx) => { dispatch(removeSubProduct({ idx })); }, [dispatch]);
  const toggleSp = useCallback((spi) => { setExpandedSps(prev => ({ ...prev, [spi]: !prev[spi] })); }, []);
  ...
  return (
    <div className="cc">
      ...
      <div className="cc-tab-bar">
        <TabBarOverflow ariaLabel="Pricing Worksheet (Complex) sub-tabs" rightSlot={
          <>
            ...
            <button className="cc-btn cc-btn-outline" onClick={handleReset}>New</button>
            <button className="cc-btn cc-btn-primary" onClick={handleSave} disabled={!isDirty || saving}>{saving ? 'Saving…' : 'Save'}</button>
          </>
        }>
          {SUB_TABS.map(tab => (
            <button
          ...
          <>
            ...
            <div className="cc-sp-table-card">
              <div className="cc-sp-table-header">
                ...
                <button className="cc-btn cc-btn-add-sp" onClick={handleAddSp}>+ Add Sub-Product</button>
              </div>
                ...
                <table className="cc-spt">
                  <thead>
                    <tr>
                      ...
                      <th className="right" style={{ width: 85 }}>Mat. Cost</th>
                      <th className="right" style={{ width: 85 }}>Mach. Cost</th>
                      <th className="right" style={{ width: 85 }}>Labor Cost</th>
                      <th className="right" style={{ width: 75 }}>Tooling</th>
                      <th className="right" style={{ width: 90 }}>G.Total</th>
                  ...
                  <tbody>
                    {sps.map((sp, spi) => {
                      ...
                      const size = sp.part_width && sp.part_length_md ? `${sp.part_width}×${sp.part_length_md}` : '\u2014';
                      return (
                        <React.Fragment key={spi}>
                          {/* Summary row */}
                          <tr className={`cc-spt-row ${isFG ? 'cc-spt-fg' : ''} ${isExpanded ? 'cc-spt-expanded' : ''}`}>
                            <td>
                              <button className={`cc-spt-toggle ${isExpanded ? 'open' : ''}`} onClick={() => toggleSp(spi)}>
                                {isExpanded ? '\u25BC' : '\u25B6'}
                              </button>
                            </td>
                            <td>
                              <input type="text" value={sp.code || ''} className="cc-spt-code" style={isFG ? { background: '#f5f3ff', color: '#7c3aed', fontWeight: 800 } : {}}
                                onChange={e => dispatch(setSpField({ spIdx: spi, field: 'code', value: e.target.value }))} />
                            </td>
                            <td>
                              <input type="text" value={sp.description || ''} placeholder="Description" className="cc-spt-input"
                                onChange={e => dispatch(setSpField({ spIdx: spi, field: 'description', value: e.target.value }))} />
                            </td>
                            <td>
                              <input type="text" value={sp.main_process || ''} placeholder="Process" className="cc-spt-input"
                                onChange={e => dispatch(setSpField({ spIdx: spi, field: 'main_process', value: e.target.value }))} />
                            </td>
                            <td className="cc-spt-size">{size}</td>
                            <td className="right cc-spt-cost" style={{ color: '#2563eb' }}>{r ? fmtN(r.s_mat_cost) : '\u2014'}</td>
                            <td className="right cc-spt-cost" style={{ color: '#d97706' }}>{r ? fmtN(r.overhead) : '\u2014'}</td>
                            <td className="right cc-spt-cost" style={{ color: '#16a34a' }}>{r ? fmtN(r.labor_cost) : '\u2014'}</td>
                            <td className="right cc-spt-cost" style={{ color: '#7c3aed' }}>{r ? fmtN(r.tooling) : '\u2014'}</td>
                            <td className="right cc-spt-gtotal">{r ? fmtN(r.s_ttl) : '\u2014'}</td>
                            <td className="cc-spt-actions">
                              <button className="cc-spt-act-btn" onClick={() => toggleSp(spi)} title="Toggle detail">{isExpanded ? '\u25B2' : '\u25BC'}</button>
                              <button className="cc-spt-act-btn cc-spt-act-del" onClick={() => removeSp(spi)} title="Remove">&times;</button>
                            </td>
                          </tr>
                  ...
                  {sps.length > 0 && (
                    <tfoot>
                      <tr className="cc-spt-footer">
                        <td colSpan={5}><b>{sps.some(s => (s.code || '').toUpperCase().startsWith('FG')) ? 'FG Total' : 'TOTAL'}</b></td>
                        <td className="right">{aggregate ? fmtN(aggregate.s_mat_cost) : '\u2014'}</td>
                        <td className="right">{aggregate ? fmtN(aggregate.overhead) : '\u2014'}</td>
                        <td className="right">{aggregate ? fmtN(aggregate.labor_cost) : '\u2014'}</td>
                        <td className="right">{aggregate ? fmtN(aggregate.tooling) : '\u2014'}</td>
                        <td className="right" style={{ fontWeight: 900, fontSize: 13 }}>{aggregate ? fmtN(aggregate.s_ttl) : '\u2014'}</td>
                        <td></td>
                      </tr>
                    </tfoot>
                  )}
              ...
              {sps.length === 0 && <div className="cc-empty">No sub-products yet. Click <b>+ Add Sub-Product</b> to begin.</div>}
            </div>
          </>
        )}
        ...
        {activeSubTab === 'packing' && (
          <div className="sc-section">
            ...
            <div className="cc-pack-hint" role="note">
              ▣ These Pack &amp; Ship values apply to the <b>whole assembly</b> (per shipped unit). They add on top of any per-sub-product packing entered in the Calculators tab, matching how Standard quotes roll up packing and shipping into the final price.
            </div>
            <div className="sc-pack-grid">
              <div className="sc-card">
                ...
                <div className="sc-card-body">
                  <div className="sc-field"><label>Packing Method</label><select value={cs.packing_method || 'Sheet'} onChange={e => setCplxField('packing_method', e.target.value)} className="sc-input"><option value="Sheet">Sheet</option><option value="Roll">Roll</option><option value="Tray">Tray</option><option value="PE Bag">PE Bag</option></select></div>
                  <div className="sc-field"><label>{labels.pcsLabel}</label><DecimalInput value={cs.pcs_per_bag} placeholder="—" onChange={v => setCplxField('pcs_per_bag', v)} className="sc-input" /></div>
                  <div className="sc-field"><label>{labels.bagLabel}</label><DecimalInput value={cs.bags_per_box} placeholder="—" onChange={v => setCplxField('bags_per_box', v)} className="sc-input" /></div>
                  <div className="sc-field"><label>{labels.containerLabel}</label><DecimalInput value={cs.container_cost} placeholder="—" onChange={v => setCplxField('container_cost', v)} className="sc-input" /></div>
                  <div className="sc-field"><label>Box Cost (USD)</label><DecimalInput value={cs.box_cost} placeholder="—" onChange={v => setCplxField('box_cost', v)} className="sc-input" /></div>
                  <div className="sc-field"><label>Other Packing/pcs</label><DecimalInput value={cs.other_packing} placeholder="—" onChange={v => setCplxField('other_packing', v)} className="sc-input" /></div>
                  <div className="sc-pack-total">Total Packing/pcs: <b>{fmtN(packTotal, 6)} USD</b></div>
                </div>
              </div>
              <div className="sc-card">
                ...
                <div className="sc-card-body">
                  <div className="sc-field"><label>Delivery Term</label><input type="text" value={cs.delivery_term || ''} onChange={e => setCplxField('delivery_term', e.target.value)} className="sc-input" placeholder="FOB, CIF, etc." /></div>
                  <div className="sc-field"><label>Deliver Quantity</label>
                    ...
                    <DecimalInput
                      ...
                      className="sc-input"
                      placeholder="—"
                      style={cs.ship_qty > 0 ? { color: 'var(--color-violet-500)', fontWeight: 700 } : {}}
                      title={cs.ship_qty > 0 ? 'Override — clear to revert to MOQ' : 'Auto-synced from MOQ'}
                    />
                  </div>
                  <div className="sc-field"><label>Shipping Cost (USD total)</label><DecimalInput value={cs.shipping_cost} placeholder="—" onChange={v => setCplxField('shipping_cost', v)} className="sc-input" /></div>
                  <div className="sc-field"><label>Other Cost/shipment</label><DecimalInput value={cs.other_ship} placeholder="—" onChange={v => setCplxField('other_ship', v)} className="sc-input" /></div>
                  <div className="sc-ship-total">Total Shipping/pcs: <b>{fmtN(shipTotal, 6)} USD</b></div>
                </div>
              </div>
            </div>
          </div>
        ...
        {activeSubTab === 'summary' && aggregate && (
          <div className="cc-summary">
            <div className="cc-sum-kpis">
              <div className="cc-sum-kpi" style={{ borderColor: '#0f2341' }}><div className="cc-sum-kpi-label">MOQ</div><div className="cc-sum-kpi-val">{(cs.moq || 0).toLocaleString()}</div><div className="cc-sum-kpi-sub">EA</div></div>
              <div className="cc-sum-kpi" style={{ borderColor: '#dc2626' }}><div className="cc-sum-kpi-label">Total Cost</div><div className="cc-sum-kpi-val" style={{ color: '#dc2626' }}>${fmtN(aggregate.s_ttl)}</div><div className="cc-sum-kpi-sub">USD</div></div>
              <div className="cc-sum-kpi" style={{ borderColor: '#1e40af' }}><div className="cc-sum-kpi-label">Selling Price</div><div className="cc-sum-kpi-val">${fmtN(cs.selling_price || 0, 4)}</div><div className="cc-sum-kpi-sub">USD</div></div>
              <div className="cc-sum-kpi" style={{ borderColor: '#0891b2' }}><div className="cc-sum-kpi-label">VA%</div><div className="cc-sum-kpi-val" style={{ color: '#0891b2' }}>{pct(aggregate.va)}</div><div className="cc-sum-kpi-sub">Value Add</div></div>
              <div className="cc-sum-kpi" style={{ borderColor: '#7c3aed' }} title={KPI_TOOLTIPS.contribution}><div className="cc-sum-kpi-label">Contr%</div><div className="cc-sum-kpi-val" style={{ color: '#7c3aed' }}>{pct(aggregate.contribution)}</div><div className="cc-sum-kpi-sub">Contribution</div></div>
              <div className="cc-sum-kpi" style={{ borderColor: gmClr(aggregate.gm) }}><div className="cc-sum-kpi-label">GM%</div><div className="cc-sum-kpi-val" style={{ color: gmClr(aggregate.gm) }}>{pct(aggregate.gm)}</div><div className="cc-sum-kpi-sub">Gross Margin</div></div>
            </div>
            <div className="cc-card" style={{ marginTop: 14 }}>
              ...
              <div className="cc-card-body">
                {[
                  ...
                  const share = aggregate.s_ttl > 0 ? ((r.val / aggregate.s_ttl) * 100).toFixed(1) : 0;
                  const maxV = Math.max(aggregate.s_mat_cost || 0, aggregate.overhead || 0, aggregate.labor_cost || 0, aggregate.tooling || 0, aggregate.packing_ship || 0, 0.001);
                  return (
                    <div key={i} className="cc-sum-bar-row">
                      <div className="cc-sum-bar-label"><span>{r.icon} {r.label}</span><span style={{ color: '#94a3b8' }}>{share}%</span></div>
                      <div className="cc-sum-bar-track"><div className="cc-sum-bar-fill" style={{ width: (r.val / maxV * 100) + '%', background: r.clr }} /></div>
                      <div className="cc-sum-bar-val" style={{ color: r.clr }}>${fmtN(r.val)}</div>
                    </div>
                  );
                })}
                <div className="cc-sum-bar-row cc-sum-bar-total">
                  <div className="cc-sum-bar-label"><b>GRAND TOTAL</b></div>
                  <div className="cc-sum-bar-track" />
                  <div className="cc-sum-bar-val" style={{ fontWeight: 900, fontSize: 14, color: '#0f2341' }}>${fmtN(aggregate.s_ttl)}</div>
                </div>
              </div>
            </div>
          </div>
...
function ComplexMoqTab({ cs, sps, dispatch, setCplxField }) {
  ...
  const addMoqTier = useCallback(() => {
    const next = [...extraMoqs, { moq: 0, eau: null, price: null, target: null, price_vnd: null, target_vnd: null }];
  ...
  const removeMoqTier = useCallback((idx) => {
    ...
    dispatch({ type: 'SET_CPLX_STATE', payload: { num_moq: Math.max(1, numMoq - 1), extra_moqs: next, active_moq_idx: 0 } });
  }, [dispatch, extraMoqs, numMoq]);
  ...
  // ── Setup overrides ──
  const setSpMatSetupForTier = useCallback((tierIdx, spi, mi, valueStr) => {
    if (tierIdx === 0) {
  ...
  const setSpProcSetupForTier = useCallback((tierIdx, spi, pi, valueStr) => {
    if (tierIdx === 0) {
      const v = valueStr === '' ? 0 : (parseFloat(valueStr) || 0);
  ...
  const setSellingUsd = useCallback((v) => {
          ...
          <table className="sc-moq-table">
            ...
            <tbody>
              ...
              <tr className={`sc-moq-tr ${activeMoqIdx === 0 ? 'active' : ''}`}>
                ...
                <td><DecimalInput value={cs.moq} onChange={v => setCplxField('moq', v)} className="sc-moq-inp" thousandSep /></td>
                <td><DecimalInput value={cs.annual_qty} onChange={v => setCplxField('annual_qty', v)} className="sc-moq-inp" thousandSep /></td>
                <td><DecimalInput value={cs.selling_price} onChange={setSellingUsd} className="sc-moq-inp sc-moq-inp-gold" /></td>
                <td><DecimalInput value={cs.selling_price_vnd} onChange={setSellingVnd} className="sc-moq-inp sc-moq-inp-gold" thousandSep /></td>
                <td><DecimalInput value={cs.target} onChange={setTargetUsd} className="sc-moq-inp" /></td>
                <td><DecimalInput value={cs.target_vnd} onChange={setTargetVnd} className="sc-moq-inp" thousandSep /></td>
                <td></td>
              </tr>
              ...
              {extraMoqs.map((em, idx) => (
                <tr key={idx} className={`sc-moq-tr ${activeMoqIdx === idx + 1 ? 'active' : ''}`}>
                  ...
                  <td><DecimalInput value={em.moq} onChange={v => updateExtraMoqWithRate(idx, 'moq', v)} className="sc-moq-inp" thousandSep /></td>
                  <td><DecimalInput value={em.eau} onChange={v => updateExtraMoqWithRate(idx, 'eau', v)} className="sc-moq-inp" thousandSep /></td>
                  <td><DecimalInput value={em.price} onChange={v => updateExtraMoqWithRate(idx, 'price', v)} className="sc-moq-inp sc-moq-inp-gold" /></td>
                  <td><DecimalInput value={em.price_vnd} onChange={v => updateExtraMoqWithRate(idx, 'price_vnd', v)} className="sc-moq-inp sc-moq-inp-gold" thousandSep /></td>
                  <td><DecimalInput value={em.target} onChange={v => updateExtraMoqWithRate(idx, 'target', v)} className="sc-moq-inp" /></td>
                  <td><DecimalInput value={em.target_vnd} onChange={v => updateExtraMoqWithRate(idx, 'target_vnd', v)} className="sc-moq-inp" thousandSep /></td>
              ...
              <table className="cc-setup-moq-table">
                ...
                <tbody>
                  <tr className="cc-smt-row-moq">
                    <td className="cc-smt-td-field"><b>MOQ</b></td>
                    <td className="cc-smt-td-db">MOQ &amp; Pricing info</td>
                    {Array.from({ length: numMoq }, (_, ti) => {
                      const qty = ti === 0 ? (cs.moq || 0) : ((extraMoqs[ti - 1] || {}).moq || 0);
                      return <td key={ti} className="cc-smt-td-num cc-smt-td-moq">{fmtIntLocal(qty)}</td>;
                    })}
                    <td className="cc-smt-td-uom">Qty</td>
                  </tr>
                  {flatMats.map(({ sp, spi, m, mi }) => {
                    const lbl = m.desc ? (m.desc.length > 22 ? m.desc.substring(0, 22) + '\u2026' : m.desc) : (m.code || `Mat ${mi + 1}`);
                    return (
                      <tr key={`m-${spi}-${mi}`}>
                        <td className="cc-smt-td-field" title={`${m.code || ''} - ${m.desc || ''}`}>{lbl}</td>
                        <td className="cc-smt-td-db"><span className="cc-smt-sp-tag">{sp.code}</span> Materials</td>
                        {Array.from({ length: numMoq }, (_, ti) => {
                          const isBase = ti === 0;
                          const em = isBase ? null : (extraMoqs[ti - 1] || {});
                          const baseVal = m.setup_lm != null ? m.setup_lm : 0;
                          const ovrVal = !isBase && em.sp_mat_setup_lm && em.sp_mat_setup_lm[spi] && em.sp_mat_setup_lm[spi][mi] != null
                            ? em.sp_mat_setup_lm[spi][mi] : null;
                  ...
                  {flatProcs.map(({ sp, spi, p, pi }) => {
                    const lbl = p.label ? (p.label.length > 22 ? p.label.substring(0, 22) + '\u2026' : p.label) : `Process ${pi + 1}`;
                    return (
                      <tr key={`p-${spi}-${pi}`}>
                        <td className="cc-smt-td-field">{lbl}</td>
                        <td className="cc-smt-td-db"><span className="cc-smt-sp-tag">{sp.code}</span> Processes</td>
                        {Array.from({ length: numMoq }, (_, ti) => {
                          const isBase = ti === 0;
                          const em = isBase ? null : (extraMoqs[ti - 1] || {});
                          const baseVal = p.setup_h != null ? p.setup_h : 0;
                          const ovrVal = !isBase && em.sp_proc_setup_h && em.sp_proc_setup_h[spi] && em.sp_proc_setup_h[spi][pi] != null
