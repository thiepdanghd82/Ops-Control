...
import { getProcessOptions, getWCOptionsByType, getToolLife, getRateByWC, calcPitch, calcLayoutPerSheet, calcPcsPerRoll } from '../../../../services/calcEngine';
import {
  setSpField, setSpMaterialField, setSpInkField, setSpProcessField,
  addSpMaterialRow, removeSpMaterialRow, addSpInkRow, removeSpInkRow,
  addSpProcessRow, removeSpProcessRow,
...
export default function SubProductRow({ sp, spi, result, allSps }) {
  ...
  const setF = useCallback((f, v) => dispatch(setSpField({ spIdx: spi, field: f, value: v })), [dispatch, spi]);
  const setMat = useCallback((mi, f, v) => dispatch(setSpMaterialField({ spIdx: spi, idx: mi, field: f, value: v })), [dispatch, spi]);
  const setInk = useCallback((ii, f, v) => dispatch(setSpInkField({ spIdx: spi, idx: ii, field: f, value: v })), [dispatch, spi]);
  const setProc = useCallback((pi, f, v) => dispatch(setSpProcessField({ spIdx: spi, idx: pi, field: f, value: v })), [dispatch, spi]);
  ...
  const setSpSetupLmActive = useCallback((mi, value) => {
  ...
  const setSpSetupHActive = useCallback((pi, value) => {
    if (activeMoqIdxCpx === 0) {
  ...
  const getSpSetupLmActive = useCallback((m, mi) => {
  ...
  const getSpSetupHActive = useCallback((p, pi) => {
  ...
  const openInkMenu = useCallback((ii, event) => {
    openMenu({
      event,
      onPick: (hit) => {
  ...
  const removeProc = useCallback((idx) => dispatch(removeSpProcessRow({ spIdx: spi, idx })), [dispatch, spi]);

  const processOpts = useMemo(() => getProcessOptions(), []);
  const printTypeOpts = useMemo(() => (lib?.ddl?.print_type_list || []).filter(Boolean), [lib]);
  const meshOpts = useMemo(() => (lib?.inkCalc?.silkscreen?.meshSpec || []).map(m => m.mesh_code).filter(Boolean), [lib]);
  const toolTypeOpts = useMemo(() => lib?.ddl?.tool_life ? Object.keys(lib.ddl.tool_life) : [], [lib]);
  ...
  const procTotals = useMemo(() => {
    let setupMach = 0, setupLabor = 0, runMach = 0, runLabor = 0, tooling = 0;
    ...
    return { setupMach, setupLabor, runMach, runLabor, tooling, total: setupMach + setupLabor + runMach + runLabor + tooling };
  }, [procResults]);
  ...
  const pitch = useMemo(() => { try { return calcPitch(sp); } catch { return 0; } }, [sp]);
  const layoutSheet = useMemo(() => { try { return calcLayoutPerSheet(sp); } catch { return 0; } }, [sp]);
  const pcsRoll = useMemo(() => { try { return calcPcsPerRoll(sp); } catch { return 0; } }, [sp]);
  ...
  const validation = useMemo(() => {
    try { return validateLayout(sp, sp.materials || [], {}, activeProfile); }
      ...
      <div className="cc-det-section">
        <div className="sc-card">
          <div className="sc-card-header sc-header-teal">
            ...
            {sp.code && (
              <span className="cl-sp-code">{sp.code}</span>
            )}
            ...
            {activeProfile && (
              <div className="cl-press-picker">
                <span className="cl-press-meta">
                  ...
                  {activeProfile.common_dies?.length ? ` · ${activeProfile.common_dies.length} dies` : ''}
                </span>
              </div>
            )}
            ...
            <div className="cl-top-grid">
              ...
              <div className="cl-params">
                ...
                <div className="sc-grid4" style={{ marginTop: 8 }}>
                  <div className="sc-field"><label>Ship Qty</label><DecimalInput value={sp.ship_qty} onChange={v => setF('ship_qty', v)} className="sc-input" /></div>
                </div>
        ...
        <div className="sc-card">
          <div className="sc-card-header sc-header-blue">
            ...
            <div className="sc-header-totals">
              <span className="sc-header-total-item">Setup: <b>${fmtN(matTotals.setup)}</b></span>
              <span className="sc-header-total-item">Run: <b>${fmtN(matTotals.run)}</b></span>
              <span className="sc-header-total-item sc-header-total-main">Total: <b>${fmtN(matTotals.total)}</b></span>
            </div>
          </div>
          ...
          <table className="cc-det-tbl">
            <thead>
              <tr>
                ...
                <th style={{ width: 60 }} title="Override pitch from layout">Pitch</th>
                ...
                <th className="sc-col-derived" style={{ width: 60 }}>QPA (m²)</th>
                <th className="sc-col-derived" style={{ width: 60 }}>QPA (lm)</th>
                <th className="sc-col-derived" style={{ width: 50 }}>L/Sheet</th>
                <th className="sc-col-derived" style={{ width: 45 }}>Webs</th>
                <th className="sc-col-derived" style={{ width: 55 }}>Scrap%</th>
                <th className="sc-col-result" style={{ width: 65 }}>Setup Cost</th>
                <th className="sc-col-result" style={{ width: 65 }}>Run Cost</th>
                <th className="sc-col-result" style={{ width: 65 }}>Total</th>
                <th style={{ width: 30 }}></th>
              </tr>
            </thead>
            <tbody>
              {(() => { let mc = 0, pc = 0; return mats.map((m, mi) => {
                ...
                const hasErr = hasCode && (!m.usage || !(m.width > 0 || sp.web_width_td > 0) || !(m.cavities > 0 || layoutSheet > 0));
                const selClass = !hasCode ? 'sc-row-type-dim' : (isMain ? 'sc-row-type-main' : 'sc-row-type-proc');
                const numClass = !hasCode ? 'sc-row-num-dim' : (isMain ? 'sc-row-num-main' : 'sc-row-num-proc');
                const rowBg = isRef ? { background: 'var(--color-violet-50)' } : {};
                return (
                  <tr key={m._mid || mi} className={hasErr ? 'sc-row-warn' : ''} style={rowBg} onContextMenu={e => openMatMenu(mi, e)}>
                    <td>
                      <span className={`sc-row-label-cell${hasCode ? '' : ' sc-row-label-empty'}`}>
                        <select value={m.row_type || 'Main.Mat'} onChange={e => setMat(mi, 'row_type', e.target.value)} className={`sc-row-type-sel ${selClass}`}>
                    ...
                    <td><input type="text" value={m.code || ''} onChange={e => setMat(mi, 'code', e.target.value)} onBlur={e => { const f = getMatByCode(e.target.value); if (f) setMat(mi, 'desc', f.description || f.desc || ''); }} className="cc-det-inp" style={isRef ? { color: 'var(--color-violet-500)', fontWeight: 700 } : {}} /></td>
                    <td><input type="text" value={m.desc || ''} onChange={e => setMat(mi, 'desc', e.target.value)} className="cc-det-inp cc-det-desc" /></td>
                    <td><DecimalInput value={m.usage} onChange={v => setMat(mi, 'usage', v)} className={`cc-det-inp cc-det-num ${!m.usage && hasCode ? 'sc-input-err' : ''}`} /></td>
                    {/* Sprint 1.6 — per-MOQ Setup LM (mirrors Standard fix). */}
                    <td><DecimalInput
                      value={getSpSetupLmActive(m, mi)}
                      onChange={v => setSpSetupLmActive(mi, v)}
                      title={isSpSetupLmOverride(mi)
                        ? `MOQ ${activeMoqIdxCpx + 1} override (base = ${m.setup_lm ?? 0})`
                        : (activeMoqIdxCpx === 0 ? 'Base value (MOQ 1)' : `Inherits MOQ 1 base (${m.setup_lm ?? 0}) — type to override`)}
                    ...
                    <td><DecimalInput
                      value={m.pitch_ovr > 0 ? m.pitch_ovr : (pitch > 0 ? Number(pitch.toFixed(1)) : 0)}
                      onChange={v => setMat(mi, 'pitch_ovr', v)}
                      className="cc-det-inp cc-det-num"
                      style={m.pitch_ovr > 0
                        ? { color: 'var(--color-violet-500)', fontWeight: 700 }
                        : { color: 'var(--color-slate-900)' }}
                      title={m.pitch_ovr > 0 ? 'Override — clear to revert to layout pitch' : 'Auto-synced from Layout'}
                    /></td>
                    <td><DecimalInput
                      value={m.width > 0 ? m.width : (sp.web_width_td > 0 ? sp.web_width_td : 0)}
                      onChange={v => setMat(mi, 'width', v)}
                      className={`cc-det-inp cc-det-num ${!(m.width > 0) && !(sp.web_width_td > 0) && hasCode ? 'sc-input-err' : ''}`}
                      style={m.width > 0
                        ? { color: 'var(--color-violet-500)', fontWeight: 700 }
                        : { color: 'var(--color-slate-900)' }}
                      title={m.width > 0 ? 'Override — clear to revert to layout Web Width TD' : 'Auto-synced from Layout Web Width TD'}
                    /></td>
                    <td><DecimalInput
                      ...
                      style={m.cavities > 0
                        ? { color: 'var(--color-violet-500)', fontWeight: 700 }
                        : { color: 'var(--color-slate-900)' }}
                      title={m.cavities > 0 ? 'Override — clear to revert to Layout/Sheet' : 'Auto-synced from Layout/Sheet'}
                    /></td>
                    <td><select value={m.offcut_yn || ''} onChange={e => setMat(mi, 'offcut_yn', e.target.value)} className="cc-det-sel cc-select-bare"><option value="Y">Y</option><option value="N">N</option><option value=""></option></select></td>
                        ...
                        <td>{yn === 'Y' ? (
                          <DecimalInput
                            ...
                            style={overridden
                              ? { color: 'var(--color-violet-500)', fontWeight: 700 }
                              : { color: 'var(--color-slate-900)' }}
                            title={overridden ? 'Override — clear to revert to auto' : 'Auto = MOD(Cavities, Width) / Cavities'}
                          />
                        ) : (
                          <input type="text"
                            value={shown}
                    ...
                    <td><select value={m.slitting_yn || ''} onChange={e => setMat(mi, 'slitting_yn', e.target.value)} className="cc-det-sel cc-select-bare"><option value="Y">Y</option><option value="N">N</option><option value=""></option></select></td>
                    <td><DecimalInput value={m.g_price} onChange={v => setMat(mi, 'g_price', v)} className="cc-det-inp cc-det-num" /></td>
                    <td><DecimalInput value={m.latest} onChange={v => setMat(mi, 'latest', v)} className="cc-det-inp cc-det-num" style={m.latest ? { color: 'var(--color-violet-500)', fontWeight: 700 } : {}} /></td>
                    ...
                    <td><button className="sc-btn-del-circle sc-btn-del-sm" onClick={() => removeMat(mi)} title="Remove">&times;</button></td>
                  </tr>
                );
              }); })()}
            </tbody>
          </table>
          <div className="sc-add-row"><button className="sc-btn-ghost" onClick={addMat}>+ Add Material Row</button></div>
          </div>
        </div>
      </div>
      ...
      <div className="cc-det-section">
        <div className="sc-card">
          <div className="sc-card-header sc-header-cyan">
            ...
            <div className="sc-header-totals">
              <span className="sc-header-total-item">Setup: <b>${fmtN(inkTotals.setup)}</b></span>
              <span className="sc-header-total-item">Run: <b>${fmtN(inkTotals.run)}</b></span>
              <span className="sc-header-total-item sc-header-total-main">Total: <b>${fmtN(inkTotals.total)}</b></span>
            </div>
          </div>
          <div className="sc-card-body cc-det-tw">
          <table className="cc-det-tbl">
            <thead>
              <tr>
                ...
                <th style={{ width: 55 }} title="Coverage override — disabled for Indigo">Cov Ovr</th>
                <th style={{ width: 50 }} title="Enabled for Indigo only">Clicks</th>
                <th className="sc-col-derived" style={{ width: 50 }} title="Scrap factor from processes">Scrap%</th>
                <th style={{ width: 60 }}>Ref Price</th>
                <th style={{ width: 60 }}>Ink Price</th>
                <th className="sc-col-result" style={{ width: 60 }}>Setup</th>
                <th className="sc-col-result" style={{ width: 60 }}>Run</th>
                <th className="sc-col-result" style={{ width: 65 }}>Total</th>
                <th style={{ width: 30 }}></th>
              </tr>
            </thead>
              ...
              {inks.map((ik, ii) => {
                ...
                return (
                  <tr key={ii} onContextMenu={e => openInkMenu(ii, e)}>
                    ...
                    <td><input type="text" value={ik.color || ''} onChange={e => setInk(ii, 'color', e.target.value)} className="cc-det-inp" placeholder="Color name" /></td>
                    <td><select value={ik.print_type || ''} onChange={e => setInk(ii, 'print_type', e.target.value)} className="cc-det-sel"><option value="">--</option>{printTypeOpts.map(p => <option key={p} value={p}>{p}</option>)}</select></td>
                    <td><select value={ik.mesh_spec || ''} onChange={e => setInk(ii, 'mesh_spec', e.target.value)} className="cc-det-sel"><option value="">--</option>{meshOpts.map(m => <option key={m} value={m}>{m}</option>)}</select></td>
                    <td><DecimalInput value={ik.pitch_mm} onChange={v => setInk(ii, 'pitch_mm', v)} className="cc-det-inp cc-det-num" /></td>
                    <td><input type="text" value={ik.base_mat || ''} onChange={e => setInk(ii, 'base_mat', e.target.value)} className="cc-det-inp" /></td>
                    <td><DecimalInput value={ik.setup_kg} onChange={v => setInk(ii, 'setup_kg', v)} className="cc-det-inp cc-det-num" /></td>
                    <td><input type="number" min="0" max="100" value={ik.area_pct != null ? Math.round(ik.area_pct * 100) : ''} onChange={e => setInk(ii, 'area_pct', numF(e.target.value) / 100)} className="cc-det-inp cc-det-num" /></td>
                    <td><DecimalInput value={ik.coverage_override} onChange={v => setInk(ii, 'coverage_override', v)} className="cc-det-inp cc-det-num" disabled={isIndigo} title={isIndigo ? 'Not used for Indigo' : 'Coverage override'} /></td>
                    <td><input type="number" value={ik.clicks || ''} onChange={e => setInk(ii, 'clicks', numF(e.target.value))} className="cc-det-inp cc-det-num" disabled={!isIndigo} title={isIndigo ? 'Indigo clicks' : 'Indigo only'} /></td>
                    <td className="sc-td-derived" style={{ color: '#059669' }}>{(scrapDisplay * 100).toFixed(1) + '%'}</td>
                    <td><DecimalInput value={ik.s_price} onChange={v => setInk(ii, 's_price', v)} className="cc-det-inp cc-det-num" /></td>
                    <td><DecimalInput value={ik.latest} onChange={v => setInk(ii, 'latest', v)} className="cc-det-inp cc-det-num" style={ik.latest ? { color: 'var(--color-violet-500)', fontWeight: 700 } : {}} /></td>
                    <td className="sc-td-result">{fmtN(r?.setup_s)}</td>
                    <td className="sc-td-result">{fmtN(r?.run_s)}</td>
                    <td className="sc-td-result sc-td-total">{fmtN(r?.total)}</td>
                    <td><button className="sc-btn-del-circle sc-btn-del-sm" onClick={() => removeInk(ii)} title="Remove">&times;</button></td>
                  </tr>
                );
              })}
            </tbody>
          </table>
          <div className="sc-add-row"><button className="sc-btn-ghost" onClick={addInk}>+ Add Ink Row</button></div>
          </div>
        </div>
      </div>
      ...
      <div className="cc-det-section">
        <div className="sc-card">
          <div className="sc-card-header sc-header-green">
            ...
            <div className="sc-header-totals">
              <span className="sc-header-total-item">Mach: <b>${fmtN(procTotals.setupMach + procTotals.runMach)}</b></span>
              <span className="sc-header-total-item">Labor: <b>${fmtN(procTotals.setupLabor + procTotals.runLabor)}</b></span>
              <span className="sc-header-total-item">Tool: <b>${fmtN(procTotals.tooling)}</b></span>
              <span className="sc-header-total-item sc-header-total-main">Total: <b>${fmtN(procTotals.total)}</b></span>
            </div>
          </div>
          <div className="sc-card-body cc-det-tw">
          <table className="cc-det-tbl">
            <thead>
              <tr>
                ...
                <th style={{ width: 40 }} title="Repeat">Rpt</th>
                <th className="sc-col-derived" style={{ width: 40 }}>Crew</th>
                <th style={{ width: 55 }}>Speed</th>
                <th className="sc-col-derived" style={{ width: 65 }}>UOM</th>
                <th style={{ width: 50 }}>Layout</th>
                <th style={{ width: 50 }}>Eff%</th>
                <th className="sc-col-derived" style={{ width: 60 }}>MC UPH</th>
                <th style={{ width: 60 }} title="Manual UPH">Man UPH</th>
                ...
                <th style={{ width: 80 }} title="Override EAU (total qty). Empty = annual × lifetime">EAU Ovr</th>
                <th className="sc-col-derived" style={{ width: 60 }} title="Production time (hours)">Prod Time</th>
                <th className="sc-col-result" style={{ width: 65 }} title="Setup machine">S.Mach</th>
                <th className="sc-col-result" style={{ width: 65 }} title="Setup labor">S.Labor</th>
                <th className="sc-col-result" style={{ width: 65 }} title="Run machine">R.Mach</th>
                <th className="sc-col-result" style={{ width: 65 }} title="Run labor">R.Labor</th>
                <th className="sc-col-result" style={{ width: 65 }}>Tooling</th>
                <th style={{ width: 30 }}></th>
              </tr>
            </thead>
            <tbody>
              {procs.map((p, pi) => {
                ...
                return (
                  <tr key={pi}>
                    ...
                    <td><select value={p.process_type || ''} onChange={e => { setProc(pi, 'process_type', e.target.value); setProc(pi, 'workcenter', ''); }} className="cc-det-sel sc-select-bare"><option value="">--</option>{processOpts.map(o => <option key={o} value={o}>{o}</option>)}</select></td>
                    <td><select value={p.workcenter || ''} onChange={e => setProc(pi, 'workcenter', e.target.value)} className="cc-det-sel sc-select-bare"><option value="">--</option>{wcOpts.map(w => <option key={w} value={w}>{w}</option>)}</select></td>
                    <td><input type="number" min="1" value={p.repeat ?? 1} onChange={e => setProc(pi, 'repeat', numF(e.target.value))} className="cc-det-inp cc-det-num" style={{ background: 'var(--color-warning-100)' }} /></td>
                    <td className="sc-td-derived">{crew || '\u2014'}</td>
                    <td><DecimalInput value={p.speed} onChange={v => setProc(pi, 'speed', v)} className="cc-det-inp cc-det-num" /></td>
                    <td className="sc-td-derived" style={{ fontSize: 10 }}>{uom || '\u2014'}</td>
                    ...
                    /></td>
                    <td><input type="number" min="0" max="100" value={p.efficiency != null ? Math.round(p.efficiency * 100) : 85} onChange={e => setProc(pi, 'efficiency', numF(e.target.value) / 100)} className="cc-det-inp cc-det-num" /></td>
                    <td className="sc-td-derived">{r?.uph ? Math.round(r.uph) : '\u2014'}</td>
                    <td><DecimalInput value={p.manual_uph} onChange={v => setProc(pi, 'manual_uph', v)} className="cc-det-inp cc-det-num" /></td>
                    {/* Sprint 1.6 — per-MOQ Setup H (mirrors Standard fix). */}
                    <td><DecimalInput
                      value={getSpSetupHActive(p, pi)}
                      onChange={v => setSpSetupHActive(pi, v)}
                      title={isSpSetupHOverride(pi)
                        ? `MOQ ${activeMoqIdxCpx + 1} override (base = ${p.setup_h ?? 0})`
                        : (activeMoqIdxCpx === 0 ? 'Base value (MOQ 1)' : `Inherits MOQ 1 base (${p.setup_h ?? 0}) — type to override`)}
                      className={`cc-det-inp cc-det-num ${isSpSetupHOverride(pi) ? 'sc-smt-inp-ovr' : ''}`}
                    /></td>
                    <td><input type="number" min="0" max="100" value={p.scrap_pct != null ? Math.round(p.scrap_pct * 100) : 3} onChange={e => setProc(pi, 'scrap_pct', numF(e.target.value) / 100)} className="cc-det-inp cc-det-num" /></td>
                    <td><DecimalInput value={p.tool_cost} onChange={v => setProc(pi, 'tool_cost', v)} className="cc-det-inp cc-det-num" /></td>
                    <td><select value={p.tool_type || ''} onChange={e => { setProc(pi, 'tool_type', e.target.value); if (lib && e.target.value) { const l = getToolLife(lib, e.target.value); if (l > 0) setProc(pi, 'tool_life', l); } }} className="cc-det-sel sc-select-bare"><option value="">--</option>{toolTypeOpts.map(t => <option key={t} value={t}>{t}</option>)}</select></td>
                    <td><DecimalInput value={p.tool_life} onChange={v => setProc(pi, 'tool_life', v)} className="cc-det-inp cc-det-num" style={p.tool_life_ovr ? { color: 'var(--color-violet-500)', fontWeight: 700 } : {}} title="Tool life shots — auto-filled from DDL or override" /></td>
                    <td><DecimalInput value={p.eau_ovr} onChange={v => setProc(pi, 'eau_ovr', v)} className="cc-det-inp cc-det-num" placeholder={r?.eau ? String(Math.round(r.eau)) : ''} /></td>
                    <td className="sc-td-derived">{r?.total_time ? (r.total_time / 60).toFixed(2) : '\u2014'}</td>
                    ...
                    <td><button className="sc-btn-del-circle sc-btn-del-sm" onClick={() => removeProc(pi)} title="Remove">&times;</button></td>
                  </tr>
                );
              })}
            </tbody>
          </table>
          <div className="sc-add-row"><button className="sc-btn-ghost" onClick={addProc}>+ Add Process Row</button></div>
          </div>
        </div>
      </div>
    </div>
