...
export default function BomTreeView() {
  ...
  const { assembly, assemblyIdx, children, orphans } = useMemo(() => {
    ...
    let orphanList = [];
    if (bom.length > 0) {
      const bomSpSet = new Set(bom.map(e => e && e.sp_index).filter(i => typeof i === 'number'));
      childEntries = bom
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        ...
        <span style={{
          ...
          background: '#fef3c7', color: '#92400e', letterSpacing: 0.3,
        }}>PREVIEW</span>
        <label style={{
          display: 'inline-flex', alignItems: 'center', gap: 6,
          fontSize: 11, color: '#525252', cursor: 'pointer',
          padding: '3px 10px', border: '1px solid ' + (bomQtyEnabled ? '#1e40af' : '#c6c6c6'),
          borderRadius: 12, background: bomQtyEnabled ? '#eff6ff' : '#fff',
        }}
      ...
      <div style={{ background: '#fff', border: '1px solid #e0e0e0', padding: 16 }}>
        ...
        {assembly ? (
          ...
          <div style={{ fontSize: 12, color: '#6f6f6f', padding: 8, background: '#f4f4f4', marginBottom: 12 }}>
            No assembly SP detected. Showing sum-of-children aggregation (legacy behavior).
            Pick one below to mark as the assembly.
          </div>
        )}
        ...
        {children.length > 0 && (
          <div style={{ position: 'relative', paddingLeft: 24, marginTop: assembly ? 4 : 0 }}>
            <div style={{
              position: 'absolute', left: 11, top: 0, bottom: 12,
            ...
            {children.map((c, i) => (
              <div key={`${c.spIdx}-${c.bomIdx}`} style={{ position: 'relative', paddingTop: 8 }}>
                <div style={{
                  ...
                  width: 14, height: 2, background: '#e0e0e0',
                }} />
                <Node
      ...
      {orphans.length > 0 && (
        <div style={{ marginTop: 14, border: '1px solid #e0e0e0', background: '#fff' }}>
          <div style={{
            ...
            color: '#6f6f6f', letterSpacing: 0.3, textTransform: 'uppercase',
            borderBottom: '1px solid #e0e0e0', background: '#f4f4f4',
          }}>
            Orphan sub-products ({orphans.length})
            <span style={{ fontWeight: 400, textTransform: 'none', letterSpacing: 0, marginLeft: 8, color: '#8d8d8d' }}>
          ...
          <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 6 }}>
            {orphans.map(({ sp, i }) => (
              <div key={i} style={{
                ...
                border: '1px dashed #c6c6c6', borderRadius: 2,
              }}>
                <div style={{
                ...
                <button
                  ...
                  style={{
                    fontSize: 10, padding: '3px 10px', border: '1px solid #16a34a',
                    background: '#16a34a', color: '#fff', cursor: 'pointer',
                    borderRadius: 2, fontWeight: 600, letterSpacing: 0.2,
                  }}
                >
                  + Add to BOM
                </button>
                <button
                  ...
                  style={{
                    fontSize: 10, padding: '3px 8px', border: '1px solid #1e40af',
      ...
      <div style={{
        marginTop: 12, padding: '10px 14px', fontSize: 11,
        ...
        {bomQtyEnabled ? (
          <>
            <strong>Qty multiplier ACTIVE.</strong> When no assembly SP is
            selected, the aggregate is a BOM-weighted sum — each child
            SP's cost is multiplied by its BOM qty. Numbers in the
            Summary + Breakdown tabs reflect this. Quotes saved now
            persist the v2 shape; loading them later with the flag off
            returns to the legacy sum.
          </>
        ) : (
          <>
            <strong>Note:</strong> BOM qty is stored per entry but does
            <em> not </em>multiply cost aggregation yet. Toggle
            <em> qty multiplier </em>above to preview the Sprint 4.4
            semantics (default off until Finance sign-off).
          </>
        )}
      </div>
    </div>
...
function Node({
  label, subtitle, qty, notes, isAssembly,
  editableQty, onQtyChange,
  canUnset, onUnsetAssembly, onMakeAssembly,
  onRemoveFromBom,
}) {
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {subtitle && (
          <div style={{ fontSize: 11, color: '#6f6f6f', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{subtitle}</div>
        )}
      </div>
      {editableQty ? (
        <label style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: '#525252' }}>
          qty
          <input
            type="number"
            min={0}
            ...
            style={{
              ...
              border: '1px solid #c6c6c6', borderRadius: 2,
      ...
      {!isAssembly && onRemoveFromBom && (
        <button
          ...
          style={{
            width: 22, height: 22, padding: 0, border: '1px solid #e0e0e0',
            background: '#fff', color: '#6f6f6f', cursor: 'pointer',
            borderRadius: 2, fontWeight: 700, fontSize: 14, lineHeight: 1,
          }}
        >
          &times;
        </button>
