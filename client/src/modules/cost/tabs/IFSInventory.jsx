...
const IMPORT_CFG = {
  inventory:     { wizardKey: 'inventory',      clear: () => importApi.clearInventory(),     label: 'Full Inventory' },
  finishedGoods: { wizardKey: 'finished-goods', clear: () => importApi.clearFinishedGoods(), label: 'Finished Goods' },
  rawMaterials:  { wizardKey: 'raw-materials',  clear: () => importApi.clearRawMaterials(),  label: 'Raw Materials' },
};
...
const TAB_CONFIG = [
  { id: 'inventory', label: 'Full Inventory', col: '#0369a1', brd: '#0ea5e9', headerBg: '#f1f5f9', hover: '#f0f9ff',
    icon: (
      <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="white" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
        <path d="M21 16V8a2 2 0 00-1-1.73l-7-4a2 2 0 00-2 0l-7 4A2 2 0 003 8v8a2 2 0 001 1.73l7 4a2 2 0 002 0l7-4A2 2 0 0021 16z"/>
        <polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/>
      </svg>
    )
  },
  { id: 'finishedGoods', label: 'Finished Goods', col: '#0f766e', brd: '#14b8a6', headerBg: '#f0fdf4', hover: '#f0fdf4',
    icon: (
      <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="white" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
  ...
  { id: 'rawMaterials', label: 'Raw Materials', col: '#7c3aed', brd: '#a855f7', headerBg: '#faf5ff', hover: '#faf5ff',
    icon: (
      <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="white" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
...
export default function IFSInventory() {
  ...
  const { data: rawInv, loading, refresh } = useCachedFetch(
    'ifs-inventory',
    (signal) => sharedApi.getInventory({ signal }),
  );
  const data = useMemo(() => ({
  ...
  useEffect(() => {
    try { localStorage.setItem(IFS_TAB_KEY, activeTab); } catch { /* private mode */ }
  }, [activeTab]);
  ...
  const handleWizardCommitted = useCallback(async (stats) => {
  ...
  const handleClear = useCallback(async () => {
    const cfg = IMPORT_CFG[activeTab];
    if (!cfg) return;
    if (!window.confirm(`Clear all ${cfg.label} data? The current file will be backed up first.`)) return;
    setBusy(true);
    setMsg('Clearing…');
    try {
      await cfg.clear();
...
function InventoryPanel({ rows, tabCfg, headerNav }) {
  ...
  const colTypes = useMemo(() => {
    ...
    allColumns.forEach(col => {
      if (ID_COL_RE.test(col)) { m[col] = 'id'; return; }
      if (DATE_COL_RE.test(col)) { m[col] = 'date'; return; }
  ...
  return (
    <>
      ...
      <div className="inv-subhead">
        ...
        <span className="inv-subhead-sub">Showing {paged.length} of {filtered.length.toLocaleString()}</span>
      </div>

      {/* Toolbar */}
      <div className="inv-toolbar">
        <div className="inv-search-wrap" style={focusVar}>
          <svg className="inv-search-icon" viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="#94a3b8" strokeWidth="2.5" strokeLinecap="round">
          ...
          {search && filtered.length < rows.length && (
            <span className="inv-search-badge">{filtered.length.toLocaleString()} / {rows.length.toLocaleString()}</span>
          )}
        </div>
        <button
        ...
        >
          ...
          Columns
          <span className="inv-col-btn-count">{displayCols.length}/{allColumns.length}</span>
        </button>
      </div>
      ...
      {showColPanel && (
        <div className="inv-col-panel">
          <div className="inv-col-panel-header">
            ...
            <button className="inv-col-panel-close" onClick={() => setShowColPanel(false)}>Done</button>
          </div>
      ...
      <div className="inv-table-area">
        <div className="inv-table-wrap" style={{ '--inv-header-bg': tabCfg.headerBg, '--inv-hover': tabCfg.hover, '--inv-brd': tabCfg.brd, '--inv-col': tabCfg.col }}>
          <table className="inv-table">
            ...
            <tbody>
              {paged.length === 0 ? (
                <tr><td colSpan={displayCols.length + 1} style={{ padding: 0 }}>
                  <EmptyState icon="📦" title="No inventory data" hint="Import an IFS inventory export (CSV/XLSX) to get started." />
                </td></tr>
              ) : paged.map((row, i) => {
                return (
                  <tr key={i} className="inv-row">
                    ...
                    {displayCols.map(col => {
                      ...
                      const isId = t === 'id';
                      const isDate = t === 'date';
                      const cls = isNum ? 'inv-td-num-val' : isId ? 'inv-td-id' : isDate ? 'inv-td-date' : '';
                      return (
      ...
      <div className="inv-pagination">
        <div className="inv-pg-left">
          <span className="inv-pg-range">
            {sorted.length === 0 ? '0' : `${startRow.toLocaleString()}\u2013${endRow.toLocaleString()}`} / {sorted.length.toLocaleString()}
          </span>
        </div>
        <div className="inv-pg-center">
          <button className="inv-pg-btn" disabled={page === 0} onClick={() => setPage(p => p - 1)}>
            &larr;
          </button>
          <span className="inv-pg-info">Pg {page + 1}/{totalPages}</span>
          <button className="inv-pg-btn" disabled={page >= totalPages - 1} onClick={() => setPage(p => p + 1)}>
            &rarr;
          </button>
        </div>
