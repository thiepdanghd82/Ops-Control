...
export default function AdminMetrics() {
  ...
  const fetchMetrics = async () => {
    ...
    try {
      const r = await fetch('/metrics/events?limit=' + RECENT_EVENTS_LIMIT, {
        signal: ctrl.signal,
        credentials: 'include',
      });
      if (!r.ok) {
  ...
  const healthLabel =
    health === 'alert' ? 'alert' : health === 'warn' ? 'degraded' : health === 'ok' ? 'ok' : 'idle';
    ...
    <section className="metrics">
      <header className="metrics__head">
        ...
        <div className="metrics__bar">
          ...
          <button type="button" className="metrics__btn" onClick={fetchMetrics} disabled={loading}>
      ...
      <div className="metrics__tiles">
        ...
        />
        <Tile
          label={'Slow routes (p95 ≥ ' + P95_ALERT_MS + 'ms)'}
          value={slowRoutes}
            ...
            <tbody>
              {latency.slice(0, 20).map((r) => (
                <tr key={r.route}>
                  ...
                  <td className="metrics__num">
                    {r.count > 0 ? (r.sum / r.count).toFixed(1) + ' ms' : '—'}
                  </td>
                </tr>
              ))}
            </tbody>
...
function LatencyChart({ routes }) {
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Per-route p95 latency"
    >
      <line
        x1={xOf(P95_WARN_MS)}
        x2={xOf(P95_WARN_MS)}
      ...
      {routes.map((r, i) => {
        ...
        const p95 = r.p95 ?? 0;
        const fill = p95 >= P95_ALERT_MS ? '#da1e28' : p95 >= P95_WARN_MS ? '#f1c21b' : '#0f62fe';
        ...
        return (
          <g key={r.route}>
            ...
            <rect
              x={PAD_L}
              y={y + 3}
              width={Math.max(2, xOf(p95) - PAD_L)}
              height={ROW_H - 6}
              fill={fill}
            />
            <text
      ...
      <tbody>
        {events.map((ev, i) => (
          <tr key={ev.id || i}>
            <td>{ev.at || ev.ts || '—'}</td>
            <td>
              <code>{ev.actor || ev.user || '—'}</code>
            </td>
            <td>
              <code>{ev.action || ev.event || '—'}</code>
            </td>
            <td>{ev.target || ev.detail || '—'}</td>
            <td
              className={
                'metrics__num ' +
                (ev.status >= 500 ? 'metrics__alert' : ev.status >= 400 ? 'metrics__warn' : '')
              }
            >
              {ev.status ?? '—'}
            </td>
