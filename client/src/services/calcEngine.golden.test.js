...
function makeGoldenStdState() {
  return {
    ...
    pcs_per_roll: 5000,
    materials: [
      { code: 'PET-80', width: 82, usage: 1, setup_lm: 50, cavities: 0, free_liner: 0, pitch_ovr: 0,
        offcut_yn: 'N', slitting_yn: 'N', df_yn: '', offcut_pct: 0,
        s_price: 8.0, g_price: 9.0, latest: 0 },
      { code: 'LIN-75', width: 82, usage: 1, setup_lm: 0, cavities: 0, free_liner: 0, pitch_ovr: 0,
        offcut_yn: 'N', slitting_yn: 'N', df_yn: '', offcut_pct: 0,
        s_price: 2.5, g_price: 3.0, latest: 0 },
    ],
    inks: [
      { color: 'Red',  print_type: 'Flexo', base_mat: 'PET-80', area_pct: 0.3, setup_kg: 0.2, s_price: 25, g_price: 25, latest: 0 },
      { color: 'Blue', print_type: 'Flexo', base_mat: 'PET-80', area_pct: 0.2, setup_kg: 0.2, s_price: 25, g_price: 25, latest: 0 },
    ],
    processes: [
      { process_type: 'Print',   workcenter: 'Flexo-A', speed: 30, layout: 1, efficiency: 0.85,
        setup_h: 1, scrap_pct: 0.03, tool_cost: 0, tool_type: '', tool_life: 0,
        manual_uph: 0, extra_cost: 0, product_life: 1, eau_ovr: 0, repeat: 1 },
      { process_type: 'Die_Cut', workcenter: 'RDC-1',   speed: 40, layout: 1, efficiency: 0.85,
        setup_h: 0.5, scrap_pct: 0.02, tool_cost: 500, tool_type: 'RDC Die', tool_life: 100000,
        manual_uph: 0, extra_cost: 0, product_life: 1, eau_ovr: 0, repeat: 1 },
    ],
    pcs_per_bag: 500, bags_per_box: 20, container_cost: 0.5, box_cost: 0.6, other_packing: 0,
    shipping_cost: 200, other_ship: 0, ship_qty: 50000,
  };
}
...
test('golden std quote: full calcAll produces canonical money outputs', () => {
  ...
  const expected_s_ttl =
    r.s_mat_cost + r.overhead + r.bd_setup_mach +
    r.labor_cost + r.bd_setup_labor +
    r.vat_loss + r.bd_extra_vat + r.tooling + r.bd_extra + r.packing_ship;
  assert.ok(Math.abs(r.s_ttl - expected_s_ttl) < 1e-9,
  ...
  assert.ok(Math.abs(r.sga - (r.g_ttl * 0.05)) < 1e-9, 'SGA = 5% of g_ttl (burden always applied on gross)');
});
...
test('golden std quote: serializeResultForPersist output can rebuild canonical KPIs', () => {
  ...
  const va2 = 1 - (persisted.s_mat_cost + persisted.tooling + persisted.packing_ship) / persisted.sp;
  assert.ok(Math.abs(va2 - full.va) < 1e-9, 'recomputed VA from persisted matches live');

  // Recompute Contribution.
  const contr2 = 1 - (persisted.s_mat_cost + persisted.tooling + persisted.packing_ship + persisted.labor_cost) / persisted.sp;
...
test('golden std quote: doubling selling price halves cost ratio (scaling invariant)', () => {
  ...
  const r1 = calcAll(buildTierState(base, 0, base.selling_price, base.moq, base.annual_qty), null, lib, null);
  const r2 = calcAll(buildTierState(base, 0, base.selling_price * 2, base.moq, base.annual_qty), null, lib, null);
...
test('golden cplx quote: aggregateComplex picks FG + sums component SP costs', () => {
  ...
  spA.selling_price = 0;
  spA.materials = [{
    row_type: 'Main.Mat', code: 'PET-80', width: 80, usage: 1, setup_lm: 10,
    pitch: 0, pitch_ovr: 0, free_liner: 0, cavities: 0,
    offcut_yn: 'N', slitting_yn: 'N', df_yn: '', offcut_pct: 0,
    s_price: 8, g_price: 9, latest: 0,
  }];
  spA.inks = [];
  spA.processes = [{
    process_type: 'Print', workcenter: 'Flexo-A', speed: 30, layout: 1, efficiency: 0.85,
    setup_h: 0.5, scrap_pct: 0.03, tool_cost: 0, tool_type: '', tool_life: 0,
    manual_uph: 0, extra_cost: 0, product_life: 1, eau_ovr: 0, repeat: 1,
  }];
  spA.part_width = 80; spA.part_length_md = 50; spA.web_width_td = 80;
  spA.sheet_length = 52; spA.parts_in_md = 1; spA.parts_web_across = 1;

  const fg = createSubProduct('FG Z');
  fg.is_assembly = true;
  fg.selling_price = 0.1;
  fg.materials = [{
    row_type: 'Main.Mat', code: 'SP A', width: 80, usage: 1, setup_lm: 0,
    pitch: 0, pitch_ovr: 0, free_liner: 0, cavities: 0,
    offcut_yn: 'N', slitting_yn: 'N', df_yn: '', offcut_pct: 0,
  ...
  fg.processes = [{
    process_type: 'Die_Cut', workcenter: 'RDC-1', speed: 40, layout: 1, efficiency: 0.85,
    setup_h: 0.3, scrap_pct: 0.02, tool_cost: 500, tool_type: 'RDC Die', tool_life: 100000,
    manual_uph: 0, extra_cost: 0, product_life: 1, eau_ovr: 0, repeat: 1,
  }];
  fg.part_width = 80; fg.part_length_md = 50; fg.web_width_td = 80;
  fg.sheet_length = 52; fg.parts_in_md = 1; fg.parts_web_across = 1;

  const cs = {
    moq: 50000, annual_qty: 500000, product_lifetime: 1,
    trade_mode: 'USD(Normal)', site: 'VN',
    selling_price: 0.1, num_moq: 1, active_moq_idx: 0, extra_moqs: [],
    subproducts: [spA, fg],
...
test('golden std quote (multi-tier): setup amortization shrinks at higher MOQ', () => {
  ...
  const tier1 = calcAll(buildTierState(st, 1, 0.07, st.extra_moqs[0].moq, st.extra_moqs[0].eau), null, lib, null);
  const tier2 = calcAll(buildTierState(st, 2, 0.06, st.extra_moqs[1].moq, st.extra_moqs[1].eau), null, lib, null);
  ...
  assert.ok(Math.abs(tier0.bd_mat_run - tier1.bd_mat_run) < 1e-9, 'mat run invariant across MOQ tiers');
  assert.ok(Math.abs(tier0.overhead - tier1.overhead) < 1e-9, 'overhead run invariant across MOQ tiers');
  assert.ok(Math.abs(tier0.labor_cost - tier1.labor_cost) < 1e-9, 'labor run invariant across MOQ tiers');
...
test('golden std quote: SGA from live lib vs frozen approval snapshot', () => {
  ...
  const rLive = calcAll(buildTierState(base, 0, base.selling_price, base.moq, base.annual_qty), null, lib, null);
  ...
  const stWithSnapshot = {
    ...base,
    approval: { rates_snapshot: { site: 'VN', sga_rate_pct: 2, frozen_at: '2024-01-01T00:00:00Z', frozen_by: 'finance_dir' } },
  };
  const rFrozen = calcAll(buildTierState(stWithSnapshot, 0, base.selling_price, base.moq, base.annual_qty), null, lib, null);
  ...
  assert.ok(rFrozen.gm_after_sga > rLive.gm_after_sga, 'frozen 2% preserves more margin than live 5%');
});
...
test('golden std quote: SGA = 0 on site with no configured rate', () => {
  ...
  const base = makeGoldenStdState();
  const r = calcAll(buildTierState(base, 0, base.selling_price, base.moq, base.annual_qty), null, lib, null);
...
test('golden cplx quote: sub-product reference amortizes into parent with scrap compound', () => {
  const lib = makeGoldenLib();
  const spA = createSubProduct('SP A');
  spA.moq = 50000; spA.selling_price = 0;
  spA.materials = [{
    row_type: 'Main.Mat', code: 'PET-80', width: 80, usage: 1, setup_lm: 0,
    pitch: 0, pitch_ovr: 0, free_liner: 0, cavities: 0,
    offcut_yn: 'N', slitting_yn: 'N', df_yn: '', offcut_pct: 0,
  ...
  spA.processes = [{
    process_type: 'Print', workcenter: 'Flexo-A', speed: 30, layout: 1, efficiency: 0.85,
    setup_h: 0.5, scrap_pct: 0.05, tool_cost: 0, tool_type: '', tool_life: 0,
    manual_uph: 0, extra_cost: 0, product_life: 1, eau_ovr: 0, repeat: 1,
  }];
  spA.part_width = 80; spA.part_length_md = 50; spA.web_width_td = 80;
  spA.sheet_length = 52; spA.parts_in_md = 1; spA.parts_web_across = 1;
  ...
  const fg = createSubProduct('FG Z');
  fg.is_assembly = true; fg.selling_price = 0.2;
  fg.materials = [{
    row_type: 'Main.Mat', code: 'SP A', width: 80, usage: 1, setup_lm: 0,
    pitch: 0, pitch_ovr: 0, free_liner: 0, cavities: 0,
    offcut_yn: 'N', slitting_yn: 'N', df_yn: '', offcut_pct: 0,
  ...
  fg.processes = [{
    process_type: 'Die_Cut', workcenter: 'RDC-1', speed: 40, layout: 1, efficiency: 0.85,
    setup_h: 0, scrap_pct: 0.1, tool_cost: 0, tool_type: '', tool_life: 0,
    manual_uph: 0, extra_cost: 0, product_life: 1, eau_ovr: 0, repeat: 1,
  }];
  fg.part_width = 80; fg.part_length_md = 50; fg.web_width_td = 80;
  fg.sheet_length = 52; fg.parts_in_md = 1; fg.parts_web_across = 1;
  ...
  const cs = {
    moq: 50000, annual_qty: 500000, product_lifetime: 1,
    trade_mode: 'USD(Normal)', site: 'VN',
    selling_price: 0.2, num_moq: 1, active_moq_idx: 0, extra_moqs: [],
    subproducts: [spA, fg],
    bom: [{ sp_index: 0, qty: 1 }],
    tooling_alloc: [],
  };
  ...
  assert.ok(aggregate.s_mat_cost > spAStandalone.s_mat_cost,
