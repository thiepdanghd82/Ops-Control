...
const notInDesktop = (op) => Promise.reject(
...
export const desktop = {
  ...
  app: {
    getConfig: () => isElectron ? ops.app.getConfig() : Promise.resolve({ mode: 'web', remoteUrl: w.location?.origin }),
  ...
  // ─── Auto-update ───────────────────────────────────────────────
  updater: {
    checkForUpdates: () => isElectron ? ops.updater.checkForUpdates() : Promise.resolve({ state: 'web-mode' }),
  ...
  printer: {
    ...
    printPdf: (pdfPath, opts) => isElectron
      ? ops.printer.printPdf(pdfPath, opts)
      : notInDesktop('printer.printPdf'),
  ...
  labelPrinter: {
    available: isElectron,
    sendZpl: (host, port, zpl) => isElectron
      ? ops.labelPrinter.sendZpl(host, port, zpl)
      : notInDesktop('labelPrinter.sendZpl'),
    sendTspl: (host, port, tspl) => isElectron
      ? ops.labelPrinter.sendTspl(host, port, tspl)
      : notInDesktop('labelPrinter.sendTspl'),
    ping: (host, port) => isElectron
      ? ops.labelPrinter.ping(host, port)
      : notInDesktop('labelPrinter.ping'),
  },
  ...
  scale: {
    ...
    open: (path, baudRate) => isElectron
      ? ops.scale.open(path, baudRate)
      : notInDesktop('scale.open'),
  ...
  scanner: {
    ...
    open: (vid, pid) => isElectron
      ? ops.scanner.open(vid, pid)
      : notInDesktop('scanner.open'),
  ...
  cache: {
    ...
    set: (key, value) => isElectron ? ops.cache.set(key, value) : Promise.resolve({ ok: false, fallback: 'web' }),
    queueWrite: (op) => isElectron ? ops.cache.queueWrite(op) : Promise.resolve({ ok: false, fallback: 'web' }),
    syncStatus: () => isElectron ? ops.cache.syncStatus() : Promise.resolve({ online: navigator?.onLine, pending: 0 }),
    ...
    read: (tableName, rowId) => isElectron ? ops.cache.read(tableName, rowId) : Promise.resolve(null),
    upsert: (tableName, rowId, payload) => isElectron ? ops.cache.upsert(tableName, rowId, payload) : Promise.resolve({ ok: false }),
  },

  // ─── Legacy data import (v1.0 → v1.1) ────────────────────────
  import: {
    available: isElectron,
    pickFolder: (opts) => isElectron
      ? ops.import.pickFolder(opts)
      : Promise.resolve({ canceled: true, reason: 'web-mode' }),
    scanFolder: (path) => isElectron
      ? ops.import.scanFolder(path)
      : Promise.resolve({ ok: false, error: 'web-mode' }),
    execute: (path, opts) => isElectron
  ...
  fs: {
    showSaveDialog: (opts) => isElectron
      ? ops.fs.showSaveDialog(opts)
      : Promise.resolve({ canceled: false, filePath: opts?.defaultPath || 'download.bin', fallback: 'web' }),
    showOpenDialog: (opts) => isElectron
