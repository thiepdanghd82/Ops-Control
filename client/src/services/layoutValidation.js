...
const DEFAULTS = {
  ...
  maxWasteTdPct: 0.20,     // warn if web > 20% wider than parts need
  maxWasteMdPct: 0.30,     // warn if sheet > 30% longer than parts need
...
export function validateLayout(st, materials = [], opts = {}, profile = null) {
  ...
  if (partW <= 0) issues.push(iss('part_width', SEVERITY.ERROR, 'required', 'Part Width TD là bắt buộc'));
  if (partL <= 0) issues.push(iss('part_length_md', SEVERITY.ERROR, 'required', 'Part Length MD là bắt buộc'));
  if (across <= 0) issues.push(iss('parts_web_across', SEVERITY.ERROR, 'required', 'Parts/Web Across là bắt buộc'));
  if (inMd <= 0) issues.push(iss('parts_in_md', SEVERITY.ERROR, 'required', 'Parts in MD là bắt buộc'));
  if (webW <= 0) issues.push(iss('web_width_td', SEVERITY.ERROR, 'required', 'Web Width TD là bắt buộc'));
  if (sheetL <= 0) issues.push(iss('sheet_length', SEVERITY.ERROR, 'required', 'Sheet Length MD là bắt buộc'));
  ...
  if (partW > 0 && across > 0 && webW > 0) {
    ...
    if (deltaTd < 0) {
      issues.push(iss('web_width_td', SEVERITY.ERROR, 'td_overlap',
      ...
      issues.push(iss('web_width_td', SEVERITY.WARNING, 'td_waste',
      ...
      issues.push(iss('web_width_td', SEVERITY.INFO, 'td_fit',
  ...
  if (webW > 0 && edgesTotal > 0) {
    ...
    if (edgesTotal >= webW) {
      issues.push(iss('edge_margin_td_left', SEVERITY.ERROR, 'edges_exceed_web',
      ...
      issues.push(iss('edge_margin_td_left', SEVERITY.WARNING, 'edges_too_wide',
  ...
  if (partL > 0 && inMd > 0 && sheetL > 0) {
    ...
    if (deltaMd < 0) {
      issues.push(iss('sheet_length', SEVERITY.ERROR, 'md_overlap',
      ...
      issues.push(iss('sheet_length', SEVERITY.WARNING, 'md_waste',
        `Sheet dư ${deltaMd.toFixed(2)}mm. Cân nhắc tăng parts_in_md hoặc giảm min_gap_md.`,
        { need: needMd, have: sheetL, delta: deltaMd },
      ));
    } else {
      issues.push(iss('sheet_length', SEVERITY.INFO, 'md_fit',
  ...
  if (partL > 0 && inMd > 1 && sheetL > 0) {
    ...
    if (lenDelta > 0.5) {
      issues.push(iss('sheet_length', SEVERITY.ERROR, 'pitch_mismatch',
  ...
  if (rotaryCols > 0 && sheetL > 0) {
    ...
    if (snapDelta > 0.05) {
      ...
      issues.push(iss('sheet_length', SEVERITY.WARNING, 'rotary_snap',
  ...
  if (numWebs > 1 && webW > 0 && logW > 0) {
    ...
    if (needLog > logW + 0.01) {
      issues.push(iss('num_webs', SEVERITY.ERROR, 'log_overflow',
  ...
  if (mainMat && mainMat.offcut_pct > cfg.maxOffcutPct) {
    issues.push(iss('web_width_td', SEVERITY.WARNING, 'offcut_high',
  ...
  if (tolC2c > 0 && gapTd > 0 && gapTd < 2 * tolC2c) {
    issues.push(iss('min_gap_td', SEVERITY.WARNING, 'gap_below_tol',
  ...
  if (tolP2c > 0 && gapMd > 0 && gapMd < 2 * tolP2c) {
    issues.push(iss('min_gap_md', SEVERITY.WARNING, 'gap_below_tol',
  ...
  if (tolSlit > 0 && edgeTd > 0 && edgeTd < 3 * tolSlit) {
    issues.push(iss('edge_margin_td', SEVERITY.WARNING, 'edge_below_slit_tol',
  ...
  if (netW > 0 && netW < 5) {
    issues.push(iss('part_net_width', SEVERITY.WARNING, 'net_too_small',
      `Net width ${netW}mm < 5mm — quá nhỏ, kiểm tra lại (đơn vị mm không phải cm?).`,
      { net: netW, min: 5 },
    ));
  }
  if (netL > 0 && netL < 5) {
    issues.push(iss('part_net_length', SEVERITY.WARNING, 'net_too_small',
  ...
  if (netW > 0 && bleedTd > 0 && bleedTd > netW * 0.10) {
    issues.push(iss('bleed_td_mm', SEVERITY.WARNING, 'bleed_too_large',
  ...
  if (netL > 0 && bleedMd > 0 && bleedMd > netL * 0.10) {
    issues.push(iss('bleed_md_mm', SEVERITY.WARNING, 'bleed_too_large',
  ...
  if (netW > 0 && partW > 0 && (netW + 2 * bleedTd) > partW + 0.01) {
    issues.push(iss('part_net_width', SEVERITY.ERROR, 'net_exceeds_die',
  ...
  if (netL > 0 && partL > 0 && (netL + 2 * bleedMd) > partL + 0.01) {
    issues.push(iss('part_net_length', SEVERITY.ERROR, 'net_exceeds_die',
  ...
  if (cornerR > 0 && cutType === 'perf-only') {
    issues.push(iss('corner_radius_mm', SEVERITY.WARNING, 'corner_vs_perf',
  ...
  if (perfOff > 0 && gapMd > 0 && gapMd < perfOff + 1) {
    issues.push(iss('perf_offset_mm', SEVERITY.WARNING, 'perf_gap_tight',
  ...
  if (colorCount > 0 && stationMax > 0 && colorCount > stationMax) {
    issues.push(iss('color_count', SEVERITY.ERROR, 'color_over_stations',
  ...
  if (colorCount > 8) {
    issues.push(iss('color_count', SEVERITY.WARNING, 'color_count_high',
  ...
  if (quietZone > 0) {
    ...
    if (effEdgeMin > 0 && effEdgeMin < minEdgeForQZ) {
      issues.push(iss('die_quiet_zone_mm', SEVERITY.WARNING, 'quiet_zone_too_tight',
    ...
    if (quietZone > 0 && gapTd > 0 && gapTd < quietZone * 0.8) {
      issues.push(iss('die_quiet_zone_mm', SEVERITY.WARNING, 'quiet_zone_gap_tight',
  ...
  if (tolP2p > 0 && tolP2p > 0.2) {
    issues.push(iss('tol_p2p_mm', SEVERITY.WARNING, 'color_reg_loose',
  ...
  if (tolP2p > 0 && colorCount >= 2) {
    ...
    if (bleedTd > 0 && bleedTd < minBleed) {
      issues.push(iss('bleed_td_mm', SEVERITY.WARNING, 'bleed_below_reg',
    ...
    if (bleedMd > 0 && bleedMd < minBleed) {
      issues.push(iss('bleed_md_mm', SEVERITY.WARNING, 'bleed_below_reg',
  ...
  if (cornerR > 0) {
    if (cornerR < 0.5) {
      issues.push(iss('corner_radius_mm', SEVERITY.ERROR, 'corner_too_sharp',
    ...
    if (partW > 0 && cornerR > partW / 4) {
      issues.push(iss('corner_radius_mm', SEVERITY.WARNING, 'corner_too_large',
    ...
    if (bleedTd > 0 && cornerR > bleedTd * 2) {
      issues.push(iss('corner_radius_mm', SEVERITY.INFO, 'corner_exceeds_bleed',
  ...
  if (slitOn && slitCnt > 1 && webW > 0) {
    ...
    const minLane = num(st?.min_slit_lane_width_mm)
      || num(profile?.min_slit_lane_mm)
      || 25;
    if (laneW < minLane) {
      issues.push(iss('slit_lane_count', SEVERITY.ERROR, 'slit_lane_too_narrow',
  ...
  if (webW > 0 && profile) {
    ...
    if (wMin > 0 && webW < wMin) {
      issues.push(iss('web_width_td', SEVERITY.ERROR, 'web_below_press_min',
        `Web ${webW}mm < min ${wMin}mm của máy ${profile.name || ''}. Chọn máy khác hoặc tăng web width.`,
        { web: webW, min: wMin, press: profile.name },
      ));
    }
    if (wMax > 0 && webW > wMax) {
      issues.push(iss('web_width_td', SEVERITY.ERROR, 'web_above_press_max',
  ...
  if (st?.include_reg_marks) {
    ...
    if (narrowEdge > 0 && narrowEdge < regW) {
      issues.push(iss('reg_mark_width_mm', SEVERITY.WARNING, 'reg_mark_edge_short',
