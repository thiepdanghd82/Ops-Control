...
const DEFAULT_TOOTH_COUNTS = [
  ...
  25, 27, 30, 32, 36, 40, 45, 48, 54, 60, 64, 72, 80, 90, 96, 104, 112, 120, 132, 144, 156, 168,
];
...
export function suggestLayouts(st, material, opts = {}) {
  ...
  const profileDies = unionDies.length
    ? unionDies
    : profile?.common_dies?.length
      ? profile.common_dies
      : null;
  ...
  const {
    ...
    maxPitchMm = num(st?.max_pitch_mm) ||
      profileMaxPitch ||
  ...
  for (const { w, l, rotated } of orientations) {
    ...
    for (let numWebs = 1; numWebs <= maxWebs; numWebs++) {
      ...
      const maxInMdForFlat = Math.max(
        1,
        Math.floor((maxPitchMm + effMinGapMd) / (l + effMinGapMd))
      );
      ...
      for (const tooth of pressType === 'flat' ? flatInMdOptions : toothOptions) {
        ...
        const finalPitch = pressType === 'flat' ? partsMdSpan + effMinGapMd : pitch;
        ...
        const targetBonus =
          targetPcsPerRoll > 0 && pcsPerShot > 0 && targetPcsPerRoll % pcsPerShot === 0
            ? 1.02
            : 1.0;
        ...
        if (toothOut != null) {
          ...
          if (hasPlate === null && hasMagnet === null) {
            ...
            reuse =
              Array.isArray(commonDies) && commonDies.length ? commonDies.includes(toothOut) : null;
          } else {
            if (hasPlate && hasMagnet) {
              reuse = true;
              reuseStatus = 'full';
            } else if (hasPlate && !hasMagnet) {
              reuse = false;
              reuseStatus = 'partial_plate';
              needsOrder = [`magnetic-${toothOut}T`];
            } else if (!hasPlate && hasMagnet) {
              reuse = false;
              reuseStatus = 'partial_magnetic';
              needsOrder = [`plate-${toothOut}T`];
            } else {
              reuse = false;
              reuseStatus = 'none';
              needsOrder = [`plate-${toothOut}T`, `magnetic-${toothOut}T`];
            }
          }
        }
        candidates.push({
          ...
          meetsTarget: targetPcsPerRoll
            ? targetPcsPerRoll % pcsPerShot === 0 ||
              Math.abs(targetPcsPerRoll / pcsPerShot - Math.round(targetPcsPerRoll / pcsPerShot)) <
                0.01
            : null,
        });
      }
  ...
  const reuseRank = (s) =>
    s === 'full' ? 0 : s === 'partial_plate' || s === 'partial_magnetic' ? 1 : s === 'none' ? 2 : 0;
  deduped.sort(
    (a, b) =>
      b.score - a.score ||
