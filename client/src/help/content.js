...
export const GLOSSARY = [
  { term: 'Anilox',         en: 'Engraved ceramic roll that meters ink onto the plate in flexographic printing. Characterised by LPI (Lines per Inch) and BCM (Billion Cubic Micrometres per square inch).', vi: 'Trục ceramic khắc kim loại cấp mực lên bản in flexo. Đặc trưng bởi LPI và BCM.' },
  { term: 'BCM',            en: 'Billion Cubic Micrometres per square inch — the volume of ink an anilox cell holds. Higher BCM = more ink laid down per rev.', vi: 'Tỉ µm³/in² — thể tích mực mà cell anilox chứa. BCM cao = xuống nhiều mực hơn.' },
  { term: 'BOM',            en: 'Bill of Materials — hierarchical list of components needed to produce one finished good.', vi: 'Định mức vật tư — danh mục phân cấp các linh kiện cần để sản xuất một thành phẩm.' },
  { term: 'CONTR%',         en: 'Contribution margin. 1 − (s_mat_cost + tooling + packing_ship + run_labor_only) / sp_price. Target ≥ 25%.', vi: 'Biên đóng góp. Mục tiêu ≥ 25%.' },
  { term: 'DDL',            en: 'Drop-Down List — reference enum library (workcenters, tool types, UoMs, currencies…). Editable by Admin.', vi: 'Danh sách lựa chọn — thư viện enum tham chiếu.' },
  { term: 'EAU',            en: 'Estimated Annual Usage — expected annual production volume per SKU. Used for tooling amortization cap.', vi: 'Ước lượng số lượng sử dụng hàng năm — dùng để cap phân bổ chi phí tooling.' },
  { term: 'G.TOTAL',        en: 'Gross total cost using purchase (g) prices for materials. Feeds SGA. See also S.TOTAL.', vi: 'Tổng chi phí theo giá mua (g). Đầu vào SGA.' },
  { term: 'GM%',            en: 'Gross Margin. 1 − s_ttl / sp_price. Target ≥ 15%.', vi: 'Biên lợi nhuận gộp. Mục tiêu ≥ 15%.' },
  { term: 'IML',            en: 'In-Mould Label — label placed inside the injection mould and fused with the part during moulding.', vi: 'Nhãn trong khuôn — nhãn đặt vào khuôn và ép dính cùng sản phẩm khi đúc.' },
  { term: 'LM',             en: 'Linear Metre — unit for web / roll material measured along the machine direction.', vi: 'Mét dài — đơn vị đo vật tư dạng web/cuộn theo chiều máy chạy.' },
  { term: 'MD / TD',        en: 'Machine Direction (along the web) / Transverse Direction (across the web).', vi: 'Hướng máy (theo web) / Hướng ngang (ngang web).' },
  { term: 'MOQ',            en: 'Minimum Order Quantity — smallest run the customer will commit to. Drives setup amortization.', vi: 'Số lượng đặt hàng tối thiểu — đơn vị nhỏ nhất khách cam kết. Ảnh hưởng phân bổ setup.' },
  { term: 'Offcut',         en: 'Waste incurred when slitting a wide log to the required material width. Computed from (cavities MOD width) / cavities.', vi: 'Hao phí khi xẻ cuộn wide thành bề rộng yêu cầu. Tính từ (cavities MOD width) / cavities.' },
  { term: 'Pitch',          en: 'Distance between successive parts along the machine direction. Drives UPH + QPA.', vi: 'Khoảng cách giữa các chi tiết liên tiếp theo hướng máy. Ảnh hưởng UPH + QPA.' },
  { term: 'QPA',            en: 'Quantity Per Assembly / Part — material consumed per label, expressed as m² or LM.', vi: 'Số lượng mỗi nhãn — vật tư tiêu thụ cho mỗi nhãn, tính theo m² hoặc LM.' },
  { term: 'RDC',            en: 'Rotary Die Cutting — continuous cylindrical die, 200k shots tool life typical.', vi: 'Máy cắt khuôn tròn liên tục — tuổi thọ khuôn 200k shots điển hình.' },
  { term: 'SGA',            en: 'Selling, General & Administrative overhead. Per-site rate configured in Finance Data. Default 0%.', vi: 'Chi phí bán hàng, quản lý chung. Cấu hình theo site trong Finance Data. Mặc định 0%.' },
  { term: 'SKU',            en: 'Stock Keeping Unit — unique product identifier. In Ops Control, the SKU is the primary key everywhere.', vi: 'Mã sản phẩm — định danh duy nhất. Trong Ops Control, SKU là khoá chính.' },
  { term: 'S.TOTAL',        en: 'Supplier total cost using s-prices. Primary basis for GM% / VA% / CONTR% (Sprint 21 Finance alignment).', vi: 'Tổng chi phí theo giá s (supplier). Cơ sở chính cho GM% / VA% / CONTR%.' },
  { term: 'UPH',            en: 'Units Per Hour — machine throughput. Formula varies by speed UOM (m/min, stamp/min, pcs/hr, sheets/hr, mtr/hr).', vi: 'Số đơn vị mỗi giờ — năng suất máy. Công thức tuỳ UOM tốc độ.' },
  { term: 'VA%',            en: 'Value-Add ratio. 1 − (s_mat_cost + tooling + packing_ship) / sp_price. Target > 30%.', vi: 'Tỉ lệ giá trị gia tăng. Mục tiêu > 30%.' },
  { term: 'VAT(Book)',      en: 'USD(Book) trade mode — domestic goods that cannot reclaim input VAT. Adds 15% VAT loss.', vi: 'Chế độ USD(Book) — hàng nội địa không hoàn VAT. Cộng thêm 15% VAT loss.' },
];
...
export const HELP_CONTENT = {
  ...
  'standard': {
    ...
    function: bi('Compute unit cost and sell price for a single-part label job', 'Tính đơn giá và giá bán cho job in nhãn một-phần'),
    path: 'Ops Cost > Calculators > Pricing (Std)',
    authorization: auth('Cost', 'User role can view; Cost role required to Save + Submit for Approval. Approved quotes auto-advance to Formal Quotation.'),
    ...
    features: [
      feat('Multi-tier MOQ pricing (up to 5 tiers per quote).',         'Báo giá theo nhiều bậc MOQ (tối đa 5 bậc/quote).'),
      feat('Live cost-waterfall: Material → Ink → Labour → Overhead → Margin → Sell.', 'Waterfall chi phí real-time.'),
      feat('Automatic integration with Print Area Calculator for ink µL per color.', 'Tự động import µL mực từ Print Area Calculator.'),
      feat('Work-center + Rate Table LOV auto-fills hourly rates + setup + run rates.', 'LOV Work-center + Rate Table tự điền đơn giá/giờ.'),
      feat('Version-controlled save: re-saving the same SKU creates a new revision in Quote History.', 'Save có kiểm soát phiên bản: lưu lại cùng SKU tạo revision mới.'),
      feat('What-if sensitivity: preview ±5% margin / ±10% qty without committing.', 'What-if: xem trước ±5% margin / ±10% qty mà không lưu.'),
    ],
    example: ex(
      bi(
      ...
      [
        bs('Enter SKU "T3000001", qty 250,000, width 82, height 52.',
           'Nhập SKU "T3000001", qty 250,000, width 82, height 52.'),
        bs('In the Material sub-tab, choose Paper → White Semi-gloss → Permanent Acrylic.',
           'Vào sub-tab Material, chọn Paper → White Semi-gloss → Permanent Acrylic.'),
        bs('In the Inks sub-tab, set Number of Inks = 1, print type = Flexo.',
           'Vào sub-tab Inks, đặt Số màu = 1, loại in = Flexo.'),
        bs('In the Processes sub-tab, pick work-center "Flexo-AT-10" and confirm the Rate Table auto-fills hourly rate.',
           'Vào sub-tab Processes, chọn work-center "Flexo-AT-10" và xác nhận Rate Table tự điền đơn giá/giờ.'),
        bs('Set target Margin % = 30 in the Summarize sub-tab.',
           'Đặt target Margin % = 30 trong sub-tab Summarize.'),
        bs('Choose Save → the quote is committed to Quote History with a new revision ID.',
    ...
    preRequisites: [
      br('Product dimensions (mm) + total quantity from the customer.', 'Kích thước sản phẩm (mm) + số lượng tổng từ khách.'),
      br('Material code from Material Library OR a known unit cost.', 'Mã vật tư từ Material Library HOẶC đơn giá đã biết.'),
      br('Print method + ink count; ideally ink volumes from Print Area Calculator.', 'Công nghệ in + số màu; tốt nhất là có volume mực từ Print Area Calculator.'),
    ],
    procedures: [
      proc('Product panel', 'Khối Sản phẩm',
        bi(
        ...
        [
          bs('Click the SKU field and type the customer part number (unique per customer, max 32 chars).',
             'Click ô SKU và nhập mã sản phẩm khách hàng (duy nhất theo khách, tối đa 32 ký tự).'),
          bs('Fill in Product name — optional but shown on the formal quote PDF, keep it customer-facing.',
             'Điền Tên sản phẩm — không bắt buộc nhưng hiện trên PDF báo giá chính thức, dùng tên khách đọc được.'),
          bs('Enter Quantity (pcs) — the run size; drives per-unit amortization of setup and tooling.',
             'Nhập Số lượng (pcs) — kích thước lô; quyết định phân bổ setup và tooling theo từng đơn vị.'),
          bs('Enter Width and Height (mm) — the trim size of the finished product, before bleed.',
             'Nhập Chiều rộng và Chiều cao (mm) — kích thước trim của thành phẩm, trước khi cộng bleed.'),
          bs('Select Drawing scale if the artwork is not 1:1 (e.g. 2:1 for artwork drawn 2× real size).',
             'Chọn Drawing scale nếu artwork không phải 1:1 (vd 2:1 nếu artwork vẽ lớn gấp 2 lần kích thước thật).'),
          bs('Click Save within this panel to stash the SKU shell before moving on — prevents loss on browser refresh.',
             'Click Save trong panel này để lưu khung SKU trước khi sang bước kế — tránh mất khi refresh browser.'),
        ],
      ),
      proc('Material panel', 'Khối Vật tư',
        bi(
        ...
        [
          bs('Click Material field → LOV popup lists all active materials filtered by substrate type.',
             'Click ô Material → popup LOV liệt kê mọi vật tư đang hoạt động, lọc theo loại substrate.'),
          bs('Select the target material; unit cost + default Waste % auto-fill.',
             'Chọn vật tư mục tiêu; đơn giá + Waste % mặc định tự điền.'),
          bs('Override Waste % only if you know the press-specific value for this material; otherwise keep the library default.',
             'Ghi đè Waste % chỉ khi biết giá trị riêng của máy cho vật tư này; nếu không giữ mặc định.'),
          bs('Optional: toggle "Sheet-fed" if the material is cut from sheets rather than web (changes waste math).',
             'Tuỳ chọn: bật "Sheet-fed" nếu vật tư cắt từ tờ thay vì cuộn (thay đổi công thức waste).'),
        ],
      ),
      proc('Print method panel', 'Khối Công nghệ in',
        bi(
        ...
        [
          bs('Select Print Method (Letterpress / Flexo / Silkscreen / Offset / Digital).',
             'Chọn Công nghệ in (Letterpress / Flexo / Silkscreen / Offset / Digital).'),
          bs('Enter Number of inks — how many separate ink colors on the label.',
             'Nhập Số màu — tổng số màu mực khác nhau trên nhãn.'),
          bs('Click the ← Import button to pull µL/label per color from a saved Print Area Calculator job. Matching is by SKU; manual override still allowed.',
             'Click nút ← Import để lấy µL/nhãn từng màu từ job Print Area đã lưu. Match theo SKU; có thể ghi đè thủ công.'),
          bs('If no Print Area run exists, the defaults fall back to the method\'s industry-baseline µL (see Legend).',
             'Nếu chưa chạy Print Area, mặc định sẽ dùng µL chuẩn ngành của công nghệ đó (xem Legend).'),
        ],
      ),
      proc('Labor & overhead panel', 'Khối Nhân công & Overhead',
        bi(
        ...
        [
          bs('Click Work center LOV → pick the press + any post-press operation.',
             'Click LOV Work center → chọn máy in + bất kỳ công đoạn post-press.'),
          bs('Setup hours and Run rate pre-populate from Routing Ops; override only for one-off non-standard jobs.',
             'Setup hours và Run rate tự điền từ Routing Ops; chỉ ghi đè cho job đặc biệt không chuẩn.'),
          bs('Verify the hourly rate shown matches the current Rate Table effective date.',
             'Xác nhận đơn giá/giờ hiển thị khớp với ngày hiệu lực hiện tại của Rate Table.'),
        ],
      ),
      proc('Margin & output panel', 'Khối Biên lợi nhuận & kết quả',
        bi(
        ...
        [
          bs('Enter target Margin % — gross margin, not markup (40% margin ≠ 40% markup).',
             'Nhập Margin % mục tiêu — gross margin, không phải markup (40% margin ≠ 40% markup).'),
          bs('Review the waterfall: Material → Ink → Labor → Overhead → Margin → Sell.',
             'Xem waterfall: Material → Ink → Labor → Overhead → Margin → Sell.'),
          bs('Click Save to commit to Quote History (key = SKU; re-saving overwrites — use History for audit).',
             'Click Save để lưu vào Quote History (key = SKU; save lại sẽ ghi đè — dùng History để audit).'),
          bs('Optional: click "What-if" to preview ±5% margin scenarios without saving.',
    ...
    keyFields: [
      ...
      field('Waste %',    'percent', 'Press setup + running waste. Default from material spec; override if you know better.'),
      field('Margin %',   'percent', 'Target gross margin; drives sell price = cost / (1 − margin/100).'),
      field('Work center', 'LOV',    'From Routing Ops library; selecting one auto-loads setup time + run rate.'),
      field('Drawing scale', 'enum', '1:1 / 2:1 / 4:1. Artwork-to-physical ratio; corrects mm² output.'),
    ],
    formulas: [
      { name: 'Material cost/unit',
        ...
        example: 'length=30mm, width=20mm, unit_price=0.0005 USD/mm², waste=8%\n= 30 × 20 × 0.0005 × 1.08 = 0.324 USD / label',
      ...
      { name: 'Ink cost/unit',
        ...
        example: 'Black 2 µL × 0.03 USD/mL + Red 0.4 µL × 0.08 USD/mL\n= (2 × 0.03 + 0.4 × 0.08) × 0.001 = 0.0000920 USD / label',
      ...
      { name: 'Labor cost/unit',
        ...
        example: 'Setup 2h for 10,000 units + run rate 500 uph → run_per_unit = 1/500 h\nRate 20 USD/h: (2/10000 + 1/500) × 20 = 0.044 USD / label',
    ...
    tips: [
      bt('Hover any field label for an inline tooltip with the formula that uses it.',
         'Hover lên label của bất kỳ ô nào để xem tooltip giải thích công thức dùng nó.'),
      bt('Import ink µL from Print Area Calculator if you have the artwork — prevents under-costing of mixed-ink jobs.',
         'Import µL mực từ Print Area Calculator nếu có artwork — tránh tính thiếu cho job nhiều màu.'),
      bt('For repeat SKUs, open the previous version from Quote History and Save As → saves 2 minutes per quote.',
         'Với SKU lặp lại, mở phiên bản cũ từ Quote History và Save As → tiết kiệm 2 phút mỗi báo giá.'),
    ],
    pitfalls: [
      bp('Forgetting to update Waste % on a new material: default is conservative (≈8%); tight-web materials can be 2–3%.',
         'Quên cập nhật Waste % cho vật tư mới: mặc định là thận trọng (~8%); vật tư tight-web có thể chỉ 2-3%.'),
      bp('Margin vs. markup confusion: 40% margin = 67% markup on cost. Always use the Margin % field, not markup.',
         'Nhầm Margin với Markup: 40% margin = 67% markup trên cost. Luôn dùng ô Margin %, không phải markup.'),
      bp('Saving overwrites the previous SKU entry. For variants (A/B pricing) use different SKU suffixes (e.g. "LBL-001-v2").',
  ...
  'complex': {
    ...
    function: bi('Multi-level BOM cost roll-up with per-level yield + shared setup', 'Tính giá BOM nhiều cấp có yield từng cấp + setup dùng chung'),
    ...
    preRequisites: [
      br('BOM structure pre-loaded in Mfg Structures or entered manually.', 'Cấu trúc BOM có sẵn trong Mfg Structures hoặc nhập thủ công.'),
      br('Rate Table with per-operation hourly rates configured.', 'Rate Table có đơn giá/giờ theo từng công đoạn đã cấu hình.'),
    ],
    procedures: [
      proc('New BOM / Open', 'Tạo BOM mới / Mở BOM có sẵn',
        bi('Cloning an existing SKU saves time — Complex BOMs often share 70-80% of structure.',
           'Clone SKU cũ tiết kiệm thời gian — BOM phức tạp thường chia sẻ 70-80% cấu trúc.'),
        [
          bs('Click New BOM to start from blank, OR click Open → pick SKU from the LOV.',
             'Click New BOM để làm từ đầu, HOẶC click Open → chọn SKU từ LOV.'),
          bs('When opening: all levels hydrate with the saved yield, routing, materials.',
             'Khi open: mọi cấp được hydrate với yield, routing, vật tư đã lưu.'),
          bs('Click Save As to clone a new SKU from the current state — preserves the source.',
             'Click Save As để clone SKU mới từ trạng thái hiện tại — giữ nguyên nguồn.'),
        ],
      ),
      proc('Per-level materials', 'Vật tư theo cấp',
        bi('Level 0 = finished product; each higher level is a sub-assembly consumed by the level below.',
           'Level 0 = thành phẩm; mỗi cấp cao hơn là sub-assembly được tiêu thụ bởi cấp dưới.'),
        [
          bs('For each level row, click + to add a material line.',
             'Với mỗi dòng level, click + để thêm dòng vật tư.'),
          bs('Pick material from Material Library LOV; qty-per-assembly is required.',
             'Chọn vật tư từ LOV Material Library; qty-per-assembly bắt buộc.'),
          bs('Override Waste % if this level\'s press has different efficiency than default.',
             'Ghi đè Waste % nếu máy ở cấp này có hiệu suất khác mặc định.'),
        ],
      ),
      proc('Per-level routing', 'Routing theo cấp', null,
        [
          bs('Click the Routing column of any level → LOV of operations from Routing Ops library.',
             'Click cột Routing của cấp bất kỳ → LOV công đoạn từ thư viện Routing Ops.'),
          bs('Multi-select to string ops in sequence: Print → Die-cut → Laminate, etc.',
             'Multi-select để xâu chuỗi công đoạn: Print → Die-cut → Laminate, v.v.'),
          bs('Check Shared setup when the same press runs multiple levels in the same window (amortizes setup across jobs).',
             'Tick Shared setup khi cùng máy chạy nhiều cấp trong cùng cửa sổ (phân bổ setup giữa các job).'),
        ],
      ),
      proc('Yield configuration', 'Cấu hình Yield',
        bi('Yield is per-level, compounds across levels. Example: 95% × 95% × 95% = 85.7% overall.',
           'Yield theo từng cấp, compound qua các cấp. Ví dụ: 95% × 95% × 95% = 85.7% tổng.'),
        [
          bs('Enter Yield % per level (column "Yield").',
             'Nhập Yield % cho từng cấp (cột "Yield").'),
          bs('System computes Required input qty upstream so final output matches the order qty.',
             'Hệ thống tính Required input qty ngược lên để output cuối khớp số lượng đơn hàng.'),
          bs('Red cell = yield below warning threshold (default 85%); review with ops before quoting.',
      ...
      proc('Waterfall review + save', 'Review waterfall + lưu', null,
        [
          bs('Right panel shows the cost waterfall: raw → after level N → final.',
             'Panel phải hiển thị waterfall chi phí: raw → sau level N → final.'),
          bs('Click any level bar → drilldown table with material + labor + overhead.',
             'Click thanh level bất kỳ → bảng drilldown vật tư + nhân công + overhead.'),
          bs('Right-click any row → "What-if yield" for ±2% sensitivity preview.',
             'Right-click dòng bất kỳ → "What-if yield" để preview ±2% sensitivity.'),
          bs('Click Save — stored in the "Complex" bucket of Quote History (separate from Standard).',
    ...
    workflow: null,
    keyFields: [
      field('Level',    'number',  'BOM depth. Level 0 is the finished product; higher numbers are sub-assemblies.'),
      field('Yield %',  'percent', 'Fraction of good units to advance. Compound: 95% × 95% × 95% ≈ 85.7% overall.'),
      field('Shared setup', 'bool',  'True = setup hours amortized across ALL levels sharing this press; False = duplicated per level.'),
    ],
    formulas: [
      ...
      { name: 'Setup amortized (shared)',
        ...
        example: 'Setup 2h × $20/h = $40 total\n2 jobs sharing: qty_A=5k + qty_B=15k = 20k units\nper_unit = 40 / 20000 = $0.002 / unit',
        notes: 'When shared_setup=True, the setup cost is split across ALL jobs on the press in the same run window.' },
    ],
    tips: [
      bt('Build reusable BOM templates in Mfg Structures — don\'t re-enter 6-level BOMs from scratch every quote.',
         'Tạo template BOM dùng lại trong Mfg Structures — đừng nhập lại BOM 6 cấp từ đầu mỗi báo giá.'),
      bt('Sensitivity view: right-click any level row → "What-if yield" to see how ±2% yield changes sell price.',
         'View nhạy cảm: right-click hàng bất kỳ → "What-if yield" để xem ±2% yield ảnh hưởng giá bán thế nào.'),
    ],
    pitfalls: [
      bp('Forgetting to mark shared setup: will over-count press-hour cost by 2–3× on multi-level jobs.',
         'Quên đánh dấu shared setup: sẽ đếm gấp 2-3× chi phí giờ máy cho job nhiều cấp.'),
      bp('Yield entered as decimal instead of percent: 0.95 vs 95 produces a 95× error. Field is always percent.',
  ...
  'lib-mat': {
    ...
    function: bi('Master catalog of materials + unit cost + waste defaults', 'Catalog gốc vật tư + đơn giá + waste mặc định'),
    ...
    preRequisites: [br('User+ role to view costs; Cost+ role to edit unit prices.', 'Role User+ để xem chi phí; Role Cost+ để sửa đơn giá.')],
    procedures: [
      proc('Search + filter', 'Tìm kiếm + lọc', null,
        [
          bs('Use the search box (top) — matches code, description, supplier.',
             'Dùng ô search (trên cùng) — match theo code, mô tả, nhà cung cấp.'),
          bs('Filter by substrate type dropdown: Paper / Film / Foil / Composite.',
             'Lọc theo dropdown loại substrate: Paper / Film / Foil / Composite.'),
          bs('Sort by clicking column headers (Code, Cost, Last updated).',
             'Sắp xếp bằng cách click header cột (Code, Cost, Last updated).'),
        ],
      ),
      proc('Detail panel', 'Panel chi tiết',
        bi('Click any row to open the right-side detail drawer.', 'Click dòng bất kỳ để mở drawer chi tiết bên phải.'),
        [
          bs('Review current unit cost + currency.', 'Xem đơn giá hiện tại + đơn vị tiền tệ.'),
          bs('Price History section shows last 12 months (chart + table).',
             'Mục Price History hiển thị 12 tháng gần nhất (chart + bảng).'),
          bs('Linked SKUs section shows every quote currently using this material.',
             'Mục Linked SKUs hiển thị mọi báo giá đang dùng vật tư này.'),
        ],
      ),
      proc('Edit unit cost (Cost+ role)', 'Sửa đơn giá (role Cost+)',
        bi('Every edit is audited: username + timestamp + old-value kept.',
           'Mọi chỉnh sửa đều được audit: username + timestamp + giá cũ được giữ.'),
        [
          bs('Click pencil icon on the row to enter edit mode.',
             'Click icon bút chì trên dòng để vào chế độ edit.'),
          bs('Change Unit cost; optionally add a Note (why, source).',
             'Đổi Unit cost; tuỳ chọn thêm Note (lý do, nguồn).'),
          bs('Save → audit entry created; Linked SKUs can be bulk-resaved to propagate.',
      ...
      proc('Export', 'Xuất', null,
        [
          ...
          bs('Current filter set is preserved — use for procurement weekly review.',
    ...
    workflow: null,
    keyFields: [
      field('Code',        'string', 'Internal material code; must match the code stored in Standard/Complex calcs.'),
      ...
      field('Waste default', 'percent', 'Pre-populated in Standard calc; operators can override per job.'),
    ],
    formulas: [],
    tips: [
      bt('Use the Price History button to audit unusual quote variations — a hidden price change is often the cause.',
         'Dùng nút Price History để audit biến thiên báo giá bất thường — đổi giá âm thầm thường là nguyên nhân.'),
    ],
    pitfalls: [
      bp('Editing unit cost without recording the source in the notes field makes future audits difficult.',
  ...
  'ink-calc': {
    ...
    function: bi('Per-process ink consumption (µL/label, mL/1k) and cost', 'Tính tiêu thụ mực theo công đoạn (µL/nhãn, mL/1k) + chi phí'),
    ...
    preRequisites: [
      br('Print Area result (per-color mm² breakdown) for the artwork.', 'Kết quả Print Area (mm² chi tiết từng màu) cho artwork.'),
      br('Anilox reference (flexo) or mesh + emulsion spec (silkscreen) for the press.',
    ...
    procedures: [
      proc('Process selector', 'Chọn công đoạn', null,
        [
          ...
          bs('Each sub-tab is independent — settings do not copy across.',
             'Mỗi sub-tab độc lập — cài đặt không copy qua lại.'),
        ],
      ),
      proc('Silkscreen — spec input', 'Silkscreen — nhập thông số',
        bi('See entry "Inks — Silkscreen detail" for the physics + calibration.',
           'Xem entry "Mực in — Chi tiết Silkscreen" để biết vật lý + hiệu chỉnh.'),
        [
          bs('Pick mesh from library, OR enter mesh count (T/cm) + thread diameter.',
             'Chọn lưới từ library, HOẶC nhập mesh count (T/cm) + đường kính sợi.'),
          bs('Enter emulsion thickness (µm).', 'Nhập độ dày emulsion (µm).'),
          bs('System computes film thickness + open area automatically.',
             'Hệ thống tự tính độ dày film + open area.'),
        ],
      ),
      proc('Flexo — anilox input', 'Flexo — nhập anilox',
        bi('See entry "Inks — Flexo detail" for BCM conversion.',
           'Xem entry "Mực in — Chi tiết Flexo" để biết cách quy đổi BCM.'),
        [
          bs('Pick anilox from library → LPI + BCM auto-populate.',
             'Chọn anilox từ library → LPI + BCM tự điền.'),
          bs('Override transfer_factor only for calibrated values specific to your press.',
             'Ghi đè transfer_factor chỉ với giá trị đã hiệu chỉnh cho máy cụ thể.'),
        ],
      ),
      proc('Area + output', 'Diện tích + kết quả', null,
        [
          bs('Enter area (mm²) OR click "← Import from Print Area" to pull by SKU.',
             'Nhập diện tích (mm²) HOẶC click "← Import from Print Area" để lấy theo SKU.'),
          bs('Output rows: µL/label, mL/1k, cost/label, cost/1k.',
             'Dòng output: µL/nhãn, mL/1k, chi phí/nhãn, chi phí/1k.'),
          bs('Compare "Calculated" vs "Actual from press logs" — delta > 15% flags a palette miss or anilox wear.',
    ...
    workflow: null,
    keyFields: [
      field('Mesh count (silk)', 'number', 'Threads per inch. Higher mesh = finer detail, less ink per pass.'),
      ...
      field('BCM (flexo)',       'number', 'Billion cubic microns per square inch — the anilox ink-carrying capacity.'),
    ],
    formulas: [
      ...
      { name: 'Flexo ink volume',
        ...
        notes: '0.06451 converts BCM (billion cubic µm per in²) to wet film µm assuming full anilox volume transfers.' },
    ],
    tips: [
      bt('Keep a calibrated anilox in the library and mark one as "reference" for the facility — it shortcuts new-job setup.',
         'Giữ một anilox đã hiệu chỉnh trong library và đánh dấu làm "reference" cho xưởng — giúp setup job mới nhanh hơn.'),
    ],
    pitfalls: [
      bp('Using last year\'s mesh spec on a new emulsion batch — re-characterize after every chemistry change.',
  ...
  'print-area': {
    ...
    function: bi('Measure per-color ink coverage on artwork → mm² → µL', 'Đo độ phủ mực từng màu trên artwork → mm² → µL'),
    path: 'Ops Cost > Calculators > Print Area',
    authorization: auth('User', 'View + analyze available to all. Cost role required to commit results to an SKU Library record.'),
    businessScenario: biz(
    ...
    features: [
      feat('Artwork input: PNG / JPG / WebP / SVG / PDF / AI (with native Adobe Illustrator support via embedded PDF stream).', 'Đầu vào: PNG / JPG / WebP / SVG / PDF / AI (hỗ trợ AI gốc qua PDF stream).'),
      feat('Perceptual color separation using CIE L*a*b* ΔE76 — JND-aware.', 'Tách màu theo cảm nhận CIE L*a*b* ΔE76.'),
      feat('Spot-color rescue: chroma-boost + outlier rescue + manual pin (3-layer defense).', 'Cứu màu spot: chroma-boost + outlier rescue + pin thủ công (3 lớp).'),
      feat('Anti-aliasing sub-pixel weighting — AA edges contribute fractional ink.', 'Trọng số sub-pixel chống răng cưa — cạnh AA đóng góp mực phần.'),
      feat('Per-method ink transfer factors + film thickness (Flexo / Silkscreen / Offset / Letterpress / Digital).', 'Transfer factor + độ dày film theo công nghệ in.'),
      feat('Separations export: one PNG per ink (film-positive for plate / screen burning).', 'Xuất separations: 1 PNG/màu (film positive).'),
    ],
    example: ex(
      bi(
      ...
      [
        bs('Drag the AI file onto the drop zone. Enter Width 240, Height 54.', 'Kéo thả file AI. Nhập Width 240, Height 54.'),
        bs('Select Print Method = Flexo. Chroma-boost and Outlier Rescue are ON by default.', 'Chọn Flexo. Chroma-boost + Outlier Rescue mặc định BẬT.'),
        bs('Choose Analyze. The system surfaces the red cluster at 0.3% coverage.', 'Chọn Analyze. Hệ thống đưa ra cluster đỏ ở 0.3% coverage.'),
        bs('If red is still missing, choose 🎯 Inspect color, click the red pixel, choose 📌 Pin as spot ink, and re-Analyze.', 'Nếu đỏ vẫn thiếu, chọn Inspect color, click pixel đỏ, Pin as spot ink, Analyze lại.'),
        bs('Choose Save → results are persisted under the SKU and available to Pricing Worksheet via Import.', 'Chọn Save → kết quả lưu theo SKU, Pricing Worksheet có thể Import.'),
      ],
      bi(
    ...
    preRequisites: [
      br('Artwork file: PNG / JPG / WebP / SVG / PDF / AI.', 'File artwork: PNG / JPG / WebP / SVG / PDF / AI.'),
      br('Product trim dimensions (mm) — CRITICAL, drives every mm² output.', 'Kích thước trim sản phẩm (mm) — QUAN TRỌNG, quyết định mọi mm² output.'),
      br('Drawing scale if artwork is not 1:1.', 'Drawing scale nếu artwork không phải 1:1.'),
    ],
    procedures: [
      proc('Product panel', 'Khối Sản phẩm',
        bi('All mm² outputs scale from these dimensions. Wrong dims = wrong quote.',
           'Mọi output mm² scale theo các kích thước này. Sai dims = sai báo giá.'),
        [
          bs('Enter SKU — identifies the saved job in the Library.',
             'Nhập SKU — định danh job đã lưu trong Library.'),
          bs('Enter Width × Height in mm (trim size, without bleed).',
             'Nhập Chiều rộng × Chiều cao mm (kích thước trim, chưa có bleed).'),
          bs('Optional: enter Bleed (mm) if the artwork includes bleed margins — ink printed on bleed still counts toward consumption.',
             'Tuỳ chọn: nhập Bleed (mm) nếu artwork có bleed — mực in trên bleed VẪN tính vào tiêu thụ.'),
          bs('Pick Drawing scale — 1:1 for most artworks; 2:1 or 4:1 if the design is oversized for detail.',
             'Chọn Drawing scale — 1:1 cho đa số; 2:1 hoặc 4:1 nếu design được vẽ lớn hơn để dễ nhìn chi tiết.'),
          bs('Set Render DPI (300 default, 600 for fine detail — doubles render time).',
             'Đặt Render DPI (300 mặc định, 600 cho chi tiết mịn — gấp đôi thời gian render).'),
        ],
      ),
      proc('Print method panel', 'Khối Công nghệ in', null,
        [
          bs('Click one of: Letterpress / Flexo / Silkscreen / Offset / Digital.',
             'Click 1 trong: Letterpress / Flexo / Silkscreen / Offset / Digital.'),
          bs('The transfer factor + film thickness are fixed per method (see Legend §4); change only if you have calibrated values for this press.',
             'Transfer factor + độ dày film cố định theo công nghệ (xem Legend §4); chỉ đổi nếu có giá trị đã hiệu chỉnh cho máy này.'),
        ],
      ),
      proc('Artwork upload', 'Tải Artwork',
        bi('Three ways to load: drag-drop, click-to-browse, or Cmd/Ctrl-V paste. PDF/AI are rendered via pdf.js at the requested DPI.',
           '3 cách tải: kéo-thả, click-chọn file, hoặc Cmd/Ctrl-V paste. PDF/AI render qua pdf.js theo DPI đã chọn.'),
        [
          bs('Drag the artwork file onto the drop zone, OR',
             'Kéo file artwork vào vùng drop, HOẶC'),
          bs('Click the drop zone to open the file picker, OR',
             'Click vùng drop để mở file picker, HOẶC'),
          bs('Paste with Cmd+V (Mac) / Ctrl+V (Windows) if you have an image on the clipboard.',
             'Paste bằng Cmd+V (Mac) / Ctrl+V (Windows) nếu đã có ảnh trong clipboard.'),
          bs('Wait for the preview canvas to show the rendered artwork (1-3 s depending on DPI).',
             'Chờ preview canvas hiển thị artwork (1-3 giây tuỳ DPI).'),
        ],
      ),
      proc('Analysis run', 'Chạy phân tích',
        bi('Runs the 9-stage pipeline: BG detect → crop → mask → quantize → merge → rescue → pin inject → count → build result.',
        ...
        [
          ...
          bs('Review the per-color table: COLOR swatch, NAME, HEX, AREA %, AREA mm², ML/LABEL, ML/1K.',
             'Xem bảng theo màu: ô COLOR, NAME, HEX, AREA %, AREA mm², ML/LABEL, ML/1K.'),
          bs('Click any color ROW to toggle "ignore" — excluded colors drop from the total (use for dieline magenta, varnish).',
             'Click HÀNG màu bất kỳ để bật/tắt "ignore" — màu loại trừ không tính vào tổng (dùng cho dieline magenta, varnish).'),
          bs('Click any color CHIP above the table to overlay that ink on the canvas (visual QC).',
             'Click CHIP màu phía trên bảng để overlay màu đó lên canvas (QC trực quan).'),
          bs('Click Total row to see aggregate printed pct, printed mm², total µL.',
             'Click dòng Total để xem tổng % in, mm² in, tổng µL.'),
        ],
      ),
      proc('Pin missing spot ink (Sprint 9)', 'Pin màu spot bị thiếu',
        bi('Three-layer defense handles 95% of spot-color cases automatically. Use this only when coverage is < 0.05% AND the ink must appear.',
           '3 lớp defense xử lý tự động 95% case. Chỉ dùng pin khi coverage < 0.05% VÀ bắt buộc phải hiện màu đó.'),
        [
          bs('Click 🎯 Inspect color — button turns purple "✓ Inspect ON".',
             'Click 🎯 Inspect color — button chuyển tím "✓ Inspect ON".'),
          bs('Zoom in if needed (scroll or pinch on the canvas).',
             'Zoom vào nếu cần (scroll hoặc pinch trên canvas).'),
          bs('Click the exact pixel of the spot ink you want preserved (e.g. the red W in "Wash").',
             'Click đúng pixel của màu spot muốn giữ (vd chữ W đỏ trong "Wash").'),
          bs('A purple pick-info panel appears below the overlay chips.',
             'Panel pick-info màu tím xuất hiện dưới các chip overlay.'),
          bs('Click 📌 Pin as spot ink — button flips to filled amber.',
             'Click 📌 Pin as spot ink — button chuyển amber đầy.'),
          bs('An amber chip strip appears above Results listing the pinned hex.',
             'Một chip strip amber xuất hiện phía trên Results liệt kê các hex đã pin.'),
          bs('Click ▶ Analyze artwork again — the pinned hex now has its own cluster.',
             'Click ▶ Analyze artwork lại — hex đã pin giờ có cluster riêng.'),
          bs('To unpin: click the × inside the amber chip, or click the pin button again.',
             'Để unpin: click × trong chip amber, hoặc click lại button pin.'),
        ],
      ),
      proc('Manual ROI', 'ROI thủ công',
        bi('When auto-crop picks up unwanted content (dim lines, multi-up layouts, front+back), draw the exact label box.',
        ...
        [
          ...
          bs('On the Analysis Area canvas, drag a rectangle around ONLY the label.',
             'Trên canvas Analysis Area, kéo hình chữ nhật bao quanh CHỈ nhãn.'),
          bs('Review the live X × Y mm readout to confirm it matches the trim size.',
             'Kiểm tra readout X × Y mm trực tiếp để xác nhận khớp kích thước trim.'),
          bs('Click ▶ Analyze — the ROI persists with the saved job.',
      ...
      proc('Save + separations export', 'Lưu + xuất Separations', null,
        [
          bs('Click 💾 Save result — persists under this SKU in the Library tab.',
             'Click 💾 Save result — lưu theo SKU này trong tab Library.'),
          bs('For film-positive export: click ⬇ Separations — downloads one PNG per non-ignored ink, rendered as solid black on white, sized to the analyzed region.',
          ...
          bs('Drop the PNGs into your screen-burning (silkscreen) or plate-imaging (flexo) workflow.',
    ...
    keyFields: [
      ...
      field('Rescue outliers',   'bool',   'Post-pass that promotes coherent outlier pixels to new clusters.'),
    ...
    formulas: [
      ...
      { name: 'Area',
        ...
        example: 'pct_black=0.887, width=30mm, height=20mm, scale=1\n= 0.887 × (30 × 20 / 1²) = 532.2 mm²' },
      { name: 'Ink volume',
        ...
        example: 'Flexo: color_mm²=532.2, film=3µm, factor=1.12\n= 532.2 × 3 × 1.12 × 0.001 = 1.788 µL / label',
    ...
    tips: [
      bt('Trust the amber banner "Background detection may be wrong" — it fires when auto-BG goes sideways on full-bleed art.',
         'Tin banner amber "Background detection may be wrong" — nó xuất hiện khi auto-BG sai trên artwork full-bleed.'),
      bt('Pin persists with the saved job; re-opening from Library keeps the red surviving automatically.',
         'Pin lưu cùng job; mở lại từ Library tự động giữ màu đỏ.'),
      bt('Use Separations export (⬇ button) to generate film positives for plate/screen burning.',
         'Dùng Separations export (nút ⬇) để tạo film positive cho bản/lưới.'),
    ],
    pitfalls: [
      bp('Drawing scale forgotten on a 2:1 artwork: mm² off by 4×, µL off by 4×, quote off by the same factor.',
         'Quên drawing scale cho artwork 2:1: mm² sai 4×, µL sai 4×, báo giá sai 4×.'),
      bp('Wrong Print Method: Flexo vs. Silkscreen differ in film thickness by 5×; ink cost will be wildly off.',
  ...
  'messages': {
    ...
    function: bi('Peer-to-peer messaging with image attachments', 'Nhắn tin nội bộ có đính kèm ảnh'),
    ...
    procedures: [
      proc('Pick recipient', 'Chọn người nhận', null,
        [
          bs('Left pane lists contacts sorted by last-interaction.',
             'Panel trái liệt kê contact sắp xếp theo last-interaction.'),
          bs('Click a name → thread opens in the middle pane.',
      ...
      proc('Send message', 'Gửi tin', null,
        [
          bs('Click into composer (bottom of middle pane).',
             'Click vào composer (đáy panel giữa).'),
          bs('Enter to send; Shift+Enter for a new line.',
             'Enter để gửi; Shift+Enter để xuống dòng.'),
          bs('Drag-drop image / paste from clipboard (Cmd+V) to attach.',
      ...
      proc('Read + mark', 'Đọc + đánh dấu', null,
        [
          ...
          bs('Opening a thread auto-marks its messages read.',
             'Mở thread tự đánh dấu các tin đã đọc.'),
          bs('Pin important threads via 📌 icon to keep at top.',
    ...
    formulas: [],
    tips: [
      bt('Pinned threads stay at the top — useful for ongoing quote reviews.',
         'Thread đã pin luôn ở trên cùng — hữu ích cho review báo giá đang theo dõi.'),
    ],
    pitfalls: [
      bp('Messages are not a ticket system — use Pending Approvals / Quote History for official tracking.',
  ...
  'summarize': {
    ...
    function: bi('Executive-view breakdown of a saved quote', 'Xem tổng quan chi phí của báo giá đã lưu'),
    ...
    preRequisites: [br('A saved quote (Standard or Complex).', 'Một báo giá đã lưu (Standard hoặc Complex).')],
    procedures: [
      proc('SKU picker', 'Chọn SKU',
        bi('Dropdown sorts by last-update descending, so the freshest quote is on top.',
           'Dropdown sắp xếp theo last-update giảm dần, báo giá mới nhất ở trên cùng.'),
        [
          bs('Click the SKU dropdown at the top of the screen.',
             'Click dropdown SKU ở đầu màn hình.'),
          bs('Type to filter if the list is long (matches SKU and customer name).',
             'Gõ để lọc nếu list dài (match theo SKU và tên khách).'),
          bs('Select — the breakdown panels refresh instantly.',
             'Chọn — các panel breakdown refresh ngay.'),
        ],
      ),
      proc('Cost waterfall panel', 'Khối Cơ cấu Chi phí',
        bi('Read top-to-bottom: raw material up, labor in the middle, margin + sell at the bottom.',
           'Đọc từ trên xuống: vật tư thô ở trên, nhân công ở giữa, margin + bán ở dưới.'),
        [
          bs('Review each line: Material → Ink → Labor → Overhead → Margin → Sell.',
             'Xem từng dòng: Material → Ink → Labor → Overhead → Margin → Sell.'),
          bs('Hover any line to see the formula, contributing fields, and the library row that sourced it (Rate Table / Material Library).',
             'Hover dòng bất kỳ để xem công thức, các ô đóng góp, và dòng library đã cung cấp (Rate Table / Material Library).'),
          bs('Click the expand arrow on a line for its sub-breakdown (e.g., Labor → per work-cell rows).',
             'Click mũi tên expand trên dòng để xem sub-breakdown (vd Labor → dòng theo work-cell).'),
        ],
      ),
      proc('What-if panel', 'Khối What-if',
        bi('Non-destructive preview — changes here do not save to the quote. Use for margin meetings or sensitivity analysis.',
           'Preview không ghi đè — thay đổi ở đây không lưu vào báo giá. Dùng cho meeting margin hoặc phân tích nhạy cảm.'),
        [
          bs('Click "What-if" button in the toolbar.',
             'Click nút "What-if" trên toolbar.'),
          bs('Adjust Margin % (slider) and/or Quantity (multiplier).',
             'Chỉnh Margin % (slider) và/hoặc Quantity (nhân).'),
          bs('The Sell price below updates live; a diff pill shows ±x% vs. the committed value.',
             'Giá bán bên dưới update live; pill diff hiển thị ±x% so với giá trị đã commit.'),
          bs('Close the What-if panel to return to the committed numbers (nothing saved).',
             'Đóng panel What-if để về số đã commit (không lưu gì).'),
        ],
      ),
      proc('Export', 'Xuất file', null,
        [
          bs('Click ⬇ PDF to download a single-page executive summary.',
             'Click ⬇ PDF để tải bản tóm tắt 1 trang cấp quản lý.'),
          bs('Click ⬇ CSV to export the line-item table for Excel reconciliation.',
             'Click ⬇ CSV để xuất bảng line-item cho đối chiếu Excel.'),
          bs('PDF filename convention: "Quote_[SKU]_[YYYYMMDD].pdf".',
    ...
    tips: [bt('Bookmark this tab for the quote review meeting — it\'s the cleanest single-page view.',
              'Bookmark tab này cho meeting review báo giá — đây là view 1 trang sạch nhất.')],
    pitfalls: [bp('Numbers shown are SNAPSHOT at save time; editing the quote elsewhere won\'t update until re-saved.',
  ...
  'formal-quote': {
    ...
    preRequisites: [br('Approved quote in Pending Approvals (or auto-approved by role).',
                       'Báo giá đã duyệt trong Pending Approvals (hoặc auto-approved theo role).')],
    procedures: [
      proc('SKU selection', 'Chọn SKU', null,
        [
          bs('Pick SKU from the dropdown — only approved quotes appear.',
             'Chọn SKU từ dropdown — chỉ báo giá đã duyệt xuất hiện.'),
          bs('Fields auto-fill from the approved snapshot (customer, qty, price).',
             'Các ô tự điền từ snapshot đã duyệt (khách, qty, giá).'),
        ],
      ),
      proc('Customer header', 'Thông tin khách hàng', null,
        [
          bs('Verify customer name + address (pulled from Drop-Down Lists).',
             'Xác nhận tên + địa chỉ khách (lấy từ Drop-Down Lists).'),
          bs('Override Attention contact if different from default.',
             'Ghi đè Attention contact nếu khác mặc định.'),
          bs('Optional: add a reference (PO#, RFQ#) for customer tracking.',
             'Tuỳ chọn: thêm reference (PO#, RFQ#) để khách theo dõi.'),
        ],
      ),
      proc('Commercial terms', 'Điều khoản thương mại',
        bi('Templates per customer segment live in Drop-Down Lists — set up once.',
        ...
        [
          ...
          bs('Payment terms (free text, e.g. "Net 45").',
             'Điều khoản thanh toán (free text, vd "Net 45").'),
          bs('Incoterms: EXW / FOB / CIF / DDP per agreement.',
      ...
      proc('Preview + send', 'Preview + gửi', null,
        [
          ...
          bs('Click Mark as Sent → freezes quote numbers; starts RFQ timer in RFQ Tracker.',
             'Click Mark as Sent → đóng băng số báo giá; khởi động timer RFQ trong RFQ Tracker.'),
          bs('Changing price AFTER Sent requires a new quote (audit integrity).',
    ...
    formulas: [],
    tips: [bt('Terms templates live in Drop-Down Lists — set up once per customer segment.',
              'Template điều khoản nằm trong Drop-Down Lists — setup 1 lần cho mỗi phân khúc khách hàng.')],
    pitfalls: [bp('Changing the SKU price after Sent requires a new quote, not an edit — for audit trail integrity.',
  ...
  'quote-history': {
    ...
    function: bi('Searchable audit trail of every quote version', 'Audit trail mọi phiên bản báo giá'),
    ...
    procedures: [
      proc('Search + filter', 'Tìm kiếm + lọc', null,
        [
          bs('Search box: SKU / customer / submitter name.',
             'Ô search: SKU / tên khách / tên người submit.'),
          bs('Date range picker: default "last 90 days".',
             'Date range picker: mặc định "90 ngày gần nhất".'),
          bs('Status chip filter: Draft / Submitted / Approved / Rejected / Sent / Won / Lost.',
             'Filter trạng thái chip: Draft / Submitted / Approved / Rejected / Sent / Won / Lost.'),
        ],
      ),
      proc('Open quote detail', 'Mở chi tiết báo giá', null,
        [
          bs('Click any row → full detail modal opens.',
             'Click dòng bất kỳ → modal chi tiết đầy đủ mở.'),
          bs('Version history tab lists all saves (each Save = new version).',
             'Tab Version history liệt kê mọi lần save (mỗi Save = phiên bản mới).'),
          bs('Click a version → loads that snapshot into the detail view.',
             'Click phiên bản → load snapshot đó vào view chi tiết.'),
        ],
      ),
      proc('Compare versions', 'So sánh các phiên bản',
        bi('Use when debugging "why is this quote different from last quarter?"',
           'Dùng khi debug "tại sao báo giá này khác quý trước?"'),
        [
          bs('In detail modal, click "Compare versions".', 'Trong modal chi tiết, click "Compare versions".'),
          bs('Select any two versions from the history list.',
             'Chọn 2 phiên bản bất kỳ từ list lịch sử.'),
          bs('Diff view highlights field-level changes (red = changed, green = new).',
             'View diff highlight thay đổi theo từng trường (đỏ = đã đổi, xanh = mới).'),
        ],
      ),
      proc('Status update + export', 'Cập nhật trạng thái + xuất',
        bi('Setting Won/Lost drives Dashboard win-rate KPI — do not skip.',
           'Đặt Won/Lost ảnh hưởng KPI tỉ lệ thắng trên Dashboard — không được bỏ.'),
        [
          bs('From row right-click → "Update status" → Won / Lost / Cancelled.',
             'Right-click dòng → "Update status" → Won / Lost / Cancelled.'),
          bs('Add a one-line reason for Lost (required).',
             'Thêm lý do 1 dòng cho Lost (bắt buộc).'),
          bs('Click ⬇ CSV in toolbar to export the filtered list.',
    ...
    formulas: [],
    tips: [bt('Set a status of Won/Lost for closed loops — drives Dashboard win-rate KPI.',
              'Đặt trạng thái Won/Lost cho deal đã đóng — ảnh hưởng KPI tỉ lệ thắng trên Dashboard.')],
    pitfalls: [bp('History is append-only — edits become new versions, never overwrites.',
  ...
  'approvals-inbox': {
    ...
    preRequisites: [br('Admin/Cost role to approve; everyone can view their own submissions.',
                       'Role Admin/Cost để duyệt; mọi người có thể xem submission của mình.')],
    procedures: [
      proc('Filter bar', 'Thanh lọc', null,
        [
          bs('Use the radio toggle: "My submissions" / "Awaiting my approval" / "All".',
             'Dùng radio toggle: "My submissions" / "Awaiting my approval" / "All".'),
          bs('Search box filters by SKU, customer, or submitter name.',
             'Ô search lọc theo SKU, khách, hoặc tên người submit.'),
          bs('Age column sorts overdue quotes to the top (default).',
             'Cột Age sắp xếp báo giá quá hạn lên đầu (mặc định).'),
        ],
      ),
      proc('Quote detail modal', 'Modal chi tiết',
        bi('Open by clicking any row. Modal shows the full Cost Breakdown + submitter comment + audit trail.',
           'Mở bằng cách click dòng bất kỳ. Modal hiển thị Cost Breakdown đầy đủ + comment người submit + audit trail.'),
        [
          bs('Click a row → modal opens on top of the queue.',
             'Click dòng → modal mở đè lên queue.'),
          bs('Review the cost waterfall, the submitter\'s comment, and the history of prior revisions.',
             'Xem waterfall chi phí, comment người submit, và lịch sử các revision trước.'),
          bs('Scroll to the bottom for the Approve / Reject action bar.',
             'Cuộn xuống đáy để thấy thanh action Approve / Reject.'),
        ],
      ),
      proc('Approve action', 'Thao tác Duyệt',
        bi('Approved quotes auto-advance to Formal Quotation, visible to the submitter immediately.',
           'Báo giá đã duyệt tự động chuyển sang Formal Quotation, người submit thấy ngay.'),
        [
          bs('Click ✓ Approve.', 'Click ✓ Approve.'),
          bs('Optionally add a one-line comment (shown in the submitter\'s Quote History).',
             'Tuỳ chọn thêm comment 1 dòng (hiển thị trong Quote History của người submit).'),
          bs('Click Confirm → quote moves to "Approved" state; badge on sidebar decrements.',
             'Click Confirm → báo giá chuyển sang state "Approved"; badge trên sidebar giảm.'),
        ],
      ),
      proc('Reject action', 'Thao tác Từ chối',
        bi('Reject reason is MANDATORY and immutable. Think before submitting — you cannot silently edit a rejection.',
        ...
        [
          ...
          bs('Type a reason (min 10 chars) — required.',
             'Gõ lý do (tối thiểu 10 ký tự) — bắt buộc.'),
          bs('Click Confirm → quote returns to the submitter as "Rejected" with your comment attached.',
             'Click Confirm → báo giá quay lại người submit dưới dạng "Rejected" kèm comment.'),
          bs('The submitter can revise + resubmit as a new version; the original rejection remains in history.',
    ...
    keyFields: [],
    formulas: [],
    tips: [bt('Set a daily reminder to clear this queue — stuck approvals are the #1 customer-wait complaint.',
              'Đặt nhắc nhở hàng ngày clear queue này — approval kẹt là phàn nàn #1 khiến khách chờ.')],
    pitfalls: [bp('Reject reason is immutable — think before submitting; review is impossible to redo silently.',
  ...
  'lib-mfg': {
    ...
    function: bi('Reusable BOM + process templates for Complex calc', 'Template BOM + công đoạn dùng lại cho Complex calc'),
    ...
    preRequisites: [br('Routing Ops library populated (templates reference operations from there).',
    ...
    procedures: [
      proc('Create template', 'Tạo template', null,
        [
          ...
          bs('Name — use descriptive convention "Multi-layer v2024-Q3" to preserve history.',
             'Name — dùng quy ước mô tả "Multi-layer v2024-Q3" để giữ lịch sử.'),
          bs('Product family — pick from LOV (drives which quotes can use this template).',
             'Product family — chọn từ LOV (quyết định báo giá nào được dùng template này).'),
        ],
      ),
      proc('Define levels', 'Định nghĩa các cấp', null,
        [
          bs('Click Add Level — level 0 = finished product, higher = sub-assemblies.',
             'Click Add Level — level 0 = thành phẩm, cao hơn = sub-assembly.'),
          bs('For each level: Materials (from Material Library) + Operations (from Routing Ops).',
             'Với mỗi cấp: Materials (từ Material Library) + Operations (từ Routing Ops).'),
          bs('Set default Yield % per level — operators can override per quote.',
             'Đặt Yield % mặc định cho mỗi cấp — operator có thể ghi đè theo báo giá.'),
        ],
      ),
      proc('Save + version', 'Lưu + quản lý phiên bản',
        bi('Templates referenced by saved quotes MUST not be deleted — rename or create v2 instead.',
           'Template đang được báo giá cũ dùng KHÔNG được xoá — đổi tên hoặc tạo v2.'),
        [
          bs('Click Save → template becomes available in Complex calc New BOM picker.',
             'Click Save → template có sẵn trong picker New BOM của Complex calc.'),
          bs('To version: click Save As → new template name (suffix v2, v3).',
             'Để version: click Save As → tên template mới (hậu tố v2, v3).'),
          bs('Deprecate old versions via toggle — hidden from picker but existing quotes still open.',
    ...
    keyFields: [],
    formulas: [],
    tips: [bt('Version templates: suffix with date (e.g., "Multi-layer v2024-Q3") to preserve history.',
              'Version template: thêm hậu tố theo ngày (vd "Multi-layer v2024-Q3") để giữ lịch sử.')],
    pitfalls: [bp('Deleting a template used by saved quotes breaks their re-open path; rename instead.',
  ...
  'lib-rop': {
    ...
    function: bi('Master list of factory operations (setup, run rate, yield)', 'Danh sách gốc công đoạn (setup, tốc độ, yield)'),
    ...
    preRequisites: [br('Rate Table populated with the hourly rate for each work-cell.',
    ...
    procedures: [
      proc('Search + filter', 'Tìm + lọc', null,
        [
          ...
          bs('Search by operation code or description.',
             'Search theo code công đoạn hoặc mô tả.'),
          bs('Sort by last-updated to see recent changes.',
             'Sort theo last-updated để xem thay đổi gần đây.'),
        ],
      ),
      proc('Add / edit operation', 'Thêm / sửa công đoạn', null,
        [
          bs('Click + (or pencil icon to edit existing).',
             'Click + (hoặc icon bút chì để sửa có sẵn).'),
          bs('Fill Operation code (5 chars) + description (max 20).',
             'Điền Operation code (5 ký tự) + mô tả (tối đa 20).'),
          bs('Pick Work cell from LOV → hourly rate auto-looks up from Rate Table.',
             'Chọn Work cell từ LOV → đơn giá/giờ tự tra từ Rate Table.'),
          bs('Enter Setup hours + Run rate (units/hour) + Yield %.',
             'Nhập Setup hours + Run rate (units/giờ) + Yield %.'),
          bs('Click Save → operation available in Mfg Structures + Complex calc.',
             'Click Save → công đoạn có sẵn trong Mfg Structures + Complex calc.'),
        ],
      ),
      proc('Periodic rate audit', 'Audit tốc độ định kỳ',
        bi('Press run rate decays 2-5% per year without maintenance. Quarterly audit.',
           'Tốc độ máy xuống cấp 2-5%/năm nếu không bảo dưỡng. Audit hàng quý.'),
        [
          bs('Pull the last 3 months of shop floor logs for the operation.',
             'Lấy 3 tháng gần nhất shop floor log cho công đoạn.'),
          bs('Compare actual avg run rate vs. library value.',
             'So sánh tốc độ trung bình thực tế vs giá trị library.'),
          bs('Update if delta > 5%; note reason in the audit trail.',
    ...
    keyFields: [
      ...
      field('Yield',    'percent',    'Good units out / units in. Defaults from typical; override per process.'),
    ],
    formulas: [],
    tips: [bt('Track run rate decay — press ages, rates drop 2-5% per year without maintenance.',
              'Theo dõi tốc độ xuống cấp — máy càng cũ, tốc độ giảm 2-5%/năm nếu không bảo dưỡng.')],
    pitfalls: [bp('Over-optimistic run rates cause under-priced quotes → margin loss at month-end.',
  ...
  'lib-inventory': {
    ...
    function: bi('Read-only IFS ERP stock levels (on-hand / on-order / allocated)', 'View chỉ đọc tồn kho IFS ERP'),
    ...
    preRequisites: [br('IFS integration active; admin has set up the ODBC/API bridge.',
                       'IFS integration đang hoạt động; admin đã setup cầu nối ODBC/API.')],
    procedures: [
      proc('Search + filter', 'Tìm + lọc', null,
        [
          bs('Search box: material code or description.',
             'Ô search: code vật tư hoặc mô tả.'),
          bs('Filter chips: In-stock only / Shortage only / Oversupply.',
             'Chip lọc: Chỉ còn hàng / Chỉ thiếu / Dư thừa.'),
          bs('Last-refresh timestamp shown top-right (data 15 min stale max).',
      ...
      proc('Row detail', 'Chi tiết dòng', null,
        [
          ...
          bs('Breakdown: On-hand / On-order / Allocated / Free-to-promise / Next ETA.',
             'Breakdown: Tồn / Đặt hàng / Đã allocate / FTP / ETA kế.'),
          bs('Click through "Open POs" → list with supplier + ETA.',
             'Click "Open POs" → list kèm NCC + ETA.'),
          bs('Click "Allocated SOs" → list with customer + ship-by.',
             'Click "Allocated SOs" → list kèm khách + ngày giao.'),
        ],
      ),
      proc('Alternate search', 'Tra vật tư thay thế',
        bi('IFS substitution chains are respected. If primary out of stock, see alternates with stock + cost delta.',
           'Chuỗi thay thế IFS được tôn trọng. Nếu vật tư chính hết, xem phương án thay thế kèm tồn + delta giá.'),
        [
          bs('Right-click row → "Show alternates".',
             'Right-click dòng → "Show alternates".'),
          bs('Alternate list with: code, stock, unit cost delta (+/− %), historical substitution rate.',
             'List thay thế kèm: code, tồn, delta đơn giá (+/− %), tỉ lệ thay thế lịch sử.'),
          bs('Cross-reference with Material Library before committing.',
    ...
    formulas: [
      { name: 'Free to promise',
        ...
        example: 'on_hand=5,000 + on_order=3,000 − allocated=4,000\nFTP = 4,000 units available for new quotes' },
    ],
    tips: [bt('Integration refresh is every 15 min. For live-critical decisions, cross-check with IFS directly.',
              'Integration refresh 15 phút/lần. Với quyết định gấp, kiểm tra trực tiếp trong IFS.')],
    pitfalls: [bp('Allocated ≠ consumed — cancelled SOs may still show allocated until IFS EOD cleanup.',
  ...
  'rfq-tracker': {
    ...
    preRequisites: [
      bi('Design file (AI / PDF) received from customer.', 'File thiết kế (AI / PDF) đã nhận từ khách.'),
      bi('Customer provided EAU, MOQ, and deadline.', 'Khách đã cung cấp EAU, MOQ, deadline.'),
    ],
    features: [
      feat('KPI bar — Total, Active, SLA Breach, Win Rate, Negotiating, Pipeline $, + 6-month win-rate / cycle-time trend chart (▼ Show 6-mo trend)',
           'Thanh KPI — Tổng, Đang chạy, Vi phạm SLA, Tỷ lệ Win, Đang đàm phán, Giá trị pipeline, + biểu đồ xu hướng 6 tháng (▼ Show 6-mo trend)'),
      feat('Kanban board — 6 columns (5 stages + Done) with per-column RFQ count + progress bars',
           'Bảng Kanban — 6 cột (5 giai đoạn + Done) với số RFQ mỗi cột + thanh tiến độ'),
      feat('List view toggle — sortable table of all RFQs with stage pill + SLA highlight + multi-select checkboxes for bulk close / delete',
           'Chuyển view dạng List — bảng sort được, pill giai đoạn, highlight SLA, checkbox multi-select cho đóng/xoá hàng loạt'),
      feat('My Inbox filter — one-click filter showing only RFQs where the current user owns the active stage',
           'Bộ lọc My Inbox — 1 click lọc chỉ RFQ mà user hiện tại là chủ của giai đoạn đang active'),
      feat('Saved Views — save the current filter as a named view (localStorage); reapply from the ⭐ menu',
           'Saved Views — lưu bộ filter hiện tại thành view có tên (localStorage); apply lại từ menu ⭐'),
      feat('Detail drawer — slides from right, 3 tabs: Detail (Identity / Print Specs / Document Flow / Pipeline) · History · Attachments',
           'Drawer chi tiết — slide từ phải, 3 tab: Detail (Identity / Print Specs / Document Flow / Pipeline) · History · Attachments'),
      feat('Required-field gates — each checklist item can be marked "req"; Next → is DISABLED until all required items in the current stage are ticked and stage required fields are filled',
      ...
      feat('Per-stage signatures — when a stage moves to Done, the owner + timestamp are stamped immutably; the stage is read-only until an explicit ⟲ Reopen (logged in audit)',
      ...
      feat('Standardized reason codes — Blocked (R01-R99), WIN (W01-W99), LOSS (L01-L99) dropdowns replace free-text for reportable analysis',
           'Mã lý do chuẩn hoá — Blocked (R01-R99), WIN (W01-W99), LOSS (L01-L99) bằng dropdown thay vì text tự do, phục vụ phân tích'),
      feat('Append-only audit trail — every field change, stage transition, reopen, checklist tick, sync, upload is logged server-side with user + timestamp. View in the History tab.',
      ...
      feat('Attachment manager — upload dieline / customer emails / quote PDFs to server-side content-addressed store; 15 MB / file default cap',
      ...
      feat('Document Flow strip — visual chain RFQ → Pricing Worksheet → Sample → Customer Order; clickable to jump to downstream tabs',
           'Dải Document Flow — chuỗi trực quan RFQ → Pricing Worksheet → Mẫu → Đơn hàng; click được để nhảy sang tab downstream'),
      feat('Sync → Pricing Worksheet — one click pushes RFQ No, Customer, Product, EAU, MOQ, owners, dimensions into Pricing (Std), and records the handoff in Document Flow + audit',
      ...
      feat('Move-stage buttons — "Next →" auto-stamps done date + signature on current stage, activates next. Disabled with a tooltip when requirements are unmet.',
    ...
    procedures: [
      proc('Stage 1 — Sale (Input)', 'Giai đoạn 1 — Sale (Nhập)', null,
        [
          bs('Click + New RFQ to create the record.',
             'Click + New RFQ để tạo hồ sơ.'),
          bs('Enter Identity: RFQ No, Customer, Product, EAU, MOQ, RFQ Date, Deadline, Sales Owner.',
             'Nhập Thông tin chung: RFQ No, Khách, Sản phẩm, EAU, MOQ, Ngày RFQ, Deadline, Sale phụ trách.'),
          bs('Tick the Sale checklist as you confirm each input (design file, specs, EAU/MOQ, deadline).',
             'Tick checklist Sale khi xác nhận mỗi mục (file thiết kế, thông số, EAU/MOQ, deadline).'),
          bs('Click Next → to advance to Feasibility.',
             'Click Next → để chuyển sang Feasibility.'),
        ],
      ),
      proc('Stage 2 — NPI Quote Specialist (Feasibility)', 'Giai đoạn 2 — NPI Quote (Tính khả thi)', null,
        [
          bs('Open the detail drawer, expand the Feasibility accordion.',
             'Mở drawer chi tiết, expand accordion Feasibility.'),
          bs('Fill Print Type (Offset / Flexo / Digital / Hybrid) in the Print Specs section.',
             'Điền Loại in (Offset / Flexo / Digital / Hybrid) trong mục Print Specs.'),
          bs('Tick feasibility checklist: risks logged, approved for costing.',
             'Tick checklist: rủi ro đã ghi nhận, đã duyệt để tính giá.'),
          bs('If blocked (e.g. tolerance impossible), set stage Status = Blocked and add notes.',
      ...
      proc('Stage 3 — NPI Design Process', 'Giai đoạn 3 — NPI Thiết kế', null,
        [
          bs('NPI Design Owner takes over — expand the Design accordion.',
             'Người phụ trách NPI Design tiếp nhận — expand accordion Design.'),
          bs('Tick imposition / dieline / wastage items as they complete.',
             'Tick các mục bình đồ / dieline / hao hụt khi hoàn thành.'),
          bs('Attach dieline file path + layout file path in Print Specs.',
             'Đính kèm đường dẫn dieline + layout trong Print Specs.'),
        ],
      ),
      proc('Stage 4 — Materials Sourcing', 'Giai đoạn 4 — Mua vật tư', null,
        [
          bs('Sourcing team quotes paper, ink, foil, die tooling from approved suppliers.',
             'Bộ phận mua hàng hỏi giá giấy, mực, nhũ, khuôn bế từ NCC đã duyệt.'),
          bs('Update the Material Library with the latest prices (see Library > Material Library).',
             'Cập nhật Library vật tư với giá mới nhất (xem Library > Material Library).'),
          bs('Tick "Prices updated in Library" to confirm Costing has the inputs it needs.',
      ...
      proc('Stage 5 — Pricing (Final)', 'Giai đoạn 5 — Báo giá (Cuối)', null,
        [
          bs('Open Pricing Worksheet directly via Sync → Pricing Worksheet in the drawer footer.',
             'Mở Pricing Worksheet trực tiếp qua nút Sync → Pricing Worksheet dưới drawer.'),
          bs('RFQ No, Customer, Product, EAU, MOQ, owners, dimensions land on the RFQ & MOQ Info tab automatically.',
             'RFQ No, Khách, Sản phẩm, EAU, MOQ, người phụ trách, kích thước sẽ tự đổ sang tab RFQ & MOQ Info.'),
          bs('Compute the full cost sheet, save the quote, then return here and set Result = WIN / LOSS / NEGOTIATING.',
             'Tính toàn bộ cost sheet, lưu báo giá, quay lại đây set Kết quả = WIN / LOSS / NEGOTIATING.'),
          bs('Enter Quote Value (USD) to update the Pipeline $ KPI.',
    ...
    keyFields: [
      { field: 'rfq_no',       label: bi('RFQ No.',         'Số RFQ'),        type: 'text',   notes: bi('Unique RFQ identifier — used as the join key for Sync → Pricing Worksheet.', 'Mã RFQ duy nhất — dùng làm khoá nối cho Sync → Pricing Worksheet.') },
      { field: 'customer',     label: bi('Customer',        'Khách hàng'),    type: 'text',   notes: bi('Direct customer code — maps to Pricing.direct_cu.', 'Mã khách trực tiếp — map sang Pricing.direct_cu.') },
      { field: 'product',      label: bi('Product',         'Sản phẩm'),      type: 'text',   notes: bi('Description — maps to Pricing.description + project_name.', 'Mô tả — map sang Pricing.description + project_name.') },
      { field: 'eau_qty',      label: bi('EAU',             'Sản lượng năm'), type: 'number', notes: bi('Estimated Annual Usage — maps to Pricing.annual_qty.', 'Estimated Annual Usage — map sang Pricing.annual_qty.') },
      { field: 'moq',          label: bi('MOQ',             'MOQ'),           type: 'number', notes: bi('Min order qty — maps to Pricing.moq.', 'Số lượng tối thiểu — map sang Pricing.moq.') },
      { field: 'deadline',     label: bi('Deadline',        'Hạn chót'),      type: 'date',   notes: bi('Card goes red if deadline has passed and result is not WIN/LOSS.', 'Card chuyển đỏ nếu quá deadline và kết quả chưa WIN/LOSS.') },
      { field: 'pipeline_stage', label: bi('Current Stage', 'Giai đoạn'),     type: 'enum',   notes: bi('Derived from per-stage status. Override by clicking Next/Back.', 'Suy ra từ trạng thái từng stage. Override bằng nút Next/Back.') },
      { field: 'result',       label: bi('Result',          'Kết quả'),       type: 'enum',   notes: bi('PENDING / WIN / LOSS / NEGOTIATING. WIN+LOSS move the card to the Done column.', 'PENDING / WIN / LOSS / NEGOTIATING. WIN+LOSS chuyển card sang cột Done.') },
      { field: 'quote_value',  label: bi('Quote Value',     'Giá trị báo giá'), type: 'number', notes: bi('USD — aggregated into the Pipeline $ KPI.', 'USD — tổng hợp vào KPI Pipeline $.') },
      { field: 'specs.width_mm / height_mm', label: bi('Dimensions', 'Kích thước'), type: 'number', notes: bi('mm — map to Pricing.part_width + part_length_md.', 'mm — map sang Pricing.part_width + part_length_md.') },
    ],
    formulas: [
      { name: bi('Win Rate %', 'Tỷ lệ Win %'),
        formula: 'wins / (wins + losses) * 100',
        notes: bi('Only closed RFQs count (WIN + LOSS). PENDING and NEGOTIATING are excluded.',
      ...
      { name: bi('SLA Breach (per RFQ)', 'Vi phạm SLA (mỗi RFQ)'),
        ...
        notes: bi('Closed RFQs (WIN/LOSS) never count as breach.',
      ...
      { name: bi('Stage progress %', '% Tiến độ giai đoạn'),
        ...
        notes: bi('Drives the progress bar on the Kanban card.',
      ...
      { name: bi('Days in stage', 'Số ngày trong giai đoạn'),
        ...
        notes: bi('Turns red on the card if > stage.sla_days.',
                 'Chuyển đỏ trên card nếu > stage.sla_days.') },
    ],
    tips: [
      bt('Start every day on the Kanban view — the SLA Breach KPI tells you what needs action before anything else.',
         'Bắt đầu mỗi ngày ở view Kanban — KPI Vi phạm SLA cho biết việc gì cần làm trước tất cả.'),
      bt('Use the List view for weekly management reports — faster to scan than 6 columns.',
         'Dùng view List cho báo cáo quản lý tuần — scan nhanh hơn 6 cột.'),
      bt('Sync → Pricing Worksheet is one-way (RFQ → Pricing). Changes in the Pricing tab do NOT push back; keep the RFQ fields up to date manually if specs change.',
      ...
      bt('Move-back is for corrections only — do NOT use it to "skip" a stage backwards; use the Status = Blocked field to represent a real hold.',
         'Chỉ dùng Move-back để sửa lỗi — KHÔNG dùng để "nhảy lùi" giai đoạn; dùng Status = Blocked để thể hiện hold thật.'),
    ],
    pitfalls: [
      bp('If the RFQ No is empty, Sync → Pricing Worksheet is disabled. Always fill RFQ No first.',
         'Nếu RFQ No rỗng, Sync → Pricing Worksheet bị disable. Luôn điền RFQ No trước.'),
      bp('The Kanban card progress bar reflects checklist ticks, not stage status. A 100%-ticked stage still needs Next → to advance to the next column.',
      ...
      bp('Deleting an RFQ is permanent — no soft-delete audit trail. Back up before bulk cleanup.',
         'Xoá RFQ là vĩnh viễn — không có audit trail soft-delete. Backup trước khi dọn hàng loạt.'),
    ],
    constraints: [
      con('Max ~500 RFQs per file before load/save latency noticeable (single JSON file).',
          'Tối đa ~500 RFQ mỗi file trước khi load/save chậm rõ rệt (một file JSON duy nhất).'),
      con('No multi-user conflict detection — last write wins. Coordinate edits over chat if multiple operators work the same RFQ.',
          'Chưa phát hiện xung đột đa người dùng — last write wins. Phối hợp qua chat nếu nhiều người cùng sửa 1 RFQ.'),
      con('Drag-and-drop between Kanban columns is NOT supported — use the Next / Back buttons on the card or drawer.',
    ...
    example: ex(
      ...
      [
        bs('+ New RFQ → fill Identity (RFQ No RFQ00042, Customer Samsung, Product 40×20 Flexo Label, EAU 200000, MOQ 50000, Deadline 2026-05-15, Sales Owner Dora).',
        ...
        bs('Tick all 5 Sale checklist items → Next → card moves to Feasibility column.',
           'Tick cả 5 mục checklist Sale → Next → card chuyển sang cột Feasibility.'),
        bs('In Print Specs, set Print Type = Flexo, Width = 40, Height = 20 → tick Feasibility checklist → Next.',
           'Trong Print Specs, set Loại in = Flexo, Rộng = 40, Cao = 20 → tick checklist Feasibility → Next.'),
        bs('NPI Design does imposition (6-up, 1 web) + dieline → tick checklist → Next.',
           'NPI Design làm bình đồ (6-up, 1 web) + dieline → tick checklist → Next.'),
        bs('Sourcing quotes substrate + ink + die → Library updated → Next.',
           'Sourcing hỏi giá vật liệu + mực + khuôn → Library cập nhật → Next.'),
        bs('In Pricing stage: click Sync → Pricing Worksheet. Standard tab opens on RFQ & MOQ Info with all fields prefilled.',
    ...
    authorization: auth(
      ...
      bi('Sales creates the RFQ; NPI + Sourcing advance middle stages; Costing releases Stage 5. Admin can delete any record.',
  ...
  'sample-tracking': {
    ...
    preRequisites: [
      bi('Customer request received (email, RFQ, or call).', 'Đã nhận yêu cầu từ khách (email, RFQ, hoặc điện thoại).'),
      bi('Quantity + due date agreed.', 'Đã thống nhất số lượng + ngày giao.'),
    ],
    features: [
      feat('KPI bar — Total, Pending, SLA Breach, OK Rate %, Finished %, Spec Released, + 6-month trend (OK Rate / avg cycle)',
           'Thanh KPI — Tổng, Pending, Vi phạm SLA, OK Rate %, Finished %, Spec Released, + xu hướng 6 tháng (OK Rate / chu kỳ TB)'),
      feat('Per-NPI-Owner summary strip — parts, finished, OK, NG, OK rate %, spec released (mirrors the xlsx summary)',
           'Strip tổng kết theo NPI Owner — parts, finished, OK, NG, OK rate %, spec released (giống xlsx)'),
      feat('Kanban board — 7 columns (6 stages + Done) with per-card progress bar',
           'Bảng Kanban — 7 cột (6 giai đoạn + Done) với thanh tiến độ trên mỗi card'),
      feat('List view toggle — sortable table with multi-select checkboxes for bulk Mark OK / NG / PARTIAL / Delete',
           'Chuyển view List — bảng sort được, multi-select để bulk Mark OK / NG / PARTIAL / Delete'),
      feat('My Inbox filter — 📥 shows only samples where current user is CS, NPI Owner, or active-stage owner',
           'Filter My Inbox — 📥 chỉ hiện mẫu mà user hiện tại là CS, NPI Owner, hoặc owner giai đoạn đang active'),
      feat('Saved Views — ⭐ menu stores named filter combos in localStorage',
           'Saved Views — menu ⭐ lưu filter đã đặt tên vào localStorage'),
      feat('Detail drawer — 3 tabs: Detail (Identity / Specs / Document Flow / Pipeline) · History · Attachments',
           'Drawer chi tiết — 3 tab: Detail (Identity / Specs / Document Flow / Pipeline) · History · Attachments'),
      feat('Parallel Prep tracking — Material / Film-Plate / Cutter each with Name, ETA, Status — all three must turn Done before the Prep stage can advance',
           'Theo dõi Prep song song — Vật liệu / Film-Plate / Dao mỗi thứ có Tên, ETA, Trạng thái — cả 3 phải Done trước khi Prep advance'),
      feat('Required-field gates — Next → is DISABLED until stage required fields are filled AND required checklist items ticked',
           'Khoá theo trường bắt buộc — Next → BỊ KHOÁ đến khi trường bắt buộc + item "req" của stage đủ'),
      feat('Immutable signatures — when a stage goes Done, owner + timestamp stamp; read-only until ⟲ Reopen',
           'Chữ ký bất biến — khi stage done, đóng dấu owner + timestamp; read-only đến khi ⟲ Reopen'),
      feat('Standardized reason codes — NG (N01-N99), Blocked (R01-R99), Accept (A01-A02) via dropdowns',
           'Mã lý do chuẩn hoá — NG (N01-N99), Blocked (R01-R99), Accept (A01-A02) bằng dropdown'),
      feat('Append-only audit trail — every field edit, stage transition, reopen, checklist tick, upload logged server-side. View in History tab.',
           'Audit trail append-only — mọi thay đổi, chuyển giai đoạn, reopen, tick checklist, upload được log bên server. Xem ở tab History.'),
      feat('Attachments — upload sample photos, customer feedback emails, QC reports; 15 MB / file default cap',
           'Đính kèm — upload ảnh mẫu, email phản hồi khách, QC report; mặc định giới hạn 15 MB / file'),
      feat('Document Flow — RFQ → Sample → Production Order → Spec Released. Fill Linked RFQ / Linked Production Order in Identity to activate.',
    ...
    procedures: [
      ...
      proc('Stage 5 — Customer Feedback', 'Giai đoạn 5 — Phản hồi khách', null,
        [
          bs('Ship the sample; attach shipment tracking as an attachment.',
             'Giao mẫu; đính kèm shipment tracking.'),
          bs('Record customer feedback: set Result = OK / NG / PARTIAL, fill OK count / NG count.',
             'Ghi nhận phản hồi: set Result = OK / NG / PARTIAL, điền OK count / NG count.'),
          bs('If NG/PARTIAL: pick an NG Reason Code (N01-N99) and log the improvement action in notes.',
      ...
      proc('Stage 6 — Spec Released', 'Giai đoạn 6 — Ra SPEC', null,
        [
          bs('Only release SPEC when Result = OK (or PARTIAL-acceptable).',
             'Chỉ release SPEC khi Result = OK (hoặc PARTIAL chấp nhận được).'),
          bs('Fill Linked Production Order (the PO / shop-order this SPEC feeds into).',
             'Điền Linked Production Order (PO / shop-order mà SPEC này đi vào).'),
          bs('Tick "Production order triggered" → Next → lands the card in Done.',
    ...
    keyFields: [
      { field: 'cs',              label: bi('CS',               'Sale'),          type: 'text',   notes: bi('Sales person who owns the customer relationship.', 'Sale phụ trách khách hàng.') },
      { field: 'npi_owner',       label: bi('NPI Owner',        'NPI Owner'),     type: 'text',   notes: bi('NPI team member coordinating the sample pipeline.', 'Người NPI điều phối pipeline mẫu.') },
      { field: 'customer',        label: bi('Customer',         'Khách hàng'),    type: 'text',   notes: bi('Customer code (Samsung, LG, SVP, ...).', 'Mã khách (Samsung, LG, SVP, ...).') },
      { field: 'ifs_code',        label: bi('IFS Code',         'IFS Code'),      type: 'text',   notes: bi('IFS material / SKU code.', 'Mã IFS / SKU.') },
      { field: 'part_number',     label: bi('Part Number',      'Mã số part'),    type: 'text',   notes: bi('Customer-facing part number on the sample.', 'Mã part khách nhìn thấy trên mẫu.') },
      { field: 'shop_order',      label: bi('Shop Order',       'Shop Order'),    type: 'text',   notes: bi('IFS shop-order number if already allocated.', 'Shop order IFS nếu đã cấp.') },
      { field: 'qty',             label: bi('Qty',              'Số lượng'),      type: 'number', notes: bi('Number of sample pieces requested.', 'Số lượng mẫu yêu cầu.') },
      { field: 'specs.width_mm / height_mm', label: bi('Size', 'Kích thước'), type: 'number', notes: bi('mm — sample dimensions for quick visual scan.', 'mm — kích thước mẫu để scan nhanh.') },
      { field: 'specs.sample_type',   label: bi('Sample Type',     'Loại mẫu'),      type: 'enum',   notes: bi('1st article / Colour match / Engineering / Limit / Pre-production.', '1st article / Colour match / Engineering / Limit / Pre-production.') },
      { field: 'specs.product_type',  label: bi('Product Type',    'Loại sản phẩm'), type: 'enum',   notes: bi('HP / Flexo / Offset / Silk Screen / Digital / Gallus / Hybrid.', 'HP / Flexo / Offset / Silk Screen / Digital / Gallus / Hybrid.') },
      { field: 'submit_date',     label: bi('Submit Date',      'Ngày nhận'),     type: 'date',   notes: bi('Date the sample request was received.', 'Ngày nhận yêu cầu mẫu.') },
      { field: 'due_date',        label: bi('Due Date',         'Deadline'),      type: 'date',   notes: bi('Customer-promised delivery date. Card goes red if passed.', 'Ngày cam kết với khách. Card đỏ nếu quá.') },
      { field: 'result',          label: bi('Result',           'Kết quả'),       type: 'enum',   notes: bi('PENDING / OK / NG / PARTIAL. OK unlocks Spec Released.', 'PENDING / OK / NG / PARTIAL. OK mở khoá Spec Released.') },
      { field: 'reason_code',     label: bi('NG Reason Code',   'Mã lý do NG'),   type: 'enum',   notes: bi('N01-N99 for NG/PARTIAL results. Drives quarterly NG analysis.', 'N01-N99 cho NG/PARTIAL. Phục vụ phân tích NG theo quý.') },
      { field: 'ok_count / ng_count', label: bi('OK / NG count', 'Số lượng OK / NG'), type: 'number', notes: bi('Breakdown within a mixed batch.', 'Phân tách trong batch hỗn hợp.') },
      { field: 'linked_rfq',      label: bi('Linked RFQ',       'RFQ liên kết'),  type: 'text',   notes: bi('Back-link to the RFQ Tracker record (for Document Flow).', 'Liên kết ngược về RFQ Tracker (cho Document Flow).') },
      { field: 'linked_production_order', label: bi('Linked Production Order', 'PO sản xuất liên kết'), type: 'text', notes: bi('Forward link to the production order that this SPEC feeds.', 'Liên kết tới PO sản xuất mà SPEC này đi vào.') },
    ],
    formulas: [
      { name: bi('OK Rate %', 'Tỷ lệ OK %'),
        formula: 'OK / (OK + NG + PARTIAL) * 100',
        notes: bi('Only judged samples count. PENDING is excluded.', 'Chỉ tính mẫu đã phán xét. PENDING bị loại.') },
      { name: bi('Finished Rate %', 'Tỷ lệ hoàn thành %'),
        formula: '(total − pending) / total * 100',
        notes: bi('How much of the backlog has a result (any of OK/NG/PARTIAL).', 'Bao nhiêu % backlog đã có kết quả.') },
      { name: bi('SLA Breach (per sample)', 'Vi phạm SLA (mỗi mẫu)'),
        formula: 'due_date < today  OR  any stage.status = "blocked"',
        notes: bi('Samples with OK/NG never count as breach.', 'Mẫu OK/NG không bao giờ là vi phạm.') },
      { name: bi('Per-NPI-Owner summary', 'Tổng kết theo NPI Owner'),
        formula: 'parts, finished, OK, NG, OK rate %, spec released',
        notes: bi('Mirrors the xlsx master summary — owner-by-owner performance.',
                 'Giống master summary trong xlsx — hiệu suất từng owner.') },
    ],
    tips: [
      bt('Start each day on My Inbox — it shows only samples the NPI / CS / QC Owner currently holds.',
         'Bắt đầu mỗi ngày ở My Inbox — chỉ hiện mẫu mà NPI / CS / QC đang giữ.'),
      bt('Use the parallel Prep tracks aggressively — blocking one (e.g. cutter not ready) doesn\'t force the other two to stall; each updates independently.',
         'Dùng 3 track Prep song song tích cực — chặn một (ví dụ dao chưa sẵn) không buộc 2 track kia đứng lại; cập nhật độc lập.'),
      bt('Attach the customer feedback email (not just the gist) — the full thread is the audit record.',
         'Đính kèm email phản hồi khách (không chỉ tóm tắt) — full thread là audit record.'),
      bt('When a sample is NG, fill BOTH the reason code AND a short notes line — the code drives the dashboard; the note helps the next person.',
         'Khi mẫu NG, điền CẢ reason code VÀ 1 dòng notes ngắn — code cho dashboard; notes cho người kế tiếp.'),
    ],
    pitfalls: [
      bp('Required Prep checklist items force all 3 sub-tracks closed before advance. If a cutter is genuinely out of scope, mark that item optional in the drawer (uncheck the "req" badge).',
      ...
      bp('Result = OK on a sample doesn\'t auto-trigger Spec Released — you still need to tick Stage 6 checklist and hit Next.',
         'Result = OK KHÔNG tự kích hoạt Spec Released — vẫn phải tick checklist Stage 6 và Next.'),
      bp('Per-NPI-Owner summary counts only top 5 owners. Rare owners roll into "(unassigned)".',
         'Strip NPI Owner chỉ đếm top 5. Owner hiếm vào "(unassigned)".'),
    ],
    constraints: [
      con('Max ~2000 samples per file before load/save latency noticeable (single JSON file).',
          'Tối đa ~2000 mẫu mỗi file trước khi load/save chậm rõ (1 JSON file).'),
      con('No multi-user conflict detection — last write wins. Coordinate over chat for the same sample.',
          'Chưa phát hiện xung đột đa người dùng — last write wins. Phối hợp qua chat trên cùng 1 mẫu.'),
      con('Attachments capped at 15 MB / file (override via OPS_SAMPLE_ATTACH_MAX_MB). Large video / CAD files must go elsewhere.',
    ...
    example: ex(
      ...
      [
        bs('+ New Sample → Identity: Customer Samsung, CS Ivy, NPI Mark, Qty 50, Submit 2026-04-23, Due 2026-05-10. Specs: Colour match / HP / 40×20.',
           '+ New Sample → Identity: Khách Samsung, CS Ivy, NPI Mark, Qty 50, Nhận 23/4/2026, Deadline 10/5/2026. Specs: Colour match / HP / 40×20.'),
        bs('Next → Prep: set Material Spec "BW-0153 + BW-0112N", Material ETA 2026-04-25 Active → Done, Film/Plate "File in HP" ETA Done, Cutter "CCL-HT-3075" ETA Done. Next →.',
        ...
        bs('Run: YCM issued, machine allocated, run started 2026-04-28, finished 2026-04-29, handed to QC. Next →.',
           'Run: YCM đã gửi, máy đã cấp, chạy bắt đầu 28/4, kết thúc 29/4, giao QC. Next →.'),
        bs('QC: visual OK, dimensional OK, QC report signed, attached to Attachments tab. Next →.',
           'QC: visual OK, dimensional OK, QC report ký, đính kèm tab Attachments. Next →.'),
        bs('Customer: ship, customer feedback "OK with minor notes" → Result = OK, Reason A02, OK count 50 / NG 0. Next →.',
           'Customer: giao, phản hồi khách "OK với note nhỏ" → Result = OK, Reason A02, OK 50 / NG 0. Next →.'),
        bs('Spec: release spec document, fill Linked Production Order "SO-12345", tick "Production order triggered", Next → card moves to Done.',
    ...
    authorization: auth(
      ...
      bi('Sales creates; NPI advances Prep/Spec; Production advances Run; QC advances QC. Admin can delete any record.',
  ...
  'dashboard': {
    ...
    function: bi('Enterprise KPI snapshot — win rate, revenue, margin trend', 'Snapshot KPI công ty — tỉ lệ thắng, doanh thu, xu hướng margin'),
    ...
    preRequisites: [br('Role user+ to view. Admin+ to export CSV.', 'Role user+ để xem. Admin+ để xuất CSV.')],
    procedures: [
      proc('Period picker', 'Chọn kỳ', null,
        [
          bs('Top-right period selector: This Month / Quarter / YTD / Custom.',
             'Bộ chọn kỳ trên phải: Tháng này / Quý / YTD / Tuỳ chỉnh.'),
          bs('All KPI cards refresh on change; auto-refresh every 5 min otherwise.',
             'Mọi card KPI refresh khi đổi; auto-refresh 5 phút/lần.'),
        ],
      ),
      proc('KPI card drill-down', 'Drill-down thẻ KPI', null,
        [
          bs('Click any card (Win rate, Revenue, Margin, Approval SLA).',
             'Click card bất kỳ (Win rate, Revenue, Margin, Approval SLA).'),
          bs('Modal opens with the underlying quote list.',
             'Modal mở với danh sách báo giá chi tiết.'),
          bs('Click a quote → jumps to Quote History with that row selected.',
             'Click báo giá → nhảy sang Quote History với dòng đó được chọn.'),
        ],
      ),
      proc('Chart interaction', 'Tương tác biểu đồ', null,
        [
          bs('Hover bars/lines for tooltip with exact numbers.',
             'Hover bar/line để xem tooltip số chính xác.'),
          bs('Click a bar → filter the KPI cards to just that slice (e.g. one month).',
             'Click bar → lọc card KPI chỉ cho slice đó (vd 1 tháng).'),
          bs('Reset filter via breadcrumb at top.',
             'Reset filter qua breadcrumb ở trên.'),
        ],
      ),
      proc('Export (admin)', 'Xuất (admin)', null,
        [
          bs('Click ⬇ CSV in toolbar — exports the CURRENT filtered dataset.',
             'Click ⬇ CSV trên toolbar — xuất dataset HIỆN TẠI đã lọc.'),
          bs('Scheduled email reports configured in Settings → Notifications.',
    ...
    formulas: [
      ...
      { name: 'Average margin',
        ...
        example: 'Q1: $10k × 40% + $100k × 25% = $4k + $25k = $29k profit on $110k\navg = 29/110 × 100 = 26.4% (not 32.5% arithmetic mean)',
      ...
      { name: 'Approval SLA',
        ...
        example: '100 quotes: median 4h, p95 = 27h\nSLA target < 24h → currently FAILING at p95 level' },
    ],
    tips: [bt('Pin the dashboard as your home tab — set in Settings → Appearance.',
              'Pin dashboard làm home tab — setup trong Settings → Appearance.')],
    pitfalls: [bp('Data is from Quote History; quotes without Won/Lost status are excluded from win rate.',
  ...
  'quote-analysis': {
    ...
    function: bi('Deep-dive win/loss analytics by segment / material / price band', 'Phân tích sâu thắng/thua theo phân khúc / vật tư / mức giá'),
    ...
    procedures: [
      proc('Select view', 'Chọn view', null,
        [
          bs('Top tabs: By Customer / By Material / By Price band / By Cost engineer.',
             'Tab trên cùng: Theo Khách / Theo Vật tư / Theo Mức giá / Theo Cost engineer.'),
          bs('Each view has its own default charts + aggregations.',
             'Mỗi view có chart + aggregation mặc định riêng.'),
        ],
      ),
      proc('Filter + time range', 'Lọc + khoảng thời gian', null,
        [
          bs('Left panel: date range + status filter (Won / Lost / All closed).',
             'Panel trái: date range + filter status (Won / Lost / Tất cả đã đóng).'),
          bs('Segment filter per view (customer group, material category, etc.).',
             'Filter phân khúc theo view (nhóm khách, danh mục vật tư, v.v.).'),
          bs('Min sample size toggle (default: hide segments with < 20 quotes).',
             'Toggle kích thước mẫu tối thiểu (mặc định: ẩn phân khúc có < 20 báo giá).'),
        ],
      ),
      proc('Drill-down', 'Drill-down', null,
        [
          bs('Click any chart bar → underlying quote list filtered to that slice.',
             'Click bar chart bất kỳ → danh sách báo giá lọc theo slice đó.'),
          bs('Click a quote → jumps to Quote History.',
             'Click báo giá → nhảy sang Quote History.'),
          bs('Breadcrumb navigation to go back to the full view.',
             'Navigation breadcrumb để quay lại full view.'),
        ],
      ),
      proc('Export for QBR', 'Xuất cho QBR', null,
        [
          bs('Click ⬇ PNG — chart-only export for presentation.',
             'Click ⬇ PNG — xuất chỉ chart cho presentation.'),
          bs('Click ⬇ CSV — tabular export for further Excel analysis.',
    ...
    keyFields: [],
    formulas: [],
    tips: [bt('Material sensitivity view surfaces which materials drive the most quote volatility.',
              'View material sensitivity cho thấy vật tư nào gây biến động báo giá nhiều nhất.')],
    pitfalls: [bp('Sample size matters — segments with < 20 quotes shouldn\'t drive strategy decisions alone.',
  ...
  'lib-rate': {
    ...
    function: bi('Per-work-center hourly rates (labor / machine / overhead)', 'Đơn giá/giờ theo work-center (labor / machine / overhead)'),
    ...
    procedures: [
      proc('Category filter', 'Lọc theo loại',
        null,
        [
      ...
      proc('Annual Q4 refresh', 'Cập nhật Q4 hàng năm',
        null,
        [
    ...
    formulas: [],
    tips: [bt('Annual Q4 rate refresh: bump every rate, publish, and communicate to the team same day.',
              'Cập nhật đơn giá hàng năm Q4: tăng mọi rate, publish, và thông báo cho team cùng ngày.')],
    pitfalls: [bp('Backdating an effective date retroactively changes historic quote recalculations. Avoid except for correction of clear errors.',
  ...
  'lib-ddl': {
    ...
    function: bi('Master reference enums for every LOV in the app', 'Enum gốc cho mọi LOV trong app'),
    ...
    workflow: [
      ...
      bs('Add / edit / deprecate entries. Deprecated = hidden in future dropdowns but kept for old records.',
         'Thêm / sửa / deprecate entry. Deprecated = ẩn khỏi dropdown tương lai nhưng giữ cho bản ghi cũ.'),
      bs('Reorder drag handles; order becomes the default display order everywhere.',
         'Kéo drag handle để sắp xếp lại; thứ tự thành default display order mọi nơi.'),
    ],
    keyFields: [],
    formulas: [],
    tips: [bt('Never HARD-delete an entry — older quotes break. Always mark deprecated.',
              'Không bao giờ XOÁ cứng entry — báo giá cũ sẽ hỏng. Luôn đánh dấu deprecated.')],
    pitfalls: [bp('Renaming an entry changes the display in old quotes too — use with care for customer-facing labels.',
  ...
  'lib-finance': {
    ...
    function: bi('Company-wide finance aggregates (P&L, balances, transfer pricing)', 'Tổng hợp tài chính công ty (P&L, số dư, transfer pricing)'),
    ...
    workflow: [
      ...
      bs('Review the aggregate; drill to source transactions if the IFS bridge is online.',
         'Xem aggregate; drill xuống transaction nguồn nếu IFS bridge online.'),
      bs('Publish overhead rate for the period → propagates to Rate Table.',
         'Publish rate overhead cho kỳ → lan sang Rate Table.'),
    ],
    keyFields: [],
    formulas: [],
    tips: [bt('Keep finance + ops views separate — this tab is the SOURCE, Rate Table is the CONSUMER.',
              'Tách view finance + ops — tab này là NGUỒN, Rate Table là NGƯỜI TIÊU DÙNG.')],
    pitfalls: [bp('Overhead rate changes mid-period cause quote inconsistencies; publish once at period start.',
  ...
  'settings': {
    ...
    function: bi('Personal preferences + admin-only system controls', 'Tuỳ chỉnh cá nhân + quyền quản trị'),
    ...
    preRequisites: [],
    workflow: [
      bs('My Profile: update avatar + name + phone.',
         'My Profile: cập nhật avatar + tên + phone.'),
      bs('My Password: old → new (min 8 chars).',
         'My Password: cũ → mới (tối thiểu 8 ký tự).'),
      bs('Appearance: language, theme (light/dark/system), date format, default home tab.',
         'Appearance: ngôn ngữ, theme (sáng/tối/theo hệ thống), format ngày, home tab mặc định.'),
      bs('(Desktop) Hardware Devices: configure label printer, scale, scanner, office printer.',
         '(Desktop) Thiết bị phần cứng: cấu hình máy in nhãn, cân, scanner, máy in văn phòng.'),
      bs('(Desktop) Connection Mode: switch Embedded / Thin / Smart, edit Remote URL, re-run setup wizard.',
         '(Desktop) Chế độ kết nối: chuyển Embedded / Thin / Smart, sửa Remote URL, chạy lại setup wizard.'),
      bs('(Admin) Account Control: create/disable users, reset passwords, generate Provisioning Card (SAP/IFS handover), change roles.',
         '(Admin) Account Control: tạo/vô hiệu user, reset mật khẩu, tạo Provisioning Card (bàn giao SAP/IFS), đổi role.'),
      bs('(Admin) Backup/Restore: download a full data snapshot; or upload to restore.',
         '(Admin) Backup/Restore: tải snapshot dữ liệu đầy đủ; hoặc upload để restore.'),
      bs('(Admin) System Logs: view last 1k events filtered by level.',
         '(Admin) System Logs: xem 1k sự kiện gần nhất lọc theo mức.'),
    ],
    keyFields: [],
    formulas: [],
    tips: [bt('Take a full backup before any risky admin action (bulk user disable, rate-table batch edit).',
              'Backup đầy đủ trước mọi thao tác admin rủi ro (disable user hàng loạt, sửa rate-table hàng loạt).')],
    pitfalls: [bp('Restore OVERWRITES the current DB — not a merge. Test on a staging URL first if available.',
  ...
  'settings-profile': {
    ...
    function: bi('Personal identity fields (avatar, name, phone, default home)', 'Thông tin cá nhân (avatar, tên, phone, home mặc định)'),
    ...
    workflow: [
      ...
      bs('Click the avatar to upload a new image (max 2 MB, square crop recommended).',
         'Click avatar để upload ảnh mới (tối đa 2 MB, nên crop vuông).'),
      bs('Edit display name / phone / email.',
         'Sửa tên hiển thị / phone / email.'),
      bs('Default home tab: pick which tab opens after login. Saves per user, not per device.',
         'Home tab mặc định: chọn tab mở sau đăng nhập. Lưu theo user, không theo thiết bị.'),
      bs('Save → immediate effect; no re-login required.',
         'Save → hiệu lực ngay; không cần đăng nhập lại.'),
    ],
    keyFields: [
      field('Display name',   'string',   'Shown everywhere your user is mentioned (quotes, messages, audit log).'),
      field('Default home',   'tab id',   'Tab that auto-opens after login; falls back to Standard Calc if cleared.'),
    ],
    formulas: [],
    tips: [bt(
  ...
  'settings-password': {
    ...
    workflow: [
      ...
      bs('Enter current password → new password (min 8 chars, mix letters + digits) → confirm.',
         'Nhập mật khẩu hiện tại → mật khẩu mới (tối thiểu 8 ký tự, có chữ + số) → xác nhận.'),
      bs('Save → you are logged out; sign back in with the new password.',
    ...
    pitfalls: [
      bp('Forgot the current one? Admin MUST reset via Account Control — no backdoor.',
         'Quên mật khẩu hiện tại? Admin PHẢI reset qua Account Control — không có backdoor.'),
      bp('If 2FA is on (Administrator account), a password change does NOT reset the TOTP secret; you still need the old Google Authenticator entry.',
  ...
  'settings-appearance': {
    ...
    function: bi('Per-user UI preferences (language, theme, formats)', 'Tuỳ chỉnh UI per-user (ngôn ngữ, theme, format)'),
    ...
    workflow: [
      ...
      bs('Language: EN / VI. All labels + Help content flip instantly.',
         'Ngôn ngữ: EN / VI. Mọi label + nội dung Help chuyển ngay.'),
      bs('Theme: Light / Dark / System (follows OS-level prefers-color-scheme).',
         'Theme: Sáng / Tối / Theo hệ thống (theo prefers-color-scheme OS).'),
      bs('Date format: DD/MM/YYYY (VI default) or MM/DD/YYYY (US).',
         'Format ngày: DD/MM/YYYY (mặc định VI) hoặc MM/DD/YYYY (US).'),
      bs('Number format: VI (1.234,56) or US (1,234.56).',
  ...
  'settings-hardware': {
    ...
    function: bi('Per-machine industrial peripheral configuration (desktop only)', 'Cấu hình thiết bị ngoại vi công nghiệp per-máy (chỉ desktop)'),
    ...
    preRequisites: [
      br('Ops Control Desktop App (web build shows a banner — browser sandbox blocks raw USB/Serial/TCP).',
         'Ops Control Desktop App (bản web hiện banner — browser sandbox chặn USB/Serial/TCP raw).'),
      br('Device drivers installed at the OS level (Windows printer driver, USB-Serial driver for the scale).',
         'Driver thiết bị cài ở mức OS (Windows printer driver, USB-Serial driver cho cân).'),
    ],
    features: [
      feat('Bilingual UI — flag toggle (🇬🇧 EN / 🇻🇳 VN) at the top-right of the tab header switches every label instantly. Same control as Settings → Appearance, surfaced inline so handover-time operators can flip without leaving the tab.',
      ...
      feat('Label printer card: IP + port, Ping connection (round-trip latency), Send test label (minimal ZPL "Ops Control TEST").',
           'Card máy in nhãn: IP + port, Ping kết nối (round-trip latency), Gửi nhãn test (ZPL tối giản "Ops Control TEST").'),
      feat('Scale card: COM port dropdown (auto-discovered), baud rate, Connect + read realtime (live weight stream from RS232/USB-Serial).',
           'Card cân: dropdown COM port (tự discover), baud rate, Connect + đọc realtime (stream trọng lượng live từ RS232/USB-Serial).'),
      feat('Scanner card: Keyboard Wedge mode (default — scanner types into active field) OR HID raw mode (open device by VID:PID, app receives scan events).',
      ...
      feat('Office printer: pick default printer name from OS spooler list + paper size (A4/A3/Letter) for PDF report output.',
           'Máy in văn phòng: chọn máy in mặc định từ danh sách OS spooler + khổ giấy (A4/A3/Letter) cho output PDF báo cáo.'),
    ],
    workflow: [
      bs('Open the Hardware Devices tab. If you see "only available in Desktop App" banner, you\'re on the web build — install the desktop app.',
         'Mở tab Thiết bị phần cứng. Nếu thấy banner "chỉ khả dụng trong Desktop App", bạn đang ở bản web — cài bản desktop.'),
      bs('Label printer: enter IP (e.g. 192.168.1.50) + port (default 9100). Click Ping — expect ✓ Connection OK with latency. Click Send test label — physical printer should produce a small "TEST" label.',
      ...
      bs('Scale: click ↻ Scan ports → pick the COM port assigned to your scale (Manufacturer name shown in parens). Set baud rate to match scale (typical 9600 for industrial scales). Click Connect + Read realtime — weight should stream live below.',
      ...
      bs('Scanner: leave Keyboard Wedge ON (default) — scanner just types into whatever input the user has focus in (works for any tab without app code changes). Toggle OFF + select HID device only if you need raw event capture (rare).',
      ...
      bs('Office printer: pick a printer from the dropdown (system default works for most cases). Set paper size matching the report templates.',
         'Máy in văn phòng: chọn máy in từ dropdown (mặc định hệ thống đủ dùng cho hầu hết). Set khổ giấy trùng template báo cáo.'),
    ],
    keyFields: [],
    formulas: [],
    tips: [
      bt('Use Ethernet over Wi-Fi for label printers when possible — Wi-Fi adds ~50-200ms ping, noticeable when batch-printing dozens of labels.',
         'Dùng Ethernet thay Wi-Fi cho máy in nhãn khi có thể — Wi-Fi thêm ~50-200ms ping, dễ nhận thấy khi in batch hàng chục nhãn.'),
      bt('Keyboard Wedge mode is the safest default for scanners — works without any app-side handler. Switch to HID raw only when you need scan-event metadata (timestamp, source) for audit.',
    ...
    pitfalls: [
      bp('Settings here are PER MACHINE (electron-store, local userData). Re-installing the app or moving to a new machine means re-entering all device IPs/ports/COM paths.',
      ...
      bp('A label printer that worked yesterday but Pings failed today is usually a DHCP-changed IP. Check the printer\'s LCD/network panel for the current IP and update here.',
  ...
  'settings-mode': {
    ...
    function: bi('Switch the desktop app between Embedded / Thin / Smart connection modes', 'Đổi desktop app giữa 3 chế độ kết nối Embedded / Thin / Smart'),
    ...
    preRequisites: [
      br('Ops Control Desktop App (web build always behaves as Thin against the current host).',
         'Ops Control Desktop App (bản web luôn hành xử như Thin với host hiện tại).'),
      br('Network reachability to the chosen mode\'s server (LAN ping for Thin/Smart; localhost for Embedded).',
         'Network với server tương ứng (LAN ping cho Thin/Smart; localhost cho Embedded).'),
    ],
    features: [
      feat('Bilingual UI — same flag toggle (🇬🇧 EN / 🇻🇳 VN) as the Hardware tab.',
           'UI song ngữ — cùng toggle cờ (🇬🇧 EN / 🇻🇳 VN) như tab Hardware.'),
      feat('3-card mode picker with Pros/Trade-off per mode + ACTIVE badge for the current selection.',
           'Picker 3 thẻ mode với Ưu điểm/Đánh đổi mỗi mode + badge ACTIVE cho lựa chọn hiện tại.'),
      feat('Server-role inline LAN URL block — when Embedded mode is active AND build is SERVER, surfaces every reachable LAN IP with one-click Copy. Replaces the once-per-install first-run dialog as a permanent reference.',
      ...
      feat('Bilingual Decision Legend (collapsible) — quick-reference matrix showing which mode fits which scenario. Includes a Smart-mode readiness warning so admins know which tabs work offline today vs. don\'t.',
      ...
      feat('Sprint 1.5 — "↻ Re-run setup wizard" button (bottom of tab) clears firstRunCompleted + mode + remoteUrl, then relaunches the app so the role-specific first-run dialog fires fresh. Useful for reassigning a workstation without uninstall. User data is NOT touched.',
    ...
    workflow: [
      bs('Open Connection Mode tab. Read the Legend (collapsible at the top of the page) if unsure which mode to pick.',
         'Mở tab Chế độ kết nối. Đọc Legend (thu gọn ở đầu trang) nếu chưa chắc chọn mode nào.'),
      bs('Click the radio on the desired mode card. Embedded = local only. Thin = remote required. Smart = hybrid (offline-capable when wiring complete).',
         'Click radio trên thẻ mode mong muốn. Embedded = chỉ local. Thin = cần remote. Smart = hybrid (offline được khi wire xong).'),
      bs('For Thin/Smart: enter the Remote Server URL (e.g. http://10.102.3.61:3000). Get this from the admin who installed the SERVER build.',
         'Cho Thin/Smart: nhập Remote Server URL (vd http://10.102.3.61:3000). Lấy từ admin cài bản SERVER.'),
      bs('Click Apply. If the server says needsRestart=true, restart the app via the green banner that appears.',
         'Click Áp dụng. Nếu server báo needsRestart=true, khởi động lại app qua banner xanh xuất hiện.'),
      bs('SERVER role + Embedded mode: copy ONE of the LAN URLs shown to give to client machines. Prefer Ethernet IPs over Wi-Fi (more stable).',
      ...
      bs('To re-onboard the machine (new user, new server, IP change): scroll to bottom → "↻ Re-run wizard" → confirm → app restarts with the role-specific first-run dialog.',
    ...
    keyFields: [
      { field: 'mode',         label: bi('Connection mode',   'Chế độ kết nối'),  type: 'enum', notes: bi('embedded | thin | smart. Persisted per machine in electron-store.', 'embedded | thin | smart. Lưu per-máy trong electron-store.') },
      { field: 'remoteUrl',    label: bi('Remote server URL', 'URL server remote'), type: 'text', notes: bi('Required for thin/smart. Format: http://host:port (no trailing slash).', 'Bắt buộc cho thin/smart. Format: http://host:port (không có dấu / cuối).') },
      { field: 'embeddedPort', label: bi('Embedded port',     'Port embedded'),     type: 'number', notes: bi('Auto-picked at first boot for the in-process server. Read-only here.', 'Tự chọn lần boot đầu cho server in-process. Chỉ đọc ở đây.') },
      { field: 'buildRole',    label: bi('Build role',        'Bản cài'),          type: 'enum', notes: bi('SERVER | CLIENT | generic. Baked into the installer; surfaces which first-run dialog fires.', 'SERVER | CLIENT | generic. Embed vào installer; quyết định dialog first-run nào fire.') },
    ],
    formulas: [],
    tips: [
      bt('When unsure between Thin and Smart: start with Thin (simpler, real-time). Move to Smart once the network is unreliable enough to be a daily problem AND the audit shows enough tabs are wired for offline cache.',
      ...
      bt('Configure a static IP or DHCP reservation for the SERVER machine on the router. Without that, the server\'s LAN IP can change after a power outage and every client will lose connection.',
    ...
    pitfalls: [
      ...
      bp('Re-running the wizard wipes mode + remoteUrl on this machine. After clicking, this client will lose its server connection until the wizard re-asks for the URL on next launch. Schedule it during downtime.',
  ...
  'settings-account-control': {
    ...
    features: [
      feat('4 sub-tabs: Users (roster CRUD) · Permissions (legacy canDeleteQuote / approval roles) · Permission Groups (SAP-style authorization profiles) · Sessions (sys-only — see all active logins, revoke any)',
      ...
      feat('Đợt 4 (v1.3) — login anomaly detection: server emits SSE security.alert when a user logs in concurrently from 2+ IPs, from a new IP not seen in 30d, or at unusual hours. Audit logged as LOGIN_ANOMALY; admins see real-time toast; user themselves sees a yellow toast post-login.',
      ...
      feat('Per-user Department dropdown (sales/cs/npi/purchasing/production/quality/finance/leader/ops) — informational + default-group suggestion',
           'Dropdown Department mỗi user (sales/cs/npi/purchasing/production/quality/finance/leader/ops) — tham chiếu + gợi ý group'),
      feat('Per-user Permission Group dropdown — controls which tabs render, which inputs disable',
           'Dropdown Permission Group mỗi user — điều khiển tab nào hiện, input nào disable'),
      feat('Permission Groups CRUD: matrix of 23 tabs × 3 modes (Hidden/Read/Edit) + bulk set + duplicate + delete (system group protected)',
           'CRUD Permission Groups: ma trận 23 tab × 3 mode (Hidden/Read/Edit) + bulk set + duplicate + xoá (group system được bảo vệ)'),
      feat('User coverage banner — counts X/Y users assigned to a group, warns if any still using default all-access',
           'Banner coverage user — đếm X/Y user đã gán group, cảnh báo nếu còn user dùng default all-access'),
      feat('Session auto-revoke on permission-group change — user must re-login for new matrix to kick in',
           'Auto-revoke session khi đổi permission-group — user phải login lại để matrix mới áp dụng'),
      feat('Sprint 1.5 — SAP/IFS-style Provisioning Card. Per-user "ID-card" button generates a cryptographically random temp password (12 chars, dash-grouped, no ambiguous glyphs), forces change-on-first-login, and opens a printable handover card with Server URL + Username + Temp Pwd + warning + 3-step instructions (bilingual EN/VN). Temp pwd is shown ONCE — Copy or Print before closing.',
      ...
      feat('Forced password change — when the server flag must_change_password=true (set by Add User OR Reset Pwd OR Generate Temp Pwd), the login screen auto-flips to change-pwd mode the moment the user types their username; the Cancel toggle is hidden so the user cannot dismiss it.',
    ...
    workflow: [
      ...
      bs('Create user: + Add User → fill username + role. Temp password auto-set + must_change_password flag enabled by default. User is forced to change pwd on first login.',
      ...
      bs('Assign Department (dropdown). Same as company org chart — helps pick the right group.',
         'Gán Department (dropdown). Giống sơ đồ tổ chức công ty — giúp chọn đúng group.'),
      bs('Assign Permission Group (dropdown). Session auto-revoked; user re-logs in with new tab access.',
         'Gán Permission Group (dropdown). Session auto-revoke; user login lại với quyền tab mới.'),
      bs('Create a NEW group: switch to Permission Groups sub-tab → + Add Group. Fill ID + name + default dept.',
         'Tạo group MỚI: chuyển sub-tab Permission Groups → + Add Group. Điền ID + name + default dept.'),
      bs('In the matrix, click a radio for each tab (Hidden = ẩn, Read = chỉ đọc, Edit = sửa). Use Bulk for mass-set.',
         'Trong ma trận, click radio cho từng tab (Hidden = ẩn, Read = chỉ đọc, Edit = sửa). Dùng Bulk để set hàng loạt.'),
      bs('Save. Users assigned to this group pick up changes on next login.',
         'Save. User gán group này nhận thay đổi khi login kế tiếp.'),
      bs('Reset password (admin types one): user row → Reset (key icon) → enter new pwd. must_change_password is auto-set, all sessions revoked.',
      ...
      bs('SAP/IFS handover (Sprint 1.5): user row → Provisioning Card (ID-card icon) → confirm → modal opens with auto-generated temp pwd. Click Print for an A6 paper card OR Copy for a multi-line text block to send via secure channel. Hand to the new user. Temp pwd is gone the moment you close the modal — re-issue if lost.',
      ...
      bs('Disable user: toggle Active OFF. For immediate session kill, click "Sign out sessions".',
         'Vô hiệu user: toggle Active OFF. Để kill session ngay, click "Sign out sessions".'),
    ],
    keyFields: [
      { field: 'username',            label: bi('Username',          'Tên đăng nhập'),   type: 'text',   notes: bi('Login identifier; cannot be changed after create.', 'ID đăng nhập; không đổi được sau khi tạo.') },
      { field: 'role',                label: bi('Role',              'Vai trò'),          type: 'enum',   notes: bi('viewonly < user < cost < admin < sys. Coarse gate; sys bypasses all tab checks.', 'viewonly < user < cost < admin < sys. Gate thô; sys bỏ qua mọi check tab.') },
      { field: 'department',          label: bi('Department',        'Phòng ban'),        type: 'enum',   notes: bi('Informational. Suggests default group but does NOT enforce on its own.', 'Tham chiếu. Gợi ý group mặc định nhưng KHÔNG enforce một mình.') },
      { field: 'permission_group_id', label: bi('Permission Group',  'Nhóm quyền'),       type: 'enum',   notes: bi('Controls per-tab Hidden/Read/Edit. Empty = fall back to role-only (all-access).', 'Điều khiển Hidden/Read/Edit từng tab. Rỗng = fallback role-only (all-access).') },
      { field: 'active',              label: bi('Active',            'Hoạt động'),        type: 'bool',   notes: bi('OFF = cannot log in. Audit log retained.', 'OFF = không login được. Audit log vẫn giữ.') },
      { field: '2fa',                 label: bi('2FA',               '2FA'),              type: 'bool',   notes: bi('Google Authenticator TOTP; shows QR on user next login.', 'Google Authenticator TOTP; hiện QR khi user login kế.') },
    ],
    formulas: [
      { name: bi('Access resolution (client & server)', 'Cách tính access (client + server)'),
        formula: "access(user, tabId) =\n  if !user                                  → 'hidden'\n  if user.role === 'sys'                    → 'edit'\n  if !user.permission_group_id              → 'edit'  (legacy fallback)\n  if group not found                        → 'edit'  (graceful)\n  group.tab_permissions[tabId]              → as declared\n  else                                      → 'edit'  (tab unlisted)",
        notes: bi('Same logic runs in client AccessContext and server permissionService.resolveTabAccess — single source of truth.',
      ...
      { name: bi('Sort order — tied suggestions', 'Thứ tự sắp xếp khi score bằng nhau'),
        ...
        notes: bi('(Layout Optimizer only — listed here for reference.)',
                 '(Chỉ áp dụng cho Layout Optimizer — liệt kê để tham khảo.)') },
    ],
    tips: [
      bt('Start with seed groups: Sales / CS / NPI / Purchasing / Production / Quality / Leader. Duplicate + customise if your team differs.',
         'Bắt đầu với group seed: Sales / CS / NPI / Purchasing / Production / Quality / Leader. Duplicate + chỉnh nếu team khác.'),
      bt('System group "all_access" is protected — cannot be edited or deleted. Use it as the implicit admin fallback.',
         'Group system "all_access" được bảo vệ — không sửa/xoá được. Dùng làm fallback ngầm cho admin.'),
      bt('Permission change auto-revokes user sessions. Warn the user before flipping their group in the middle of the day.',
         'Đổi permission tự revoke session user. Báo trước khi đổi group giữa giờ làm.'),
      bt('Grant sys role only to 1-2 people (ops). Sys bypasses all tab checks + can reset 2FA — over-grant is a real risk.',
         'Cấp role sys chỉ cho 1-2 người (ops). Sys bỏ qua mọi check tab + reset 2FA được — cấp nhiều là rủi ro.'),
      bt('Prefer the Provisioning Card flow over the manual Reset prompt — admin doesn\'t need to invent a password, the card has all info on one page (server URL + username + temp pwd + instructions), and the printed handout is a clean record of who got which credentials when.',
    ...
    pitfalls: [
      bp('Role and Permission Group are ORTHOGONAL. A user with role=admin but group=sales_default still SEES only what sales_default allows — until they\'re also assigned all_access or a leader group.',
      ...
      bp('Hard-deleting a user breaks quote ownership on old records. Deactivate, don\'t delete.',
         'Xoá cứng user phá quote ownership trên record cũ. Deactivate, không xoá.'),
      bp('Resetting a password DOES NOT reset 2FA. User still needs their Authenticator entry unless you also toggle 2FA off + on.',
         'Reset mật khẩu KHÔNG reset 2FA. User vẫn cần entry Authenticator trừ khi toggle 2FA off + on.'),
      bp('Users without a permission group fall back to edit on EVERY tab. That\'s the default until an admin assigns one — the coverage banner warns about this.',
         'User không có permission group fallback sang edit MỌI tab. Đó là default đến khi admin gán — banner coverage cảnh báo.'),
      bp('Provisioning Card temp pwd is shown ONCE — the server hashes + stores it but does not echo it back. If the admin closes the modal before copying or printing, the only recovery is to re-issue (which generates a new pwd and revokes the old one). Always Copy or Print BEFORE clicking Done.',
      ...
      bp('Generating a temp pwd revokes ALL of the target user\'s active sessions (server-side audit: PWD_TEMP_REVOKE). If you do this on a user mid-shift they\'ll be logged out instantly — coordinate with them or do it during downtime.',
    ...
    example: ex(
      ...
      [
        bs('Settings → Account Control → Users → + Add User. Username=minh.nguyen, Role=user (Password field can stay default; will be replaced in next step).',
        ...
        bs('Row minh.nguyen → Dept dropdown → sales. Permission Group dropdown → Sales Team (default).',
           'Dòng minh.nguyen → dropdown Dept → sales. Dropdown Permission Group → Sales Team (default).'),
        bs('Same row → click Provisioning Card icon (ID-card) → confirm → modal opens with auto-generated temp pwd e.g. "Tx9-mK2p-Qv8L".',
           'Cùng dòng → click icon Provisioning Card (ID-card) → confirm → modal hiện temp pwd tự tạo vd "Tx9-mK2p-Qv8L".'),
        bs('Click Print to print an A6 paper card OR Copy to send via Slack DM. Hand to minh.',
           'Click Print để in thẻ giấy A6 HOẶC Copy để gửi qua Slack DM. Đưa cho minh.'),
        bs('Minh installs Ops Control CLIENT, opens app → first-run wizard asks for server URL → she types it from the card → test → save → login screen.',
           'Minh cài Ops Control CLIENT, mở app → first-run wizard hỏi server URL → cô ấy gõ URL từ thẻ → test → save → màn login.'),
        bs('Minh types her username — login form auto-flips to change-pwd mode + amber banner appears: "Admin đã cấp mật khẩu tạm". She types temp pwd + new pwd + confirm → Submit → in.',
        ...
        bs('Minh sees sidebar: RFQ Tracker (edit), Quote History (read), Formal Quotation (edit), Dashboard (read). Pricing (Std/Cpx), Material Cost, Finance Data — NOT rendered.',
    ...
    authorization: auth(
      'Admin + Sys only',
      bi('CRUD on Permission Groups requires admin/sys. Assigning groups to users requires admin+. System group "all_access" cannot be modified.',
         'CRUD Permission Groups yêu cầu admin/sys. Gán group cho user yêu cầu admin+. Group system "all_access" không sửa được.'),
    ),
    constraints: [
      con('Unassigned users fall back to edit-all — NOT a default-deny system. Migration to default-deny is opt-in per user.',
          'User chưa gán group fallback sang edit-all — KHÔNG phải hệ thống default-deny. Migration sang default-deny opt-in từng user.'),
      con('Permission changes take effect on NEXT login, not instantly (sessions revoked but active tabs keep their matrix until refresh).',
          'Thay đổi permission áp dụng ở lần login KẾ (session bị revoke nhưng tab đang mở giữ matrix cũ đến khi refresh).'),
      con('Group deletion cascades: users assigned to a deleted group auto-fall back to edit-all. Review user assignments before deleting.',
  ...
  'settings-backup-restore': {
    ...
    function: bi('Full-system data snapshot + its inverse (admin only)', 'Snapshot toàn bộ dữ liệu + ngược lại (chỉ admin)'),
    ...
    preRequisites: [br('Admin role; at least 500 MB free disk space on the client.',
                       'Role Admin; ít nhất 500 MB trống trên client.')],
    workflow: [
      bs('Settings → Backup / Restore. Default tab: Data Backups.',
         'Settings → Backup / Restore. Tab mặc định: Data Backups.'),
      bs('Create Data Backup → click → snapshot JSON ghi xuống server/data/Backup & restore/Data/manual_<ts>.json.',
         'Create Data Backup → click → snapshot JSON ghi xuống server/data/Backup & restore/Data/manual_<ts>.json.'),
      bs('Auto daily backup chạy nền (đợt 5 default ON) — auto_<YYYYMMDD>_<HHMMSS>.json. Kept 30 ngày, prune tự động.',
         'Auto daily backup chạy nền (đợt 5 default ON) — auto_<YYYYMMDD>_<HHMMSS>.json. Giữ 30 ngày, tự prune.'),
      bs('SQLite online backup chạy song song mỗi ngày 02:00 → server/data/Backup/SQLite/ops_<ts>.sqlite (PRAGMA integrity_check sau mỗi backup).',
      ...
      bs('Đợt 3 (v1.3) — UPLOAD từ máy khác: nút "📤 Upload từ máy khác…" (sys-only). Pick file .json từ USB / off-site copy → server validate có ≥1 known dataset key → save vào Backup & restore/Data/uploaded_<ts>_<original>.json. Sau đó click Restore từ list.',
      ...
      bs('Off-site backup script (scripts/backup-offsite.sh) chạy cron 02:30 → rsync newest local backup sang USB/NAS/SSH target + sha256 verify + webhook alert on fail. Cấu hình env OPS_DATA_DIR + OPS_OFFSITE_TARGET.',
      ...
      bs('Restore: pick row → Restore → confirm prompt. Pre-restore snapshot tự tạo (pre_restore_<ts>.json) cho rollback.',
         'Restore: chọn dòng → Restore → confirm. Pre-restore snapshot tự tạo (pre_restore_<ts>.json) cho rollback.'),
      bs('Code backup (admin) — snapshot toàn bộ source tree vào Backup & restore/Code/code_<ts>/ (loại trừ node_modules, dist, .git, server/data).',
         'Code backup (admin) — snapshot toàn bộ source vào Backup & restore/Code/code_<ts>/ (loại trừ node_modules, dist, .git, server/data).'),
    ],
    keyFields: [],
    formulas: [],
    tips: [
      bt('Keep at least 3 off-site snapshots: latest + last Monday + last end-of-month.',
         'Giữ ít nhất 3 snapshot off-site: mới nhất + thứ Hai trước + cuối tháng trước.'),
      bt('Restore on a STAGING URL first if available — verify the data looks right before touching production.',
         'Restore trên URL STAGING trước nếu có — xác nhận data đúng trước khi động vào production.'),
    ],
    pitfalls: [
      bp('Restore OVERWRITES the current DB in full. It is NOT a merge or selective restore.',
         'Restore GHI ĐÈ toàn bộ DB hiện tại. KHÔNG phải merge hoặc selective restore.'),
      bp('A restore rolls back every record, including saved quotes made since the backup. Communicate to users BEFORE restoring.',
         'Restore rollback mọi bản ghi, kể cả báo giá lưu sau backup. Thông báo cho user TRƯỚC khi restore.'),
      bp('File manifest mismatch (SHA-256) aborts the restore — don\'t hex-edit backup zips to "fix" them.',
  ...
  'settings-system-logs': {
    ...
    function: bi('Last 10k server events filtered by level (admin only)', '10k sự kiện server gần nhất lọc theo mức (chỉ admin)'),
    ...
    workflow: [
      ...
      bs('Filter by level (errors first), user, time window (last 1h / 24h / 7d / custom).',
         'Lọc theo mức (errors trước), user, khoảng thời gian (1h / 24h / 7d / tuỳ chỉnh).'),
      bs('Click a row → full request context: method, path, params (redacted), duration, stack trace (if error).',
         'Click dòng → context đầy đủ: method, path, params (đã redact), duration, stack trace (nếu lỗi).'),
      bs('Export filtered rows as JSON/CSV for attaching to a bug report.',
    ...
    keyFields: [
      ...
      field('Duration',   'ms',       'Total time from receipt to response. > 2000 ms on simple reads means DB contention.'),
    ],
    formulas: [],
    tips: [
      bt('Bookmark "level=error + last 24h" — opens the actionable subset in one click.',
         'Bookmark "level=error + last 24h" — mở subset actionable trong 1 click.'),
      bt('Correlate by request id when investigating — a single user action may produce 3–4 log lines.',
         'Tương quan theo request id khi điều tra — 1 action user có thể tạo 3-4 log line.'),
    ],
    pitfalls: [
      bp('Logs are in-memory; restart wipes them. Configure a Loki / Promtail sidecar for long-term retention.',
         'Log in-memory; restart sẽ xoá. Cấu hình Loki / Promtail sidecar để retention dài hạn.'),
      bp('Level fatal auto-pages the on-call engineer via the error beacon — don\'t test with ?simulate=fatal in prod.',
  ...
  'ink-silkscreen': {
    ...
    function: bi('Silkscreen-specific ink volume math (mesh + emulsion → film → µL)', 'Công thức thể tích mực silkscreen (lưới + emulsion → film → µL)'),
    ...
    procedures: [
      proc('Mesh spec input', 'Nhập thông số lưới', null,
        [
          bs('Pick mesh from library → count + thread diameter auto-fill.',
             'Chọn lưới từ library → count + đường kính sợi tự điền.'),
          bs('Manual: enter mesh count (T/cm) and thread diameter (µm).',
             'Thủ công: nhập mesh count (T/cm) và đường kính sợi (µm).'),
          bs('System computes Open area % = (1 − d × count)².',
             'Hệ thống tính Open area % = (1 − d × count)².'),
        ],
      ),
      proc('Emulsion + film', 'Emulsion + độ dày film',
        bi('Film thickness is the stencil thickness plus a mesh-dependent correction.',
           'Độ dày film là độ dày stencil cộng hiệu chỉnh theo lưới.'),
        [
          bs('Enter emulsion thickness (µm) from datasheet.',
             'Nhập độ dày emulsion (µm) từ datasheet.'),
          bs('System computes film_µm ≈ emulsion_µm × 1.2 + k × √open_area.',
             'Hệ thống tính film_µm ≈ emulsion_µm × 1.2 + k × √open_area.'),
          bs('k calibrated via step-wedge test (typical 3-5, emulsion-dependent).',
             'k hiệu chỉnh qua step-wedge test (thường 3-5, phụ thuộc emulsion).'),
        ],
      ),
      proc('Area + output', 'Diện tích + kết quả', null,
        [
          bs('Import area (mm²) from Print Area run, or enter manually.',
             'Import diện tích (mm²) từ run Print Area, hoặc nhập thủ công.'),
          bs('Output: µL/label, mL/1k, cost (when ink unit-price in Material Library).',
             'Output: µL/nhãn, mL/1k, chi phí (khi đơn giá mực có trong Material Library).'),
          bs('Compare "Calculated" vs "Actual press logs" — delta > 15% flags mesh batch variance.',
    ...
    keyFields: [
      ...
      field('Ink open area',      'percent', 'Fraction of mesh that passes ink; derived from thread diameter.'),
    ],
    formulas: [
      { name: 'Mesh open area',
        ...
        example: 'Mesh 120 T/cm, thread diameter 35 µm = 0.0035 cm\nopen = (1 − 0.0035 × 120)² = (1 − 0.42)² = 0.336 = 33.6%',
      ...
      { name: 'Silkscreen film thickness',
        ...
        example: 'emulsion=8 µm, open=0.336, k=4\nfilm ≈ 8 × 1.2 + 4 × √0.336 = 9.6 + 2.32 = 11.9 µm',
    ...
    tips: [
      bt('Step-wedge calibration once per emulsion batch — run a 5/25/50/75/100% patch on a scrap label and weigh ink consumed.',
         'Hiệu chỉnh step-wedge 1 lần/lô emulsion — chạy patch 5/25/50/75/100% trên nhãn scrap và cân mực tiêu thụ.'),
    ],
    pitfalls: [
      bp('Using anilox-style BCM math for silkscreen is wrong — silkscreen has no anilox cells; film thickness is directly the mesh + emulsion stack.',
  ...
  'ink-flexo': {
    ...
    function: bi('Flexo-specific ink volume math (anilox BCM → film → µL)', 'Công thức thể tích mực flexo (BCM anilox → film → µL)'),
    ...
    procedures: [
      proc('Anilox pick', 'Chọn anilox',
        bi('Keep a "reference" anilox flagged in the library — new jobs can clone it as a starting point.',
           'Giữ một anilox "reference" trong library — job mới có thể clone làm điểm xuất phát.'),
        [
          bs('Click Anilox LOV → pick by engraving code.',
             'Click LOV Anilox → chọn theo engraving code.'),
          bs('LPI + BCM auto-fill from the library.',
             'LPI + BCM tự điền từ library.'),
          bs('Certification date column shows age — re-cert quarterly on high-volume rolls.',
             'Cột ngày cert hiện tuổi — re-cert hàng quý với roll volume cao.'),
        ],
      ),
      proc('BCM → film conversion', 'Quy đổi BCM → film', null,
        [
          bs('System computes film_µm = BCM × 0.06451 (BCM bn µm³/in² × in² per mm² × mm per µm).',
             'Hệ thống tính film_µm = BCM × 0.06451 (BCM tỉ µm³/in² × in² mỗi mm² × mm mỗi µm).'),
          bs('Transfer factor default 1.12 (10-15% cell residue for flexo).',
             'Transfer factor mặc định 1.12 (10-15% đọng trong cell anilox cho flexo).'),
          bs('Override only with calibrated press-specific data.',
             'Ghi đè chỉ với data đã hiệu chỉnh cho máy cụ thể.'),
        ],
      ),
      proc('Output + compare', 'Kết quả + đối chiếu', null,
        [
          bs('Import area from Print Area run, OR enter manually.',
             'Import diện tích từ run Print Area, HOẶC nhập thủ công.'),
          bs('Output: µL/label, mL/1k, cost/label.',
             'Output: µL/nhãn, mL/1k, chi phí/nhãn.'),
          bs('Cross-check "calculated µL" vs "metered µL from press tank" — delta > 10% flags anilox wear or bad BCM cert.',
    ...
    workflow: null,
    keyFields: [
      field('LPI',   'lines/in', 'Engrave fineness. Higher LPI = finer detail, smaller cells, less ink.'),
      field('BCM',   'bn µm³/in²', 'Anilox ink-carrying capacity.'),
      field('Transfer factor', 'x', 'Fraction of BCM that transfers to plate → substrate. Default 1.12.'),
    ],
    formulas: [
      { name: 'BCM → film thickness',
        ...
        notes: '0.06451 converts BCM (bn µm³/in²) to wet film µm assuming full transfer; discount by transfer_factor for reality.' },
      { name: 'Ink volume',
      ...
      { name: 'Effective coverage with dot gain',
        ...
        example: 'File tint P=0.25 (25%):\neffective = 0.25 + 0.18 × 4 × 0.25 × 0.75 = 0.385 (38.5% press)',
        notes: 'g=18% at 50% peak is the flexo default.' },
    ],
    tips: [
      bt('Anilox wear is real — re-certify BCM quarterly on high-volume rolls. Wear drops BCM 5-10% per year.',
         'Anilox mòn là có thật — re-cert BCM hàng quý với roll volume cao. Mòn giảm BCM 5-10%/năm.'),
      bt('Mark one anilox as the facility "reference" — most new jobs can clone it as a starting point.',
         'Đánh dấu 1 anilox là "reference" của xưởng — hầu hết job mới có thể clone làm điểm xuất phát.'),
    ],
    pitfalls: [
      bp('Using the nameplate BCM from the anilox vendor without certified test data is unreliable — always use the QC certificate.',
         'Dùng BCM nameplate của NCC anilox mà không có data test đã chứng nhận là không đáng tin — luôn dùng QC certificate.'),
      bp('Anilox cleaning with aggressive chemistry can actually INCREASE BCM temporarily (the cells widen); re-certify after a deep clean.',
  ...
  'standard-layout': {
    ...
    function: bi('Configure product dimensions + roll/sheet layout + MOQ tiers', 'Cấu hình kích thước sản phẩm + layout cuộn/tờ + bậc MOQ'),
    ...
    preRequisites: [br('Product trim dimensions known; press web width known.',
                       'Biết kích thước trim sản phẩm; biết web width máy in.')],
    procedures: [
      proc('Product dimensions', 'Kích thước sản phẩm', null,
        [
          bs('Enter Length × Width (mm) — the trim size, exclusive of bleed.',
             'Nhập Chiều dài × Chiều rộng (mm) — kích thước trim, không gồm bleed.'),
          bs('Optional: add bleed if material utilization depends on it.',
             'Tuỳ chọn: thêm bleed nếu sử dụng vật tư phụ thuộc vào nó.'),
          bs('Thickness (µm) — drives material weight + UoM conversions.',
             'Độ dày (µm) — ảnh hưởng cân nặng vật tư + quy đổi UoM.'),
        ],
      ),
      proc('Roll / Sheet layout', 'Layout cuộn / tờ', null,
        [
          bs('Pick "Roll" or "Sheet" feed per the target press.',
             'Chọn "Roll" hoặc "Sheet" theo máy in mục tiêu.'),
          bs('Enter web width (roll) or sheet dims (sheet).',
             'Nhập web width (cuộn) hoặc kích thước tờ (sheet).'),
          bs('Across count: labels per row. Down count: rows per sheet (sheet only).',
             'Across count: nhãn mỗi hàng. Down count: hàng mỗi tờ (chỉ cho sheet).'),
          bs('Gap between labels (mm): press-dependent; default 3 for flexo.',
             'Khoảng cách giữa các nhãn (mm): phụ thuộc máy; mặc định 3 cho flexo.'),
        ],
      ),
      proc('MOQ tiers', 'Bậc MOQ',
        bi('Customers often ask for tiered pricing. Configure up to 5 tiers; each tier gets its own Summarize row.',
           'Khách thường yêu cầu báo giá theo bậc. Cấu hình tối đa 5 bậc; mỗi bậc có dòng Summarize riêng.'),
        [
          bs('Enter Tier 1 qty — the base MOQ.',
             'Nhập số lượng Bậc 1 — MOQ cơ sở.'),
          bs('Enter up to 4 additional tier quantities (ascending).',
             'Nhập tối đa 4 số lượng bậc tiếp theo (tăng dần).'),
          bs('Each tier amortizes setup over its qty — larger tier = lower per-unit cost.',
             'Mỗi bậc phân bổ setup theo qty — bậc càng lớn = chi phí/đơn vị càng thấp.'),
        ],
      ),
    ],
    tips: [
      bt('Layout counts cascade: a wrong Across = N changes labels/hr, waste %, and material cost on EVERY sub-tab.',
         'Layout count chảy xuống: Across = N sai sẽ đổi labels/hr, waste %, và chi phí vật tư ở MỌI sub-tab.'),
    ],
    pitfalls: [
      bp('Forgetting Gap causes Materials to over-estimate utilization — your quote will be optimistic.',
  ...
  'standard-material': {
    ...
    function: bi('Pick substrate + adhesive + liner; compute material cost per label', 'Chọn face + adhesive + liner; tính chi phí vật tư/nhãn'),
    ...
    whenToUse: bi('After Layout is complete. Re-visit when switching substrate.', 'Sau khi Layout xong. Quay lại khi đổi substrate.'),
    preRequisites: [br('Layout sub-tab completed; Material Library populated.',
                       'Sub-tab Layout đã xong; Material Library đã có data.')],
    procedures: [
      proc('Face material', 'Vật tư mặt (Face)', null,
        [
          bs('Click Face LOV → pick material code.',
             'Click LOV Face → chọn code vật tư.'),
          bs('Unit cost auto-populates from Material Library.',
             'Đơn giá tự điền từ Material Library.'),
          bs('Waste % override if press-specific value differs from library default.',
             'Ghi đè Waste % nếu giá trị riêng của máy khác mặc định library.'),
        ],
      ),
      proc('Adhesive + Liner', 'Keo + Đế', null,
        [
          bs('Pick Adhesive from LOV (permanent / removable / freezer-grade).',
             'Chọn Keo từ LOV (vĩnh viễn / gỡ được / cấp đông).'),
          bs('Pick Liner (PET 30µm, glassine, etc.).',
             'Chọn Đế (PET 30µm, glassine, v.v.).'),
          bs('Per-layer waste accumulates to the material cost line.',
             'Waste từng lớp cộng dồn vào dòng chi phí vật tư.'),
        ],
      ),
      proc('Optional layers', 'Lớp tuỳ chọn', null,
        [
          bs('Laminate (over-laminate film) — toggle ON and pick from LOV.',
             'Laminate (film over-laminate) — bật ON và chọn từ LOV.'),
          bs('Varnish — toggle ON, select type (water / UV).',
             'Varnish — bật ON, chọn loại (gốc nước / UV).'),
          bs('Each optional layer adds a cost line; toggle OFF to skip.',
             'Mỗi lớp tuỳ chọn thêm 1 dòng chi phí; tắt OFF để bỏ qua.'),
        ],
      ),
      proc('Cost summary', 'Tóm tắt chi phí', null,
        [
          bs('Right panel shows per-layer breakdown + total material cost / label.',
             'Panel phải hiển thị breakdown từng lớp + tổng chi phí vật tư/nhãn.'),
          bs('Hover any row for formula + library row reference.',
  ...
  'standard-inks': {
    ...
    function: bi('Per-color ink consumption + cost for Standard quote', 'Tiêu thụ mực từng màu + chi phí cho báo giá Standard'),
    ...
    preRequisites: [br('Print method selected in header; Print Area run for this SKU (recommended).',
                       'Đã chọn công nghệ in trong header; đã chạy Print Area cho SKU (khuyến nghị).')],
    procedures: [
      proc('Ink count', 'Số màu', null,
        [
          bs('Enter Number of inks (the separation count).',
             'Nhập Số màu (số separation).'),
          bs('Table shows one row per ink.',
             'Bảng hiển thị một dòng mỗi màu.'),
        ],
      ),
      proc('Import from Print Area', 'Import từ Print Area',
        bi('Biggest accuracy win — replaces ink-count-based defaults with real pixel measurements.',
           'Cú bứt chính xác lớn nhất — thay mặc định theo số màu bằng đo pixel thực tế.'),
        [
          bs('Click ← Import from Print Area in toolbar.',
             'Click ← Import from Print Area trên toolbar.'),
          bs('System matches by SKU; µL/label auto-fills per row.',
             'Hệ thống match theo SKU; µL/nhãn tự điền từng dòng.'),
          bs('If no match found, fill manually (see defaults table below).',
             'Nếu không match, nhập thủ công (xem bảng mặc định bên dưới).'),
        ],
      ),
      proc('Per-ink adjustment', 'Điều chỉnh từng màu', null,
        [
          bs('Override ink name (e.g. "Red 485C") for the customer-facing quote.',
             'Ghi đè tên mực (vd "Red 485C") cho báo giá khách thấy.'),
          bs('Unit cost / mL — auto-pulled from Material Library (ink category).',
             'Đơn giá / mL — tự lấy từ Material Library (danh mục mực).'),
          bs('Waste % per ink — press tank min level drives this (5-10% typical).',
  ...
  'standard-process': {
    ...
    function: bi('Configure process routing (work-centers, setup, run rate) per tier', 'Cấu hình routing (work-center, setup, tốc độ) theo bậc'),
    ...
    whenToUse: bi('After Inks. Use Balancing sub-tab if press has capacity bottleneck.', 'Sau Inks. Dùng Balancing nếu máy tới hạn.'),
    preRequisites: [br('Routing Ops library populated; Rate Table current.',
    ...
    procedures: [
      proc('Add process row', 'Thêm process row', null,
        [
          ...
          bs('Pick Process Type (Print / Cut / Laminate / Slit / Rewind / QC).',
             'Chọn Loại công đoạn (Print / Cut / Laminate / Slit / Rewind / QC).'),
          bs('Pick Work-center → hourly rate auto-loads from Rate Table.',
             'Chọn Work-center → đơn giá/giờ tự load từ Rate Table.'),
          bs('Setup hours + Run rate + Yield populate from Routing Ops library.',
             'Setup hours + Run rate + Yield tự điền từ thư viện Routing Ops.'),
          bs('Override for press-specific anomalies.',
             'Ghi đè cho bất thường riêng của máy.'),
        ],
      ),
      proc('Layout batch count', 'Layout batch',
        bi('Required for machine work-centers. Usage bumps from 0 → 1 → 2 as the operation consumes sheets in batches.',
           'Bắt buộc cho work-center loại máy. Usage tăng từ 0 → 1 → 2 khi công đoạn tiêu thụ sheet theo batch.'),
        [
          bs('Set Layout / batch count — how many layouts run per press-hour.',
             'Đặt Layout / batch count — bao nhiêu layout chạy mỗi giờ máy.'),
          bs('Machine workcenters REQUIRE this > 0; manual skips.',
             'Work-center loại máy BẮT BUỘC > 0; thủ công có thể bỏ qua.'),
        ],
      ),
      proc('Per-tier tuning', 'Tuỳ chỉnh theo bậc', null,
        [
          bs('Each process has N copies (one per MOQ tier from Layout).',
             'Mỗi process có N bản (một cho mỗi bậc MOQ từ Layout).'),
          bs('Override setup/run rate per-tier ONLY if the press behavior differs at scale (rare).',
             'Ghi đè setup/run rate theo từng bậc CHỈ khi máy hoạt động khác theo quy mô (hiếm).'),
          bs('Visible column toggle: show/hide tiers to reduce horizontal scroll.',
  ...
  'standard-balancing': {
    ...
    function: bi('Identify the bottleneck operation + compute realistic lead-time', 'Xác định công đoạn bottleneck + tính lead-time thực tế'),
    ...
    whenToUse: bi('Tight delivery; multiple presses available; considering overtime.', 'Giao hàng gấp; nhiều máy; cân nhắc tăng ca.'),
    preRequisites: [br('Processes sub-tab filled with work-center + rates.',
                       'Sub-tab Processes đã điền work-center + đơn giá.')],
    procedures: [
      proc('Review per-tier timings', 'Xem thời lượng từng bậc', null,
        [
          bs('Chart rows = process steps; bar length = run time.',
             'Dòng chart = các bước process; độ dài bar = thời gian chạy.'),
          bs('Red bar = bottleneck (longest-running op at that tier).',
             'Bar đỏ = bottleneck (công đoạn chạy lâu nhất ở bậc đó).'),
          bs('Hover for start/end times + work-center.',
             'Hover để xem thời gian start/end + work-center.'),
        ],
      ),
      proc('What-if parallelization', 'What-if chạy song song', null,
        [
          bs('Click any bottleneck bar → suggestion: "Run on 2 machines, save X h".',
             'Click bar bottleneck → gợi ý: "Chạy 2 máy, tiết kiệm X h".'),
          bs('Apply → new process row added at half the run time each.',
             'Apply → dòng process mới thêm với một nửa thời gian mỗi máy.'),
          bs('Review cost impact on the Cost Breakdown tab.',
  ...
  'standard-cost-breakdown': {
    ...
    function: bi('Itemized cost waterfall per MOQ tier', 'Waterfall chi phí chi tiết theo từng bậc MOQ'),
    ...
    preRequisites: [br('Layout, Material, Inks, Process sub-tabs filled.',
                       'Đã điền các sub-tab Layout, Material, Inks, Process.')],
    procedures: [
      proc('Review cost lines', 'Review các dòng chi phí', null,
        [
          bs('Rows: Material, Ink, Labor (per process), Machine (per process), Overhead, SGA.',
             'Dòng: Vật tư, Mực, Nhân công (mỗi process), Máy (mỗi process), Overhead, SGA.'),
          bs('Columns: one per MOQ tier.',
             'Cột: một cột mỗi bậc MOQ.'),
          bs('Total row at bottom; margin-before-tier-selection at very bottom.',
             'Dòng Total ở đáy; margin-trước-chọn-bậc ở dưới cùng.'),
        ],
      ),
      proc('Drilldown', 'Drilldown', null,
        [
          bs('Click any cell → detail modal with formula + source fields.',
             'Click ô bất kỳ → modal chi tiết kèm công thức + trường nguồn.'),
          bs('Red flag icon = variance > 20% vs historical avg for this SKU / product family.',
             'Icon cờ đỏ = chênh lệch > 20% so với trung bình lịch sử SKU / dòng sản phẩm.'),
        ],
      ),
      proc('Export', 'Xuất', null,
        [
          bs('Click ⬇ CSV to export the full matrix.',
             'Click ⬇ CSV để xuất full matrix.'),
          bs('For customer-facing, use Summarize → Formal Quotation instead (filters internal cost lines).',
  ...
  'standard-pack-ship': {
    ...
    function: bi('Packaging material cost + freight estimate', 'Chi phí bao bì + ước tính cước vận chuyển'),
    ...
    preRequisites: [br('Material sub-tab completed for net weight calc.',
                       'Sub-tab Material đã điền để tính net weight.')],
    procedures: [
      proc('Roll/Box config', 'Cấu hình cuộn/thùng', null,
        [
          bs('Pick Roll core type (3-inch / 6-inch) OR Box type for sheet-fed.',
             'Chọn loại lõi cuộn (3-inch / 6-inch) HOẶC loại thùng cho sheet-fed.'),
          bs('Labels per roll / sheets per box — drives box count.',
             'Số nhãn/cuộn hoặc sheet/thùng — quyết định số thùng.'),
          bs('Core + box unit cost auto-pulled from Material Library (Packaging category).',
             'Đơn giá lõi + thùng tự lấy từ Material Library (danh mục Packaging).'),
        ],
      ),
      proc('Pallet', 'Pallet', null,
        [
          bs('Boxes per pallet (typical 24-48).',
             'Số thùng/pallet (thường 24-48).'),
          bs('Pallet type (wood / plastic) from LOV.',
             'Loại pallet (gỗ / nhựa) từ LOV.'),
          bs('Stretch-wrap + strap cost auto-included.',
             'Chi phí stretch-wrap + dây đai tự bao gồm.'),
        ],
      ),
      proc('Freight', 'Cước vận chuyển', null,
        [
          bs('Pick Incoterms (EXW / FOB / CIF / DDP).',
             'Chọn Incoterms (EXW / FOB / CIF / DDP).'),
          bs('For non-EXW: fill Origin → Destination; Freight rate auto-looks up from Freight library (if configured).',
             'Với non-EXW: điền Điểm đi → Điểm đến; cước tự tra từ thư viện Freight (nếu đã cấu hình).'),
          bs('Manual override when the freight library misses the lane.',
  ...
  'standard-summarize': {
    ...
    function: bi('Per-tier margin + sell price; the final decision screen', 'Margin + giá bán theo từng bậc; màn hình quyết định cuối'),
    ...
    whenToUse: bi('Last sub-tab. Final margin call before saving to Quote History.', 'Sub-tab cuối. Quyết định margin cuối trước khi lưu Quote History.'),
    preRequisites: [br('All upstream sub-tabs (Layout, Material, Inks, Process, Cost Breakdown) filled.',
                       'Mọi sub-tab phía trước (Layout, Material, Inks, Process, Cost Breakdown) đã điền.')],
    procedures: [
      proc('Set margin per tier', 'Đặt margin theo bậc',
        bi('Different margins per tier is common — larger tiers can absorb tighter margins due to better setup amortization.',
           'Margin khác nhau theo bậc là bình thường — bậc lớn có thể chịu margin chặt hơn nhờ amortize setup tốt hơn.'),
        [
          bs('For each MOQ tier row, enter Margin %.',
             'Với mỗi dòng bậc MOQ, nhập Margin %.'),
          bs('Sell price updates live.', 'Giá bán update live.'),
          bs('Use Copy icon to propagate one tier\'s margin to the rest.',
             'Dùng icon Copy để áp margin 1 bậc sang các bậc còn lại.'),
        ],
      ),
      proc('Per-tier review', 'Review theo bậc', null,
        [
          bs('Columns: Cost → Margin → Sell → Profit → Profit/1k.',
             'Cột: Cost → Margin → Sell → Profit → Profit/1k.'),
          bs('Red cell = margin below minimum threshold (default 15%); needs justification.',
             'Ô đỏ = margin dưới ngưỡng tối thiểu (mặc định 15%); cần giải trình.'),
        ],
      ),
      proc('Commit save', 'Commit save', null,
        [
          bs('Click Save → snapshot committed to Quote History.',
             'Click Save → snapshot lưu vào Quote History.'),
          bs('Click Submit for Approval → enters Pending Approvals queue.',
             'Click Submit for Approval → vào queue Pending Approvals.'),
          bs('Click Formal Quotation → opens the customer-facing doc generator.',
             'Click Formal Quotation → mở trình tạo tài liệu gửi khách.'),
        ],
      ),
    ],
    relatedTabs: ['standard', 'standard-cost-breakdown', 'summarize', 'formal-quote', 'approvals-inbox'],
    screenshot: null,
  },
  ...
  'complex-bom-tree': {
    ...
    preRequisites: [br('BOM structure defined in Mfg Structures or inline.',
                       'Cấu trúc BOM định nghĩa trong Mfg Structures hoặc inline.')],
    procedures: [
      proc('Expand / collapse', 'Mở rộng / thu gọn', null,
        [
          bs('Click ▸ icon on any level to expand its children.',
             'Click icon ▸ trên cấp bất kỳ để mở rộng con.'),
          bs('Click ▾ to collapse.', 'Click ▾ để thu gọn.'),
          bs('Ctrl+click any level to expand all descendants at once.',
             'Ctrl+click cấp bất kỳ để mở rộng tất cả con cháu.'),
        ],
      ),
      proc('Add / remove levels', 'Thêm / xoá cấp', null,
        [
          bs('Right-click any level → Add Child Level.',
             'Right-click cấp bất kỳ → Add Child Level.'),
          bs('Right-click → Delete Level (cascades to all descendants; confirmation required).',
             'Right-click → Delete Level (cascade xuống mọi con cháu; cần xác nhận).'),
          bs('Drag a level onto another to re-parent.',
             'Kéo một cấp lên cấp khác để đổi cha.'),
        ],
      ),
      proc('Cost overlay', 'Overlay chi phí',
        bi('Toggle in toolbar shows cost contribution per level directly on the tree.',
           'Toggle trên toolbar hiện đóng góp chi phí từng cấp trực tiếp trên cây.'),
        [
          bs('Click "Show costs" toggle.', 'Click toggle "Show costs".'),
          bs('Each level shows its % of total cost — drives drill-down priority.',
             'Mỗi cấp hiển thị % tổng chi phí — quyết định thứ tự drill-down.'),
          bs('Orange ≥ 15%, Red ≥ 30% — review first.',
             'Cam ≥ 15%, Đỏ ≥ 30% — review trước.'),
        ],
      ),
    ],
  ...
  'complex-cost-breakdown': {
    ...
    function: bi('Per-level + aggregated cost waterfall', 'Waterfall chi phí theo từng cấp + tổng hợp'),
    ...
    preRequisites: [br('Complex BOM filled with materials + routing.',
                       'Complex BOM đã điền vật tư + routing.')],
    procedures: [
      proc('Matrix review', 'Review matrix', null,
        [
          bs('Rows = BOM levels; columns = cost categories.',
             'Dòng = cấp BOM; cột = danh mục chi phí.'),
          bs('Cell = cost contribution of that level × category.',
             'Ô = đóng góp chi phí của cấp × danh mục đó.'),
          bs('Heatmap gradient: darker = larger contribution.',
             'Gradient heatmap: đậm = đóng góp lớn hơn.'),
        ],
      ),
      proc('Yield compound inspection', 'Kiểm tra Yield compound',
        bi('Yield hits compound across levels — 95%³ = 85.7%. This view surfaces where material waste hides.',
           'Yield compound qua các cấp — 95%³ = 85.7%. View này chỉ ra waste vật tư ẩn ở đâu.'),
        [
          bs('Click "Show yield" toggle in toolbar.',
             'Click toggle "Show yield" trên toolbar.'),
          bs('Each level row shows compounded input qty + implied material loss.',
             'Mỗi dòng cấp hiện input qty đã compound + mất mát vật tư ngầm.'),
          bs('Red cells = yield below floor threshold.',
             'Ô đỏ = yield dưới ngưỡng sàn.'),
        ],
      ),
      proc('Export', 'Xuất', null,
        [
          bs('Click ⬇ CSV — 2D matrix flattens to long format.',
             'Click ⬇ CSV — matrix 2D chuyển sang dạng long.'),
          bs('Click ⬇ PDF — produces the sign-off-ready exec summary.',
  ...
  'metrics': {
    ...
    function: bi('Runtime health (latency, errors, cache, sessions) — sys role only', 'Health runtime (latency, lỗi, cache, session) — chỉ sys'),
    ...
    workflow: [
      bs('Home tile grid: green = healthy, amber = warning, red = critical.',
         'Grid tile: xanh = OK, amber = cảnh báo, đỏ = nguy cấp.'),
      bs('Click a tile → time-series + last N events.',
         'Click tile → time-series + N sự kiện gần nhất.'),
      bs('Error beacon feed: client-side JS errors grouped by stack trace.',
         'Feed error beacon: lỗi JS client-side nhóm theo stack trace.'),
    ],
    keyFields: [],
    formulas: [],
    tips: [bt('Subscribe to the error beacon via email in Settings → Notifications.',
              'Đăng ký error beacon qua email trong Settings → Notifications.')],
    pitfalls: [bp('Metrics are in-memory; restart resets them. Ship to Prometheus for long-term retention.',
  ...
  'whats-new': {
    ...
    function: bi('Release notes — 6 đợt enterprise hardening', 'Ghi chú phát hành — 6 đợt hardening enterprise'),
    ...
    preRequisites: [],
    features: [
      feat('Đợt 1: Connection banner (top-of-app) when server unreachable, useAutoRefresh on QuoteHistory/RFQ/Sample/Approvals tabs (poll every 30-60s, pause when hidden)',
      ...
      feat('Đợt 2: SSE event bus — server emits quote.saved, rfq.updated, sample.updated, approval.transition, library.imported. Tabs refetch instantly (< 50ms) instead of waiting for poll. ConflictModal replaces blunt window.confirm() with 3-button (Reload / Overwrite / Cancel) preserving user edits.',
      ...
      feat('Đợt 3: Active users indicator (TopBar góc phải — "● N online" pill, click xem list). Backup upload from disk (sys-only). Off-site backup helper script (scripts/backup-offsite.sh). HTTPS Caddy helper (scripts/setup-https-caddy.sh).',
      ...
      feat('Đợt 4: Login anomaly detection — server marks LOGIN_ANOMALY in audit + emits SSE security.alert when (a) same user logged in from 2+ IPs in 5min, (b) login from new IP not seen in 30d, (c) login at unusual hour (22h-6h). Admins see real-time toast; user themselves sees yellow toast post-login to spot session hijack.',
      ...
      feat('Đợt 5: SQLite primary backend default — removes JSON-mutex bottleneck @ 20 concurrent saves. AboutSection 3 new diagnostic tests (Quote backend parity, SSE subscribers, Active users count).',
      ...
      feat('Đợt 6: Active Sessions admin tab (sys-only, in Account Control) — see all active logins (username, role, 2FA status, token prefix, expires-in). Per-row Revoke button kicks user from all machines.',
    ...
    workflow: [
      bs('Read through the 6 đợt highlights above.',
         'Đọc qua 6 đợt highlights ở trên.'),
      bs('Settings → ℹ️ About / Diagnostics → click "Quote backend" — verify backend=sqlite + parity OK.',
         'Settings → ℹ️ About / Diagnostics → click "Quote backend" — xác nhận backend=sqlite + parity OK.'),
      bs('Login from máy thứ 2 (cùng tài khoản) → bạn sẽ thấy toast vàng "Có session khác đang mở từ IP …".',
         'Login từ máy thứ 2 (cùng tài khoản) → bạn sẽ thấy toast vàng "Có session khác đang mở từ IP …".'),
      bs('Settings → Account Control → Sessions tab (chỉ sys) — verify thấy 2 sessions; click Revoke để kick session kia.',
         'Settings → Account Control → Sessions tab (chỉ sys) — verify thấy 2 sessions; click Revoke để kick session kia.'),
      bs('Settings → Backup / Restore → click "📤 Upload từ máy khác…" để test restore từ snapshot ngoài.',
         'Settings → Backup / Restore → click "📤 Upload từ máy khác…" để test restore từ snapshot ngoài.'),
    ],
    keyFields: [],
    formulas: [],
    tips: [
      bt('SSE real-time push only works when server + client share the same origin or proxy passes Connection: keep-alive (default). nginx behind Caddy: ensure no buffering on /api/events/stream.',
      ...
      bt('Login anomaly thresholds tuned for low false-positive: night-shift operators won\'t fire after 3 nights of normal shift logins (history baseline).',
      ...
      bt('SQLite backend gives 5-10× faster list queries (indexed columns) vs JSON scan. Switch back to file via OPS_DATA_BACKEND=file env if SQLite issue.',
      ...
      bt('Sessions tab is sys-only (not admin). Grant sys role only to ops/IT.',
         'Tab Sessions chỉ sys (không phải admin). Cấp role sys chỉ cho ops/IT.'),
    ],
    pitfalls: [
      bp('SQLite cutover: JSON file is still mirrored as backup safety net. Don\'t delete quote_history.json manually — let the next /save-all rewrite it.',
         'Cutover SQLite: file JSON vẫn mirror làm safety net. Đừng xoá quote_history.json bằng tay — để /save-all kế tự ghi lại.'),
      bp('Revoking sessions kills the user instantly on every machine. Communicate before doing it on a prod user mid-shift.',
         'Revoke session kill user ngay trên mọi máy. Báo trước khi làm với prod user giữa ca.'),
      bp('SSE doesn\'t survive nginx default buffering. If clients show stale data despite save, check proxy_buffering off; on /api/events/stream location.',
  ...
  'whats-new-sprint9': {
    ...
    preRequisites: [],
    workflow: [
      bs('Read through the Sprint 9 highlights below.',
         'Đọc qua các điểm nổi bật Sprint 9 bên dưới.'),
      bs('Confirm the Help tab\'s own entry works via F1 deep-link.',
         'Xác nhận entry Help tab hoạt động qua F1 deep-link.'),
      bs('Skim the changed tabs (Print Area, Help — both have new capabilities).',
         'Lướt qua các tab đã đổi (Print Area, Help — cả hai có khả năng mới).'),
      bs('If you had existing saved jobs in Print Area, re-analyze a few to confirm the spot-color detection now surfaces the red / brand inks you expect.',
    ...
    formulas: [],
    tips: [
      bt('Sprint 9 added a centralized Help system — this screen — covering 37 tabs. Press F1 from any tab for context-sensitive help.',
         'Sprint 9 thêm Help system tập trung — màn hình này — phủ 37 tab. Ấn F1 ở tab bất kỳ để help theo ngữ cảnh.'),
      bt('Print Area Calculator now rescues rare spot colors automatically (chroma-boost + outlier rescue). Red warning text on black labels no longer vanishes from the palette.',
      ...
      bt('The 📌 Pin as spot ink eyedropper lets you force a cluster for colors with < 0.05% coverage. See "Print Area Calculator" for the workflow.',
         'Eyedropper 📌 Pin as spot ink cho phép force cluster cho màu coverage < 0.05%. Xem "Print Area Calculator" để biết workflow.'),
      bt('New NaN-guards on corrupt saved-job configs prevent the "blank palette" failure that hit a few legacy quotes.',
         'NaN-guard mới trên config job hỏng ngăn lỗi "blank palette" đã gặp ở vài báo giá cũ.'),
    ],
    pitfalls: [
      bp('Re-analyzing an old saved job with Sprint 9 defaults (K=8, chroma boost ON) may produce slightly different per-color splits than the archived numbers. Total coverage is unchanged; per-ink µL may shift.',
  ...
  'help': {
    ...
    preRequisites: [],
    workflow: [
      bs('Search box: type any keyword — matches titles, purposes, and body text across all sections.',
         'Ô search: gõ keyword bất kỳ — match title, purpose, và body text qua mọi section.'),
      bs('Sidebar: browse by section (Calculators → Quoting → …).',
         'Sidebar: duyệt theo section (Calculators → Quoting → …).'),
      bs('Content panel: scroll; related-tab links jump to the related entry.',
         'Panel nội dung: scroll; link related-tab nhảy đến entry liên quan.'),
      bs('Download button (top right): exports the full user guide as a Word file.',
         'Nút Download (trên phải): xuất full user guide dạng Word file.'),
    ],
    keyFields: [],
    formulas: [],
    tips: [
      bt('Press F1 for context-sensitive help — jumps to the entry for the tab you\'re on.',
         'Ấn F1 để Help theo ngữ cảnh — nhảy đến entry của tab đang mở.'),
      bt('The Word export is a printable version of what you see here — useful for onboarding packets.',
  ...
  'order-entry': {
    ...
    function: bi('Intake customer orders (SKU, qty, ship date)', 'Tiếp nhận đơn khách (SKU, qty, ngày giao)'),
    ...
    whenToUse: bi('Whenever a customer order is confirmed — before Planning can touch capacity.',  'Khi đơn hàng được xác nhận — trước khi Planning bắt đầu tính capacity.'),
    preRequisites: [br('Customer in Drop-Down Lists; SKU in Material Library or a quoted SKU.',
                       'Khách hàng trong Drop-Down Lists; SKU trong Material Library hoặc SKU đã báo giá.')],
    workflow: [
      bs('Create order: pick customer → pick SKU → enter qty + ship-by date.',
         'Tạo đơn: chọn khách → chọn SKU → nhập qty + ngày giao.'),
      bs('Save → auto-passes to BOM Explosion for material requirements.',
         'Save → tự chuyển sang BOM Explosion để tính nhu cầu vật tư.'),
    ],
    keyFields: [],
    formulas: [],
    tips: [bt('Use the Clone button for repeat orders — saves retyping.',
              'Dùng nút Clone cho đơn lặp lại — tiết kiệm nhập lại.')],
    pitfalls: [bp('Ship-by date must be realistic; Capacity Planning downstream will flag it but won\'t auto-reject.',
  ...
  'bom-explosion': {
    ...
    function: bi('Compute total material requirements from finished-goods orders', 'Tính tổng nhu cầu vật tư từ đơn thành phẩm'),
    ...
    preRequisites: [br('Complete BOM definition in Mfg Structures.',
                       'Định nghĩa BOM đầy đủ trong Mfg Structures.')],
    workflow: [
      bs('Pick the order → click Explode.', 'Chọn đơn → click Explode.'),
      bs('Review the expanded material list grouped by level + lead-time category.',
    ...
    formulas: [
      { name: 'Required qty',
        ...
        example: 'Order 10,000 FG; component at level 2, consumption 1.2 per FG, yield chain 0.95 × 0.95\nqty = 10000 × 1.2 / 0.9025 = 13,297 units' },
    ],
    tips: [bt('Review Level 1 only if short on time — it\'s 80% of material dollar volume.',
              'Review Level 1 trước nếu gấp — nó chiếm 80% giá trị vật tư.')],
    pitfalls: [bp('BOM changes mid-run break the explosion — freeze BOM version for active orders.',
  ...
  'material-check': {
    ...
    function: bi('Feasibility check: stock + open POs vs. requirements', 'Kiểm tra khả thi: tồn + PO mở so với nhu cầu'),
    ...
    whenToUse: bi('After BOM Explosion. Must pass before Work Order generation.', 'Sau BOM Explosion. Phải pass trước khi phát Work Order.'),
    preRequisites: [br('IFS Inventory live.', 'IFS Inventory đang hoạt động.')],
    workflow: [
      bs('Select exploded BOM → system matches against IFS stock.',
         'Chọn BOM đã phân rã → hệ thống match với tồn IFS.'),
      bs('Red rows = shortage; amber = borderline; green = OK.',
         'Dòng đỏ = thiếu; amber = giáp ranh; xanh = OK.'),
      bs('For shortages: jump to RFQ Tracker to source or to Material Library for alternates.',
    ...
    formulas: [
      { name: 'Shortage',
        ...
        example: 'required=13,297; on_hand=5,000; on_order=3,000; allocated=4,000\nshortage = 13,297 − (5,000 + 3,000 − 4,000) = 9,297 units' },
    ],
    tips: [bt('Snapshot the check result — IFS is live, a week later the picture changes.',
              'Snapshot kết quả check — IFS live, tuần sau bức tranh sẽ khác.')],
    pitfalls: [bp('Allocated-to-cancelled SOs may still show allocated until IFS nightly cleanup.',
  ...
  'capacity': {
    ...
    function: bi('Gantt work-cell load vs. committed orders', 'Gantt tải work-cell so với đơn đã cam kết'),
    ...
    whenToUse: bi('Weekly planning meeting. On-demand when a new large order arrives.', 'Họp kế hoạch hàng tuần. On-demand khi đơn lớn mới đến.'),
    preRequisites: [br('Routing Ops with run rates; Order Entry backlog.',
                       'Routing Ops có run rates; Order Entry có backlog.')],
    workflow: [
      bs('Pick a horizon (2 weeks default).', 'Chọn horizon (mặc định 2 tuần).'),
      bs('Rows = work-cells; color = utilization. Click a bar = which order it serves.',
         'Dòng = work-cell; màu = utilization. Click bar = đang phục vụ đơn nào.'),
      bs('Drag to re-schedule; system recomputes downstream ship dates.',
    ...
    formulas: [
      { name: 'Utilization',
        ...
        example: 'Press A: 160h available/week, 136h committed\nutilization = 136/160 × 100 = 85% → practical ceiling (amber)',
        notes: '85%+ flashes amber; 100% is theoretical — real-world ceiling is ~85%.' },
    ],
    tips: [bt('Over 85% utilization flashes amber — that\'s the practical capacity ceiling, not 100%.',
              'Utilization > 85% flash amber — đó là trần thực tế, không phải 100%.')],
    pitfalls: [bp('Drag-reschedule without approval from shop-floor leads causes friction — loop them in.',
  ...
  'work-orders': {
    ...
    function: bi('Generate + release floor work orders from capacity plan', 'Tạo + phát lệnh sản xuất từ kế hoạch năng lực'),
    ...
    whenToUse: bi('Daily, at end-of-shift, for the next day\'s run.', 'Hàng ngày, cuối ca, cho sản xuất ngày hôm sau.'),
    preRequisites: [br('Capacity Planning approved; Material Check green.',
                       'Capacity Planning đã duyệt; Material Check xanh.')],
    workflow: [
      bs('Select orders to release → click Generate Work Orders.',
         'Chọn đơn để release → click Generate Work Orders.'),
      bs('Review the generated routing cards per cell.',
         'Xem routing card đã tạo cho từng cell.'),
      bs('Print + hand to shop floor, OR push to the floor terminal if integrated.',
         'In + đưa shop floor, HOẶC đẩy sang terminal shop floor nếu có tích hợp.'),
    ],
    keyFields: [],
    formulas: [],
    tips: [bt('Batch releases by press — reduces setup time by sharing tooling.',
              'Batch release theo máy — giảm thời gian setup bằng cách dùng chung tooling.')],
    pitfalls: [bp('Releasing without material check pass causes floor stall — always confirm green first.',
  ...
  'wip-tracker': {
    ...
    function: bi('Real-time WIP status per work order (completion %, holds, yield)', 'Trạng thái WIP theo WO (% hoàn thành, giữ, yield)'),
    ...
    whenToUse: bi('Shop-floor supervisor hourly; office team for customer ETA queries.', 'Supervisor hàng giờ; office team khi khách hỏi ETA.'),
    preRequisites: [br('Shop floor terminals reporting to the system.',
                       'Terminal shop floor báo cáo về hệ thống.')],
    workflow: [
      bs('Filter by work-cell or customer.', 'Lọc theo work-cell hoặc khách.'),
      bs('Click a WO for the full step timeline + yield per step.',
         'Click WO để xem timeline các bước + yield từng bước.'),
      bs('Flag a quality hold → stops downstream release until resolved.',
    ...
    formulas: [
      { name: 'Completion %',
        ...
        example: 'WO-2026-0042: released 10,000, completed 7,230\ncompletion = 7230/10000 × 100 = 72.3%' },
    ],
    tips: [bt('Drive stand-up meetings off this screen — single source of truth.',
              'Họp stand-up dựa trên màn hình này — single source of truth.')],
    pitfalls: [bp('Completion % is not always accurate if operators forget to scan. Enforce the scan with SOP.',
