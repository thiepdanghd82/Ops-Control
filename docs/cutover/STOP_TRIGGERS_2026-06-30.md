...
| T5  | Backup không run trên prod tối D-1 (`server/data/Backup/SQLite/` file >24h cũ)                                                         | HALT cutover. Backup MUST be running before any new data lands.                         | Lead Engineer (sysadmin function) fix within 2h.        |
